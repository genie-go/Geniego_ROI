// corrections2.json 의 수동 검수값을 en_translation_workbook_final.json 에 반영
// 출력: en_translation_workbook_final2.json (신규 파일, 덮어쓰기 없음)
// 사용법: node tools/session144_apply_corrections2.mjs [--dry|--apply]

import fs from 'node:fs';

const MODE        = process.argv.includes('--apply') ? 'apply' : 'dry';
const INPUT_FILE  = 'en_translation_workbook_final.json';
const CORR_FILE   = 'corrections2.json';
const OUT_FILE    = 'en_translation_workbook_final2.json';
...
const result = workbook.map(entry => {
  ...
  return { ...entry, en_suggestion: corr.en, needs_review: false, _source: `corrected2` };
});
