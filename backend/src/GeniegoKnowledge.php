...
■ 네이버 스마트스토어 [naver_smartstore]
   ...
   ※네이버 상품등록에 필요한 것
   - 카테고리(leafCategoryId): 네이버는 리프(말단) 카테고리 ID가 필수입니다. [카탈로그 동기화] > [카테고리 매핑]에서 지정하거나, 확정 필요 패널의 추천 후보에서 고르면 됩니다. GenieGo가 상품명·카테고리 텍스트로 후보를 자동 추천합니다.
   - 상품정보제공고시·배송/반품·AS·원산지: [상품 최적화(가격/상품)] 상품 등록 폼의 "쇼핑몰 필수정보" 항목입니다. 비어 있으면 네이버가 등록을 거부(400)합니다.

■ 11번가 [st11]
   1. 준비물: 11번가 셀러(판매자) 계정이 필요합니다. ★일반회원으로 가입했다면 셀러오피스에서 반드시 "Seller로 회원전환"을 먼저 해야 합니다. 일반 Open API 키로는 상품/카테고리 "조회"만 되고, 상품등록·주문관리 같은 셀러 API는 쓸 수 없습니다.
   2. openapi.11st.co.kr (11번가 OPEN API CENTER)에 셀러 아이디로 로그인합니다.
   3. 상단 [API 관리] > [서비스 등록·확인] > [서비스등록]에서 셀러 정보를 입력하고 약관에 동의해 등록하면 OPEN API KEY(32자리)가 발급됩니다.
   4. ★[서비스 등록·확인] > [Seller API 정보 수정] > "접속권한"에서 **API를 호출할 서버의 IP 주소를 반드시 등록**합니다. IP를 등록해야만 셀러 API Key 승인이 가능합니다(IP 미등록 = 승인 불가). 카페24·메이크샵 호스팅을 쓰면 "호스팅여부"에서 선택하면 자동 등록됩니다.
   5. [마이페이지] > [API 이용신청/승인 현황]에서 상품 API·주문 API가 "승인완료"인지 확인합니다.
   6. [서비스확인]에서 발급된 API Key(32자리)를 복사합니다.
   7. GenieGo [연동 허브] > [등록]에 api_key와 seller_id(셀러 ID)를 입력해 저장합니다.

   ※11번가 오류코드 안내
   - resultCode -997 "등록된 API 정보가 존재하지 않습니다": 인증(키/IP/이용신청) 실패가 아니라, **호출한 API 주소(경로+방식)가 11번가에 존재하지 않는다**는 뜻입니다. 키를 재발급해도 해결되지 않습니다.
   - "API로 이용할 수 없는 판매 방식입니다": 판매방식코드 문제입니다. 11번가 API는 고정가판매(01)·예약판매(04)·중고판매(05)만 지원하며, GenieGo는 일반 판매를 고정가판매(01)로 자동 전송합니다.
   - "브랜드코드가 없습니다": 상품에 브랜드가 지정되지 않았습니다. [카탈로그 동기화] > [브랜드 관리]에서 브랜드를 등록하고 상품에 지정하세요.

   ※11번가 상품등록에 반드시 필요한 3가지 (하나라도 없으면 등록이 거부됩니다)
   - 표시카테고리(dispCtgrNo): [카탈로그 동기화] > [카테고리 매핑] 또는 확정 필요 패널에서 지정합니다.
   - 브랜드: [카탈로그 동기화] > [브랜드 관리]에서 등록 후 상품에 지정합니다.
   - 판매방식: GenieGo가 고정가판매(01)로 자동 전송하므로 사용자가 입력할 필요 없습니다.
...
■ 토스페이먼츠 [toss]
   ...
   5. GenieGo [등록]에 secret_key를 입력해 저장합니다(client_key 는 결제위젯용 · 정산수집 불필요 · 선택).
...
■ Stripe [stripe]
   ...
   5. GenieGo [등록]에 secret_key를 입력해 저장합니다(publishable_key 는 클라이언트용 · 정산수집 불필요 · 선택).
...
■ Checkout.com [checkout]
   ...
   4. GenieGo [등록]에 secret_key를 입력해 저장합니다(public_key 는 클라이언트용 · 정산수집 불필요 · 선택).
...
■ DHL [dhl]
   ...
   6. GenieGo [등록]에 api_key를 입력해 저장합니다(account_number(계정번호)는 요율·예약 시에만 입력 · 선택).

■ FedEx [fedex]
   1. 준비물: FedEx 계정번호가 필요합니다.
   2. developer.fedex.com 에 가입하고 로그인합니다.
   3. 프로젝트를 생성하면 API Key와 Secret Key가 발급됩니다.
   4. GenieGo [등록]에 api_key, api_secret를 입력해 저장합니다(account_number 는 선택).

■ UPS [ups]
   1. 준비물: UPS 계정번호가 필요합니다.
   2. developer.ups.com 에 가입하고 앱을 생성합니다.
   3. client_id와 client_secret을 발급받습니다.
   4. GenieGo [등록]에 client_id, client_secret를 입력해 저장합니다(account_number 는 선택).
...
■ CJ대한통운 [cj]
   ...
   4. GenieGo [등록]에 api_key 를 입력해 저장합니다(cust_code 는 전용 API 계약 시에만 입력 · 선택).
...
■ TNT [tnt]
   ...
   4. GenieGo [등록]에 api_key, api_secret 를 입력해 저장합니다(account_number 는 선택).
...
TXT;
    ...
    public static function menuGuides(): string {
        return <<<'TXT'
● 메뉴: catalogSync
   - 이게 무슨 기능인가요?: 내 상품 카탈로그를 11번가·네이버 스마트스토어·쿠팡 같은 판매 채널에 실제로 "등록·수정·판매중지"하는 메뉴입니다. 브랜드·카테고리처럼 채널이 요구하는 필수 정보를 여기서 갖춘 뒤 전송합니다.
   - 어떤 역할을 하나요?: 채널마다 요구하는 필수 항목이 다릅니다(11번가=표시카테고리+브랜드, 네이버=리프 카테고리+상품정보제공고시). 이 메뉴가 그 차이를 흡수해서, 한 번 갖춰 두면 여러 채널에 반복 전송할 수 있게 해줍니다.
   - 탭 구성: 카탈로그 / 동기화 실행 / 카테고리 매핑 / 브랜드 관리 / 가격 규칙 / 재고 정책 / 이력 / 사용 가이드.
   - 따라하기 (11번가에 상품을 처음 올리는 경우):
       ① [브랜드 관리] 탭에서 브랜드 등록 → 브랜드명(예: 청정원)을 입력하고 [＋ 추가]를 누릅니다. 11번가는 브랜드가 없으면 상품등록을 거부합니다("브랜드코드가 없습니다" 오류). 브랜드코드는 몰라도 되며, 브랜드명만으로 전송됩니다.
       ② 화면 위 [카테고리·브랜드 확정 필요] 패널 확인 → 아직 카테고리나 브랜드가 없는 상품이 여기 모입니다. 다 갖춘 상품은 이 패널에 나오지 않습니다.
       ③ 상품마다 브랜드 선택 → 각 상품 카드의 브랜드 드롭다운에서 고릅니다. 목록에 없으면 옆 칸에 새 브랜드명을 입력하고 [＋ 추가·지정]을 누르면 등록과 지정이 한 번에 됩니다.
       ④ 상품마다 카테고리 선택 → 추천 후보 버튼을 누르거나, [🔍 카테고리 검색]으로 직접 찾습니다. 카테고리를 확정하는 순간 채널로 자동 전송됩니다.
       ⑤ 결과 확인 → "채널 응답"에 성공/실패가 그대로 표시됩니다. 실패하면 11번가가 알려준 진짜 사유(예: 필수 필드 누락)가 그대로 보이므로, 그 항목을 채우고 다시 시도하면 됩니다.
   - 브랜드 관리 상세: 브랜드는 상품 하나당 1개를 지정합니다. 상품 등록 폼(상품 최적화 메뉴)에서 직접 입력한 브랜드도 이 목록에 자동으로 등록되어, 다음 상품부터는 골라 쓸 수 있습니다. 사용 중인 브랜드는 삭제되지 않습니다(상품의 브랜드가 사라져 등록이 깨지는 것을 막기 위함).
   - 카테고리 매핑 상세: 11번가는 카테고리를 2개 지정할 수 있습니다 — 표시카테고리(dispCtgrNo, 필수)와 기본카테고리(선택). 다른 채널은 1개만 씁니다.
   - 이 정보로 무엇을 할 수 있나요?: 등록 실패 상품을 [카테고리·브랜드 확정 필요] 패널에서 한눈에 파악하고, 필수 항목만 채워 재전송하세요. 채널이 거부한 이유가 그대로 표시되므로 추측할 필요가 없습니다.
