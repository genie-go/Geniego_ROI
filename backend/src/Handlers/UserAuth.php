...
final class UserAuth
{
     ...
     *   ★Phase2(현재): 조회는 `token = hashToken(입력)` **hash-only**. 배포 시 기존 평문행을 in-place
     *     해시화(UPDATE SET token=SHA2(token,256))했으므로 원문 토큰은 `token=hashToken(원문)` 으로 매칭되고,
     *     DB 덤프로 얻은 저장 해시는 `token=hashToken(해시)≠저장` 이라 replay 불가(실 보호 실현). Phase1 의
     *     dual-read 평문분기 제거 완료.
    ...
    private static function userByToken(string $token): ?array
    {
        ...
        try {
            ...
            $stmt->execute([self::hashToken($token), self::now()]);
        ...
        if (array_key_exists('_sess_last_seen', $row)) {
            ...
            if ($almMin > 0 && $lastTs > 0 && ($nowTs - $lastTs) >= $almMin * 60) {
                // 유휴 초과 → 세션 폐기(재로그인 강제).
                try { $pdo->prepare('DELETE FROM user_session WHERE token = ?')->execute([self::hashToken($token)]); } catch (\Throwable $e) {}
                return null;
            }
            // 활동 갱신(throttle: 최초 또는 60s 초과 시에만 write).
            if ($lastTs === 0 || ($nowTs - $lastTs) >= 60) {
                try { $pdo->prepare('UPDATE user_session SET last_seen_at = ? WHERE token = ?')->execute([self::now(), self::hashToken($token)]); } catch (\Throwable $e) {}
            }
        }
    ...
    public static function logout(ServerRequestInterface $req, ResponseInterface $res): ResponseInterface
    {
        ...
        if ($token) {
            ...
            Db::pdo()->prepare('DELETE FROM user_session WHERE token = ?')->execute([self::hashToken($token)]);
    ...
    public static function changePassword(ServerRequestInterface $req, ResponseInterface $res): ResponseInterface
    {
        ...
        try { $pdo->prepare('DELETE FROM user_session WHERE user_id=? AND token<>?')->execute([$user['id'], self::hashToken($token)]); } catch (\Throwable $e) {}
    ...
    private static function recordSessionMeta(\PDO $pdo, ServerRequestInterface $req, string $token): void
    {
        try {
            ...
            $pdo->prepare('UPDATE user_session SET ip=?, ua=?, last_seen=? WHERE token = ?')
                ->execute([self::clientIp($req), $ua, self::now(), self::hashToken($token)]);
    ...
    public static function listSessions(ServerRequestInterface $req, ResponseInterface $res): ResponseInterface
    {
        ...
        $out = array_map(function ($r) use ($token) {
            ...
            return [
                ...
                'current'    => hash_equals($tk, self::hashToken($token)), // [P5-Phase2] 저장값=해시(hash-only)
    ...
    public static function revokeOtherSessions(ServerRequestInterface $req, ResponseInterface $res): ResponseInterface
    {
        ...
        try {
            ...
            $st->execute([(int)$user['id'], self::hashToken($token)]);
            $revoked = $st->rowCount();
        } catch (\Throwable $e) {}
