...
final class Connectors
{
    ...
    {
        $campaigns = ['브랜드_봄시즌', '리타겟팅_구매자', '루카라이브_홍보', '신규유입_여성30대'];
        $rows = [];
    ...
    {
        $campaigns = ['브랜드검색_크리에이티브', '경쟁사키워드_방어', '쇼핑_시즌상품', '유튜브_브랜딩'];
        ...
        for ($ts = $d; $ts <= $e && count($rows) < 20; $ts += 86400) {
            foreach ($campaigns as $c) {
                ...
                $rows[] = [
                    ...
                    'conversions' => rand(1, 40), 'mock' => true,
                ];
                if (count($rows) >= 20) break;
            }
        }
    ...
    {
        return [
            ['campaignId' => 'C001', 'campaignName' => '브랜드_키워드', 'status' => 'ELIGIBLE', 'budget' => 500000, 'clicks' => 1240, 'impressions' => 38000, 'cost' => 310000, 'mock' => true],
            ['campaignId' => 'C002', 'campaignName' => '상품_카테고리_타겟팅', 'status' => 'ELIGIBLE', 'budget' => 300000, 'clicks' => 870, 'impressions' => 22000, 'cost' => 215000, 'mock' => true],
            ['campaignId' => 'C003', 'campaignName' => '경쟁사_방어', 'status' => 'ELIGIBLE', 'budget' => 200000, 'clicks' => 450, 'impressions' => 15000, 'cost' => 145000, 'mock' => true],
        ];
    }
