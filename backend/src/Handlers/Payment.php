...
final class Payment
{
    ...
    public static function getPublicPricingPlans(ServerRequestInterface $req, ResponseInterface $res): ResponseInterface
    {
        try {
            ...
            // ── 2단계 파싱: 먼저 raw 데이터 수집, 이후 월간 기준가(monthly base)로 정규화 ──
            $rawPricing = []; // [planKey][acct][cycle] => { price_krw, discount_pct }
            ...
            // ── 월간 기본가(monthly base_price) 기준으로 모든 cycle 정규화 ──
            // Admin이 저장한 monthly의 price_krw를 SSOT(Single Source of Truth)로 사용
            // quarterly/semi_annual/yearly의 total = monthlyBase × months × (1 - discount%)
            $pricingMap = [];
            foreach ($rawPricing as $planKey => $acctMap) {
                foreach ($acctMap as $acct => $cycleMap) {
                    // 1) 월간 기본가 결정
                    $monthlyBase = $cycleMap['monthly']['price_krw'] ?? 0;
                    // 월간이 없으면 다른 cycle에서 역산 시도
                    if ($monthlyBase <= 0) {
                        foreach (['quarterly', 'semi_annual', 'yearly'] as $fc) {
                            if (isset($cycleMap[$fc]) && $cycleMap[$fc]['price_krw'] > 0) {
                                $monthlyBase = $cycleMap[$fc]['price_krw'];
                                break;
                            }
                        }
                    }

                    foreach ($cycleMap as $cycle => $data) {
                        ...
                        $discPct = $data['discount_pct'];

                        if ($cycle === 'monthly') {
                            // 월간: 할인 없이 그대로
                            $finalMonthly = $monthlyBase;
                            $total = $monthlyBase;
                        } else {
                            // 장기결제: 월간기본가 × 개월수 × (1 - 할인율)
                            $totalBeforeDisc = $monthlyBase * $cycleMonths;
                            $total = $discPct > 0
                                ? (int)round($totalBeforeDisc * (1 - $discPct / 100))
                                : $totalBeforeDisc;
                            $finalMonthly = (int)round($total / $cycleMonths);
                        }

                        $pricingMap[$planKey][$acct][$cycle] = [
                            'base_price'    => $monthlyBase,
                            'discount_pct'  => $discPct,
    ...
    private static function checkMaster(ServerRequestInterface $req): bool
    {
        ...
        try {
            ...
            $stmt->execute([UserAuth::hashToken($token),$now]); $user=$stmt->fetch(\PDO::FETCH_ASSOC);
    ...
    // ─────────────────────────────────────────────────────────────
    // GET /auth/pricing/menu-access?plan=growth  (관리자 전용)
    // 저장된 플랜별 메뉴 접근 권한을 DB에서 로드하여 반환
    // ─────────────────────────────────────────────────────────────
    public static function getMenuAccess(ServerRequestInterface $req, ResponseInterface $res): ResponseInterface
    {
        if (!self::checkAdmin($req)) {
            return self::json($res->withStatus(403), ['ok' => false, 'error' => '관리자 권한이 필요합니다.']);
        }

        $plan = trim((string)($req->getQueryParams()['plan'] ?? 'growth'));
        if (!$plan) $plan = 'growth';
        ...
        try {
            ...
            // 해당 플랜으로 저장된 메뉴 접근 권한 로드
            $stmt = $pdo->prepare(
                "SELECT menu_key FROM menu_tier_pricing WHERE plan=? AND is_active=1 AND price_krw IN (1,2) AND menu_key NOT LIKE '__plan_acct__%'"
            );
            $stmt->execute([$plan]);
            $rows = $stmt->fetchAll(\PDO::FETCH_COLUMN);

            $permissions = [];
            foreach ($rows as $menuKey) {
            ...
            return self::json($res, ['ok' => true, 'permissions' => [], 'count' => 0]);
        }
    }

    // ─────────────────────────────────────────────────────────────
    // POST /auth/pricing/menu-access  (관리자 전용)
    // 새 형식: { plan: "growth", permissions: { "home||dashboard||...": true, ... } }
    // 구 형식: { permissions: { [menuId]: { [role]: "✅"|"👁"|"🔒" } }, items: [...] }
    // ─────────────────────────────────────────────────────────────
    public static function saveMenuAccess(ServerRequestInterface $req, ResponseInterface $res): ResponseInterface
    {
        ...
        // 새 형식: { plan: "growth", permissions: { "key": true/false } }
        if (empty($items) && $plan && isset($body['permissions']) && is_array($body['permissions'])) {
            $permissions = $body['permissions'];
            // boolean 맵인지 확인 (첫 번째 값이 bool인 경우)
            $firstVal = reset($permissions);
            if (is_bool($firstVal) || $firstVal === 1 || $firstVal === 0) {
                foreach ($permissions as $menuKey => $enabled) {
                    if ($enabled) {
                        $items[] = [
                            'menu_key'  => $menuKey,
                            'menu_path' => $menuKey,
                ...
                // 구 형식 (role-based emoji)
                foreach ($permissions as $menuId => $rolePerm) {
                    if (!is_array($rolePerm)) continue;
        ...
        try {
            ...
            // 해당 플랜의 기존 메뉴접근권한 항목 전체 비활성화 (요금 항목 __plan_acct__ 는 제외)
            if ($plan) {
                $stmt = $pdo->prepare("UPDATE menu_tier_pricing SET is_active=0 WHERE plan=? AND menu_key NOT LIKE '__plan_acct__%' AND price_krw IN (1,2)");
                $stmt->execute([$plan]);
            } else {
            ...
            foreach ($items as $item) {
                ...
                )->execute([$menuKey, $menuPath, $itemPlan, $cycle, $priceKrw]);
                $saved++;
            }
            ...
            return self::json($res, [
                ...
                'saved' => $saved,
                'plan'  => $plan,
                'msg'   => "✅ {$plan} 플랜 메뉴 접근 권한 {$saved}건이 저장되었습니다.",
            ]);
        } catch (\Throwable $e) {
            return self::json($res->withStatus(500), ['ok' => false, 'error' => '서버 오류가 발생했습니다.']);
        }
    }
}
