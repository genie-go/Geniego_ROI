...
final class UserAdmin
{
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // [263차 제거] 기간별 구독요금 관리(plan_prices) — 유령테이블(CREATE 부재·프론트 호출0) 참조 고아.
    //   기간별 구독요금 SSOT 는 plan_period_pricing(AdminPlans UI·PlanPricing/AuthPage/GlobalDataContext 소비)
    //   으로 일원화됨. listPlanPrices/upsertPlanPrice/deletePlanPrice 및 라우트 3종 제거(중복 재신설 금지).
    // [289차후속 P3] admin_roles/user_roles 폐기 — 부여된 커스텀 역할이 어떤 인가 게이트에서도 소비되지 않는
    //   DORMANT(장식) RBAC 였다(실 RBAC=TeamPermissions acl_permission/data_scope). ensureRoleTables + 6메서드
    //   (listRoles/upsertRole/deleteRole/assignRole/getUserRoles/revokeRole) + 라우트 + FE 역할탭 동반 제거(재신설 금지).
    //   admin_roles/user_roles 테이블은 파괴적 DROP 대신 고아 유지(비파괴).
