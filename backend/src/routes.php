...
// v418.3 - highest spec aliases for v418.2 endpoints (fully backward compatible)
        ...
        // ── 메뉴 접근권한 저장 (관리자 전용 — MenuAccessTab 저장 버튼) ────
    ...
    $app->get('/api/v423/creds',          [\Genie\Handlers\ChannelCreds::class, 'list']);
    $app->post('/api/v423/creds',         [\Genie\Handlers\ChannelCreds::class, 'upsert']);
    $app->delete('/api/v423/creds/{id}',  [\Genie\Handlers\ChannelCreds::class, 'delete']);
    $app->post('/api/v423/creds/{id}/test', [\Genie\Handlers\ChannelCreds::class, 'test']);
    // SmartConnect: 채널 스캔 / 자동 연동 테스트 / 발급신청
    $app->get('/api/v423/creds/scan',                        [\Genie\Handlers\ChannelCreds::class, 'scan']);
    $app->post('/api/v423/connectors/{channel}/test',        [\Genie\Handlers\ChannelCreds::class, 'channelTest']);
    $app->post('/api/v423/connectors/apply',                 [\Genie\Handlers\ChannelCreds::class, 'apply']);
    // ConnectorSyncContext: 연결된 채널 요약 (메뉴간 동기화)
    $app->get('/api/v423/creds/summary',                     [\Genie\Handlers\ChannelCreds::class, 'summary']);
    ...
    // ── 메뉴 접근권한 저장 (관리자 전용 — MenuAccessTab 저장) ────────────
