...
(async () => {
  console.log('🚀 Nginx root 경로로 클린 배포...');
  ...
  // 1. Clean server assets
  await ex(c, `rm -rf ${NGINX_ROOT}/assets/*`);
  
  // 2. Upload index.html
  await sftpUpload(c, path.join(LOCAL_DIST, 'index.html'), `${NGINX_ROOT}/index.html`);
  ...
  for (const file of assets) {
    try {
      await sftpUpload(c, path.join(LOCAL_DIST, 'assets', file), `${NGINX_ROOT}/assets/${file}`);
      ok++;
      if (ok % 20 === 0) console.log(`   ${ok}/${assets.length}`);
    } catch {
      await new Promise(r => setTimeout(r, 300));
      try { const c2 = await conn(); await sftpUpload(c2, path.join(LOCAL_DIST, 'assets', file), `${NGINX_ROOT}/assets/${file}`); c2.end(); ok++; } catch { console.log(`   ❌ ${file}`); }
    }
  }
  console.log(`✅ ${ok}/${assets.length}`);

  // 4. Verify
  const idx = await ex(c, `grep 'index-' ${NGINX_ROOT}/index.html`);
