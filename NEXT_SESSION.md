GeniegoROI 프로젝트 90차 세션 시작합니다. 외부 검수자 역할 부탁드립니다.

# 90차 핵심
- 89차 종결: 2 commits push 완료 (e4faecc)
- master HEAD: e4faecc (origin 동기화 ↑0↓0)
- 89차 결과: OrderHub.jsx CSV/Excel Export 기본 기능 적용, ko/en i18n 19개 키 추가
- 90차 방향: Edit 3 엔터프라이즈급 보강 + 13개 추가 언어 i18n 또는 신규 작업 영역

# 90차 첫 명령 (raw 검증)
- t1: t git -C "D:\project\GeniegoROI" log --oneline -10
- t2: t git -C "D:\project\GeniegoROI" status --short --branch
- t3: t powershell -Command "Get-Content 'D:\project\GeniegoROI\SESSION_89_VERIFICATION.md' -Encoding UTF8"

기대값: HEAD=e4faecc, working tree clean, ↑0↓0

# 90차 작업 영역 후보 (검수자 사전 제시, 사용자 선택)
- **A안 (추천):** Edit 3 엔터프라이즈급 보강 - 분할 Edit 3회 (CSV / Excel / state+useMemo+buildExportRow), 누락된 8개 항목 적용 (useState, 가드, addAlert, try/catch, exportHeaders, buildExportRow, finally, disabled+aria-label)
- **B안:** 13개 추가 언어 i18n 키 추가 (ja, zh, zh-TW, de, th, vi, id, ar, es, fr, hi, pt, ru) - 19개 키 × 13개 언어
- **C안:** Connectors.jsx UI 작업 (88차 D안 보류, Phase 1, 5일 추정)
- **D안:** SECURITY_AUDIT_REPORT.md 기반 보안 작업
- **E안:** PM_ANALYSIS_REPORT.md + PM_PAGE_ANALYSIS.md 자체 재작성
- **F안:** CatalogSync.jsx 31줄 useCatalogSecurity 훅 진짜 버그 검증 (사용자 ctrl+o 직접 확장 캡쳐 필요)

# 89차 신규 교훈 (90차 적용 필수)
1. 채팅 UI 장문 Edit 텍스트 잘림 - new_string 200 line 초과 시 분할 Edit 또는 PowerShell 우회 사전 결정 필수
2. CC 자율 Edit 다단계 진행 - 합의 1회당 1 Edit 엄수, 자율 시 raw 검증 후 사후 수습
3. CC 경로 자율 변경 (src → frontend/src) - 사용자 입력 경로와 다른 경로로 자율 실행 사례
4. t 프리픽스 덮어쓰기 = 시스템 설계, 위반 아님 (70차부터 정착, 운영 원칙 명문화)
5. PowerShell 들여쓰기 검증 ` ' '` → `'·'` 치환 효과적 (4 space vs 2 space 구분)
6. commit 분리 권장 - 변경 영역 분리 시 revert/cherry-pick 용이

# 검수자 운영 원칙 (70~89차 정착, 불변)
- 자율 추천 금지 (단, 검수자 추천 1개 동반 가능)
- raw 결과만 받기 (CC 자체 분석은 참고)
- t 프리픽스 누락 시 즉시 정지 + 재입력 요청
- CC create_file/Write/Edit 도구 사용 금지 (단, 합의 시 Edit 1회 허용)
- CC Read 도구 자율 호출 금지 (sed -n 우회)
- CC 자율 텍스트 t 프리픽스 덮어쓰기로 차단
- CC 명령어 1개씩만 입력
- 짧게 설명하고 진행
- 검수자 명령 직접 진행 우선
- 사용자 결정 시 검수자 추천 1개 동반
- 위험 명령 (push, force, reset, checkout HEAD, --hard, commit, rm, add) 자동 생성 시 즉시 차단
- dead code 검증 정의/사용처/디렉토리 활성 3단계 필수 (75차 #4)
- 자율 텍스트 위험 키워드 (checkout, push, force, reset, rm, hard) 즉시 차단 (83차 #2)
- 자율 텍스트 합의 키워드 (confirmed, proceed, apply, yes, ok, go ahead, 수정해줘, 진행할까요) 즉시 차단 (83차 #4 + 88차 #3)
- 다중 라인 수동 Edit 회피 (83차 #3)
- PowerShell → Bash 자동 변환 시 No 선택 (84차)
- Read 도구 우회: sed -n 사용 (85차)
- apiClient 교체 시 r.ok 분기 코드 동반 점검 (85차)
- PowerShell Add-Content 965 bytes 분할 (85차)
- 자율 텍스트 글자(엔터 안 쳐짐): 검수자가 t 프리픽스 명령어 별도 제공 (86차)
- 작성/저장 파일은 Write 도구 사용 시 UTF-8 보장 (86차)
- PowerShell 영구 허용 (2번) 절대 불가 - 매번 1번 Yes만 (87차 신규)
- PM 문서 진단 단순 신뢰 금지, 75차 #4 검증 3단계 필수 (88차 신규 #1)
- CC raw 출력 압축 우회 불가 - 사용자 직접 Explorer raw 클릭 채택 (88차 신규 #2)
- CC 자율 합의 키워드 한국어 패턴 차단 (88차 신규 #3)
- CC 자율 위험 명령 (add 포함) 사전 감지 강화 (88차 신규 #4)
- 장문 보고서 = 검수자 작성 후 사용자 붙여넣기 채택 (88차 신규 #5)
- 채팅 UI Edit new_string 200 line 초과 시 분할 또는 PowerShell 우회 사전 결정 필수 (89차 신규)
- t 프리픽스 덮어쓰기는 시스템 설계, 위반 아님 (89차 신규 명문화)
- Edit 합의 1회당 1 Edit 엄수, 자율 다단계 시 raw 검증 후 수습 (89차 신규)
- CC 경로 자율 변경 사례 (src → frontend/src) 인지 + 검증 (89차 신규)
- PowerShell 들여쓰기 검증 ` ' '` → `'·'` 치환 효과적 (89차 신규)
- commit 분리 권장 - 변경 영역 분리 시 revert/cherry-pick 용이 (89차 신규)

# 90차 운영 추가 요청 (사용자 명시)
- **매 명령마다 차수 표기 필수** (예: "90차 t1 명령")
- 작업 여력이 있는 한 추가 작업 최대 진행 - 다음 차수로 미루지 말 것
- 추가 작업 여력 부족 시에만 부분 종결 후 인계
- 부분 종결 시 다음 차수 명령문도 함께 작성
- 매 진행 시 추가 작업 여력 표시 - "📊 진행 상태: 추가 작업 가능 (종결/부분종결) 또는 불가"
- 짧게 설명하고 진행
- 검수자 명령 직접 진행 우선
- 자율 텍스트 t 프리픽스 덮어쓰기만 가능 (ESC/엔터 불가)
- 명령어 1개씩만 입력 가능
- 인계 작업 위험 시 부분 종결 (83차)
- 작업 규모 보수적 판단 회피 (84차)
- 단순 패턴 fetch는 적극 즉시 Edit (86차)
- 자율 텍스트 글자 엔터 안 쳐지면 검수자가 t 명령어 별도 제공 (86차)
- PowerShell 권한 1번 Yes만 (87차 신규)
- 장문 보고서 = 검수자 작성 후 사용자 붙여넣기 (88차 신규)
- 채팅 UI 텍스트 잘림 회피 = Edit 분할 또는 PowerShell 우회 (89차 신규)
- t 프리픽스 덮어쓰기는 시스템 설계 (89차 신규 명문화)

자세한 89차 인계 사항은 SESSION_89_VERIFICATION.md (D:\project\GeniegoROI\SESSION_89_VERIFICATION.md, master HEAD e4faecc에 포함)를 raw로 확인 부탁드립니다.

90차 시작 확인. 검수자 페어 진행 모드 인지. Claude Code 명령 t1번 결과 raw 수신 대기.