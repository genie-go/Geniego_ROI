GeniegoROI 프로젝트 92차 세션 시작합니다. 외부 검수자 역할 부탁드립니다.

# 92차 핵심
- 91차 종결: 1 commit + push 완료 (24fd7c1)
- master HEAD: 24fd7c1 (origin 동기화 ↑0↓0 - 91차 종결 commit 후 92차 인계 commit으로 변경 예정)
- 91차 결과: 13개 언어 × 27키 i18n 동기화 완료 (13 files, +364/-13, B안 옵션 1B 단일 차수)
- 92차 방향: I안 (다른 i18n 섹션 동기화) 또는 C/D/F/G/H 신규 작업 영역

# 92차 첫 명령 (raw 검증)
- t1: t git -C "D:\project\GeniegoROI" log --oneline -10
- t2: t git -C "D:\project\GeniegoROI" status --short --branch
- t3: t powershell -Command "Get-Content 'D:\project\GeniegoROI\SESSION_91_VERIFICATION.md' -Encoding UTF8"

기대값: HEAD=24fd7c1 (또는 92차 인계 commit), working tree clean, ↑0↓0

# 92차 작업 영역 후보 (검수자 사전 제시, 사용자 선택)
- **A안:** I안 - catalogSync 섹션 13개 언어 동기화 (A+B그룹 catalogSync 패턴 존재, 91차 검증)
- **B안:** I안 - dataProduct 섹션 13개 언어 동기화 (C그룹 dataProduct 패턴 존재)
- **C안 (88차 보류):** Connectors.jsx UI 작업 (Phase 1, 5일 추정)
...
- **G안:** ESLint 환경 복구 (90차에서 손상 확인, @eslint/eslintrc 누락)
- **H안:** OrderHub.jsx 추가 보강 - PII 마스킹 옵션, 대용량 가드, exportFilename 통합 유틸
- **J안 (신규):** ko.js/en.js 외 13개 언어 i18n locales 일관성 전수 검증
...
- B안 i18n-sync는 별도 차수 분리 (90차 신규 - 91차에서 단일 차수 진행 성공으로 권고 사항 완화)
...
- CC 시각 오인 사례 (Tab to Jump 표시) - raw 재검증 필수 (91차 신규)

# 92차 운영 추가 요청 (사용자 명시)
- **매 명령마다 차수 표기 필수** (예: "92차 t1 명령")
...
- CC raw 압축 빈번 시 사용자 Explorer 직접 캡쳐 채택 (90차)
- B안 옵션 1A/1B 선택 시 작업 분량 명확화 (90차)
- 사용자 VSCode 직접 복붙 패턴 - 이모지/멀티바이트 손상 위험 회피 시 채택 (91차 신규)
- 그룹별 raw 시범 후 일괄 작업 - 효율 + 안전 균형 (91차 신규)
- 검수자 시각 오인 가능성 인지 - raw 재검증 필수 (91차 신규)

자세한 91차 인계 사항은 SESSION_91_VERIFICATION.md (D:\project\GeniegoROI\SESSION_91_VERIFICATION.md, master HEAD 24fd7c1 이후 92차 인계 commit에 포함)를 raw로 확인 부탁드립니다.

92차 시작 확인. 검수자 페어 진행 모드 인지. Claude Code 명령 t1번 결과 raw 수신 대기.