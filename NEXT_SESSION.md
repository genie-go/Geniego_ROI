...
## F. ★ 다음 차수 순서 (우선순위)
1. **가독성 전수 마감**: 라이브 대비 스캐너를 전 메뉴(특히 각 페이지 비기본 탭·모달·active 상태)에 순회 실행해 잔여 흰-on-밝음/어두운-on-어두운/흰-on-흰 0 확인. 컬러 배지/버튼/탭 active 패턴 위주.
2. **프리미엄 일관화(페이지별 안전)**: 제목/서브탭/버튼 글자 크기·박스 높이 일관성 — 전역 강제는 위험(오류0 원칙), 고트래픽 페이지부터 페이지별 검수+검증.
3. **모바일 17페이지 시각 전수**: 섹션6 페이지 1:1 스크린샷(레이아웃 품질=오버플로 외 셀 적층 등).
4. **외부의존 약점**: 실광고집행(OAuth/토큰)·TOSS 빌링키·TikTok 영상 업로드·리프라이서 채널 push.
5. **브랜치 머지**: feat/n236-admin-growth-automation 다수 커밋 → main/master 전략(★master push=CI 자동배포, 수동배포 이미 완료).

## G. 트랩/도구 (238차 학습)
- ★라이트테마 darkening/gradient-conversion 규칙은 **흰글자 컬러 UI를 over-match** → 신규 컬러 배지/버튼 추가 시 `:not([style*="color: rgb(255, 255, 255)"])` 패턴 의식. **틴트(rgba)는 흰글자 금지·솔리드/그라데이션만 흰글자**.
...
- 라이브 대비 스캐너(luminance)·gradient 배경 rgb 추출이 진단 핵심 도구. 이모지/hex그라데이션 오탐 주의.
- 산출물: docs/NATIVE_MOBILE_UX_UPGRADE_REPORT.md 외 5종.

(★본 인계서·전 커밋·push = 사용자 명시 승인. 자격증명 평문 노출 0. 다음 대형 = F-1~5.)
