...
| **★Change Approval 워크플로** | ✅ **REAL**(v2.1 정정 — 초판 오분류) — `action_request` **승인 워크플로**: `decision`(approve/reject)·`approvals_json`·`status`·**테넌트 소유 검증(IDOR 차단·208차 P0)**([Alerting.php:578-582](../../backend/src/Handlers/Alerting.php)/545-546)·승인 후 실 집행+정직 상태(287차·:608-611) | **재사용(§18 Approval Reference·중복 승인엔진 신설 금지)** |
| **Change History / Audit Registry** | ✅ **REAL**(v2.1 정정 — "2종"은 과소) — **도메인별 audit_log 12파일**: `audit_log`([AdminGrowth.php:157](../../backend/src/Handlers/AdminGrowth.php))·`menu_audit_log`([AdminMenu.php:123](../../backend/src/Handlers/AdminMenu.php))·`dsar_audit_log`([Dsar.php:213](../../backend/src/Handlers/Dsar.php))·auth_audit_log 등 · `.githooks/baseline.json` sacred SHA(Version Gate) | **재사용(§49 Audit Event·도메인별 KEEP_SEPARATE)** |
| **★Scheduled 실행(예약)** | ✅ **REAL**(v2.1 신규 실측) — EmailMarketing **`scheduled_at`+`status 'draft'`+예약 큐+`attempts` 재시도**([EmailMarketing.php:57](../../backend/src/Handlers/EmailMarketing.php)/83/101-103)·KakaoChannel 예약·**예약 드레인 워커 cron**(286차 무음유실 종결) | **재사용(§19 SCHEDULED Activation·§7 SCHEDULED State)** |
