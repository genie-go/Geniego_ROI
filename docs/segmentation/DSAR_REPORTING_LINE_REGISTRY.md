...
- 🔴 **`evidence`(#22)는 `menu_audit_log` 스키마를 복제하지 말고 알고리즘(SHA-256 prev-chain)만 이식하라.** 원본은 `tenant_id` 가 없어 **테넌트 교차 오염**을 물려받는다. 테넌트별 체인은 `lastHash()` 에 `WHERE tenant_id=?` 가 **필수**.
