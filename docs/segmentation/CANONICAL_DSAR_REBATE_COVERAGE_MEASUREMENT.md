...
| ID | 항목 | 등급 |
|---|---|---|
| **MR-1-6-01** | **§53 요구 목록(분모) 저장소 영속화** — COV-GAP-01 → 🟡 **289차 ⑤ 부분 이행**: E-02 원장 **실체 신설** = [`COVERAGE_REQUIREMENT_REGISTRY_06A.md`](./COVERAGE_REQUIREMENT_REGISTRY_06A.md)(블록 16 · `source_persisted` 판정 · **측정불가 명시적 영속** · 복구절차). 🔴 **요구 목록 자체의 복원은 불가** — 원문이 채팅과 함께 소멸했고 **사용자만 보유**. ★**산출물에서 역산 금지**(분모=분자 → 정의상 100% = 가짜녹색) | 🟡 **부분**(잔여=사용자 원문 제공) |
| **MR-1-6-02** | **보고 52 vs 실측 47 판별** — MR-1-6-01 선행 필요 | 🔴 `UNVERIFIED` **불변** — 289차 ⑤가 분모를 **복원하지 못했으므로**(구조만 신설) 판별 조건 **미충족 유지** |
