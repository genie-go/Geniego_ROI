...
- **#8 historical reconstruction 의 이식 선례** = `pm_audit_log`(migration `20260526_168_008` — `tenant_id NOT NULL`+`entity`+`diff_json`+3인덱스+append-only). 🔴**`menu_audit_log` 스키마 복제 금지**(`tenant_id` 없음 · `lastHash()` 에 tenant 술어 없음) — **hash_chain 알고리즘만 이식**(`AdminMenu.php:128`,`:182-197`)하고 `WHERE tenant_id=?` 필수. 🔴 단 **`menu_audit_log` 는 쓰기 체인만 실재** — `verify()` 0·preimage ts(`:195`) 소실로 **tamper-evident 아님** → 검증형 정본(`SecurityAudit::verify():56-68`)의 검증기까지 이식해야 무결성 검증이 성립한다.
