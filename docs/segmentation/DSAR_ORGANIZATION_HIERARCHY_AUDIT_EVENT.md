...
| 계층 | 실측 | 판정 |
|---|---|---|
| ★**`menu_audit_log`** | **`hash_chain CHAR(64) DEFAULT NULL`**(`AdminMenu.php:128`) = **SHA-256 prev-chain 실구현**<br>· 생성 `:182-197` — `$prevHash = self::lastHash($pdo)`(`:182`) → payload 에 `'prev' => $prevHash`(`:194`)+`'ts'`(`:195`) 포함 → `$hash = hash('sha256', (string)$payload)`(`:197`) → INSERT 시 `hash_chain` 컬럼에 기록(`:199-210`)<br>· `lastHash()` `:214-219` — `SELECT hash_chain FROM menu_audit_log ORDER BY id DESC LIMIT 1`<br>· **tamper-evident 주석** `:18`<br>· 컬럼: `menu_id`·`action`·`old_value` JSON·`new_value` JSON·`changed_by`·`changed_by_role`·`reason`·`ip_address`·`user_agent`·`request_id`·`hash_chain`·`created_at`(`:123-129`)<br>· 마이그레이션 실재: `backend/migrations/20260526_168_102_create_menu_audit_log.sql`<br>· 🔴 **`tenant_id` 컬럼 없음**(전역 단일 메뉴 트리 도메인) | ★**`VALIDATED_LEGACY`(해시체인 패턴)** — **§63 의 정본 확장 대상** |
...
**실측 개수: 36 / 36 전사.** 판정 = `NOT_APPLICABLE` 36 / 36(이벤트 어휘 축). **단 저장 계층 축 = `VALIDATED_LEGACY`** — `menu_audit_log`/`pm_audit_log` 확장.
