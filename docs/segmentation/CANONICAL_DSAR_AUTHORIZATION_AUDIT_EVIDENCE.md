...
|---|---|---|
| **★Hash Chain(tamper-evident) 감사** | ⚠️ **연결 실재 · 검증 부재** — **`menu_audit_log.hash_chain CHAR(64)`**([AdminMenu.php:128](../../backend/src/Handlers/AdminMenu.php)/143) 의 **체인 연결은 실재**한다(`:194` `'prev'=>$prevHash` + `lastHash():216` `SELECT hash_chain … ORDER BY id DESC LIMIT 1`). 🔴**그러나 "tamper-evident" 주석(`:18`·`:166`)은 거짓이다**: preimage 의 `'ts'=>date('c')`(`:195`)가 **INSERT 컬럼 목록(`:199-203`)에 `created_at` 이 없어 저장되지 않는다**(`:129` DB DEFAULT 가 채움) → preimage 재구성 불가 → **변조 탐지가 원리적으로 불가능**. 게다가 **검증기도 없다**(`hash_equals` 레포 24히트 중 AdminMenu **0건**). ★**"체인이 있다"가 "변조를 탐지할 수 있다"를 보증하지 않는다.** 실 정본 = `SecurityAudit`(`verify():56-68`) | **`PARTIAL`** — 연결 실재·검증 영구 불가 → **감사 정본 아님 · 재사용 강제 근거 없음** |
| **★풍부한 감사 필드** | ✅ **REAL** — menu_audit_log: id · menu_id · action · **old_value JSON** · **new_value JSON** · **changed_by** · **changed_by_role** · **reason** · **ip_address** · **user_agent** · **request_id** · created_at · `KEY idx_audit_menu`(AdminMenu.php:123-131). **필드 목록 자체는 실재하며 유효한 선례다**(위 Hash Chain 행과 **분리 서술** — ⚠️`hash_chain` 은 검증 불가한 장식이므로 이 선례에서 제외). 🔴**단 `tenant_id` 부재**(DDL `:123-131`) — 멀티테넌트 도메인에 이식 시 반드시 추가 | **★VALIDATED_LEGACY — Evidence 필드 정본(필드 축 한정 · `tenant_id` 보강 조건부)** |
