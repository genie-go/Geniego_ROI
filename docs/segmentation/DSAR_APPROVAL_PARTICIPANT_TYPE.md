...
> **전사 근거**: [SPEC_06A_4_5_3_1_5_3_1_VERBATIM.md](SPEC_06A_4_5_3_1_5_3_1_VERBATIM.md) §19 Participant Type — 원문 그대로 전사.
> **분모 정합**: REQ 집계 15 ↔ **원문 실측 15 — 일치**.
...
스펙 §19 **원문 전사**(실측 15 · REQ 집계 15 와 **일치**):

| # | Participant Type | # | Participant Type |
|---|---|---|---|
| 1 | `REQUESTER` | 9 | `ESCALATION_OWNER` |
| 2 | `REQUESTED_FOR` | 10 | `DELEGATE_REFERENCE` |
| 3 | `APPROVER_CANDIDATE` | 11 | `SUBSTITUTE_REFERENCE` |
| 4 | `APPROVER` | 12 | `SYSTEM_ACTOR` |
| 5 | `REVIEWER` | 13 | `AUDITOR` |
| 6 | `OBSERVER` | 14 | `NOTIFICATION_RECIPIENT` |
| 7 | `RESOURCE_OWNER` | 15 | `OTHER` |
| 8 | `CASE_OWNER` | | |

**현행 커버리지 = 15 중 0종.** `participant_type` grep 0(§0 실측) — 열거형 자체가 부재하므로 **현행 어느 값도 위 15종에 매핑되지 않는다**.
유일한 인접 선례(`mapping_change_request` 의 `requested_by` ↔ `approvals_json[].user` 분리 저장 · `Mapping.php:268,285`)는
원문 기준 **`REQUESTER`(#1) 와 `APPROVER`(#4) 를 사후 구별 가능한 수준**이나, **유형 필드가 아니라 컬럼 배치의 부산물**이므로 커버리지 산입 불가.
...
- 원문 수령 후 확정: **15종 대비 현행 커버리지 = 0종**(전사 전 "0~1종 예상" 은 원문 대조로 **0종 확정**).

## 2. 규칙

- **§19 원문 15종이 정본** — 전사 완료(위 §1). 이후 항목명 변경·추가·삭제 금지(원문이 정본).
