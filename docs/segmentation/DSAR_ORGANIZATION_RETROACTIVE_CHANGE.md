...
- ★**감사 기록은 `menu_audit_log`/`pm_audit_log` 패턴 확장**(ⓑ §8). 🔴 **"해시체인 선례 없음"을 전역 명제로 인용하면 오염이다** — 참인 것은 **전역 `audit_log`(`Db.php:540-545`)에 한해서**다. `menu_audit_log.hash_chain`(`AdminMenu.php:128`)은 **SHA-256 prev-chain 쓰기 실구현**(`:182-197`)이다. ⚠️ 단 **쓰기 체인만** — `verify()` 0 · preimage ts(`:195`) 소실로 **tamper-evident 는 아니다**(검증형 정본 = `SecurityAudit::verify():56-68`).
