...
| **★Data Backfill** | ✅ **REAL**(v2.1 정정 — 초판 "부재"는 **오류**) — `Attribution::backfillOwnedTouches($pdo,$tenant,$orderId,$email,$phone,$orderAt,$windowDays=30)`([Attribution.php:282](../../backend/src/Handlers/Attribution.php))=**과거 터치 소급 생성(윈도 제한)** · 레거시 NULL→demo 데이터 보정(Alerting.php:57/214)·런타임 컬럼 backfill(UserAuth.php:571) | **재사용(§30 data_backfill_requirement·소급 생성 패턴)** |
| **Migration Batch / Checkpoint / 건수·금액 Validation / Dual Run / Shadow** | ❌ **부재(grep 0)** — `migration_batch/batch_sequence` 전무·배치 프레임워크 없음 | **NOT_APPLICABLE → 신설(§34·§35·§36)** |
| **Contract/Provider/Rule/Claim/Accrual/Settlement Migration·Historical Program Mapping** | ❌ **부재(grep 0 각각)** | **NOT_APPLICABLE → 신설(§32·§33·§38)** |
| **Contract Version / Product Taxonomy Version / Customer Identity Version / Provider Account Version Registry** | ❌ **부재(grep 0 각각)** — 스펙 §3 선행조건이나 실측 부재 | **NOT_APPLICABLE(신설·Reference 필드만)** |
| **Program Clone / Program Revision** | ❌ **부재** — `clone` 히트는 PHP object clone(ChannelSync.php:2200)·XML `cloneNode`(EnterpriseAuth.php:609)=**오탐** · `revision` 히트는 **Klaviyo API 버전 헤더**(Connectors.php:4670)=**오탐** | **NOT_APPLICABLE(신설)** |
