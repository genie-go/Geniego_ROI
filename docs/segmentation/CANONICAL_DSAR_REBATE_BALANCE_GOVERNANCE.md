...
> 선행: Funding(4-5-3-1-3 Commitment/Reservation)·Lifecycle(4-5-3-1-4)·Rule/Tier(선행설계 R1)·Eligibility(선행설계 R2 Holdback/Maturity).
> **범위**: 잔액 파생/집계/정합만 — Claim/Settlement/Payout/Recovery 실행 아님(후속 선행설계 R4~R5).
...
| **Budget/Cap 대비 잔여** | ✅ **REAL** — monthly_budget MTD cap·`SUM(budget) FROM auto_campaign WHERE status IN('active','pending','running')`(:437)·enforceBudgetCaps 97%(284차) | **재사용(§55 Cap 대비 잔여·선행설계 R1 §25c)** |
...
| **취소 제외 술어 SSOT** | ✅ **REAL** — 취소제외 **2축 통일**(Attribution 전환집합·배송비 OrderHub/Pnl·CRM affinity·286차) | **재사용(§52 Basis 정합·선행설계 R1 §23)** |
...
- **Aggregation Definition(§52)**: **포함 state 집합(§53)·포함 entry type·부호 규약·통화/FX 시점(선행설계 R1 §28a)·기간 경계·취소/반품 제외 술어(★현행 2축 SSOT 재사용·286차)·Gross/Net·VAT**·evidence. **★§4.6 평균의 평균 금지(ratio-of-sums)** — 비율 지표는 SUM(분자)/SUM(분모)(현행 정본: Rollup avg_return_rate 268차 정정). **★집계 정의 없는 Balance 표시 금지(무엇을 더했는지 불명=근거 없는 숫자·헌법 Vol4 Explainable)**.
- **Balance by State(§53b, 8)**: PENDING·RESERVED·ACCRUED·**LOCKED(Holdback)**·AVAILABLE·CLAIMED·SETTLED·PAID·REVERSED **각각 분리 표시**. **★§4.7 단일 "잔액" 숫자 금지 — 상태별 분리 필수**(Locked를 Available로 합산 표시=허위 가용잔액·선행설계 R2 §4.2 Qualification≠Availability 계승·Cashback 4-5-2-3 Pending≠Available 대칭).
...
- **Cap 대비 잔여(§55)**: 잔여 = **Cap/Commitment − (확정 + in-flight)**(선행설계 R1 §25c Cap·4-5-3-1-3 §16 Commitment 연결). **현행 재사용**: monthly_budget MTD cap·`SUM(budget) FROM auto_campaign WHERE status IN('active','pending','running')`(:437·**진행중 status 포함=§4.4 동일 원리**)·enforceBudgetCaps 97%(284차). **★잔여 음수 시 Accrual/Payout 생성 금지(fail-closed)**.
...
- **Carry-forward(§57b)**: 기간 미달/미청구 잔액의 차기 이월 정책(선행설계 R1 §20 "미달 시 CARRY_FORWARD" 연결)·이월 한도·이월 Entry.

## 6. Reconciliation (§59) · Discrepancy (§60) · Guard/Error (§61)

- **Reconciliation(§59, 14)**: **Balance↔원장 SUM(항등식)**·Snapshot↔원장·Statement opening+Σ=closing·**내부 잔액↔외부 지급 원장(★"우리 DB 가 아니라 매입사 원장이 진실"·BillingMethod.php:549 계승)**·Accrual↔Accrual Line 합계·Accrual↔Source·Reversal↔원본·true-up↔기 Accrual(선행설계 R1 §22b)·Reserved↔Commitment(4-5-3-1-3 §16)·in-flight↔실제 진행 작업·Locked↔Holdback(선행설계 R2 §20d)·통화별 잔액↔FX·hash-chain 무결성(Ledger §49)·Historical↔Applied.
