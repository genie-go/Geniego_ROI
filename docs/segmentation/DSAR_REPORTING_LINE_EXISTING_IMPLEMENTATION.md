...
| 30 | `BLOCKED_HISTORICAL_INTEGRITY_RISK` | 🔴**현행이 정면 반례**: `AgencyPortal.php:304`·`:381` 이 **`revoked_at=NULL` 로 이전 해지 시각을 소거** → **이력 물리적 소멸**(§55 "과거 Snapshot 대체 금지" 위반) · 선례 = `menu_audit_log.hash_chain`(`AdminMenu.php:128` SHA-256 prev-chain · `lastHash():214-219`) 🔴**`tenant_id` 없음 → 스키마 복제 금지·알고리즘만 이식** | `BLOCKED_HISTORICAL_INTEGRITY_RISK`(**차단 대상 실재**) |
