...
| **SEG-H2** | Audience Removal 부재 | Meta/Google/TikTok | 세그먼트 이탈·동의철회·삭제가 목적지에 전파 안 됨(Google 540일 잔존) | HIGH | 철회 후 잔존 타겟팅 | 광고 오디언스 | **잔여(라이브검증세션 대기)** | `AdAdapters.php`(users_remove/삭제op grep 무매치) | Removal orchestrator(Meta users_remove·Google remove op·TikTok delete). **289차 결정: 외부 3-provider API·라이브 광고계정 검증 필요→블라인드 구현 지양(287차 죽은스켈레톤 교훈). 설계=Part3-3-2 완료. 라이브 광고계정+배포 가능 세션에서 구현·즉시검증.** |
