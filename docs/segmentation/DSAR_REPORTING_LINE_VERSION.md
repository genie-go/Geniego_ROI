...
| 22 | immutable_hash | **알고리즘만 REAL** = `menu_audit_log.hash_chain CHAR(64)`(DDL `AdminMenu.php:128` · SHA-256 prev-chain `:182-197` · `lastHash():214-219`) · `schema_migrations.checksum`(`Migrate.php:50` `hash('sha256',$sql)`). 🔴**쓰기 체인만 실재·검증 불가 장식** — `menu_audit_log` 는 `verify()` 검증기 0(AdminMenu `hash_equals` 0) · preimage `'ts'=date('c')`(`:195`)가 INSERT 컬럼(`:199-203`)에 없어 `created_at` DB DEFAULT(`:129`)가 덮음 → 재계산 불가 → **tamper-evident 아님**(`:18` 주석≠근거). **검증형 정본 = `SecurityAudit::verify():56-68`**(`:64` hash_equals+prev_hash 교차 · `:31` preimage $now 를 created_at 에 저장→`:63` 재계산). 🔴**스키마 복제 금지** — `menu_audit_log` **`tenant_id` 없음** · **`lastHash():216` 에 tenant 술어 없음** → 테넌트별 체인 시 `WHERE tenant_id=?` **필수**. 권장 = `pm_audit_log` 스키마(`tenant_id NOT NULL` `…168_008:7`) + `SecurityAudit::verify` 의 검증형 알고리즘 | `LEGACY_ADAPTER` |
...
- 🔴 **`immutable_hash`(#22)는 `menu_audit_log` 을 검증형 해시체인 선례로 삼지 마라.** 그것은 **쓰기 체인만 실재**하고 `verify()` 검증기가 없으며(AdminMenu `hash_equals` 0), preimage `'ts'=date('c')`(`:195`)가 INSERT 컬럼에서 소실돼 `created_at` DB DEFAULT 가 덮으므로 **재계산이 불가능한 장식** = **tamper-evident 아님**(`:18` 주석은 근거가 아니다). **검증형 정본은 `SecurityAudit::verify():56-68`**(preimage 시각을 저장하고 재계산·`hash_equals`·prev_hash 교차)이며, 이식 대상은 이 검증형 알고리즘이다. 또한 원본은 `tenant_id` 가 없고 `lastHash():216` 에 tenant 술어가 없어 **테넌트 교차 체인**이 된다. 권장 = **`pm_audit_log` 스키마(`tenant_id NOT NULL`·`diff_json`·3인덱스·append-only) + `SecurityAudit::verify` 의 검증형 SHA-256 prev-chain**.
