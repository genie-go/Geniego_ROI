...
### 3-1. ✅ **GUARD-GAP-01 — `guard_headerless_getjson.mjs` 호출처 0 (실효 0)** — **289차 해소(배선 완료)**

> **289차 정정(승인 세션):** 아래 §3-1 의 결함 서술은 **발견 시점(5-8)에는 정확했고, 289차에 해소됐다.**
> `.githooks/pre-commit` 에 **G15** 로 배선(`frontend/src/**/*.{js,jsx,ts,tsx}` staged 시 트리거).
> **양방향 실증**: 위반 파일 stage → **hook exit 1 로 차단 확인** · 정상 import → **G15 통과 확인** · 현행 저장소 **위반 0건**.
> ⚠️ **단 "CI 가드" 라는 기존 서술은 배선 후에도 여전히 거짓이다** — pre-commit 은 **로컬 훅**이고
> `core.hooksPath` 는 **클론별 로컬 config**(새 클론·CI runner 는 미실행) · **`--no-verify` 우회 가능**.
> **정확한 현재 등급 = `WIRED(pre-commit, 로컬)` ≠ `CI_ENFORCED`.** CI 승격은 **별건(사슬 ④ B4 와 동일 과제)**.

**증명(부재증명 수행 · 발견 시점 기록 — 보존):**
...
→ **289차 승인 세션에서 배선 완료.** 예측대로 **판단이 아니라 승인 문제였고**, 승인 즉시 해소됐다
(가드 자체는 275차 이후 무수정 — **파일은 처음부터 옳았고 배선만 없었다**).
**잔여 = CI 승격**(`WIRED(로컬)` → `CI_ENFORCED`) → **GUARD-GAP-02 와 병합해 추적**.

### 3-2. 🟡 **GUARD-GAP-02 — pre-commit은 강제가 아니다** — **289차 ④ 부분 해소**

> **289차 정정(승인 세션 ④):** 아래 "세 겹의 구멍" 중 **①②를 CI 실행으로 해소**했다.
> `.github/workflows/security-scan.yml`(283차)에 **`repo-guards` 잡 추가** — **B4**(자격증명) + **G15**(인가 회귀).
> **283차 파일을 확장**했고(신규 워크플로 신설 안 함) `deploy.yml` 은 **무접촉**이다.
> **의존성 스캔의 실행 조건은 283차 그대로 보존**(`if: github.event_name != 'push'`) — push 트리거는 `repo-guards` 전용.
>
> ★**B4 규칙은 `tools/scan_secrets.sh` 로 SSOT 이관** — 훅과 CI 가 **같은 스크립트**를 부른다.
> 정규식을 CI 에 복사했다면 **규칙이 두 벌로 갈라져 따로 낡는다**(289차 ② '351 15벌'과 정확히 같은 클래스).
> ★**리포트 전용이 아니라 차단 게이트**(`continue-on-error` 없음) — 현행 위반 0 실측이라 차단해도 무영향이고,
> 리포트 전용으로 두면 **"가드가 있는데 아무것도 막지 않는" 275차 사고를 CI 에서 재현**하게 된다.
>
> 🔴 **③ `--no-verify` 는 해소되지 않았고, 더 중요한 한계가 남는다 → `G-06b`(신설)**:
> **탐지(detection)이지 예방(prevention)이 아니다.** 브랜치 보호·required check 가 없어
> **master 직 push 시 `repo-guards` 실패는 배포를 막지 못하고 사후 통보만** 한다(`deploy.yml` 은 독립 트리거).
> **진짜 차단은 GitHub 저장소 설정**(브랜치 보호 + required check)이며 **코드로 해결 불가 · 사용자 결정사항**이다.
> ⚠️ **"이제 CI 가드가 있으니 안전"으로 읽지 말 것** — 그 오독이 정확히 §3-1(275차)이 4층을 오염시킨 방식이다.
...
| ID | 항목 | 등급 | 비고 |
|---|---|---|---|
| ~~**MR-5-8-01**~~ | ~~`guard_headerless_getjson.mjs` **배선**(pre-commit 또는 CI)~~ → ✅ **289차 CLOSED** — `.githooks/pre-commit` **G15** 배선·양방향 실증 | ✅ | 예측대로 **판단 아닌 승인 문제** |
| ~~**MR-5-8-01b**~~ | ~~**G15 CI 승격**~~ → ✅ **289차 ④ CLOSED** — `security-scan.yml` `repo-guards` 잡(B4 와 함께 승격·예고대로 **묶어서 해결**) | ✅ | 예고대로 GUARD-GAP-02 와 **동일 근본**이었다 |
| **MR-5-8-01c** | 🔴 **브랜치 보호 + required check** — 없으면 `repo-guards` 는 **탐지일 뿐 예방이 아니다**(master 직 push 를 못 막음) · `--no-verify` 잔존 | 🔴 | **GitHub 저장소 설정 · 코드 불가 · 사용자 결정**(= `G-06b`) |
