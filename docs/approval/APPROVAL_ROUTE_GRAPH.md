...
| §18 #22 · 전 섹션 evidence | `backend/src/SecurityAudit.php:27`,`:45-52`,`:56-68` | tenant 포함 해시 · `prev_hash`/`hash_chain` DDL · **`verify()` `hash_equals` 검증기**. 🔴**`menu_audit_log.hash_chain` 을 선례로 인용 금지** — preimage 의 `'ts'=>date('c')`(`AdminMenu.php:195`) 가 **`:199-203` INSERT 컬럼 목록에 `created_at` 이 없어 미저장**(`:129` DB DEFAULT) → **재구성 불가** · `hash_equals` grep 0(`AdminMenu` 내) = 검증 불가능한 장식. ★근거 정정 — `prev` 는 `lastHash():216` 이 직전 행 `hash_chain` 을 읽어 공급하므로 재구성 가능. |
