...
| 36 | `APPROVAL_CHAIN_AUDIT_EVENT` | 승인 도메인 0. 감사 정본 선례 = `SecurityAudit`(`backend/src/SecurityAudit.php:27` tenant 포함 해시 · `:45-52` DDL `prev_hash`/`hash_chain` · `verify():56-68` `hash_equals`). 🔴 `menu_audit_log.hash_chain` 은 **검증 불가 장식**(인용 금지 — 막히는 축 = preimage 의 `ts`(`AdminMenu.php:195`) 가 `:199-203` INSERT 컬럼 목록에 `created_at` 이 없어 미저장 · ★근거 정정: `prev` 는 `lastHash():216` 으로 재구성 가능) | ABSENT |
...
7. **검증 불가 해시 장식** — `menu_audit_log.hash_chain`(★근거 정정 · 289차 10회차 ⓔ: 막히는 축은 **`ts` 하나**로 preimage 의 `'ts'=>date('c')`(`AdminMenu.php:195`) 가 **`:199-203` INSERT 컬럼 목록에 `created_at` 이 없어 미저장**(`:129` DB DEFAULT) → 재구성 불가 · `hash_equals` grep 0. `prev` 는 `lastHash():216` 으로 재구성 가능하므로 `prev_hash` 컬럼 부재는 결함이 아니다). **감사 정본 선례 = `SecurityAudit`**(`:27` tenant 포함 해시 · `:45-52` DDL · `verify():56-68`).
