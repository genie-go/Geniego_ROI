...
| SonarQube | 없음 |
| 테스트 프레임워크 | **없음** (`npm test` · PHPUnit 부재) |
...
| 항목 | 사유 |
|------|------|
| Unit Test | 테스트 스위트가 **존재하지 않는다**(`npm test`·PHPUnit 부재) |
...
| 런타임 회귀 확인 | **추천코드 검증 복구는 코드 레벨 수정까지만 확인**했다. 실제 화면에서 "추천인 확인" 이 뜨는지는 배포 후 확인 필요 |
...
| 4 | 테스트 스위트 부재 | Part004 범위 밖이나, 품질 게이트의 최대 공백이다 |
