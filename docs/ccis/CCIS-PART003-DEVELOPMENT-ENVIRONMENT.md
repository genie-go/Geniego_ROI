...
| 구분 | 도구 | 실측 버전 | 판정 |
|------|------|-----------|------|
| 필수 | Node.js | **24.16.0** | 충족 (단 CI 와 major 불일치 — §8) |
| 필수 | npm | 11.13.0 | 충족 |
...
| # | 항목 | 현황 | 영향 |
|---|------|------|------|
| 1 | **Node 버전 3중 분기** | 로컬 24 · CI 18(`deploy.yml:43,88`) · `frontend/Dockerfile` 20. `engines`·`.nvmrc` 없음 | Part003 목표 "CI/CD와 동일한 Build 결과" 미달. 통일 시 어느 버전으로 맞출지 결정 필요(CI 를 올릴지, 로컬을 내릴지) |
