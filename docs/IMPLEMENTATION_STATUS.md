...
- **★266차(2026-07-06 완료·배포·검증) 전메뉴 1:1 전수스윕(104P+89H=193/193)에서 검출한 HIGH2 수정**: ①`ResultSection.jsx:142` 가 참조하던 `<ChannelResultCard>` 가 231차 pages_backup 삭제 시 소실(259차 ResultSection 추출 이후 잠복)→`/ai-recommend` 채널탭 렌더 ReferenceError 화이트스크린. 손상된 자동번역 원본 복원 대신 현 프리미티브(AdMockup export·CHANNEL_COLORS/ICONS)로 승인/집행/재생성 워크플로 보존한 클린 `ChannelResultCard` 재구성(`ResultSection.jsx`·`AIRecommendTab.jsx` AdMockup export). ②`ReturnsPortal::updateStatus` 정산전파 블록이 returns.sqlite3(3테이블만) PDO로 orderhub_claims/channel_orders 조회→무음실패였던 것 `Db::pdo()` 라우팅(수동 반품 승인 시 returnFee claim+원월 재롤업 복구). +MED3(Health 무인증정보 차폐·EventNorm creator_handle·CRM 거짓토스트→이동안내). 잔여 LOW 백로그·확정양호 상세=FP레지스트리 266차. 빌드EXIT0·번들토큰실재·홈200 검증.
...
## 13-B) 워크스페이스 상태 영속 (WorkspaceState) — ✅ 266차 신설(재구현 금지)
- **테넌트 스코프 범용 KV**: `tenant_kv`(tenant_id,skey,sval·PK(tenant_id,skey)) + `WorkspaceState`(GET/POST `/v424/workspace`, 세션 self-auth authedTenant, key 화이트리스트, 512KB캡, 데모 write skip). app_setting 은 전역·민감(cred_enc_key 등)이라 재사용 불가 → 테넌트 격리 신설. `WorkspaceState.php`·`routes.php`·`index.php` bypass. 라이브 E2E(admin PUT/GET roundtrip·badkey 422) 확인.
- **운영 영속 배선(그간 localStorage/state 전용→새로고침 소실 해소)**: ContentCalendar(calendar_events)·FeedbackCenter(feedback_entries)·Approvals 설정탭(approval_cfg)·CatalogSync 4보조탭(catalog_price_rules/inventory/schedules/category_map). 프론트 헬퍼 `services/workspaceState.js`(데모=no-op·기존 localStorage 유지). 신규 페이지 편입 시 KEYS 화이트리스트에 추가.
- **CampaignManager pause/resume 실 매체 반영**: 인메모리 no-op → `POST /v423/auto-campaign/status`(AutoCampaign::setStatus·AdAdapters 실 pause/activate) 재사용 배선(신규 EP 0). 실 백엔드 캠페인=매체 반영·로컬 id=422 무해·데모=로컬.
