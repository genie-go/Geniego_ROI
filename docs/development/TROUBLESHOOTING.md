...
**현재 상태**: Node 버전 정본은 **`.nvmrc`(20)** 하나다. CI 두 워크플로 4개소가
`setup-node` 의 `node-version-file` 로 이 파일을 읽고, `package.json` 의 `engines` 는 `>=20` 이다.

로컬이 24 여도 `engines` 는 충족하므로 빌드는 된다(검증: `✓ built in 50.52s`).
다만 CI 와 **동일 산출물**을 보장하려면 `nvm use` 로 20 을 쓰는 편이 안전하다 —
`check-environment.sh` 가 이 차이를 WARN 으로 알려준다.

버전을 올릴 때 고칠 곳은 **`.nvmrc` 와 `frontend/Dockerfile` 의 `ARG NODE_VERSION` 둘뿐**이다.
