const fs = require('fs');
const file = 'd:/project/GeniegoROI/frontend/src/pages/AIInsights.jsx';

const content = `import React, { useState, useRef, useEffect, useCallback, useMemo } from "react";
            ...
            <div style={{ maxWidth: '80%', padding: '14px 18px', borderRadius: isAI ? '6px 18px 18px 18px' : '18px 6px 18px 18px', background: isAI ? 'rgba(15,23,42,0.8)' : 'linear-gradient(135deg,#4f8ef7,#3b82f6)', border: \`1px solid \${isAI ? 'rgba(79,142,247,0.2)' : 'rgba(255,255,255,0.1)'}\`, fontSize: 13, lineHeight: 1.7, color: isAI ? '#e2e8f0' : '#ffffff', boxShadow: '0 8px 24px rgba(0,0,0,0.1)' }}>
                {loading ? (
                    ...
                    <>
                        ...
                        {insight?.bullets && <div style={{ marginTop: 12, display: 'grid', gap: 6, background: 'rgba(255,255,255,0.03)', padding: 12, borderRadius: 10 }}>{insight.bullets.map((b, i) => <div key={i} style={{ display: 'flex', gap: 8, color: '#cbd5e1' }}><span style={{ color: '#a855f7' }}>✦</span><span>{b}</span></div>)}</div>}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: 20, alignItems: 'start' }}>
            ...
            <div style={CARD}>
                ...
                <div style={{ fontSize: 11, color: '#94a3b8', textAlign: 'center', marginTop: 12, display: 'flex', justifyContent: 'center', gap: 16 }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><div style={{ width: 10, height: 3, background: '#4f8ef7' }}/> Actual Data</span>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><div style={{ width: 10, height: 3, borderTop: '2px dashed #a855f7' }}/> ML Prediction</span>
                </div>
            </div>
        </div>
    );
                ...
                <div style={{ display: 'flex', gap: 4, background: 'rgba(30,41,59,0.5)', padding: 6, borderRadius: 16, border: '1px solid rgba(255,255,255,0.05)', backdropFilter: 'blur(10px)' }}>
                    {TABS.map(tb => (
                        <button key={tb.id} onClick={() => setTab(tb.id)} style={{
                            flex: 1, padding: '12px', border: 'none', cursor: 'pointer', textAlign: 'center', borderRadius: 12,
...
console.log('AI Insights ultra-advanced UI applied via script.');
