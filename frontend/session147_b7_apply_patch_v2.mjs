// session147_b7_apply_patch_v2.mjs
// V2: Resolve each entry by FULL DOTTED PATH, not bare finalKey.
...
//
// Safety guards (N-145-B, 7 of them) — same as v1:
//   G1. Backup ko.js to backup_session147_B7/
...
//
// Usage:
//   node session147_b7_apply_patch_v2.mjs              (dry-run)
//   node session147_b7_apply_patch_v2.mjs --apply      (commit)
...
const LOG_PATH   = path.join(ROOT, 'b7v2_patch_log.txt');
...
async function main() {
  ...
  const logLines = [];
  logLines.push(`# B7 v2 patch log — ${new Date().toISOString()}`);
  ...
  console.log('================ B7 V2 PATCH SUMMARY ================');
  ...
  console.log('=====================================================');
  console.log('[SUCCESS] B7 v2 apply complete. All 7 guards passed.');
