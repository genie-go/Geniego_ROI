...
if (!enContent.includes('explainTitle')) {
    // marketing 섹션에서 첫 번째로 나오는 cat_platform 키 다음에 삽입
    enContent = enContent.replace(
        /(cat_platform: "[^"]*",)/,
        '$1\n' + EN_KEYS
    );
    fs.writeFileSync(enFile, enContent, 'utf8');
    console.log('DONE en.js');
} else {
    console.log('SKIP en.js');
}

// ko.js, ja.js, zh.js에 추가
['ko', 'ja', 'zh'].forEach(lang => {
    const data = LOCALES_DATA[lang];
    const file = path.join(localesDir, lang + '.js');
    let c = fs.readFileSync(file, 'utf8');
    if (c.includes('explainTitle')) { console.log('SKIP ' + lang); return; }
    const keys = Object.entries(data).map(([k, v]) => `    ${k}: "${v.replace(/"/g, '\\"')}",`).join('\n');
    c = c.replace(/(cat_platform: "[^"]*",)/, '$1\n' + keys);
    fs.writeFileSync(file, c, 'utf8');
    console.log('DONE ' + lang);
});

// de, th, vi, id, zh-TW는 영어로 fallback (en.js에 있으므로 i18n deepGet이 자동 처리)
console.log('de/th/vi/id/zh-TW will fallback to en.js (deepGet fallback)');
