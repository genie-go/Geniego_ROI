// Restore corrupted Korean/CJK text in DashInfluencer.jsx
// The ASCII code structure is intact but all Korean/CJK chars became replacement chars (U+FFFD)
// Strategy: Read the file, extract the code structure, and re-inject proper LOC dictionary
...
// The LOC dictionary spans from "const LOC = {" to the closing "};"
// We need to replace the entire LOC block with a fresh one

// Find the LOC block boundaries
const locStart = content.indexOf('const LOC = {');
const locEndMarker = '\n// ── Reusable';
const locEnd = content.indexOf(locEndMarker, locStart);

if (locStart === -1 || locEnd === -1) {
  console.log('Could not find LOC block boundaries');
  console.log('LOC start:', locStart, 'LOC end:', locEnd);
  
  // Try alternate marker
  const altEnd = content.indexOf('\n// ──', locStart + 100);
  console.log('Alt end:', altEnd);
  process.exit(1);
}

console.log(`LOC block: ${locStart} to ${locEnd}`);
...
const FRESH_LOC = `const LOC = {
  ...
  zh: {
    ...
    aiEngineTitle:'Claude AI 达人分析引擎',
    aiEngineDesc:'{count}位创作者 × 5维度全数据 Claude {model} 分析',
    aiRunning:'🧠 Claude AI 分析中...(15~30秒)', aiRun:'🧠 执行AI分析',
    aiResultTitle:'AI综合分析报告', tokensUsed:'Token使用', model:'模型',
    noCreators:'暂无注册创作者',
    selectCreator:'从左侧创作者列表中选择，或连接API后自动加载数据',
    threatsBlocked:'威胁已拦截', secureConnection:'安全连接',
    colFollowers:'粉丝', colLikes:'点赞', colComments:'评论',
    colRevenue:'收益', colPurchases:'购买数', colShares:'分享',
    colSaves:'收藏', colEngRate:'互动率',
    colScore:'评分', colCampaigns:'活动',
    filterAll:'全部', filterActive:'活跃', filterPaused:'暂停', filterCompleted:'完成',
    sortFollowers:'按粉丝', sortEngagement:'按互动', sortRevenue:'按收益',
  ...
  'zh-TW': {
    ...
    quality:'📐品質', report:'📊 報告', ai:'🧠 AI分析',
    noCreators:'尚無註冊的創作者',
    selectCreator:'從左側創作者列表中選擇',
  ...
  de: {
    ...
    noCreators:'Keine Creator registriert',
    selectCreator:'Wählen Sie einen Creator aus der Liste',
...
// Replace the LOC block
const before = content.substring(0, locStart);
const after = content.substring(locEnd);
content = before + FRESH_LOC + after;

// Verify no replacement chars remain in LOC area
const newLocEnd = content.indexOf(locEndMarker);
const locSection = content.substring(locStart, newLocEnd);
const badChars = (locSection.match(/\ufffd/g) || []).length;
console.log(`LOC section replacement chars: ${badChars}`);

// Check remaining bad chars in the rest of the file
const totalBad = (content.match(/\ufffd/g) || []).length;
console.log(`Total remaining replacement chars: ${totalBad}`);

if (totalBad > 0) {
  // Fix remaining replacement chars in component code
  // These would be in Korean strings inside component functions
  // Replace common patterns that were corrupted
  
  // Show context around remaining bad chars
  let idx = 0;
  let count = 0;
  while ((idx = content.indexOf('\ufffd', idx)) !== -1 && count < 5) {
    const lineStart = content.lastIndexOf('\n', idx);
    const lineEnd = content.indexOf('\n', idx);
    const lineNum = content.substring(0, idx).split('\n').length;
    console.log(`  L${lineNum}: ...${content.substring(Math.max(lineStart, idx-30), Math.min(lineEnd, idx+30))}...`);
    idx++;
    count++;
  }
}
...
console.log('LOC dictionary restored and saved!');
