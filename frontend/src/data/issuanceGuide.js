...
export const ISSUANCE_GUIDE_KO = {
  ...
  twitch: [
    '준비물: Twitch 계정이 필요합니다. (앱 등록을 위해 계정에 2단계 인증이 활성화돼 있어야 합니다.)',
    'dev.twitch.tv/console 에 접속해 [Twitch 로그인] 버튼으로 로그인합니다. (아래 화면)',
    '로그인하면 [개발자 콘솔]이 열립니다. 우측 "응용 프로그램" 영역의 [내 응용 프로그램 등록] 버튼을 누릅니다. (아래 화면)',
    '[내 응용 프로그램 등록] 폼에서 [이름] 칸에 애플리케이션 이름(3~100자)을 입력합니다. (예: GenieGoROI 연동)',
    '[OAuth 리디렉션 URL] 칸에 아래 [OAuth 리디렉션 URL] 박스의 [복사] 버튼으로 복사한 값을 붙여넣고 우측 [추가] 버튼을 누릅니다. ★"리디렉션 URI는 HTTPS 프로토콜을 사용해야 합니다" 안내대로 https 값이며, 끝 슬래시·대소문자까지 완전히 일치해야 합니다. (아래 화면)',
    '[범주]는 "Analytics Tool"(분석 도구) 또는 "기타"를 선택하고, [클라이언트 유형]은 "기밀"(Confidential)로 둔 뒤 하단 [만들기] 버튼을 누릅니다.',
    '생성이 완료되면 [개발자 애플리케이션] 목록에 앱이 나타나고 [링크]에 방금 입력한 리디렉션 URL이 표시됩니다. [관리]를 눌러 Client ID를 확인하고, [New Secret](비밀키 생성)으로 Client Secret을 발급합니다. ★Secret은 한 번만 표시되니 즉시 복사해 두세요. (아래 화면)',
    'GenieGo 연동허브 > "Twitch" [등록]에 client_id, client_secret, 그리고 조회할 채널 로그인명(login: twitch.tv/ 뒤의 이름)을 입력해 저장하면 채널 통계가 바로 동작합니다.',
    '※ 채널 조회수·동시시청자 통계는 client_id/secret(App Access Token)만으로 동작하며, 위 리디렉션 URL은 Twitch가 앱 등록 시 필수로 요구하는 항목입니다(브로드캐스터 팔로워 등 권한 데이터 연동 시 사용).',
  ],
  coupang: [
  ...
  naver_smartstore: [
    '준비물 ①: 스마트스토어 판매자센터 계정에서 "통합매니저" 또는 "판매자(대표)" 권한이 필요합니다. 일반 스태프 권한으로는 API 애플리케이션을 발급할 수 없습니다(권한 부족 시 애플리케이션 등록 메뉴가 보이지 않거나 저장이 거부됩니다).',
    '준비물 ②: 커머스 API는 "네이버 커머스 API 센터"에서 발급합니다. 검색광고(searchad)·네이버쇼핑(developers) API 센터와 다른 곳이니 반드시 apicenter.commerce.naver.com 으로 접속하세요.',
    'apicenter.commerce.naver.com (네이버 커머스 API 센터)에 접속해 판매자센터와 동일한 네이버 계정으로 로그인합니다.',
    '최초 이용이면 우측 상단 [계정 생성]을 눌러 개발업체(회사) 정보·서비스 URL·장애대응 담당자 연락처를 입력하고 이용약관에 동의해 개발자 계정을 생성합니다.',
    '[애플리케이션] > [애플리케이션 등록]에서 애플리케이션 이름·설명을 입력하고, 사용할 API 권한(상품/주문/정산/문의 등)을 선택해 등록합니다. 등록 즉시 "애플리케이션 ID(client_id)"와 "애플리케이션 시크릿(client_secret)"이 발급됩니다.',
    '★중요(자주 실패하는 지점): 애플리케이션 상세에서 API를 호출할 "서버 IP 주소"를 반드시 등록해야 합니다. IP 미등록 시 인증은 되더라도 실제 API 호출이 "허용되지 않은 IP" 오류로 거부됩니다. (GenieGo 서버 IP는 연동허브 등록 화면에 안내된 값을 넣으세요.)',
    '★중요(전자서명 인증 방식): 커머스 API는 client_secret 을 키로 한 전자서명(bcrypt 해시 기반 서명)으로 토큰을 발급합니다. GenieGo는 이 서명·토큰 발급을 서버에서 자동 처리하므로, 여러분은 client_id / client_secret 두 값만 정확히 입력하면 됩니다(시크릿 앞뒤 공백·줄바꿈이 섞이지 않도록 복사에 주의).',
    '★중요(애플리케이션 인증 갱신): 커머스 API 센터는 보안상 애플리케이션 인증을 주기적으로 갱신하도록 요구합니다. 갱신이 만료되면 토큰 발급이 막히므로, API 센터 안내 메일이 오면 재인증(갱신)해 두세요. 갱신해도 client_id/secret 값 자체는 바뀌지 않아 재입력은 불필요합니다.',
    'GenieGo 연동허브 > "네이버 스마트스토어" [등록]에 client_id(애플리케이션 ID)와 client_secret(애플리케이션 시크릿)을 입력하고 저장합니다. 저장 즉시 서버가 토큰을 발급해 상품·주문·정산 동기화를 시작합니다.',
    '연동 확인: 저장 후 "연동완료(초록)"로 표시되면 정상입니다. 만약 "미완료/오류"로 뜨면 (1) IP 등록 여부 (2) 시크릿 값 공백 혼입 (3) 애플리케이션 인증 만료 (4) 통합매니저 권한 여부 순으로 점검하세요. 상품이 채널에 등록되는데 매핑이 안 되면 판매자센터의 상품이 채널상품(channelProducts)으로 정상 연결됐는지 확인합니다.',
  ],
  st11: [
    "준비물: 11번가 셀러(판매자) 계정이 필요합니다. ★일반회원으로 가입했다면 셀러오피스에서 반드시 \"Seller로 회원전환\"을 먼저 하세요. 일반 키로는 상품·카테고리 \"조회\"만 되고 상품등록·주문관리는 할 수 없습니다.",
    "openapi.11st.co.kr (11번가 OPEN API CENTER)에 셀러 아이디로 로그인합니다.",
    "상단 [API 관리] > [서비스 등록·확인] > [서비스등록]에서 셀러 정보를 입력하고 약관에 동의하면 OPEN API KEY(32자리)가 발급됩니다.",
    "★[서비스 등록·확인] > [Seller API 정보 수정] > \"접속권한\"에서 API를 호출할 서버의 IP 주소를 등록합니다. IP를 등록해야만 셀러 API 키가 승인됩니다(IP 미등록 = 승인 불가). 카페24·메이크샵 호스팅은 \"호스팅여부\"에서 선택하면 자동 등록됩니다.",
    "[마이페이지] > [API 이용신청/승인 현황]에서 상품 API·주문 API가 \"승인완료\"인지 확인합니다.",
    "[서비스확인]에서 발급된 API Key(32자리)를 복사합니다.",
    "GenieGo [등록]에 api_key와 seller_id(셀러 ID)를 입력해 저장합니다.",
    "★상품등록 전 준비물: 11번가는 (1) 표시카테고리 (2) 브랜드 (3) 판매방식이 모두 있어야 등록됩니다. 표시카테고리는 [카탈로그 동기화] > [카테고리 매핑]에서, 브랜드는 [카탈로그 동기화] > [브랜드 관리]에서 지정하세요(판매방식은 고정가판매로 자동 전송). 오류 \"-997 등록된 API 정보가 존재하지 않습니다\"는 키·IP 문제가 아니라 호출 주소 문제이니 키를 재발급하지 마세요.",
  ],
  gmarket: [
  ...
  toss: [
    ...
    'GenieGo [등록]에 secret_key를 입력해 저장합니다(client_key 는 결제위젯용 · 정산수집 불필요 · 선택).',
  ],
  kcp: [
  ...
  stripe: [
    ...
    'GenieGo [등록]에 secret_key를 입력해 저장합니다(publishable_key 는 클라이언트용 · 정산수집 불필요 · 선택).',
  ],
  paddle: [
  ...
  checkout: [
    ...
    'GenieGo [등록]에 secret_key를 입력해 저장합니다(public_key 는 클라이언트용 · 정산수집 불필요 · 선택).',
  ],
  mollie: [
  ...
  dhl: [
    ...
    'GenieGo [등록]에 api_key를 입력해 저장합니다(account_number(계정번호)는 요율·예약 시에만 입력 · 선택).',
  ],
  fedex: [
    '준비물: FedEx 계정번호가 필요합니다.',
    'developer.fedex.com 에 가입하고 로그인합니다.',
    '프로젝트를 생성하면 API Key와 Secret Key가 발급됩니다.',
    'GenieGo [등록]에 api_key, api_secret를 입력해 저장합니다(account_number 는 선택).',
  ],
  ups: [
    '준비물: UPS 계정번호가 필요합니다.',
    'developer.ups.com 에 가입하고 앱을 생성합니다.',
    'client_id와 client_secret을 발급받습니다.',
    'GenieGo [등록]에 client_id, client_secret를 입력해 저장합니다(account_number 는 선택).',
  ],
  google_analytics: [
  ...
  instagram: [
    '준비물: Meta(Facebook) 개발자 계정 + 페이스북 페이지에 연결된 Instagram 비즈니스/크리에이터 계정이 필요합니다.',
    'developers.facebook.com > [내 앱] > [앱 만들기]에서 비즈니스 유형 앱을 생성합니다.',
    '[Instagram Graph API] 제품을 추가하고 앱 검수(App Review)로 instagram_basic 등 권한을 신청합니다.',
    '시스템 사용자/페이지 토큰으로 액세스 토큰을 발급하고 IG 비즈니스 계정 ID를 확인합니다.',
    'GenieGo [등록]에 access_token, ig_user_id 를 입력해 저장합니다.',
  ],
  facebook: [
    '준비물: Meta(Facebook) 개발자 계정 + 관리 권한이 있는 페이스북 페이지가 필요합니다.',
    'developers.facebook.com > [내 앱] > [앱 만들기]에서 비즈니스 유형 앱을 생성합니다.',
    '[페이지]/[Facebook 로그인] 제품을 추가하고 앱 검수로 pages_read_engagement 등 권한을 신청합니다.',
    '그래프 API 탐색기 또는 시스템 사용자로 페이지 액세스 토큰을 발급하고 페이지 ID를 확인합니다.',
    'GenieGo [등록]에 access_token(페이지 토큰), page_id 를 입력해 저장합니다.',
  ],
  kakao_moment: [
    '준비물: 카카오모먼트 광고계정 + 카카오 비즈니스 채널. ★오픈API 권한은 카카오 공식 광고대행사에만 부여됩니다(일반 광고주 자가발급 불가).',
    'developers.kakao.com 에서 앱을 생성하고 [비즈니스 앱]으로 전환합니다.',
    '카카오모먼트 오픈API [추가기능 신청] — 공식 대행사 자격 검토 후 권한이 부여됩니다.',
    '권한 승인 후 OAuth 인가로 액세스 토큰을 발급하고 광고계정 ID를 확인합니다(연동허브 OAuth 원클릭 가능).',
    'GenieGo [등록]에 access_token, ad_account_id 를 입력해 저장합니다.',
  ],
  naverpay: [
    '준비물: 네이버페이 가맹점 가입(계약)이 완료되어야 합니다. ★자가발급이 아니라 계약 후 자격증명이 전달됩니다.',
    '네이버페이 비즈니스 센터에서 가맹 계약을 신청·완료합니다.',
    '심사·계약 완료 후 담당자로부터 Client ID·Client Secret·Chain ID 를 전달받습니다.',
    'GenieGo [등록]에 client_id, client_secret, chain_id 를 입력해 저장합니다.',
  ],
  cj: [
    '준비물: CJ대한통운 계약(고객) 코드가 필요합니다(계약형).',
    '★배송 추적만 필요하면 [스마트택배(통합 추적)] 채널에 t_key 하나로 전 택배사를 연동할 수 있습니다(권장).',
    'CJ대한통운 전용 API가 필요하면 영업 담당으로 계약(고객) 코드와 API 키를 발급받습니다.',
    'GenieGo [등록]에 api_key 를 입력해 저장합니다(cust_code 는 전용 API 계약 시에만 입력 · 선택).',
  ],
  lotte: [
    '준비물: 롯데글로벌로지스 계약(고객) 코드가 필요합니다(계약형).',
    '★배송 추적만 필요하면 [스마트택배(통합 추적)] 채널에 t_key 하나로 전 택배사를 연동할 수 있습니다(권장).',
    '전용 API가 필요하면 영업 담당으로 계약(고객) 코드와 API 키를 발급받습니다.',
    'GenieGo [등록]에 api_key 를 입력해 저장합니다(cust_code 는 전용 API 계약 시에만 입력 · 선택).',
  ],
  hanjin: [
    '준비물: 한진택배 계약(고객) 코드가 필요합니다(계약형).',
    '★배송 추적만 필요하면 [스마트택배(통합 추적)] 채널에 t_key 하나로 전 택배사를 연동할 수 있습니다(권장).',
    '전용 API가 필요하면 영업 담당으로 계약(고객) 코드와 API 키를 발급받습니다.',
    'GenieGo [등록]에 api_key 를 입력해 저장합니다(cust_code 는 전용 API 계약 시에만 입력 · 선택).',
  ],
  logen: [
    '준비물: 별도 발급 없이 스마트택배(스윗트래커) 통합 추적 키로 연동됩니다.',
    'tracking.sweettracker.co.kr 에 가입해 t_key 를 발급합니다.',
    '연동허브에서 [스마트택배(통합 추적)] 채널을 선택합니다(로젠택배 채널 대신).',
    'GenieGo [등록]에 t_key(api_key)를 입력하면 로젠택배 배송추적이 연동됩니다.',
  ],
  epost: [
    '준비물: 별도 발급 없이 스마트택배(스윗트래커) 통합 추적 키로 연동됩니다.',
    'tracking.sweettracker.co.kr 에 가입해 t_key 를 발급합니다.',
    '연동허브에서 [스마트택배(통합 추적)] 채널을 선택합니다(우체국택배 채널 대신).',
    'GenieGo [등록]에 t_key(api_key)를 입력하면 우체국택배 배송추적이 연동됩니다.',
  ],
  ocl_sameday: [
    '준비물: 지니고 당일배송 서비스 계약이 필요합니다(운영팀).',
    '운영팀에 당일배송 이용을 신청·계약합니다.',
    '계약 완료 후 머천트 ID·API 키·서비스 권역을 발급받습니다.',
    'GenieGo [등록]에 api_key, merchant_id, region 을 입력해 저장합니다.',
  ],
  fulfillment: [
    '준비물: 3PL 풀필먼트 입출고 위탁 계약이 필요합니다.',
    '풀필먼트 사업자/운영팀과 위탁 계약을 체결합니다.',
    '계약 완료 후 창고 ID·3PL API 키를 발급받습니다.',
    'GenieGo [등록]에 api_key, warehouse_id 를 입력해 저장합니다.',
  ],
  ems: [
    '준비물: 우체국 국제특송(EMS) 계약(고객) 코드가 필요합니다(계약형).',
    '인터넷우체국/계약창구에서 EMS 계약을 진행합니다.',
    '계약 완료 후 고객(계약) 코드와 우체국 API 키를 발급받습니다.',
    'GenieGo [등록]에 api_key 를 입력해 저장합니다(cust_code 는 전용 API 계약 시에만 입력 · 선택).',
  ],
  cj_intl: [
    '준비물: CJ대한통운 국제특송 계약(고객) 코드가 필요합니다(계약형).',
    'CJ대한통운 국제특송 영업 담당과 계약을 진행합니다.',
    '계약 완료 후 고객(계약) 코드와 API 키를 발급받습니다.',
    'GenieGo [등록]에 api_key 를 입력해 저장합니다(cust_code 는 전용 API 계약 시에만 입력 · 선택).',
  ],
  tnt: [
    '준비물: TNT는 FedEx에 통합되어 FedEx Developer Portal 자격증명을 사용합니다.',
    'developer.fedex.com 에 가입하고 프로젝트를 생성합니다.',
    'API Key/Secret 을 발급하고 FedEx 계정번호를 확인합니다.',
    'GenieGo [등록]에 api_key, api_secret 를 입력해 저장합니다(account_number 는 선택).',
  ],
  yahoo_japan: [
