...
// Basic 플랜 정의 (Starter·Free 제거, Paid Plan만)
const DEFAULT_PLANS = [
  { id: "growth",     label: "Growth",     color: "#4f8ef7", emoji: "📈", is: false },
  { id: "pro",        label: "Pro",        color: "#a855f7", emoji: "🚀", is: false },
  { id: "enterprise", label: "Enterprise", color: "#f59e0b", emoji: "🌐", is: false },
];

// Basic 계정 수 티어 (동적 Add/삭제 가능)
const DEFAULT_TIERS = [
  { key: "1",         label: "1계정",  count: 1  },
  { key: "5",         label: "5계정",  count: 5  },
  { key: "10",        label: "10계정", count: 10 },
  { key: "30",        label: "30계정", count: 30 },
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={onPaddleSync} disabled={paddleSyncing || priceItems.length === 0} style={{
            ...
            color: 'var(--text-1)', fontWeight: 700, fontSize: 11,
            ...
            <div onClick={() => togglePlan(plan.id)} style={{
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  }}>{Object.keys(acctMap).length}개 계정 티어</span>
                {/* Pricing 요약도 한 줄 */}
                    ...
                    {summaryEntries.map(({ acct, item }) => {
                      ...
                      return (
                        <span key={acct.key} style={{ fontSize: 10, color: '#94a3b8' }}>
                          ...
                          {' '}<span style={{ color: 'var(--text-1)', fontWeight: 700 }}>₩{final.toLocaleString('ko-KR')}</span>
                      ...
                      {CYCLES.map(c => {
                        ...
                        return (
                          <div key={c.key} style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--text-1)' }}>₩{final.toLocaleString()}<span style={{ fontSize: 8, color: '#7c8fa8', fontWeight: 400 }}>/월</span></div>
                            {months > 1 && (
                              <div style={{ fontSize: 9, color: plan.color, marginTop: 1 }}>₩{total.toLocaleString()} 합계액</div>
                            )}
                            {parseFloat(it.discount_pct) > 0 && (
                              <div style={{ fontSize: 8, color: '#4ade80' }}>{it.discount_pct}% 할인</div>
                            )}
...
nst [filterCycle,  setFilterCycle]  = useState("all");
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: "grid", gap: 4 }}>
          ...
          <div style={{
            ...
            <span>플랜</span><span>계정수</span><span>주기</span>
            <span>기본 요금</span><span>할인율</span><span>최종 요금</span>
            <span>Paddle 상태</span>
            {showHistory && <span style={{ color: "#eab308" }}>등록일</span>}
            <span>관리</span>
...
function PricingMatrix({ cycle, data, onChange, onTabChange }) {
  ...
  const [tiers, setTiers] = useState(() => {
    // 영문 → 한글 라벨 자동 변환 맵
    const labelFixMap = { '1Account':'1계정','5Account':'5계정','10Account':'10계정','30Account':'30계정','Unlimited':'무제한' };
    const fixLabel = (t) => ({ ...t, label: labelFixMap[t.label] || (t.label?.includes('Account') ? t.label.replace('Account','계정') : t.label) });
    try {
      const saved = localStorage.getItem(TIERS_STORAGE_KEY);
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) {
          const fixed = parsed.map(fixLabel);
          // 변환 발생 시 localStorage 업데이트
          if (JSON.stringify(fixed) !== saved) {
            try { localStorage.setItem(TIERS_STORAGE_KEY, JSON.stringify(fixed)); } catch {}
          }
          return fixed;
        }
      }
  ...
  // Paid Plan만 (Free 제외)
  const paidPlans = plans.filter(p => !p.is);
  ...
  const addPlan = () => {
    ...
    setPlans(prev => [...prev, { id, label, color: PLAN_COLORS[idx], emoji: PLAN_EMOJIS[idx], is: false }]);
    setNewPlanLabel("");
  ...
  const addTier = () => {
    if (newTierUnlimited) {
      ...
      saveTiers([...tiers, { key: 'unlimited', label: '무제한', count: 0, unlimited: true }]);
    } else {
      const count = parseInt(newTier수);
      if (!count || count <= 0) return;
      const key = String(count);
      if (tiers.some(t => t.key === key)) return;
      saveTiers([...tiers, { key, label: `${count}계정`, count }].sort((a, b) => {
    ...
    <div style={{ display: "grid", gap: 8 }}>
      ...
      {isMonthly ? (
        <div style={{ padding: "10px 14px", borderRadius: 8, fontSize: 11, background: "rgba(34,197,94,0.05)", border: "1px solid rgba(34,197,94,0.2)", color: "#86efac", display: "flex", alignItems: "center", gap: 8 }}>
          <span>✏️</span>
          <span><strong>월간 기본요금</strong>을 계정 수별로 입력하세요. 3개월·6개월·연간 탭의 기본요금은 <strong>월간요금 × 개월수</strong>로 자동 계산됩니다.</span>
      ) : (
        <div style={{ padding: "10px 14px", borderRadius: 8, fontSize: 11, background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.2)", color: "#93c5fd", display: "flex", alignItems: "center", gap: 8 }}>
          <span>🔒</span>
          <div><strong>기본요금</strong>은 <strong>월간요금 × {cycleInfo.months}개월</strong>로 자동 계산됩니다. <span style={{ color: "#4ade80", marginLeft: 6 }}>할인율(%)만 이 탭에서 직접 입력하세요.</span></div>
      )}
          ...
          <div key={plan.id} style={{
            ...
            <div onClick={() => setOpenPlan(isOpen ? null : plan.id)} style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                <span style={{ fontSize: 10, color: "#7c8fa8", marginLeft: 8 }}>유료 플랜</span>
              {hasValues && (
                <span style={{ fontSize: 9, padding: "1px 7px", borderRadius: 99, fontWeight: 700, background: `${plan.color}18`, border: `1px solid ${plan.color}33`, color: plan.color }}>요금 등록됨</span>
              )}
              {/* 플랜 삭제 Button */}
            ...
            {isOpen && (
              <div style={{ paddingBottom: 12 }}>
                ...
                <div style={{
                  display: "grid",
                  gridTemplateColumns: !isMonthly ? "80px 170px 100px 120px 130px 30px" : "80px 170px 100px 120px 30px",
                  gap: 8, padding: "6px 16px 10px", borderBottom: `1px solid ${plan.color}22`, alignItems: "center",
                }}>
                  <span style={{ fontSize: 10, fontWeight: 700, color: "#7c8fa8" }}>계정 수</span>
                  {isMonthly && <span style={{ fontSize: 10, fontWeight: 700, color: "#4ade80" }}>월간 기본요금 (₩) ✏️</span>}
                  {!isMonthly && <span style={{ fontSize: 10, fontWeight: 700, color: "#60a5fa" }}>기본요금 🔒 자동</span>}
                  <span style={{ fontSize: 10, fontWeight: 700, color: "#4ade80" }}>할인율 (%) ✏️</span>
                  <span style={{ fontSize: 10, fontWeight: 700, color: "#7c8fa8" }}>할인 후 월 요금</span>
                  {!isMonthly && <span style={{ fontSize: 10, fontWeight: 700, color: plan.color }}>{cycleInfo.label} 합계액</span>}
                  <span />
                    ...
                    <div key={tier.key} style={{
                      ...
                      {isMonthly ? (
                        ...
                        <div style={{
                          ...
                          ) : <span style={{ fontSize: 10, color: "#3b4d6e", fontStyle: "italic" }}>월간요금 미입력</span>}
                      )}
                      ...
                      <div style={{ fontSize: 12, fontWeight: 800, color: finalMonthly > 0 ? "#e2e8f0" : "#3b4d6e" }}>
                        {krw(finalMonthly)}
                        {parseFloat(disc) > 0 && monthlyBase > 0 && <div style={{ fontSize: 9, color: "#4ade80", marginTop: 1 }}>{disc}% 할인</div>}

                      {/* Total액 (비Monthly) */}
                      {!isMonthly && (
                        <div style={{ fontSize: 12, fontWeight: 800, color: total > 0 ? plan.color : "#3b4d6e" }}>
                          {krw(total)}
                      ...
                      <button onClick={() => removeTier(tier.key)} title="이 계정 수 제거" style={{
                        ...
                        background: "rgba(239,68,68,0.05)", color: "#ef4444", fontSize: 10, cursor: "pointer",
                      }}>✕</button>
                      ...
                      {!newTierUnlimited && (
                        <input
                          ...
                          style={{
                            ...
                            borderRadius: 7, color: "var(--text-1)", padding: "6px 10px",
                            fontSize: 12, outline: "none", width: 120,
                          }}
                        />
                      )}
                      <button onClick={addTier} style={{ padding: "5px 12px", borderRadius: 6, border: "none", background: "#22c55e", color: 'var(--text-1)', fontSize: 11, fontWeight: 700, cursor: "pointer" }}>추가</button>
                      <button onClick={() => setShowAddTier(false)} style={{ padding: "5px 10px", borderRadius: 6, border: '1px solid var(--border)', background: "transparent", color: "#94a3b8", fontSize: 11, cursor: "pointer" }}>취소</button>
                  ) : (
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      <button onClick={resetTiers} title="1·5·10·무제한으로 초기화" style={{
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <input
              ...
              style={{
                ...
                borderRadius: 7, color: "var(--text-1)", padding: "7px 12px",
                fontSize: 12, outline: "none",
              }}
            />
            <button onClick={addPlan} style={{ padding: "7px 16px", borderRadius: 7, border: "none", background: "linear-gradient(135deg,#4f8ef7,#6366f1)", color: 'var(--text-1)', fontSize: 12, fontWeight: 700, cursor: "pointer" }}>➕ 플랜 추가</button>
            <button onClick={() => setShowAddPlan(false)} style={{ padding: "7px 12px", borderRadius: 7, border: '1px solid var(--border)', background: "transparent", color: "#94a3b8", fontSize: 12, cursor: "pointer" }}>취소</button>
        ) : (
          <button onClick={() => setShowAddPlan(true)} style={{
            width: "100%", padding: "8px", borderRadius: 8, border: "none",
            background: "transparent", color: "#4f8ef7",        fontSize: 12, fontWeight: 700,
            cursor: "pointer", textAlign: "center",
          }}>➕ 유료 플랜 추가하기 (Growth / Pro / Enterprise 외 커스텀 플랜)</button>
        )}      : Basic 대시보드·Onboarding·Help만 ( 체험 Min)
...
// 플랜별 접근 Grade 요약도
const PLAN_SUMMARY = {
  free:       { label: " 체험",   desc: "주요 위젯·빠른 링크·튜토리얼·고객센터 열람 (오가닉 데이터 한정)",                       icon: "🆓" },
...
function MenuAccessTab({ menuData, onMenuChange }) {
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
          {[
            ...
            <button key={plan.id} type="button"
              ...
              {plan.popular && (
                <div style={{ position: "absolute", top: -8, left: "50%", transform: "translateX(-50%)", background: "#a855f7", color: 'var(--text-1)', fontSize: 8, fontWeight: 800, padding: "2px 8px", borderRadius: 99 }}>POPULAR</div>
              )}
              <div style={{ fontWeight: 800, fontSize: 14, color: plan.color, marginBottom: 4 }}>{plan.emoji} {plan.label}</div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "var(--text-1)", marginBottom: 4 }}>{plan.price}</div>
      ...
      {/* 빠른 도구 */}
      <div style={{ padding: "12px 16px", borderRadius: 10, background: 'var(--surface)', border: '1px solid var(--border)', display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <div key={section.key} style={{ borderRadius: 10, overflow: "hidden", border: "1px solid rgba(79,142,247,0.18)", background: "rgba(79,142,247,0.02)" }}>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 900, color: "var(--text-1)" }}>{section.label}</span>
...
function DiscountPanel({ pricingData, setPricingData, handle저장, saving, setMsg, setMsgType }) {
  ...
  const DISC_PLANS = PLANS.filter(p => !p.is); // Growth/Pro/Enterprise
      ...
      <div style={{ padding: "16px 20px" }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: "#7c8fa8", marginTop: 2 }}>
              Discount율은 각 플랜의 <strong style={{ color: "var(--text-1)" }}>모든 계정 수 단계</strong>에 동일하게 Apply됩니다. 月간 Tab에서 BasicPricing, 여기서 Discount율 설정 후 저장하세요.
          </div>
          <button
            onClick={async () => {
            ...
            style={{
              ...
              color: 'var(--text-1)', fontWeight: 800, fontSize: 13, cursor: saving ? "not-allowed" : "pointer",
...
export default function SubscriptionPricing() {
  ...
  /* ── 모든 기능보기 실시간 동기화 State ── */
  const [featureData, setFeatureData] = useState({});
  const [featureSyncTs, setFeatureSyncTs] = useState(null);
  const bcRef = useRef(null);

  /* ── BroadcastChannel: MenuAccessPanel에서 저장 시 즉시 업데이트 ── */
  useEffect(() => {
    try {
      bcRef.current = new BroadcastChannel("geniego_menu_access_sync");
      bcRef.current.onmessage = (e) => {
        if (e.data?.type === "menu_access_updated") {
          const { plan, featureSummary, timestamp } = e.data;
          setFeatureData(prev => ({ ...prev, [plan]: featureSummary }));
          setFeatureSyncTs(timestamp);
          setMsg(`🔄 ${plan.toUpperCase()} 플랜 메뉴 접근 권한이 실시간 동기화되었습니다!`);
          setMsgType("ok");
  ...
  /* ── localStorage에서 초기 featureData 로드 (페이지 새로고침 대비) ── */
  useEffect(() => {
    try {
      const stored = JSON.parse(localStorage.getItem("geniego_menu_features") || "{}");
      if (Object.keys(stored).length > 0) {
        setFeatureData(stored);
        const ts = localStorage.getItem("geniego_menu_features_ts");
        if (ts) setFeatureSyncTs(Number(ts));
      }
    } catch {}
  }, []);

  /* ── window custom event 감지 (같은 탭 내 동기화) ── */
  useEffect(() => {
    const handler = () => {
      try {
        const stored = JSON.parse(localStorage.getItem("geniego_menu_features") || "{}");
        if (Object.keys(stored).length > 0) {
          setFeatureData(stored);
          setFeatureSyncTs(Date.now());
        }
      } catch {}
    };
    window.addEventListener("menu-access-saved", handler);
    return () => window.removeEventListener("menu-access-saved", handler);
  }, []);

  /* ── Load: 서버 API를 단일 진실 원천(Single Source of Truth)으로 사용 ── */
  const loadData = useCallback(() => {
    setLoading(true);
    fetch(`${API}/auth/pricing/plans`, { headers: authHeader() })
      .then(r => r.json())
      .then(d => {
        /* ── 서버 응답만 사용 (localStorage 폴백 영구 제거) ── */
  ...
  const handle저장 = async () => {
    ...
    try {
      ...
      if (items.length === 0) {
        setMsg("⚠️ 저장할 요금 또는 권한 설정이 없습니다. 값을 입력해 주세요."); setMsgType("warn");
        setSaving(false); return;
      }

      /* ── 항상 서버에 저장 (localStorage 의존성 완전 제거) ── */
      ...
      if (d.ok) {
        setMsg(`✅ ${d.saved ?? items.length}건 서버 저장 완료! 요금제 페이지에 즉시 동기화됩니다.`);
        setMsgType("ok");
        loadData();
        setInnerTab("list");
        if (reloadMenuAccess) reloadMenuAccess();
      } else {
        setMsg("❌ " + (d.error || "저장 실패")); setMsgType("err");
      }
    } catch (e) {
      setMsg("❌ 네트워크 오류: " + e.message); setMsgType("err");
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      {/* ── Inner Tab Bar (4개 탭) ── */}
      <div style={{ display: "flex", gap: 6, background: 'var(--surface)', borderRadius: 12, padding: 6 }}>
        {[
          {
          ...
          {
            id: "features", icon: "📋", label: "모든 기능 보기",
            sub: featureSyncTs ? `마지막 동기화: ${new Date(featureSyncTs).toLocaleTimeString("ko-KR")}` : "플랜별 기능 상세",
            color: "#ec4899",
            highlight: featureSyncTs && (Date.now() - featureSyncTs < 10000),
          },
          {
          ...
          return (
            <button key={tab.id} onClick={() => setInnerTab(tab.id)} style={{
              ...
              background: isActive
                ...
                : tab.highlight ? `${tab.color}0C` : "transparent",
              borderBottom: `2px solid ${isActive ? tab.color : "transparent"}`,
              transition: "all 150ms", textAlign: "center",
              outline: tab.highlight && !isActive ? `1px solid ${tab.color}40` : "none",
              animation: tab.highlight && !isActive ? "pulse 1.5s ease infinite" : "none",
            }}>
              ...
              {tab.highlight && !isActive && (
                <div style={{ fontSize: 7, padding: "1px 6px", borderRadius: 99, background: tab.color, color: 'var(--text-1)', fontWeight: 800, marginTop: 3, display: "inline-block" }}>🔄 NEW</div>
              )}
            </button>
          );
        })}
          ...
          <div style={{
            ...
            <button
              ...
              style={{
                ...
                background: saving ? "rgba(79,142,247,0.3)" : "linear-gradient(135deg,#4f8ef7,#6366f1)",
                color: 'var(--text-1)', fontWeight: 800, fontSize: 14, cursor: saving ? "not-allowed" : "pointer",
      ...
      {/* ── 모든 기능보기 Tab ── */}
      {!loading && innerTab === "features" && (
        <AllFeaturesViewPanel featureData={featureData} featureSyncTs={featureSyncTs} menuData={menuData} savedItems={savedItems} />
      )}
...
/* ══════════════════════════════════════════════════════════════════════════
   AllFeaturesViewPanel — 플랜별 모든 기능보기 (실시간 동기화)
   ══════════════════════════════════════════════════════════════════════════ */
const FEATURE_PLANS = [
  { id: "growth", label: "Growth", emoji: "📈", color: "#4f8ef7", desc: "국내 마케팅·커머스·CRM 핵심" },
  { id: "pro", label: "Pro", emoji: "🚀", color: "#a855f7", desc: "AI 자동화·글로벌·고급 분석", popular: true },
  { id: "enterprise", label: "Enterprise", emoji: "🌐", color: "#f59e0b", desc: "전체 무제한 + Writeback" },
];

function AllFeaturesViewPanel({ featureData, featureSyncTs, menuData, savedItems }) {
  const [selectedPlan, setSelectedPlan] = useState("growth");
  const [expandedSections, setExpandedSections] = useState({});

  const planData = featureData?.[selectedPlan] || [];
  const pc = FEATURE_PLANS.find(p => p.id === selectedPlan)?.color || "#4f8ef7";
  const hasSyncData = planData.length > 0;
  ...
  const expandAll = () => {
    const next = {};
    planData.forEach(sec => { next[sec.key] = true; });
    setExpandedSections(next);
  };
  const collapseAll = () => setExpandedSections({});

  const totalEnabled = planData.reduce((s, sec) => s + sec.sections.reduce((s2, sub) => s2 + sub.enabled.length, 0), 0);
  const totalDisabled = planData.reduce((s, sec) => s + sec.sections.reduce((s2, sub) => s2 + sub.disabled.length, 0), 0);
  const totalAll = totalEnabled + totalDisabled;
  const coveragePct = totalAll > 0 ? Math.round((totalEnabled / totalAll) * 100) : 0;
    ...
    <div style={{ display: "grid", gap: 14 }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
          ...
          <div>
            <div style={{ fontWeight: 900, fontSize: 16, color: "#f9a8d4" }}>플랜별 제공 기능 전체 보기</div>
            <div style={{ fontSize: 11, color: "#7c8fa8", marginTop: 3 }}>
              관리자 &gt; 메뉴 접근 권한에서 저장한 플랜별 권한 설정이 실시간으로 반영됩니다
          </div>
          {featureSyncTs && (
            <div style={{ marginLeft: "auto" }}>
              <span style={{
                ...
                animation: (Date.now() - featureSyncTs < 10000) ? "pulse 1.5s ease infinite" : "none",
              }}>
                🔄 마지막 동기화: {new Date(featureSyncTs).toLocaleString("ko-KR")}
              </span>
          )}
        ...
        {hasSyncData && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginTop: 12 }}>
            {[
              { label: "플랜", value: selectedPlan.toUpperCase(), color: pc, icon: "🏷" },
              { label: "활성 기능", value: `${totalEnabled}개`, color: "#4ade80", icon: "✅" },
              { label: "제한 기능", value: `${totalDisabled}개`, color: "#f87171", icon: "🔒" },
              { label: "커버리지", value: `${coveragePct}%`, color: "#ec4899", icon: "📈" },
            ].map((kpi, i) => (
              <div key={i} style={{
                ...
                <div style={{ fontSize: 9, color: "#7c8fa8" }}>{kpi.label}</div>
            ))}
        )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
        {FEATURE_PLANS.map(plan => {
          const pData = featureData?.[plan.id] || [];
          const en = pData.reduce((s, sec) => s + sec.sections.reduce((s2, sub) => s2 + sub.enabled.length, 0), 0);
          const dis = pData.reduce((s, sec) => s + sec.sections.reduce((s2, sub) => s2 + sub.disabled.length, 0), 0);
          const total = en + dis;
          ...
          return (
            <button key={plan.id} onClick={() => { setSelectedPlan(plan.id); setExpandedSections({}); }}
              style={{
                padding: "16px 14px", borderRadius: 14, border: "none", cursor: "pointer", textAlign: "left",
                background: isActive ? `${plan.color}1A` : "rgba(255,255,255,0.02)",
                outline: `2px solid ${isActive ? plan.color : "rgba(255,255,255,0.08)"}`,
                position: "relative", transition: "all 150ms",
              }}>
              {plan.popular && (
                <div style={{ position: "absolute", top: -8, left: "50%", transform: "translateX(-50%)", background: plan.color, color: 'var(--text-1)', fontSize: 8, fontWeight: 800, padding: "2px 8px", borderRadius: 99 }}>POPULAR</div>
              )}
              <div style={{ fontWeight: 800, fontSize: 14, color: plan.color, marginBottom: 4 }}>{plan.emoji} {plan.label}</div>
              <div style={{ fontSize: 10, color: "#94a3b8", marginBottom: 8 }}>{plan.desc}</div>
              {hasData ? (
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: "#4ade80" }}>✅ {en}개 활성</span>
                    {dis > 0 && <span style={{ fontSize: 10, fontWeight: 700, color: "#f87171" }}>🔒 {dis}개</span>}
                  <div style={{ height: 5, borderRadius: 3, background: "rgba(255,255,255,0.06)", overflow: "hidden" }}>
                    <div style={{ width: `${pct}%`, height: "100%", borderRadius: 3, background: `linear-gradient(90deg,${plan.color},${plan.color}88)` }} />
                  <div style={{ fontSize: 9, color: plan.color, fontWeight: 700, marginTop: 3 }}>{pct}% 커버리지</div>
              ) : (
                <div style={{ fontSize: 10, color: "#475569", fontStyle: "italic" }}>⚠ 권한 데이터 없음</div>
              )}
              {isActive && <div style={{ marginTop: 6, fontSize: 10, color: plan.color, fontWeight: 800 }}>✓ 선택됨</div>}
            </button>
...
때 가이드 */}
      {!hasSyncData && (
        <div style={{
          ...
          <div style={{ fontSize: 15, fontWeight: 800, color: "var(--text-1)", marginBottom: 8 }}>
            {selectedPlan.toUpperCase()} 플랜의 기능 데이터가 아직 동기화되지 않았습니다
          <div style={{ fontSize: 12, color: "#7c8fa8", lineHeight: 1.8, maxWidth: 500, margin: "0 auto" }}>
            1. 상단 <strong style={{ color: "#a855f7" }}>「메뉴 접근 권한」</strong> 탭으로 이동<br/>
            2. <strong style={{ color: "#4f8ef7" }}>{selectedPlan.toUpperCase()} 플랜</strong> 선택 → <strong style={{ color: "#ec4899" }}>AI 추천 자동 적용</strong> 버튼 클릭<br/>
            3. <strong style={{ color: "#22c55e" }}>권한 저장 & 동기화</strong> 버튼 클릭<br/>
            4. 이 탭으로 돌아오면 <strong style={{ color: "#ec4899" }}>실시간으로 모든 기능이 표시</strong>됩니다
        </div>
      )}
      ...
      {hasSyncData && (
        <div style={{ display: "grid", gap: 8 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <button onClick={expandAll} style={{ padding: "6px 14px", borderRadius: 8, border: `1px solid ${pc}30`, background: `${pc}08`, color: pc, fontWeight: 700, fontSize: 10, cursor: "pointer" }}>📂 전체 열기</button>
            <button onClick={collapseAll} style={{ padding: "6px 14px", borderRadius: 8, border: '1px solid var(--border)', background: "transparent", color: "#7c8fa8", fontWeight: 700, fontSize: 10, cursor: "pointer" }}>📁 전체 접기</button>
            <div style={{ marginLeft: "auto", fontSize: 10, color: "#475569" }}>
              총 <strong style={{ color: pc }}>{planData.length}</strong>개 카테고리 · <strong style={{ color: "#4ade80" }}>{totalEnabled}</strong>개 활성 기능
          </div>

          {planData.map((secInfo, si) => {
            const secEnabled = secInfo.sections.reduce((s, sub) => s + sub.enabled.length, 0);
            const secDisabled = secInfo.sections.reduce((s, sub) => s + sub.disabled.length, 0);
            const secTotal = secEnabled + secDisabled;
            const secPct = secTotal > 0 ? Math.round((secEnabled / secTotal) * 100) : 0;
            const isExpanded = expandedSections[secInfo.key];

            return (
              <div key={si} style={{
                borderRadius: 12, overflow: "hidden",
                border: `1px solid ${isExpanded ? `${pc}30` : "rgba(255,255,255,0.06)"}`,
              }}>
                <button onClick={() => toggleSection(secInfo.key)} style={{
                  width: "100%", padding: "14px 18px", border: "none", cursor: "pointer",
                  background: isExpanded ? `${pc}0A` : "rgba(15,23,42,0.4)",
                  display: "flex", alignItems: "center", gap: 12, textAlign: "left",
                }}>
                  <span style={{ fontSize: 12, transition: "transform 200ms", transform: isExpanded ? "rotate(90deg)" : "rotate(0deg)", color: "#7c8fa8" }}>▶</span>
                  <span style={{ fontWeight: 800, fontSize: 13, color: isExpanded ? "#e2e8f0" : "#94a3b8", flex: 1 }}>{secInfo.label}</span>
                  <div style={{ width: 60, height: 6, borderRadius: 3, background: "rgba(255,255,255,0.06)", overflow: "hidden" }}>
                    <div style={{ width: `${secPct}%`, height: "100%", borderRadius: 3, background: secPct === 100 ? "#4ade80" : pc }} />
                  <span style={{
                    fontSize: 9, padding: "3px 10px", borderRadius: 99, fontWeight: 700,
                    background: secPct === 100 ? "rgba(34,197,94,0.12)" : `${pc}12`,
                    border: `1px solid ${secPct === 100 ? "rgba(34,197,94,0.3)" : `${pc}30`}`,
                    color: secPct === 100 ? "#4ade80" : pc,
                  }}>
                    {secPct === 100 ? "✅ FULL" : `${secEnabled}/${secTotal}`}
                  </span>
                </button>

                {isExpanded && (
                  <div style={{ padding: "12px 18px", background: "rgba(15,23,42,0.2)" }}>
                    {secInfo.sections.map((sub, sj) => (
                      <div key={sj} style={{ marginBottom: 10 }}>
                        <div style={{ fontSize: 11, fontWeight: 700, color: "#a5b4fc", marginBottom: 6, display: "flex", alignItems: "center", gap: 6 }}>
                          📂 {sub.itemLabel} → {sub.subLabel}
                          <span style={{ fontSize: 8, color: "#475569" }}>({sub.enabled.length + sub.disabled.length}개)</span>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                          {sub.enabled.map((feat, fi) => (
                            <span key={fi} style={{
                              fontSize: 10, padding: "5px 12px", borderRadius: 8,
                              background: feat.value === "high"
                                ? "linear-gradient(135deg,rgba(34,197,94,0.3),rgba(34,197,94,0.15))"
                                : feat.value === "premium"
                                  ? "linear-gradient(135deg,rgba(79,142,247,0.3),rgba(79,142,247,0.15))"
                                  : "rgba(59,130,246,0.2)",
                              border: `1px solid ${feat.value === "high" ? "rgba(34,197,94,0.6)" : feat.value === "premium" ? "rgba(79,142,247,0.6)" : "rgba(59,130,246,0.5)"}`,
                              color: feat.value === "high" ? "#86efac" : "#93c5fd",
                              fontWeight: 600, display: "flex", alignItems: "center", gap: 4,
                            }}>
                              {feat.value === "high" ? "⭐" : "✅"} {feat.label}
                              {feat.tier === "enterprise" && <span style={{ fontSize: 7, padding: "0 3px", borderRadius: 3, background: "rgba(245,158,11,0.3)", color: "#f59e0b" }}>ENT</span>}
                              {feat.tier === "pro" && <span style={{ fontSize: 7, padding: "0 3px", borderRadius: 3, background: "rgba(168,85,247,0.3)", color: "#a855f7" }}>PRO</span>}
                            </span>
                          ))}
                          {sub.disabled.map((feat, fi) => (
                            <span key={"d" + fi} style={{
                              fontSize: 10, padding: "5px 12px", borderRadius: 8,
                              background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.35)",
                              color: "#fca5a5", fontWeight: 600, display: "flex", alignItems: "center", gap: 4,
                            }}>
                              🔒 {feat.label}
                              {feat.tier !== "growth" && (
                                <span style={{
                                  fontSize: 7, padding: "0 4px", borderRadius: 3,
                                  background: feat.tier === "enterprise" ? "rgba(245,158,11,0.2)" : "rgba(168,85,247,0.2)",
                                  color: feat.tier === "enterprise" ? "#f59e0b" : "#a855f7",
                                }}>
                                  {feat.tier === "enterprise" ? "ENT 전용" : "PRO+"}
                                </span>
                              )}
                            </span>
                          ))}
        ...
        padding: "12px 18px", borderRadius: 10,
            ...
            fontSize: 10, color: "#7c8fa8", display: "flex", alignItems: "center", gap: 8,
          }}>
            <span style={{ fontSize: 14 }}>🔄</span>
            <span>
              이 데이터는 <strong style={{ color: "#4ade80" }}>관리자 &gt; 메뉴 접근 권한</strong>에서 저장할 때 실시간으로 동기화됩니다.
              권한을 변경하려면 <strong style={{ color: "#a855f7" }}>메뉴 접근 권한</strong> 탭에서 AI 추천을 적용 후 저장하세요.
            </span>
        </div>
      )}
                                                </div>
