...
function generateRecommendations(inventory, orders, channelBudgets, settlement, crmSegments, pnlStats, t, fmt) {
    ...
    const nb = 8000000;
    const roi5 = calcROI(nb, 3.2, avgMargin);
    recs.push({
        id: 'rec_new_customer', type: 'acquisition', priority: 'MEDIUM',
        title: t('aiHub.recAcqTitle'),
        reason: `${t('aiHub.currentOrders')} ${(orders || []).length}${t('aiHub.casesUnit')}. ${t('aiHub.acqReason')}`,
        insight: `${t('aiHub.dataSource')}: ${t('aiHub.acqInsight')} ${fmt(nb)} → ${t('aiHub.netProfit')} ${fmt(Math.round(roi5.netProfit))}, ROI ${roi5.roi.toFixed(1)}%`,
        channels: [
            { id: 'tiktok', name: 'TikTok', budget: 5000000, icon: '🎵' },
            { id: 'influencer', name: t('aiHub.influencer'), budget: 3000000, icon: '⭐' },
        ],
        budget: nb, estimatedRoas: 3.2,
        estimatedRevenue: Math.round(roi5.revenue),
        estimatedProfit: Math.round(roi5.netProfit),
        estimatedROI: roi5.roi,
        category: `🚀 ${t('aiHub.catAcquisition')}`, color: '#ec4899',
        dataPoints: [
            { label: t('aiHub.dpEstConvRate'), value: '3.8%', status: 'good' },
            { label: t('aiHub.dpEstCpa'), value: '₩15,000', status: 'info' },
            { label: t('aiHub.dpExpectedProfit'), value: fmt(Math.round(roi5.netProfit)), status: roi5.netProfit > 0 ? 'good' : 'bad' },
            { label: t('aiHub.dpAdRoi'), value: roi5.roi.toFixed(1) + '%', status: roi5.roi > 0 ? 'good' : 'bad' },
        ],
    });
        ...
        <div className="card card-glass" style={{ border: `1.5px solid ${approved ? rec.color + '55' : executed ? 'rgba(34,197,94,0.3)' : 'rgba(255,255,255,0.07)'}`, padding: 20, transition: 'border 0.3s' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    {rec.estimatedRoas > 0 && <><div style={{ fontSize: 9, color: 'var(--text-3)' }}>{t('aiHub.estRoas')}</div><div style={{ fontSize: 20, fontWeight: 900, color: rec.color }}>{rec.estimatedRoas}x</div></>}
            ...
            {rec.channels.length > 0 && (
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
                    {rec.channels.map(ch => (
                        <div key={ch.id} style={{ padding: '5px 12px', borderRadius: 8, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', fontSize: 11 }}>
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    {editing ? (
                        <input autoFocus type="number" value={inputVal} onChange={e => setInputVal(e.target.value)}
                            onBlur={handleBudgetSave} onKeyDown={e => { if (e.key === 'Enter') handleBudgetSave(); if (e.key === 'Escape') setEditing(false); }}
                            style={{ width: 110, padding: '5px 8px', borderRadius: 7, background: 'rgba(0,0,0,0.4)', border: `1px solid ${rec.color}55`, color: '#fff', fontSize: 12 }} />
                    ) : (
                        <button onClick={() => { setEditing(true); setInputVal(String(budget)); }}
                            style={{ padding: '5px 12px', borderRadius: 7, background: rec.color + '10', border: `1px dashed ${rec.color}44`, color: rec.color, fontWeight: 800, fontSize: 12, cursor: 'pointer' }}>
                            {fmt(budget)} ✏️
                        </button>
                    )}
                    {rec.estimatedRevenue > 0 && <span style={{ fontSize: 10, color: 'var(--text-3)' }}>→ {t('aiHub.revenue')} <span style={{ color: '#22c55e', fontWeight: 700 }}>{fmt(rec.estimatedRevenue)}</span></span>}
                </div>
                {defaultCard && (
    ...
    return (
        <div className="card card-glass" style={{ padding: 20, border: '1px solid rgba(168,85,247,0.25)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 10 }}>
                ...
                {!editing ? (
                    <button onClick={() => { setEditing(true); setEditVal(totalBudget ? String(totalBudget) : ''); }}
                        style={{ padding: '8px 16px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg,#a855f7,#7c3aed)', color: '#fff', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                        <input autoFocus type="number" value={editVal} onChange={e => setEditVal(e.target.value)}
                            ...
                            onKeyDown={e => { if (e.key === 'Enter') handleCalc(); if (e.key === 'Escape') setEditing(false); }}
                            style={{ width: 160, padding: '8px 12px', borderRadius: 9, background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(168,85,247,0.4)', color: '#fff', fontSize: 13, fontWeight: 700 }} />
                        ...
                        <button onClick={() => setEditing(false)} style={{ padding: '8px 12px', borderRadius: 9, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: 'var(--text-3)', fontSize: 12, cursor: 'pointer' }}>{t('cancel')}</button>
                    </div>
                )}
            </div>
    ...
    return (
        <div className="card card-glass" style={{ padding: 18 }}>
            ...
            <div style={{ display: 'grid', gap: 8 }}>
                {logs.map((log, i) => (
                    <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '8px 12px', borderRadius: 8, background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <div style={{ display: 'grid', gap: 18, padding: 4 }}>
            ...
            {isDemo && (
                <div style={{ padding: '12px 18px', borderRadius: 12, background: 'linear-gradient(135deg,rgba(168,85,247,0.1),rgba(79,142,247,0.06))', border: '1.5px solid rgba(168,85,247,0.3)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
                    <div>
                        <div style={{ fontWeight: 800, fontSize: 13, color: '#c084fc' }}>🎭 {t('aiHub.demoBanner')}</div>
                        <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2 }}>
                            {t('aiHub.demoDesc')} <strong style={{ color: '#c084fc' }}>{t('aiHub.demoNeedPaid')}</strong>
                        </div>
                    </div>
            ...
            <div className="card card-glass" style={{ padding: '18px 22px', background: 'linear-gradient(135deg,rgba(79,142,247,0.08),rgba(168,85,247,0.06))' }}>
                <div style={{ fontWeight: 900, fontSize: 20, background: 'linear-gradient(135deg,#4f8ef7,#a855f7)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                    🤖 {t('aiHub.heroTitle')}
                </div>
                        ...
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                            {FILTERS.map(([v, l]) => (
                                <button key={v} onClick={() => setFilter(v)}
                                    style={{ padding: '5px 14px', borderRadius: 20, border: '1px solid rgba(255,255,255,0.1)', background: filter === v ? '#6366f1' : 'transparent', color: filter === v ? '#fff' : 'var(--text-2)', cursor: 'pointer', fontSize: 11, fontWeight: 700 }}>
                                    {l}
                                </button>
                            ))}
                        </div>
