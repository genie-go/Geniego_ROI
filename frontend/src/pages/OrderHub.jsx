...
const SKUS = ["JART-CICA-50", "JART-CERA-100", "JART-VITAL-30", "JART-DERMA-01", "JART-CRYO-02", "JART-V7-03"];
const NAMES = ["시카페어(Cicapair) 미스트", "세라마이딘(Ceramidin) 세라마이드 크림", "바이탈 하이드라 콜라겐 앰플", "더마클리어 마이크로 폼 수딩 젤", "크라이오 고무 마스크 워터풀", "V7 핑크 토닝 라이트 V3"];

const IS_DEMO = () => typeof window !== 'undefined' && localStorage.getItem('genie_has_real_keys') !== '1';

const ORDERS = IS_DEMO() ? Array.from({ length: 60 }, (_, i) => {
    const si = i % 6;
    ...
    return {
        ...
        price,
        total,
        buyer: `user_${String(i + 1).padStart(3, "0")}@drjart-demo.kr`,
...
const INTL_ORDERS = IS_DEMO() ? Array.from({ length: 20 }, (_, i) => {
    ...
    return {
        ...
        buyer: `global_user${i+1}@drjart-global.com`,
...
const CLAIMS = IS_DEMO() ? ORDERS.filter(o => o.hasClaim).map((o, i) => ({
...
const DELIVERIES = IS_DEMO() ? ORDERS.filter(o => o.carrier).map((o, i) => ({
...
const SETTLEMENTS = IS_DEMO() ? CHANNELS.map((ch, i) => {
...
const SAMPLE_ROWS = [
    ['시카페어(Cicapair) 미스트', 'DJ-CICA-101', '89000', '150', '전자제품 > 음향기기', '소니 WH-1000XM5 블랙'],
    ['세라마이딘(Ceramidin) 세라마이드 크림', 'KB-MXM-RGB-01', '149000', '80', '전자제품 > 주변기기', 'Cherry MX 스위치'],
    ['더마클리어 마이크로 폼 수딩 젤', 'CAM-4K-PRO-01', '129000', '45', '전자제품 > 카메라', '4K 화질 웹캠'],
];
                ...
                return (
                    <div key={c.id} className="card card-glass">
                        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                            ...
                            <span className="badge badge-green" style={{ fontSize: 9, marginLeft: "auto" }}>{t('super.ihConnected')}</span>
                        </div>
