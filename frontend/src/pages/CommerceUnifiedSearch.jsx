...
export default function CommerceUnifiedSearch() {
  ...
  const results = useMemo(() => {
    ...
    const products = (tab === 'all' || tab === 'product') ? inventory.map(item => {
      ...
      const catalog = DEMO_CATALOG.find(c => c.sku === item.sku);
  ...
  const hl = (text = '') => {
    ...
    return parts.map((p, i) =>
      re.test(p) ? <mark key={i} style={{ background: 'rgba(79,142,247,0.35)', color: '#93c5fd', borderRadius: 3, padding: '0 1px' }}>{p}</mark> : p
    );
  };
    ...
    <div style={{ minHeight: '100%', color: '#e8eaf6', fontFamily: "'Inter','Segoe UI',sans-serif" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: 20, fontWeight: 900, color: '#fff' }}>커머스·물류 Unified Search</h1>
            <p style={{ margin: '2px 0 0', fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>Product·Stock·Orders·Channel·정산을 한 번에 Search</p>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8, marginBottom: 16 }}>
          {[
            ...
            <div key={i} style={{ ...card, borderColor: `${k.color}33`, textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.45)', marginTop: 1 }}>{k.label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <select value={filterChannel} onChange={e => setFilterChannel(e.target.value)} style={{ padding: '5px 8px', background: '#0d1829', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 7, color: '#e8eaf6', fontSize: 11 }}>
              <option value="all">All Channel</option>
              {Object.entries(CH_MAP).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
            </select>
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} style={{ padding: '5px 8px', background: '#0d1829', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 7, color: '#e8eaf6', fontSize: 11 }}>
              <option value="all">All Status</option>
              {Object.keys(STATUS_COLOR).map(s => <option key={s} value={s}>{s}</option>)}
            </select>
            <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{ padding: '5px 8px', background: '#0d1829', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 7, color: '#e8eaf6', fontSize: 11 }}>
      ...
      <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', marginBottom: 10 }}>
        ...
        {query && <span> · Search어: <strong style={{ color: '#93c5fd' }}>"{query}"</strong></span>}
      </div>

      {results.length === 0 ? (
        <div style={{ ...card, textAlign: 'center', padding: '60px 20px', color: 'rgba(255,255,255,0.3)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <span style={{ fontWeight: 800, fontSize: 13, color: '#fff' }}>{hl(item.name)}</span>
          ...
          <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 6 }}>
              {stockEntries.map(([wh, qty]) => (
                <div key={wh} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 8, padding: '8px 10px', textAlign: 'center' }}>
      ...
      <div style={{ ...card, cursor: 'pointer' }} onClick={() => setExpanded(e => !e)}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 14, marginTop: 4, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 11, color: '#7c8fa8' }}>Orders <strong style={{ color: '#93c5fd' }}>{hl(item.id)}</strong></span>
              <span style={{ fontSize: 11, color: '#7c8fa8' }}>구매자 <strong style={{ color: '#e2e8f0' }}>{hl(item.buyer)}</strong></span>
        ...
        {expanded && (
          <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.06)', display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 8 }}>
            {[
    ...
    return (
      <div style={{ ...card, borderColor: `${ch.color}33` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 800, fontSize: 14, color: '#fff', marginBottom: 4 }}>{ch.label}</div>
  ...
  if (item.type === 'settlement') {
    ...
    return (
      <div style={{ ...card, borderColor: isPending ? 'rgba(245,158,11,0.3)' : 'rgba(34,197,94,0.2)', cursor: 'pointer' }} onClick={() => setExpanded(e => !e)}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              <span style={{ fontWeight: 800, fontSize: 13, color: '#fff' }}>{ch.label}</span>
        ...
        {expanded && (
          <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.06)', display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
            {[
              ...
              <div key={k} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 8, padding: '8px 10px' }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 700, color: '#e2e8f0', marginTop: 2 }}>{v}</div>
              </div>
            ))}
          </div>
        )}
