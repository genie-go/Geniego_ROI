...
export default function CommerceUnifiedSearch() {
  ...
  const tabs = ["All Results","Products","Orders","Customers"];
  const kpis = [{"emoji":"🔍","label":"Searches Today","val":1240},{"emoji":"📦","label":"Products","val":"8.4K"},{"emoji":"🛒","label":"Orders","val":"2.1K"},{"emoji":"👥","label":"Customers","val":"5.6K"}];
    ...
    <div style={{ padding: 24, minHeight: "100%", color: "var(--text-1, #1e293b)" }}>
      <div style={{ borderRadius: 18, padding: "28px 32px", marginBottom: 22, background: "linear-gradient(135deg, rgba(79,142,247,0.08), rgba(99,102,241,0.06))", border: "1px solid rgba(79,142,247,0.12)", backdropFilter: "blur(16px)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            <div style={{ fontSize: 22, fontWeight: 800, color: "var(--text-1, #1e293b)" }}>🔍 Unified Search</div>
            <div style={{ fontSize: 13, color: "var(--text-3, #64748b)", marginTop: 2 }}>Search across all products, orders, customers, and channels</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ borderRadius: 16, padding: "28px 32px", minHeight: 320, background: "rgba(255,255,255,0.85)", border: "1px solid rgba(0,0,0,0.06)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 12 }}>
          {["Fuzzy search","Filter by channel","Sort by relevance","Export results"].map((f, i) => (
