...
const CHANNELS = [
  ...
  { key: 'auction', label: '옥션', emoji: '🅐' },
];

const SEVERITY_COLOR = { high: '#e74c3c', medium: '#f39c12', low: '#3498db' };
const STATUS_BADGE = {
  open: { label: '미처리', bg: '#e74c3c' },
  investigating: { label: '조사in progress', bg: '#f39c12' },
  resolved: { label: '해결', bg: '#27ae60' },
  waived: { label: '면제', bg: '#95a5a6' },
};
...
function UploadTab() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      catch { setError('JSON 형식 Error. 배열 형태로 입력하세요. 예: [{"Orders번호":"P001",...}]'); setLoading(false); return; }
      if (!Array.isArray(parsed)) { setError('배열 형태가 아닙니다.'); setLoading(false); return; }
  ...
  return (
    <div style={{ padding: 24 }}>
      <h3 style={{ marginBottom: 16, color: 'var(--text-primary)' }}>📤 정산 데이터 Upload</h3>
      <p style={{ color: 'var(--text-secondary)', marginBottom: 20 }}>
        Channel에서 받은 정산 데이터를 JSON 배열로 붙여넣으세요. 한글 컬럼명을 Auto으로 표준 필드로 변환합니다.
      </p>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <textarea
          ...
          rows={10}
          placeholder={`[\n  {"Orders번호":"P001","판매Amount":50000,"정산Amount":42000,"Commission":5000,"Coupon할인":3000}\n]`}
          style={{
...
function ReconTab({ onReconDone }) {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      else setError(data.error || '대사 Run Failed');
    } catch (e) { setError(e.message); }
    finally { setLoading(false); }
  };
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <p style={{ color: 'var(--text-secondary)', marginBottom: 20 }}>
        Upload된 정산 데이터와 Commission 규칙을 Compare해 차이를 Auto Calculate합니다.
      </p>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <div>
          <label style={{ fontSize: 13, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>Start일</label>
      ...
      <button onClick={handleRun} disabled={loading}
        ...
        {loading ? 'Analysis in progress...' : '⚖️ 대사 Run'}
      </button>
        ...
        <div style={{ marginTop: 20 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
            {[
              ...
              { label: '매칭', value: result.matched, color: '#27ae60' },
              { label: '불일치', value: result.mismatch, color: '#e74c3c' },
              { label: '티켓 Create', value: result.tickets_created, color: '#f39c12' },
            ].map(item => (
          ...
          <div style={{ marginTop: 12, background: '#fffbf0', border: '1px solid #f39c12', borderRadius: 8, padding: 12 }}>
            💰 Commission 차이: <strong>₩{(result.fee_diff_krw ?? 0).toLocaleString()}</strong> &nbsp;|&nbsp;
...
function ReportsTab({ refresh }) {
  ...
  return (
    <div style={{ padding: 24 }}>
      <h3 style={{ marginBottom: 16, color: 'var(--text-primary)' }}>📋 대사 리포트 List</h3>
      {reports.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-secondary)' }}>
          <div style={{ fontSize: 40, marginBottom: 12 }}>📭</div>
          <div>아직 대사 리포트가 없습니다. 먼저 데이터를 Upload하고 대사를 Run하세요.</div>
        </div>
      ) : (
        <div style={{ display: 'flex', gap: 20 }}>
          ...
          <div style={{ flex: 1 }}>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
                  {[
                    ...
                    ['매칭', detail.matched],
                    ['불일치', detail.mismatch],
                    ['티켓 Count', detail.tickets?.length ?? 0],
                  ].map(([k, v]) => (
            ...
            ) : <div style={{ color: 'var(--text-secondary)', textAlign: 'center', paddingTop: 40 }}>리포트를 Clicks하세요</div>}
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h3 style={{ color: 'var(--text-primary)', margin: 0 }}>🎫 불일치 티켓 대응</h3>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-secondary)' }}>해당 Status의 티켓이 없습니다.</div>
      ) : (
        <div>
          {filtered.map(t => (
            <div key={t.id} style={{
...
function FeeRulesTab() {
  ...
  return (
    <div style={{ padding: 24 }}>
      <h3 style={{ marginBottom: 16, color: 'var(--text-primary)' }}>⚙️ Channelper Commission 규칙</h3>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20, maxWidth: 600 }}>
        {[
          { field: 'platform_fee_rate', label: 'Platform Commission율', placeholder: '0.05 = 5%' },
          { field: 'ad_fee_rate', label: 'Ad Spend율', placeholder: '0.01 = 1%' },
          { field: 'vat_rate', label: '부가세율', placeholder: '0.1 = 10%' },
          { field: 'note', label: '비고', placeholder: 'Note' },
      ...
      <button onClick={handleSave} disabled={loading}
        ...
        {loading ? 'Save in progress...' : '💾 규칙 Save'}
      </button>
      {saved && <span style={{ marginLeft: 12, color: '#27ae60', fontWeight: 700 }}>✅ Save되었습니다</span>}

      {rules.length > 0 && (
        <div style={{ marginTop: 24 }}>
          <h4 style={{ marginBottom: 12 }}>Register된 규칙 이력</h4>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '2px solid var(--border)' }}>
                <th style={{ textAlign: 'left', padding: '8px 12px', color: 'var(--text-secondary)' }}>Apply일</th>
                ...
                <th style={{ textAlign: 'right', padding: '8px 12px', color: 'var(--text-secondary)' }}>부가세</th>
                <th style={{ textAlign: 'left', padding: '8px 12px', color: 'var(--text-secondary)' }}>비고</th>
              </tr>
            </thead>
            <tbody>
              {rules.map(r => (
...
export default function Reconciliation() {
  ...
  const TABS = [
    ...
    { key: 'recon', label: '⚖️ 대사 Run', },
    { key: 'reports', label: '📋 리포트', },
    { key: 'tickets', label: '🎫 티켓 대응', },
    ...
    <div style={{ padding: 24, maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: 'var(--text-secondary)', marginTop: 6 }}>
          Coupang·Naver·11Street·Gmarket 정산 데이터를 Upload하고, Commission 규칙과 Auto으로 대조합니다.
        </p>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ padding: '8px 16px', borderRadius: 10, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)', fontSize: 12 }}>
            ✅ <strong style={{ color: '#4ade80' }}>정산 Done</strong> ₩{(settlementStats.settledAmount / 1e6).toFixed(1)}M
          </div>
          <div style={{ padding: '8px 16px', borderRadius: 10, background: 'rgba(234,179,8,0.08)', border: '1px solid rgba(234,179,8,0.2)', fontSize: 12 }}>
            ⏳ <strong style={{ color: '#fbbf24' }}>정산 Pending</strong> ₩{(settlementStats.pendingAmount / 1e6).toFixed(1)}M
          </div>
          <div style={{ padding: '8px 16px', borderRadius: 10, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', fontSize: 12 }}>
            💸 <strong style={{ color: '#f87171' }}>Ad Spend 차감</strong> ₩{(settlementStats.totalAdFee / 1e6).toFixed(1)}M
          </div>
          ...
          <button onClick={() => navigate('/pnl-dashboard')} style={{ padding: '8px 16px', borderRadius: 10, border: '1px solid rgba(79,142,247,0.3)', background: 'rgba(79,142,247,0.08)', color: '#60a5fa', fontSize: 12, cursor: 'pointer', fontWeight: 700 }}>
            🌊 P&L Dashboard에서 Confirm →
          </button>
        </div>
        <div style={{ marginTop: 6, fontSize: 11, color: 'var(--text-secondary)' }}>
          🔴 실Time P&L 영업Profit: <strong style={{ color: pnlStats.operatingProfit >= 0 ? '#4ade80' : '#f87171' }}>₩{(pnlStats.operatingProfit / 1e6).toFixed(1)}M</strong>
          &nbsp;· 정산 데이터 Change 시 P&L Auto 반영됩니다
        </div>
      </div>
