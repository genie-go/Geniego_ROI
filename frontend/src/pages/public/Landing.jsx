...
const t = (key, lang) => DICT[key]?.[lang] || DICT[key]?.en || key;
...
function LangSelector({ lang, setLang }) {
    ...
    return (
        <div ref={ref} style={{ position: "fixed", top: 80, right: 28, zIndex: 1100 }}>
            <button onClick={() => setOpen(!open)} style={{
                display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 10,
                background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)",
                color: "var(--text-1)", cursor: "pointer", fontSize: 12, fontWeight: 600,
                backdropFilter: "blur(12px)", transition: "all 200ms",
            }}
            ...
            {open && (
                <div style={{
                    ...
                    background: "rgba(10,18,36,0.95)", border: "1px solid rgba(79,142,247,0.2)",
                    borderRadius: 12, padding: 6, minWidth: 160,
                    backdropFilter: "blur(20px)", boxShadow: "0 12px 40px rgba(0,0,0,0.5)",
                }}>
                    {LANGS.map(l => (
                        <button key={l.code} onClick={() => { setLang(l.code); setOpen(false); localStorage.setItem("landing_lang", l.code); }}
                            style={{
                                display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "8px 12px",
                                borderRadius: 8, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600,
                                background: lang === l.code ? "rgba(79,142,247,0.15)" : "transparent",
                                color: lang === l.code ? "#4f8ef7" : "#e2e8f0",
                            ...
                            <span>{l.label}</span>
                            {lang === l.code && <span style={{ marginLeft: "auto", fontSize: 10, color: "#4f8ef7" }}>✓</span>}
                        </button>
                    ))}
                </div>
            )}
...
export default function Landing() {
    ...
    const FEATURES = [
        { icon: "🛒", title: t("f1t", lang), desc: t("f1d", lang), color: "#4f8ef7" },
        { icon: "🏭", title: t("f2t", lang), desc: t("f2d", lang), color: "#22c55e" },
        { icon: "📣", title: t("f3t", lang), desc: t("f3d", lang), color: "#a855f7" },
        { icon: "🤝", title: t("f4t", lang), desc: t("f4d", lang), color: "#f59e0b" },
        { icon: "📊", title: t("f5t", lang), desc: t("f5d", lang), color: "#ef4444" },
        { icon: "💳", title: t("f6t", lang), desc: t("f6d", lang), color: "#14b8a6" },
        { icon: "🤖", title: t("f7t", lang), desc: t("f7d", lang), color: "#ec4899" },
        { icon: "🔌", title: t("f8t", lang), desc: t("f8d", lang), color: "#6366f1" },
    ];

    const STATS = [
        { label: t("statChannels", lang), value: "30+", icon: "🌐" },
        { label: t("statSKUs", lang), value: "1M+", icon: "📦" },
        { label: t("statData", lang), value: "500K+", icon: "📈" },
        { label: t("statClients", lang), value: "200+", icon: "🏢" },
    ];

    const TESTIMONIALS = [
        { name: "Sarah Kim", role: "Head of E-Commerce, Luxe Beauty Co.", text: t("t1", lang), avatar: "SK" },
        { name: "James Park", role: "CEO, GlobalTrade Inc.", text: t("t2", lang), avatar: "JP" },
        { name: "MinJi Lee", role: "Marketing Director, K-Style Fashion", text: t("t3", lang), avatar: "ML" },
    ];
    ...
    return (
        <PublicLayout>
            ...
            <section style={{ padding: "100px 28px 80px", textAlign: "center", position: "relative", overflow: "hidden" }}>
                <div style={{ position: "absolute", top: "10%", left: "50%", transform: "translateX(-50%)", width: 800, height: 400, borderRadius: "50%", background: "radial-gradient(ellipse, rgba(79,142,247,0.12) 0%, transparent 70%)", pointerEvents: "none" }} />

                <div className="pub-section pub-fadeUp">
                    <div style={{ display: "inline-block", padding: "5px 20px", borderRadius: 99, background: "rgba(79,142,247,0.08)", border: "1px solid rgba(79,142,247,0.25)", fontSize: 11, color: "#4f8ef7", fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 28 }}>
                        {t("heroBadge", lang)}
                    </div>

                    <h1 style={{ fontSize: "clamp(24px,3.2vw,40px)", fontWeight: 900, lineHeight: 1.2, margin: "0 0 24px", letterSpacing: -0.5 }}>
                        {t("heroTitle", lang).split("\n").map((line, i) => (
                            <React.Fragment key={i}>
                                {i === 0 ? (
                                    <span style={{ display: "block", fontSize: "clamp(18px,2.6vw,30px)", fontWeight: 800, letterSpacing: 2, marginBottom: 12, background: "linear-gradient(90deg,#22d3ee 0%,#4f8ef7 40%,#a855f7 100%)", backgroundSize: "200% 200%", animation: "pub-gradient 4s ease infinite", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", filter: "drop-shadow(0 0 24px rgba(34,211,238,0.35))" }}>{line}</span>
                                ) : (
                                    <span style={{ display: "block", background: "linear-gradient(135deg,#fff 30%,#4f8ef7 60%,#7c3aed 100%)", backgroundSize: "200% 200%", animation: "pub-gradient 6s ease infinite", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{line}</span>
                                )}
                            </React.Fragment>
                        ))}
                    </h1>

                    <p style={{ fontSize: 17, color: "rgba(255,255,255,0.5)", maxWidth: 640, margin: "0 auto 44px", lineHeight: 1.8, fontWeight: 400 }}>
                        {t("heroDesc", lang)}
                    </p>

                    <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap", marginBottom: 72 }}>
                        <Link to="/pricing" className="pub-btn-primary" style={{ padding: "14px 36px", fontSize: 15 }}>
                            {t("btnTrial", lang)}
                        </Link>
                        <Link to="/login" className="pub-btn-outline" style={{ padding: "14px 36px", fontSize: 15 }}>
                            {t("btnLogin", lang)}
                        </Link>
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, maxWidth: 800, margin: "0 auto" }}>
                        {STATS.map(s => (
                            <div key={s.label} style={{ padding: "24px 16px", borderRadius: 16, background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.06)", backdropFilter: "blur(10px)", textAlign: "center", transition: "border-color 300ms" }} onMouseEnter={e => e.currentTarget.style.borderColor = "rgba(79,142,247,0.3)"} onMouseLeave={e => e.currentTarget.style.borderColor = "rgba(255,255,255,0.06)"}>
                                <div style={{ fontSize: 14, marginBottom: 8 }}>{s.icon}</div>
                                <div style={{ fontSize: 32, fontWeight: 900, color: "#4f8ef7", letterSpacing: -1 }}>
                                    <AnimatedCounter target={s.value} suffix={s.value.includes("+") ? "+" : ""} />
                                </div>
                                <div style={{ fontSize: 11, color: "var(--text-3)", marginTop: 4, fontWeight: 500 }}>{s.label}</div>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <section style={{ padding: "32px 0", borderTop: "1px solid rgba(255,255,255,0.04)", borderBottom: "1px solid rgba(255,255,255,0.04)", overflow: "hidden" }}>
                <div style={{ textAlign: "center", marginBottom: 20 }}>
                    <span style={{ fontSize: 11, color: "var(--text-3)", fontWeight: 600, textTransform: "uppercase", letterSpacing: 2 }}>{t("integrationsLabel", lang)}</span>
                </div>
                <div style={{ display: "flex", gap: 48, justifyContent: "center", flexWrap: "wrap", padding: "0 28px" }}>
                    {INTEGRATIONS.map(name => (
                        <div key={name} style={{ padding: "8px 20px", borderRadius: 8, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", fontSize: 12, fontWeight: 700, color: "var(--text-3)", letterSpacing: 0.5 }}>{name}</div>
                    ))}
                </div>
            </section>
            ...
            <section style={{ padding: "100px 28px" }}>
                <div className="pub-section">
                    <div style={{ textAlign: "center", marginBottom: 64 }}>
                        <div style={{ display: "inline-block", padding: "4px 16px", borderRadius: 99, background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.25)", fontSize: 11, color: "#22c55e", fontWeight: 700, letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 20 }}>{t("featuresBadge", lang)}</div>
                        <h2 style={{ fontSize: 40, fontWeight: 900, margin: "0 0 14px", background: "linear-gradient(135deg,#fff 40%,rgba(255,255,255,0.6))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", letterSpacing: -1 }}>{t("featuresTitle", lang)}</h2>
                        <p style={{ fontSize: 15, color: "var(--text-3)", maxWidth: 520, margin: "0 auto" }}>{t("featuresDesc", lang)}</p>
                    </div>

                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(270px,1fr))", gap: 16 }}>
                        {FEATURES.map((f, i) => (
                            <div key={f.title} style={{ padding: "28px 24px", borderRadius: 18, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", transition: "all 300ms cubic-bezier(0.4,0,0.2,1)", cursor: "default", animationDelay: `${i * 0.05}s` }}
                                onMouseEnter={e => { e.currentTarget.style.borderColor = f.color + "50"; e.currentTarget.style.background = f.color + "08"; e.currentTarget.style.transform = "translateY(-4px)"; }}
                                ...
                                <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 10, color: "#fff" }}>{f.title}</div>
                                <div style={{ fontSize: 12, color: "var(--text-3)", lineHeight: 1.8 }}>{f.desc}</div>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {/* ═══ TESTIMONIALS ═══ */}
            <section style={{ padding: "80px 28px", background: "rgba(79,142,247,0.02)", borderTop: "1px solid rgba(79,142,247,0.08)", borderBottom: "1px solid rgba(79,142,247,0.08)" }}>
                <div className="pub-section">
                    <div style={{ textAlign: "center", marginBottom: 56 }}>
                        <div style={{ display: "inline-block", padding: "4px 16px", borderRadius: 99, background: "rgba(168,85,247,0.08)", border: "1px solid rgba(168,85,247,0.25)", fontSize: 11, color: "#a855f7", fontWeight: 700, letterSpacing: 1.2, textTransform: "uppercase", marginBottom: 20 }}>{t("testimonialBadge", lang)}</div>
                        <h2 style={{ fontSize: 36, fontWeight: 900, color: "#fff", letterSpacing: -0.5 }}>{t("testimonialTitle", lang)}</h2>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 20 }}>
                        {TESTIMONIALS.map(tm => (
                            <div key={tm.name} style={{ padding: "32px 28px", borderRadius: 18, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", transition: "border-color 300ms" }}
                                onMouseEnter={e => e.currentTarget.style.borderColor = "rgba(168,85,247,0.3)"}
                                onMouseLeave={e => e.currentTarget.style.borderColor = "rgba(255,255,255,0.07)"}>
                                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", lineHeight: 1.8, marginBottom: 24, fontStyle: "italic" }}>"{tm.text}"</div>
                                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                    <div style={{ width: 40, height: 40, borderRadius: 10, background: "linear-gradient(135deg,#4f8ef7,#7c3aed)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 13, color: "#fff" }}>{tm.avatar}</div>
                                    <div>
                                        <div style={{ fontSize: 13, fontWeight: 700, color: "#fff" }}>{tm.name}</div>
                                        <div style={{ fontSize: 11, color: "var(--text-3)" }}>{tm.role}</div>
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            <section style={{ padding: "100px 28px", textAlign: "center" }}>
                <div className="pub-section">
                    <div style={{ maxWidth: 700, margin: "0 auto", padding: "60px 48px", borderRadius: 24, background: "linear-gradient(135deg,rgba(79,142,247,0.08),rgba(124,58,237,0.08))", border: "1px solid rgba(79,142,247,0.15)", position: "relative", overflow: "hidden" }}>
                        <div style={{ position: "absolute", top: -60, right: -60, width: 200, height: 200, borderRadius: "50%", background: "radial-gradient(circle, rgba(79,142,247,0.15) 0%, transparent 70%)" }} />
                        <h2 style={{ fontSize: 34, fontWeight: 900, margin: "0 0 14px", color: "#fff", letterSpacing: -0.5, position: "relative" }}>{t("ctaTitle", lang)}</h2>
                        <p style={{ fontSize: 15, color: "rgba(255,255,255,0.5)", marginBottom: 36, position: "relative" }}>{t("ctaDesc", lang)}</p>
                        <div style={{ display: "flex", gap: 14, justifyContent: "center", position: "relative" }}>
                            <Link to="/pricing" className="pub-btn-primary" style={{ padding: "16px 44px", fontSize: 15 }}>{t("ctaBtnPricing", lang)}</Link>
                            <a href="mailto:support@genie-go.com" className="pub-btn-outline" style={{ padding: "16px 44px", fontSize: 15 }}>{t("ctaBtnContact", lang)}</a>
                        </div>
                    </div>
                </div>
            </section>
