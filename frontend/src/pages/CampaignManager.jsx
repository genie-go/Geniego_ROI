...
export default function CampaignManager(){
  ...
  const [_retryCount, _setRetryCount] = React.useState(0);
    ...
    // 184차 #5: enterprise 가이드 헬퍼(campMgr.* 직접 해석, 빈값 폴백)
    const g=useCallback(k=>{const v=t('campMgr.'+k);return (v&&v!=='campMgr.'+k)?v:'';},[t]);
    ...
    return(
        <div style={{ display: 'flex', flexDirection: 'column', maxWidth: 1200, margin: '0 auto', width: '100%', height: 'calc(100vh - 54px)', overflow: 'hidden', color: 'var(--text-1, #1e293b)', background: 'transparent' }}>
                    ...
                    <div style={CARD}>
                        ...
                        <div style={{ overflowX: 'auto' }}>
                            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                                ...
                                <tbody>{(sharedCampaigns || []).filter(c => c.impressions > 0).sort((a, b) => (b.impressions || 0) - (a.impressions || 0)).map(c => { const cfg = STS[c.status] || STS.draft; const ctr = c.impressions ? ((c.clicks || 0) / c.impressions * 100).toFixed(2) : '0'; const cvr = c.clicks ? ((c.conv || 0) / c.clicks * 100).toFixed(2) : '0'; return (<tr key={c.id} onClick={() => setDetailId(c.id)} style={{ borderBottom: '1px solid #f1f5f9', cursor: 'pointer', transition: 'background 0.15s', padding: '10px 12px', fontWeight: 700, color: c.roas >= 5 ? '#22c55e' : c.roas >= 3 ? '#4f8ef7' : '#f59e0b', borderRadius: 6, fontSize: 12, background: cfg.bg }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(79,142,247,0.04)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}><td>{c.name}</td><td><span>{stsLabel(c.status)}</span></td><td>{(c.impressions || 0).toLocaleString()}</td><td>{(c.clicks || 0).toLocaleString()}</td><td>{ctr}%</td><td>{(c.conv || 0).toLocaleString()}</td><td>{cvr}%</td><td>{c.roas}x</td></tr>); })}</tbody>
                            </table>
            ...
            {/* ══ TAB: GUIDE ════════════════════════════════════════ */}
            {tab==='guide'&&(()=>{
                // 184차 #5: enterprise 패턴 렌더러(CRM/OmniChannel/PriceOpt/Kakao/Email 정본 동일, NS=campMgr).
                ...
                <div className="guide-section" style={{ display:'grid', gap:18, minHeight:CONTENT_MIN, alignContent:'start', color:'#1e293b' }}>
                    ...
                    {tips.length>0 && <div style={{ ...card, background:'rgba(34,197,94,0.04)', borderColor:'rgba(34,197,94,0.25)' }}>
                        <div style={secTitle}>💡 {tr(T.guideTipsTitle)}</div>
