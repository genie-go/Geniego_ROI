...
const _SUMMARY = {
    total: _CUSTOMERS.length, mode: "",
    high_risk: _CUSTOMERS.filter(c => c.risk_level === "high").length,
    medium_risk: _CUSTOMERS.filter(c => c.risk_level === "medium").length,
    low_risk: _CUSTOMERS.filter(c => c.risk_level === "low").length,
    predicted_revenue_30d: 124000000,
};
const _LTV_SEGMENTS = [
    { tier: "diamond", label: "💎 Diamond", color: "#60a5fa", threshold: fmtKRW(1000000) + "+", customer_count: 847, avg_ltv: 2840000, total_ltv: 2403480000, max_ltv: 8900000, action: "Provide exclusive VIP benefits" },
    { tier: "gold", label: "🥇 Gold", color: "#fbbf24", threshold: fmtKRW(500000) + "+", customer_count: 2341, avg_ltv: 720000, total_ltv: 1685520000, max_ltv: 990000, action: "Premium membership upgrade" },
    { tier: "silver", label: "🥈 Silver", color: "#94a3b8", threshold: fmtKRW(200000) + "+", customer_count: 4123, avg_ltv: 310000, total_ltv: 1278130000, max_ltv: 498000, action: "Gold upgrade incentive campaign" },
    { tier: "bronze", label: "🥉 Bronze", color: "#cd7c4b", threshold: fmtKRW(50000) + "+", customer_count: 3892, avg_ltv: 102000, total_ltv: 396984000, max_ltv: 198000, action: "Repurchase incentive email" },
    { tier: "new", label: "🌱 New", color: "#4ade80", threshold: "~" + fmtKRW(50000), customer_count: 1644, avg_ltv: 18000, total_ltv: 29592000, max_ltv: 49000, action: "Start onboarding journey series" },
];
const _MODEL_METRICS = {
    ...
    product_recommendation: { model: "Collaborative Filtering", hit_rate: 0, ndcg: 0, coverage: 0, last_trained: new Date().toISOString().slice(0, 10), training_samples: 88420 },
};
...
function ModelMetricsCard({ metrics }) {
    ...
    const cards = [
        ...
        { title: "💰 LTV Forecast", m: metrics.ltv_prediction, keys: [["MAPE", "mape", "%"], ["R²", "r2", ""], ["RMSE", "rmse", "KRW"]] },
                    ...
                    {keys.map(([label, key, unit]) => (
                        <div key={key} style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                            ...
                            <span style={{ fontSize: 13, fontWeight: 700, color: C.green }}>
                                {unit === "KRW" ? fmtKRW(m[key] || 0) : `${m[key] || "-"}${unit}`}
                            </span>
                        </div>
                    ))}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                    {[["3개월", c.ltv_3m], ["12개월", c.ltv_12m], ["CLV", c.clv]].map(([label, val]) => (
                        <div key={label} style={{ textAlign: "center" }}>
                            ...
                            <div style={{ fontSize: 14, fontWeight: 800, color: C.purple }}>{fmtKRW(val || 0)}</div>
                        </div>
                    ))}
                </div>
            </div>
            ...
            {tab === "overview" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {[
                        ...
                        ["Cumulative LTV", fmtKRW(c.total_ltv || 0)],
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {(prodRecs.length ? prodRecs : (c.recommended_products || [])).map((p, i) => (
                        <div key={i} style={{ background: C.card, borderRadius: 10, padding: 12 }}>
                            ...
                            <div style={{ fontSize: 11, color: C.muted, marginBottom: 6 }}>{p.category} · {fmtKRW(p.price || 0)}</div>
...
function AIPredictionInner() {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            if (churnRes?.ok && churnRes.customers?.length) {
                ...
                setSummary(churnRes.summary || _SUMMARY);
                setMode(churnRes.mode || "");
            } else {
            ...
            setModelMetrics(modelRes?.ok ? modelRes.metrics : _MODEL_METRICS);
            // LTV 세그먼트: API Success 우선, Failed 시 로컬 폴백
            setLtvSegments(ltvRes?.ok && ltvRes.segments?.length ? ltvRes.segments : _LTV_SEGMENTS);
        } catch (e) {
            // 완전한 네트워크 Failed → 로컬 시뮬레이션 폴백
            setCustomers(_CUSTOMERS);
            setSummary(_SUMMARY);
            setModelMetrics(_MODEL_METRICS);
            setLtvSegments(_LTV_SEGMENTS);
            setMode("");
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div style={{ background: C.bg, minHeight: "100%", color: C.text, position: "relative" }}>
            ...
            {/* Header */}
            <div style={{ background: `linear-gradient(135deg,${C.surface},${C?.surface || "var(--surface)"})`, border: `1px solid ${C.border}`, borderRadius: 16, padding: "22px 28px", marginBottom: 20 }}>
                    ...
                    <div>
                        ...
                        <div style={{ fontSize: 12, color: C.muted, marginTop: 4 }}>
                            ...
                            <span style={{ marginLeft: 12, padding: "2px 8px", borderRadius: 6, fontSize: 10, background: mode === "live" ? `${C.green}22` : `${C.yellow}22`, color: mode === "live" ? C.green : C.yellow, fontWeight: 700 }}>
                                {mode === "live" ? t('aiPredict.liveDB') : mode === "" ? t('aiPredict.Sim') : "⚡ Simulation"}
                            </span>
                        </div>
                    </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 12, marginBottom: 20 }}>
                {[
                    ...
                    { icon: "💰", label: t('aiPredict.kpi.revenue'), value: summary.predicted_revenue_30d ? fmtKRW(summary.predicted_revenue_30d) : fmtKRW(124000000), color: C.green, sub: t('aiPredict.kpi.revenueSub') },
            ...
            {!loading && activeTab === "customers" && (
                <div>
                    ...
                    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden" }}>
                        <div style={{ display: "grid", gridTemplateColumns: "1.8fr 1fr 110px 120px 140px 120px 110px 80px", padding: "10px 18px", background: 'var(--surface)', fontSize: 10, color: C.muted, fontWeight: 700 }}>
                        ...
                        {filtered.map((c, i) => {
                            ...
                            return (
                                <div key={c.id || i} style={{ display: "grid", gridTemplateColumns: "1.8fr 1fr 110px 120px 140px 120px 110px 80px", padding: "12px 18px", borderTop: `1px solid ${C.border}`, alignItems: "center", transition: "background 0.15s", cursor: "default" }}
                                    ...
                                    <div style={{ fontSize: 13, fontWeight: 700, color: C.purple }}>{fmtKRW(c.ltv_12m || 0)}</div>
                        ...
                        <div key={seg.tier} style={{ background: C.card, borderRadius: 14, padding: 20, border: `1px solid ${C.border}`, borderLeft: `4px solid ${seg.color}` }}>
                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14 }}>
                                ...
                                <div style={{ textAlign: "right" }}>
                                    <div style={{ fontSize: 12, fontWeight: 400, color: C.muted }} >{(seg.customer_count || 0).toLocaleString()}<span> customers</span></div>
                                </div>
                            </div>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
                                {[["Average LTV", fmtKRW(seg.avg_ltv || 0)], ["Total LTV", fmtKRW(seg.total_ltv || 0)], ["Max LTV", fmtKRW(seg.max_ltv || 0)]].map(([label, val]) => (
