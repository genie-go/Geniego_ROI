...
const ROLES = [
    {
        ...
        desc: 'Ads Performance Analysis, AI Campaign Auto화, CRM 세그먼트',
        tags: ['Ads 최적화', 'A/B Test', 'Auto화'],
        steps: [
            { icon: '🌐', title: 'Ads Channel Integration', desc: 'Meta·Google·TikTok·Naver·Kakao API 키 입력만으로 Ads 데이터가 Auto Count집됩니다.', route: '/omni-channel', kpi: 'Average 30분 내 첫 데이터 Count집' },
            { icon: '🎯', title: '어트리뷰션 Analysis', desc: '어떤 Channel이 실제 Revenue에 기여했는지 Shapley Value 기반으로 공정하게 측정합니다.', route: '/attribution', kpi: 'Channel 낭비 Budget Average 23% 절감' },
            { icon: '🤖', title: 'AI Marketing Auto화', desc: 'AI가 Customer Segment를 Auto Generate하고, 최적 Send Time에 Email·Kakao·SMS를 Auto 집행합니다.', route: '/auto-marketing', kpi: 'CTR +41% Average 향상' },
            { icon: '👥', title: 'CRM 세그먼트 Settings', desc: 'VIP·이탈 위험·재구매 가능 Customer을 AI가 Auto Classify합니다.', route: '/crm', kpi: '이탈 Forecast 정확도 87%' },
            { icon: '🧪', title: 'A/B Test Run', desc: 'Bayesian 최적화로 승자 변형을 Statistics적으로 95% 신뢰도에서 Auto 확정합니다.', route: '/attribution', kpi: 'Conv. Rate Average +18% 향상' },
        ],
    },
    {
        id: 'commerce', icon: '🛒', title: 'Commerce Owner', color: '#f97316',
        desc: '멀티Channel Orders·Stock·WMS·반품 Unified Management',
        tags: ['Stock Sync', 'Orders Management', '3PL'],
        steps: [
            { icon: '🔗', title: '판매 Channel Integration', desc: 'Shopify·Coupang·Naver·Amazon SP-API를 Connect하면 Product·Stock·Orders이 실Time Sync됩니다.', route: '/omni-channel', kpi: 'Stock 오차 99.2% 제거' },
            { icon: '📦', title: 'Orders 허브 Settings', desc: '모든 Channel Orders을 한 곳에서 처리하고 배송사 Auto 배정 Rule을 Settings합니다.', route: '/order-hub', kpi: 'Orders 처리 Time 68% 단축' },
            { icon: '🏭', title: 'WMS 창고 Settings', desc: '입고·피킹·출고·Stock 실사를 바코드 스캔 기반으로 Auto화합니다.', route: '/wms', kpi: '피킹 Error율 0.08% 이하' },
            { icon: '💰', title: '정산 Auto화', desc: 'Channelper Commission를 Auto 대사하고 실질 순Profit을 실Time 계산합니다.', route: '/settlements', kpi: '정산 처리 Time 84% 절감' },
            { icon: '🔔', title: 'Stock Notification Settings', desc: '안전Stock 이하 진입 시 즉시 Notification, Count요Forecast 기반 Auto 발주 권고를 받습니다.', route: '/alert-policies', kpi: 'Stock 소진 사전 방지' },
        ],
    },
    {
        id: 'finance', icon: '📊', title: 'Finance·경영자', color: '#22c55e',
        desc: 'P&L Analysis, Channelper Profit성, ESG·리포트 Auto화',
        tags: ['P&L', 'Profit성 Analysis', 'ESG'],
        steps: [
            { icon: '💹', title: 'P&L Dashboard', desc: 'Channelper·SKUper 실질 순Profit을 Ad Spend·Commission·물류비 차감 후 실Time으로 Confirm합니다.', route: '/pnl-dashboard', kpi: 'Profit 구조 즉시 파악' },
            { icon: '📈', title: 'Channelper Profit성 Analysis', desc: 'LTV/CAC Rate로 어떤 Channel이 장기적으로 가장 Profit성이 높은지 Analysis합니다.', route: '/attribution', kpi: 'LTV/CAC Average 7.2x 유지' },
            { icon: '💳', title: '정산 대조', desc: 'Platform Settlement History과 실제 입Amount을 Auto 매칭해 미지급·오지급을 즉시 감지합니다.', route: '/reconciliation', kpi: '매칭율 97.6%' },
            { icon: '🌱', title: 'ESG Report', desc: '탄소 배출·지속가능성 Metric를 Auto Aggregate해 투자자·규제 대응 리포트를 Create합니다.', route: '/pnl-dashboard', kpi: '규제 준Count Auto화' },
            { icon: '📄', title: '리포트 Auto화', desc: '경영진용 Weekly/Monthly 리포트를 PDF·Excel로 스케줄 Auto Send Settings합니다.', route: '/report-builder', kpi: '리포트 작성 Time 90% 절감' },
        ],
    },
    {
        id: 'ops', icon: '⚙️', title: '운영 Owner', color: '#a855f7',
        desc: '3PL·물류·공급망·반품 Unified 운영',
        tags: ['3PL', '공급망', '반품'],
        steps: [
            { icon: '🚚', title: '3PL 파트너 Register', desc: 'CJ대한통운·Coupang풀필먼트·CFS 등 3PL 파트너를 Register하고 Auto 배차 Rule을 Settings합니다.', route: '/supply-chain', kpi: '물류비 최적화 Average 15%' },
            { icon: '🗺️', title: '공급망 Management', desc: '공급업체부터 소비자까지 All 공급망 가시성을 실Time으로 Confirm합니다.', route: '/supply-chain', kpi: '공급망 리스크 조기 감지' },
            { icon: '↩️', title: '반품 포털 Settings', desc: 'Customer 반품 신청→검Count→재입고→환불을 Auto화하고 반품 원인을 Analysis합니다.', route: '/returns-portal', kpi: '반품 처리 42% 단축' },
            { icon: '🌏', title: '아시아 물류 Integration', desc: 'in progress국·일본·동남아 3PL, 관세·통관 Auto화, 다국 창고 Management를 Settings합니다.', route: '/asia-logistics', kpi: '해외 배송 Error 78% Decrease' },
            { icon: '⚠️', title: '운영 Notification Settings', desc: '배송 지연, Stock 임박, 시스템 Error 발생 시 즉각 Notification을 Slack·Email로 받습니다.', route: '/alert-policies', kpi: '문제 감지-대응 Time 단축' },
        ],
    },
    {
        id: 'developer', icon: '💻', title: 'Developer·IT Management자', color: '#14d9b0',
        desc: 'API Integration, 시스템 모니터링, 데이터 스키마',
        tags: ['API', '시스템 모니터링', '데이터'],
        steps: [
            { icon: '🔑', title: 'API 키 Issue', desc: 'REST API 키를 Issue받아 외부 시스템과 Geniego-ROI를 Unified합니다.', route: '/api-keys', kpi: '400+ API 엔드포인트 지원' },
            { icon: '🔌', title: '커넥터 Settings', desc: 'ERP·PIM·WMS 등 기존 시스템과 양방향 Data Pipeline을 구성합니다.', route: '/connectors', kpi: '100+ 시스템 Integration 지원' },
            { icon: '📋', title: '데이터 스키마 Confirm', desc: 'Count집되는 모든 Event·Orders·Customer Data의 스키마와 정규화 규칙을 검토합니다.', route: '/data-schema', kpi: '실Time 데이터 검증' },
            { icon: '🖥️', title: '시스템 모니터링', desc: 'API 레이턴시, DLQ Error, 배치 Run Status를 실Time 모니터링합니다.', route: '/system-monitor', kpi: 'p99 < 200ms Goal' },
            { icon: '🛡️', title: 'AI 정책 Settings', desc: 'AI Auto화 Rule의 Run 조건·Approval 플로우·감사 로그를 구성합니다.', route: '/ai-policy', kpi: 'Auto화 신뢰도 Management' },
        ],
    },
];
        ...
        return (
            <div style={{ maxWidth: 1000, margin: '0 auto', padding: 4 }}>
                <div className="hero fade-up" style={{ textAlign: 'center', marginBottom: 24 }}>
                    ...
                    <div className="hero-title grad-blue-purple">Geniego-ROI에 오신 것을 환영합니다!</div>
                    <div className="hero-desc" style={{ maxWidth: 520, margin: '8px auto' }}>
                        역할을 Select하면 <strong>30분 내 첫 인사이트</strong>를 얻을 Count 있는 맞춤 5단계 경로를 안내합니다.
                    </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'center' }}>
                    <button onClick={() => selectedRole && startWithRole(selectedRole)} disabled={!selectedRole} style={{
                        ...
                        {selectedRole ? `${ROLES.find(r => r.id === selectedRole)?.title} Start하기 →` : '역할을 Select하세요'}
                    </button>
                </div>
            </div>
        );
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', padding: 4 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        <div>
                            ...
                            <div style={{ fontSize: 18, fontWeight: 900 }}>5단계 맞춤 경로</div>
                        </div>
                    </div>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                        ...
                        <button onClick={resetOnboarding} style={{ padding: '5px 12px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.12)', background: 'transparent', color: 'var(--text-3)', fontSize: 11, cursor: 'pointer' }}>역할 Change</button>
                    </div>
                </div>
            ...
            {step && (
                <div className="card card-glass fade-up" style={{ border: `1px solid ${role.color}22`, background: `linear-gradient(145deg,rgba(5,10,25,0.9),${role.color}05)` }}>
                    ...
                    <div style={{ padding: '10px 14px', borderRadius: 10, background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.2)', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                            <div style={{ fontSize: 10, color: 'var(--text-3)', marginBottom: 1 }}>이 단계 Done 시 예상 효과</div>
            ...
            {progress === 100 && (
                <div className="card card-glass" style={{ textAlign: 'center', marginTop: 16, border: '1px solid rgba(34,197,94,0.3)', background: 'rgba(34,197,94,0.04)' }}>
                    ...
                    <div style={{ fontSize: 22, fontWeight: 900, color: '#22c55e', marginBottom: 8 }}>5단계 Done!</div>
