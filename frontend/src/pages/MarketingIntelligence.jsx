...
function SectionTitle({ children }) {
    return <div style={{ fontSize: 13, fontWeight: 800, color: "var(--text-1)", marginBottom: 12 }}>{children}</div>;
}

/* ═══════════════════════ DATA ═══════════════════════════ */
const FUNNEL = [
    { id: "meta", name: "Meta Ads", icon: "📘", color: "#4f8ef7", imp: 4_820_000, clicks: 136_800, cart: 18_420, orders: 12_400, spend: 148_200, revenuePer: 68.4, newCustomerPct: 34, mobileConvPct: 62, desktopConvPct: 38 },
    { id: "tiktok", name: "TikTok Biz", icon: "🎵", color: "#a855f7", imp: 7_200_000, clicks: 72_100, cart: 9_800, orders: 8_200, spend: 89_400, revenuePer: 54.2, newCustomerPct: 58, mobileConvPct: 81, desktopConvPct: 19 },
    { id: "shopify", name: "Shopify", icon: "🛒", color: "#22c55e", imp: 2_100_000, clicks: 24_300, cart: 8_400, orders: 7_600, spend: 22_100, revenuePer: 89.1, newCustomerPct: 22, mobileConvPct: 55, desktopConvPct: 45 },
    { id: "amazon", name: "Amazon Ads", icon: "📦", color: "#eab308", imp: 1_950_000, clicks: 19_640, cart: 5_200, orders: 4_800, spend: 63_100, revenuePer: 112.3, newCustomerPct: 41, mobileConvPct: 48, desktopConvPct: 52 },
];

const INF_FUNNEL = [
    { name: "TechVibe", tier: "Macro", followers: 890_000, reach: 1_980_000, clicks: 28_400, orders: 1_240, spend: 2_800_000, conv: 4.37, roi: 117, color: "#4f8ef7" },
    { name: "Tech Unboxing", tier: "Micro", followers: 85_000, reach: 131_000, clicks: 8_900, orders: 412, spend: 800_000, conv: 4.63, roi: 93, color: "#22c55e" },
    { name: "Daily Gadget", tier: "Mid", followers: 210_000, reach: 254_000, clicks: 12_100, orders: 520, spend: 1_200_000, conv: 4.30, roi: 72, color: "#a855f7" },
    { name: "IT꿀팁 박해진", tier: "Micro", followers: 320_000, reach: 890_000, clicks: 14_200, orders: 42, spend: 600_000, conv: 0.30, roi: 12, color: "#ef4444" },
    { name: "뷰티테크 지나", tier: "Nano", followers: 9_400, reach: 38_000, clicks: 2_800, orders: 38, spend: 200_000, conv: 1.36, roi: 28, color: "#eab308" },
];
...
const STD_KPIS = [
    ...
    { id: "revenue_per_imp", cat: "Profit성", name: "Impressions당 Profit", desc: "인상당 기대 Revenue", weight: 5, calc: (d) => (d.orders * d.revenuePer / d.imp) * 1000, format: v => "₩" + v.toFixed(1) + "/천", target: 0.5 },
...
const CONTRIB_DIMENSIONS_KEYS = [
    { key: "channel", labelKey: "dimChannel", items: [{ name: "Paid Ad", value: 38, color: "#4f8ef7" }, { name: "인플루언서", value: 28, color: "#a855f7" }, { name: "SEO/오가닉", value: 18, color: "#22c55e" }, { name: "Email/SMS", value: 10, color: "#14d9b0" }, { name: "직접 유입", value: 6, color: "#eab308" }] },
    { key: "content", labelKey: "dimContent", items: [{ name: "숏폼 영상", value: 32, color: "#a855f7" }, { name: "Product Image", value: 24, color: "#4f8ef7" }, { name: "리뷰/UGC", value: 21, color: "#22c55e" }, { name: "인포그래픽", value: 13, color: "#eab308" }, { name: "Text Ad", value: 10, color: "#ef4444" }] },
    { key: "tier", labelKey: "dimTier", items: [{ name: "Macro (100K~1M)", value: 42, color: "#4f8ef7" }, { name: "Micro (10~100K)", value: 31, color: "#22c55e" }, { name: "Mid (1~10M)", value: 18, color: "#a855f7" }, { name: "Nano (<10K)", value: 9, color: "#eab308" }] },
    { key: "device", labelKey: "dimDevice", items: [{ name: "모바일 앱", value: 44, color: "#4f8ef7" }, { name: "모바일 웹", value: 28, color: "#a855f7" }, { name: "PC/태블릿", value: 28, color: "#22c55e" }] },
    { key: "time", labelKey: "dimTime", items: [{ name: "저녁(18~22시)", value: 34, color: "#4f8ef7" }, { name: "오후(12~18시)", value: 26, color: "#22c55e" }, { name: "오전(9~12시)", value: 18, color: "#eab308" }, { name: "심야(22~24시)", value: 14, color: "#a855f7" }, { name: "새벽(0~9시)", value: 8, color: "#ef4444" }] },
    { key: "customer", labelKey: "dimCustomer", items: [{ name: "신규 Customer", value: 38, color: "#22c55e" }, { name: "재방문 Customer", value: 35, color: "#4f8ef7" }, { name: "리타겟팅", value: 27, color: "#a855f7" }] },
    { key: "coupon", labelKey: "dimCoupon", items: [{ name: "Coupon Use", value: 28, color: "#eab308" }, { name: "타임세일", value: 24, color: "#ef4444" }, { name: "General 구매", value: 28, color: "#4f8ef7" }, { name: "멤버십 혜택", value: 20, color: "#22c55e" }] },
    { key: "touchpoint", labelKey: "dimTouch", items: [{ name: "1회 접촉", value: 22, color: "#ef4444" }, { name: "2~3회 접촉", value: 31, color: "#eab308" }, { name: "4~6회 접촉", value: 28, color: "#22c55e" }, { name: "7회+ 접촉", value: 19, color: "#4f8ef7" }] },
];

const INIT_RULES = [
    { id: 1, name: "ROAS < 2.0 Campaign Auto 일시정지", trigger: "ROAS < 2.0 (3일 연속)", action: "Campaign 일시정지 + Slack Notification", status: "pending", channel: "Meta Ads", risk: "low", savings: "₩24,000/일", confidence: 91 },
    { id: 2, name: "Budget 92% 소진 시 10% 증액", trigger: "Budget Burn Rate ≥ 92%", action: "일 Budget 10% Auto 증액", status: "pending", channel: "TikTok Biz", risk: "medium", savings: "예상 ROAS +0.4x", confidence: 78 },
    { id: 3, name: "CTR 4%+ Ad세트 Budget 2배", trigger: "CTR ≥ 4% (5일 유지)", action: "Ad세트 Budget ×2.0", status: "approved", channel: "Meta Ads", risk: "medium", savings: "예상 Conversion +180건", confidence: 84 },
    { id: 4, name: "인플루언서 ROI < 30x 계약 End 검토", trigger: "ROI < 30x (2개월)", action: "계약 End 검토 Notification", status: "pending", channel: "Influencer", risk: "low", savings: "₩800,000/월", confidence: 89 },
    { id: 5, name: "Shopify 이탈 카트 리Marketing Auto Run", trigger: "카트 이탈 후 24Time", action: "Email + Meta 리타겟 Ad Run", status: "active", channel: "Shopify", risk: "low", savings: "Conv. Rate +2.1%", confidence: 93 },
    { id: 6, name: "신규 Customer Rate < 20% 시 신규 Ad 증액", trigger: "신규Customer Rate < 20%", action: "신규 Customer 타겟 Ad Budget +30%", status: "pending", channel: "Meta Ads", risk: "medium", savings: "CAC 최적화", confidence: 72 },
];

const INIT_RECS = [
    { id: 1, title: "TikTok UGC 콘텐츠 Budget 2배 증액", category: "Budget 재배분", confidence: 91, expectedROAS: "4.2x (+1.0x)", effort: "낮음", reason: "숏폼 영상 기여도 32%로 1위. UGC_Challenge ROAS 3.95x로 최고 효율.", actions: ["TikTok 월 Budget ₩89K → ₩150K 증액", "UGC_Challenge 복제 Campaign 2개 Add", "Hashtag_Spring 크리에이티브 교체"], color: "#a855f7" },
    { id: 2, title: "Meta Catalog_Dynamic 정지 → Retargeting Conversion", category: "Campaign 최적화", confidence: 88, expectedROAS: "3.8x (+0.8x)", effort: "낮음", reason: "Catalog_Dynamic ROAS 2.91x로 Goal치 미달. Retargeting_Q1 ROAS 6.84x로 최고.", actions: ["Catalog_Dynamic 즉시 일시정지 (₩12,300 절감)", "Budget Retargeting_Q1으로 Previous", "Lookalike 오디언스 분리 Test"], color: "#4f8ef7" },
    { id: 3, title: "저녁 18~22시 집in progress Budget 배분 전략", category: "Timezone 최적화", confidence: 83, expectedROAS: "4.0x (+0.6x)", effort: "in progress간", reason: "저녁 Timezone Conversion 기여도 34%로 최고. 현재 균등 배분으로 황금 Timezone Impressions 손실.", actions: ["Timezoneper 입찰 조정 Apply (저녁 ×1.4배)", "Meta/TikTok Ad 스케줄 Settings", "A/B Test로 2주 검증"], color: "#22c55e" },
    { id: 4, title: "Micro 인플루언서 포트폴리오 확장", category: "인플루언서 전략", confidence: 79, expectedROAS: "Inf ROI 80x+", effort: "High", reason: "Micro 기여도 31%로 Macro 대비 CPO 40% 저렴. Tech Unboxing ROI 93x.", actions: ["IT/테크 분야 Micro 5명 발굴 (팔로워 30~80K)", "CPS(판매Commission) 계약 우선 협의", "3개월 시범 운영 후 정규 계약"], color: "#14d9b0" },
    { id: 5, title: "Coupon 전략 재설계 — 신규 Customer 전용 Pro모", category: "Pro모션 최적화", confidence: 76, expectedROAS: "CVR +1.8%p", effort: "in progress간", reason: "Coupon 기여도 28%이나 재방문 Customer in progress복 사용으로 마진 잠식.", actions: ["신규/재방문 Coupon Code 분리 Issue", "신규 전용 15% 할인 + 재방문 5% 포인트 Conversion", "Shopify 이탈 카트 Coupon A/B Test"], color: "#eab308" },
];
                                ...
                                {selected?.id === d.id && (
                                    <div style={{ marginTop: 14, padding: "12px 14px", borderRadius: 10, background: "rgba(0,0,0,0.3)", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, borderTop: `1px solid ${d.color}22` }}>
                                        {[
                                            ...
                                            <div key={s.label} style={{ padding: "8px 10px", background: "rgba(255,255,255,0.02)", borderRadius: 8 }}>
            ...
            <div className="card card-glass">
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
                    {CONTRIB_DIMENSIONS_KEYS.map(d => (
                        <button key={d.key} onClick={() => setSelDim(d.key)} style={{
                            padding: "5px 14px", borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: "pointer", border: "none",
                            background: selDim === d.key ? "#4f8ef7" : "rgba(255,255,255,0.07)", color: selDim === d.key ? "#fff" : "var(--text-2)",
                        }}>{t(`marketingIntel.${d.labelKey}`)}</button>
                    ))}
                </div>
                {dim && (
                    <div>
        ...
        <div style={{ display: "grid", gap: 18 }}>
            <div className="card card-glass" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 20px" }}>
                ...
                <button onClick={runAI} disabled={loading} style={{
                    padding: "8px 20px", borderRadius: 10, background: loading ? "rgba(99,102,241,0.3)" : "linear-gradient(135deg,#6366f1,#4f8ef7)",
                    border: "none", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer",
                }}>{loading ? t("marketingIntel.analyzing") : t("marketingIntel.runAI")}</button>
            </div>
                                ...
                                <div>
                                    ...
                                    {rec.actions.map((a, j) => (
                                        <div key={j} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginBottom: 6, fontSize: 12 }}>
                                            ...
                                            <span style={{ color: "var(--text-1)" }}>{a}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div key={r.id} style={{ padding: "14px 16px", borderRadius: 12, background: "rgba(0,0,0,0.25)", border: `1px solid ${sc.color}22` }}>
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <div style={{ fontWeight: 700, fontSize: 13, color: "var(--text-1)", marginBottom: 4 }}>{r.name}</div>
                                        ...
                                        <div style={{ fontSize: 11, marginTop: 4 }}>
                                            ...
                                            <span style={{ color: "var(--text-3)" }}>{t("marketingIntel.confidence")}: <strong style={{ color: "#4f8ef7" }}>{r.confidence}%</strong></span>
                                        </div>
                                    </div>
            ...
            <div style={{ display: "flex", gap: 6, padding: "6px", background: "rgba(0,0,0,0.25)", borderRadius: 14, flexWrap: "wrap" }}>
                {TABS.map(tab_item => (
                    <button key={tab_item.id} onClick={() => setTab(tab_item.id)} style={{
                        padding: "8px 16px", borderRadius: 10, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 12,
                        background: tab === tab_item.id ? "linear-gradient(135deg,#4f8ef7,#6366f1)" : "transparent",
                        color: tab === tab_item.id ? "#fff" : "var(--text-2)", transition: "all 150ms",
                    }}>
