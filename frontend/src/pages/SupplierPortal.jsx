...
export default function SupplierPortal() {
  ...
  const tabs = ["Supplier List","Purchase Orders","Performance","Contracts"];
  const kpis = [{"emoji":"🏭","label":"Suppliers","val":32},{"emoji":"📋","label":"Active POs","val":18},{"emoji":"✅","label":"On-time Rate","val":"94%"},{"emoji":"⭐","label":"Avg Score","val":"4.2"}];
    ...
    <div style={{ padding: 24, minHeight: "100%", color: "var(--text-1, #1e293b)" }}>
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            <div style={{ fontSize: 22, fontWeight: 800, color: "var(--text-1, #1e293b)" }}>🏭 Supplier Portal</div>
            <div style={{ fontSize: 13, color: "var(--text-3, #64748b)", marginTop: 2 }}>Manage suppliers, purchase orders, and vendor performance</div>
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 12 }}>
          {["Supplier scoring","Auto PO generation","Lead time tracking","Contract management"].map((f, i) => (
            <div key={i} style={{
