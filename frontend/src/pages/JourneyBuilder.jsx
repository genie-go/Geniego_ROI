...
export default function JourneyBuilder() {
    ...
    // 184차 #5: enterprise 가이드 헬퍼(jb.* 직접 해석, 빈값 폴백)
    const g = useCallback(k => { const v = t('jb.' + k); return (v && v !== 'jb.' + k) ? v : ''; }, [t]);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', maxWidth: isMobile ? '100%' : 1200, margin: '0 auto', width: '100%', flex: 1, minHeight: 0, color: '#1e293b', background: 'transparent', padding: isMobile ? '0 8px' : '0' }}>
            ...
            <div className="fade-up" style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', padding: isMobile ? '10px 4px 20px' : '16px 8px 28px' }}>
                            ...
                            <div style={{ ...CARD, padding: 0, overflow: isMobile ? 'visible' : 'hidden' }}>
                                {isMobile ? (
                                    ...
                                    <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
                                        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(56px,2fr) minmax(52px,90px) minmax(52px,100px) minmax(48px,80px) minmax(48px,80px) minmax(48px,80px) minmax(64px,140px)', gap: 6, padding: '14px 20px', background: 'rgba(0,0,0,0.02)', borderBottom: '1px solid rgba(0,0,0,0.06)', fontSize: 12, fontWeight: 700, color: '#64748b' }}>
                                        ...
                                        {journeys.map(j => {
                                            ...
                                            const tCfg = TRIGGER_CFG[j.trigger_type] || TRIGGER_CFG.manual;
                                            return (
                                                <div key={j.id} style={{ display: 'grid', gridTemplateColumns: 'minmax(56px,2fr) minmax(52px,90px) minmax(52px,100px) minmax(48px,80px) minmax(48px,80px) minmax(48px,80px) minmax(64px,140px)', gap: 6, padding: '12px 20px', borderBottom: '1px solid rgba(0,0,0,0.03)', alignItems: 'center', transition: 'background 0.15s' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(79,142,247,0.04)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                ...
                {/* ══ TAB: GUIDE ════════════════════════════════════════ */}
                {tab === 'guide' && (() => {
                    // 184차 #5: enterprise 패턴 렌더러(CRM/OmniChannel/PriceOpt/Kakao/Email/CampaignMgr 정본 동일, NS=jb).
                    const COLORS = ['#4f8ef7','#22c55e','#a855f7','#f59e0b','#f97316','#06b6d4','#ec4899','#14b8a6','#8b5cf6','#3b82f6','#10b981','#e11d48'];
                    const ICONS = ['🎯','👥','🗺️','📡','⏱️','🔀','🔍','🚀','📜','📈','♻️','🔐'];
                    ...
                    const tips = []; for (let i = 1; i <= 8; i++) { const tip = g('guideTip' + i); if (tip) tips.push(tip); }
                    ...
                    <div className="guide-section" style={{ display: 'grid', gap: 18, minHeight: CONTENT_MIN, alignContent: 'start', color: '#1e293b' }}>
                        <div style={{ background: 'linear-gradient(135deg,#eef2ff,#cffafe)', borderRadius: 16, border: '1px solid #a5f3fc', padding: '28px 24px', textAlign: 'center' }}>
                        ...
                        {tips.length > 0 && <div style={{ ...card, background: 'rgba(34,197,94,0.04)', borderColor: 'rgba(34,197,94,0.25)' }}>
                            <div style={secTitle}>💡 {tr(K.guideTipsTitle)}</div>
                        ...
                        {faqs.length > 0 && <div style={card}>
                            <div style={secTitle}>❓ {g('guideFaqTitle')}</div>
