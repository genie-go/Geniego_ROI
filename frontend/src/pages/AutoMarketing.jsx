        ...
        <div style={{ display: 'flex', flexDirection: 'column', maxWidth: 1200, margin: '0 auto', width: '100%', height: 'calc(100vh - 54px)', /* topbar ~54px — viewport-level scroll constraint */
            ...
            {tab === "guide" && (
                <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
                    ...
                    <div className="am-banner-guide" style={{ ...cardStyle, background: isLight ? 'rgba(79,142,247,0.06)' : 'linear-gradient(135deg,rgba(79,142,247,0.12),rgba(168,85,247,0.08))', borderColor:'rgba(79,142,247,0.3)', textAlign:'center', padding:32 }}>
                        ...
                        <div style={{ fontSize:13, color: isLight ? '#374151' : '#64748b', maxWidth:640, margin:'6px auto 0', lineHeight:1.7 }}>{t('marketing.guideSub')}</div>
                        <div style={{ display:'flex', gap:8, justifyContent:'center', flexWrap:'wrap', marginTop:14 }}>
                            {[{i:'🔰',k:'guideBeginnerBadge',c:'#22c55e'},{i:'⏱️',k:'guideTimeBadge',c:'#4f8ef7'},{i:'🌐',k:'guideLangBadge',c:'#a855f7'}].map((b,i)=>(
                                <span key={i} style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'5px 12px', borderRadius:99, background:`${b.c}14`, color:b.c, fontSize:12, fontWeight:700 }}>{b.i} {t('marketing.'+b.k)}</span>
                            ))}
                        </div>
                    ...
                    <div style={cardStyle}>
                        ...
                        {[
                          ...
                          {phaseKey:'guidePhaseC',phaseIcon:'🤖',phaseColor:'#22c55e',steps:[{n:'8',k:'guideStep8',c:'#22c55e',icon:'🤖'},{n:'9',k:'guideStep9',c:'#16a34a',icon:'🎨'},{n:'10',k:'guideStep10',c:'#15803d',icon:'👁️'}]},
                          {phaseKey:'guidePhaseD',phaseIcon:'📈',phaseColor:'#f97316',steps:[{n:'11',k:'guideStep11',c:'#f97316',icon:'✅'},{n:'12',k:'guideStep12',c:'#ea580c',icon:'📊'},{n:'13',k:'guideStep13',c:'#c2410c',icon:'🔄'}]},
                          {phaseKey:'guidePhaseE',phaseIcon:'🏆',phaseColor:'#06b6d4',steps:[{n:'14',k:'guideStep14',c:'#06b6d4',icon:'📋'},{n:'15',k:'guideStep15',c:'#0e7490',icon:'🏆'}]},
                        ].map((ph,pi) => (
                          <div key={pi} style={{ marginBottom:20 }}>
                            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12, padding:'8px 14px', borderRadius:10, background:`linear-gradient(135deg,${ph.phaseColor}14,${ph.phaseColor}06)` }}>
                              ...
                              <span style={{ fontWeight:800, fontSize:14, color:ph.phaseColor }}>{t(`marketing.${ph.phaseKey}`)}</span>
                            </div>
                        ...
                        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                            {[1,2,3,4,5].map(n => (
                                <div key={n} style={{ padding:'12px 14px', background:'rgba(241,245,249,0.6)', borderRadius:10, border:'1px solid rgba(99,140,255,0.08)' }}>
                                    <div style={{ fontWeight:700, fontSize:13, color:'#4f8ef7', marginBottom:5 }}>Q. {t(`marketing.guideFaq${n}Q`)}</div>
                                    <div style={{ fontSize:12, color:'#64748b', lineHeight:1.7 }}>A. {t(`marketing.guideFaq${n}A`)}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                    {/* ── 완료 CTA ── */}
                    <div style={{ ...cardStyle, background:'linear-gradient(135deg,rgba(34,197,94,0.08),rgba(79,142,247,0.06))', borderColor:'rgba(34,197,94,0.3)', textAlign:'center', padding:28 }}>
                        <div style={{ fontSize:32 }}>🎉</div>
                        <div style={{ fontWeight:900, fontSize:18, marginTop:6, color:'#1e293b' }}>{t('marketing.guideReadyTitle')}</div>
                        <div style={{ fontSize:13, color:'#64748b', maxWidth:600, margin:'8px auto 0', lineHeight:1.7 }}>{t('marketing.guideReadyDesc')}</div>
                        <button onClick={() => setTab('setup')} style={{ marginTop:16, padding:'12px 32px', borderRadius:12, border:'none', cursor:'pointer', background:'linear-gradient(135deg,#22c55e,#4f8ef7)', color:'#fff', fontWeight:800, fontSize:14 }}>🚀 {t('marketing.guideStartBtn')}</button>
                    </div>
                </div>
            )}
            ...
            {tab === "setup" && (
                <div style={{ display: "grid", gap: 18 }}>
                    ...
                    <div style={cardStyle} data-onboard-cta="mkt-category" data-onboard-hint={t("marketing.onboardCatHint", "먼저 판매 상품/서비스 카테고리를 선택하세요")}>
