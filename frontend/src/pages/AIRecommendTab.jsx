            ...
            <div style={{ padding: '10px 14px', borderRadius: 10, background: '#4f8ef70f', border: budgetEditing ? '1.5px solid #4f8ef7' : '1px solid #4f8ef722', textAlign: 'center', cursor: 'pointer' }}
                ...
                {budgetEditing ? (
                    <input autoFocus type="number" value={budgetInputVal}
                        ...
                        onKeyDown={e => { if (e.key === 'Enter') e.target.blur(); if (e.key === 'Escape') { setBudgetEditing(false); setBudgetInputVal(''); }}} style={{ width: '100%', background: 'transparent', border: 'none', outline: 'none', textAlign: 'center', fontSize: 14, fontWeight: 900, color: '#4f8ef7', appearance: 'textfield' }}
                    />
                ) : (
        ...
        <div key={ch.channel_id} style={{ marginBottom: 9 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, marginBottom: 3 }}>
                ...
                <span style={{ color: 'var(--text-2)' }}>
                    {t('gAiRec.monthly')} \u20a9{(ch.monthly_budget || 0).toLocaleString()} ({ch.budget_pct}%)
                    {ch.effectiveness_score ? ` · ${t('gAiRec.effectiveness')} ${ch.effectiveness_score}pts` : ''}
                </span>
            </div>
        ...
        <div key={ch.channel_id} className="card card-glass" style={{ padding: 18, border: `1px solid ${color}30` }}>
            ...
            <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                <svg id={svgId} width={W} height={H}
                    ...
                    <text x="26" y="30" fontSize={size === 'banner' ? 10 : 12} fill="white" fontFamily="sans-serif" fontWeight="700">{catLabel || t('gAiRec.deliveryDefault')}</text>
                    {size === 'banner' ? (
                        <>
                            <text x="16" y={H * 0.55} fontSize="16" fill="white" fontFamily="sans-serif" fontWeight="900">{(cr.headline || ch.channel_name + ' ' + t('gAiRec.adDefault')).slice(0, 22)}</text>
    ...
    return (
        <div style={{ display: 'grid', gap: 16 }}>
            ...
            <div style={{ textAlign: 'center', padding: '8px 0 4px' }}>
                <div style={{ fontSize: 22, fontWeight: 900, background: 'linear-gradient(135deg,#4f8ef7,#a855f7)', marginBottom: 4 }}>
                    {t('gAiRec.title')}
                </div>
                        ...
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                            {['Naver Shopping', 'Coupang', 'Own Mall', '11Street', 'Gmarket', 'Kakao Shopping', 'Amazon', 'Rakuten'].map(ch => (
                                <button key={ch} onClick={() => toggleSalesCh(ch)}
                                    ...
                                    {t('gAiRec.channel_' + ch.replace(/\s/g, '')) || ch}
                                </button>
                            ))}
                        </div>
                    </div>
            ...
            {status === 'loading' && (
                <div style={{ padding: '44px 0', textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 4 }}>"{searchQ}" — {t('gCat.' + cat.id + '_label')} {t('gAiRec.analyzingBest')}</div>
