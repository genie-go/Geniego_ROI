...
const VendorBadge = ({ vendor }) => (
    <span style={{
        ...
        textTransform: "uppercase" }}>{vendor}</span>
);
...
/* ─── Fallback Data (empty — live API only) ── */
const FALLBACK_RAW = [];
const FALLBACK_NORM = [];
                ...
                {[
                    ...
                    <React.Fragment key={s.label}>
                        <div style={{
                            ...
                            display: "flex", gap: 5, alignItems: "center" }}><span>{s.icon}</span>{s.label}</div>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                        <thead>
                            <tr style={{ borderBottom: "1px solid rgba(99,140,255,0.1)" }}>
                                {[t('auto.l2spnk', '필드명'), t('auto.xfucnv', '타입'), "Description"].map(h => (
        ...
        <div style={{ display: "grid", gap: 12 }}>
            <div style={{ display: "flex", gap: 6 }}>
                {["all", "ad", "market", "influencer"].map(d => (
                    <button key={d} onClick={() => setDomain(d)} style={{ padding: "5px 14px", borderRadius: 8, border: "1px solid", borderColor: domain === d ? (DOMAIN_COLORS[d] || "#4f8ef7") : "var(--border)", background: domain === d ? (DOMAIN_COLORS[d] || "#4f8ef7") + "15" : "transparent", color: domain === d ? (DOMAIN_COLORS[d] || "#4f8ef7") : "var(--text-3)", fontSize: 11, cursor: "pointer", fontWeight: 700 }}>{d === "all" ? "All" : d}</button>
                ))}
    ...
    return (
        <div style={{ display: "grid", gap: 20 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
                ...
                {/* UGC Breakdown */}
                <div className="card card-glass" style={{ padding: 16 }}>
                    <div style={{ fontWeight: 800, fontSize: 11, marginBottom: 12 }}>{t('auto.pw7qil', '🎬 UGC 권리/화이트리스트')}</div>
...
export default function EventNorm() {
  ...
  initGlobals(t);
    ...
    const [rawRows, setRawRows] = useState(FALLBACK_RAW);
    const [normRows, setNormRows] = useState(FALLBACK_NORM);
    ...
    const handleSeed = async () => {
        ...
        try {
            ...
            if (r.ok) {
                ...
                showToast(`✓ ${d.seeded ?? 0}개  Event 시드 Done`);
                await fetchAll();
            } else {
        ...
        <div style={{ display: "grid", gap: 16, position: "relative" }}>
            ...
            {toast && (
                <div style={{
                    ...
                    border: `1px solid ${toast.color}40`, backdropFilter: "blur(8px)" }}>{toast.msg}</div>
            )}

            {/* Hero */}
            <div className="hero fade-up" style={{ background: "linear-gradient(135deg,rgba(249,115,22,0.05),rgba(79,142,247,0.05))", borderColor: "rgba(249,115,22,0.15)" }}>
