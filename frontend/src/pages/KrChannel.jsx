...
// ── Tab: Channels ─────────────────────────────────────────────────────────────
const MOCK_CHANNELS = [
    { channel_key: "coupang", display_name: "Coupang", currency: "KRW", settlement_cycle: "Monthly 15일", vat_rate: 0.1, note: "Rocket Delivery/Wing Commission per도" },
    { channel_key: "naver", display_name: "Naver 스마트스토어", currency: "KRW", settlement_cycle: "매주 화요일", vat_rate: 0.1, note: "포인트/Coupon 차감 후 지급" },
    { channel_key: "11st", display_name: "11Street", currency: "KRW", settlement_cycle: "Monthly 20일", vat_rate: 0.1, note: "SK페이 Payment Commission per도" },
    { channel_key: "gmarket", display_name: "Gmarket", currency: "KRW", settlement_cycle: "Monthly 20일", vat_rate: 0.1, note: "옥션과 공동 정산" },
    { channel_key: "kakaogift", display_name: "Kakao선물하기", currency: "KRW", settlement_cycle: "Monthly 말일", vat_rate: 0.1, note: "Kakao페이 Integration" },
    { channel_key: "lotteon", display_name: "Lotte ON", currency: "KRW", settlement_cycle: "Monthly 15일", vat_rate: 0.1, note: "L.POINT 사용" },
];
...
function ChannelsTab() {
    ...
    useEffect(() => {
        fetch(`${API}/v419/kr/channels`)
            ...
            .catch(() => { setChannels(MOCK_CHANNELS); setIsMock(true); });
    }, []);
        ...
        <div>
            {isMock && (
                <div style={{ marginBottom: 12, padding: "6px 14px", borderRadius: 8, background: "rgba(234,179,8,0.08)", border: "1px solid rgba(234,179,8,0.2)", fontSize: 11, color: "#eab308" }}>
                    🟡 MOCK — 백엔드 API Disconnected (샘플 데이터)
                </div>
            )}
                ...
                {channels.map((ch) => (
                    <div key={ch.channel_key} className="card" style={{ borderLeft: `3px solid ${chColor(ch.channel_key)}` }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, fontSize: 11, color: "#94a3b8" }}>
                            <span>화폐</span><span style={{ color: "#e2e8f0" }}>{ch.currency}</span>
                            <span>정산 주기</span><span style={{ color: "#e2e8f0" }}>{ch.settlement_cycle}</span>
                            <span>부가세</span><span style={{ color: "#e2e8f0" }}>{(ch.vat_rate * 100).toFixed(0)}%</span>
                        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            <div>
                ...
                <div style={{ marginBottom: 10 }}>
                    ...
                    <select value={form.channel_key} onChange={(e) => { setForm((f) => ({ ...f, channel_key: e.target.value })); if (e.target.value) loadRules(e.target.value); }}
                        style={{ display: "block", width: "100%", background: "#0f172a", border: "1px solid #1c2842", borderRadius: 6, color: "#e2e8f0", padding: "5px 8px", fontSize: 12, marginTop: 3 }}>
                ...
                <div style={{ display: "grid", gap: 7, marginBottom: 10 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}>
                        <Inp label="Basic 배송비 (₩)" k="shipping_standard" ph="3000" />
                        <Inp label="Basic 반품비 (₩)" k="return_fee_standard" ph="5000" />
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}>
                ...
                {rules.map((r, i) => (
                    <div key={i} style={{ padding: "8px 10px", background: "#0f172a", borderRadius: 6, marginBottom: 6, fontSize: 11 }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 4, color: "#94a3b8" }}>
                            <span>Commission <b style={{ color: "#e2e8f0" }}>{PCT(r.platform_fee_rate * 100)}</b></span>
                            <span>Ad <b style={{ color: "#e2e8f0" }}>{PCT(r.ad_fee_rate * 100)}</b></span>
                            <span>배송 <b style={{ color: "#e2e8f0" }}>{KRW(r.shipping_standard)}</b></span>
                            <span>부가세 <b style={{ color: "#e2e8f0" }}>{PCT(r.vat_rate * 100)}</b></span>
                        </div>
...
function IngestTab() {
    ...
    const [linesJson, setLinesJson] = useState(JSON.stringify(DEMO_LINES["coupang"], null, 2));
    ...
    const loadDemo = (key) => {
        setSel(key);
        setLinesJson(JSON.stringify(DEMO_LINES[key] || DEMO_LINES["coupang"], null, 2));
    };
    ...
    return (
        <div>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 10, flexWrap: "wrap" }}>
                <select value={sel} onChange={(e) => loadDemo(e.target.value)}
                    style={{ background: "#0f172a", border: "1px solid #1c2842", borderRadius: 6, color: "#e2e8f0", padding: "5px 10px", fontSize: 12 }}>
                    {channels.map((c) => <option key={c.channel_key} value={c.channel_key}>{c.display_name}</option>)}
                </select>
                <button className="btn" onClick={() => loadDemo(sel)}>샘플 데이터 로드</button>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 14, alignItems: "center" }}>
                {[["since", since, setSince], ["until", until, setUntil]].map(([k, v, s]) => (
                    <input key={k} type="date" value={v} onChange={(e) => s(e.target.value)}
                        style={{ background: "#0f172a", border: "1px solid #1c2842", borderRadius: 6, color: "#e2e8f0", padding: "5px 8px", fontSize: 12 }} />
                ))}
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
                {reports.map((r) => (
                    <button key={r.id} onClick={() => openReport(r.id)}
                        style={{ background: selReport?.id === r.id ? "#1c2842" : "transparent", border: `1px solid ${selReport?.id === r.id ? "#3b4d6e" : "#1c2842"}`, borderRadius: 8, padding: "6px 12px", color: "#e2e8f0", cursor: "pointer", fontSize: 11, display: "flex", gap: 6, alignItems: "center" }}>
