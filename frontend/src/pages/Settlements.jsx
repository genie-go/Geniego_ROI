...
const MOCK_SETTLEMENTS = [
  { id: 1,  channel: "Coupang",      period_start: "2026-02-01", period_end: "2026-02-28", gross_sales: 280800000, platform_fee: 30688000, ad_fee: 28080000, payment_fee: 0,        refunds: 11232000, net_payout: 210800000, currency: "KRW", status: "confirmed", fee_rate: 10.9, orders: 2340, icon: "🇰🇷", color: "#00bae5" },
  { id: 2,  channel: "Naver",        period_start: "2026-02-01", period_end: "2026-02-28", gross_sales: 145200000, platform_fee: 7986000,  ad_fee: 14520000,  payment_fee: 4356000,  refunds: 5808000,  net_payout: 112530000, currency: "KRW", status: "confirmed", fee_rate: 5.5,  orders: 1210, icon: "🟢", color: "#03c75a" },
  { id: 3,  channel: "Amazon KR",    period_start: "2026-02-01", period_end: "2026-02-28", gross_sales: 48200000,  platform_fee: 7230000,  ad_fee: 4820000,   payment_fee: 0,        refunds: 2400000,  net_payout: 33750000,  currency: "KRW", status: "confirmed", fee_rate: 15.0, orders: 387,  icon: "📦", color: "#ff9900" },
  { id: 4,  channel: "11Street",       period_start: "2026-02-01", period_end: "2026-02-28", gross_sales: 56000000,  platform_fee: 5600000,  ad_fee: 5040000,   payment_fee: 1120000,  refunds: 2240000,  net_payout: 42000000,  currency: "KRW", status: "confirmed", fee_rate: 10.0, orders: 560,  icon: "🏬", color: "#ff0000" },
  { id: 5,  channel: "Shopify",      period_start: "2026-02-01", period_end: "2026-02-28", gross_sales: 158000,    platform_fee: 4740,     ad_fee: 15800,     payment_fee: 4582,     refunds: 6320,     net_payout: 126558,    currency: "USD", status: "confirmed", fee_rate: 3.0,  orders: 412,  icon: "🛒", color: "#96bf48" },
  { id: 6,  channel: "Amazon US",    period_start: "2026-02-01", period_end: "2026-02-28", gross_sales: 224000,    platform_fee: 33600,    ad_fee: 22400,     payment_fee: 0,        refunds: 8960,     net_payout: 159040,    currency: "USD", status: "confirmed", fee_rate: 15.0, orders: 728,  icon: "📦", color: "#ff9900" },
  { id: 7,  channel: "Rakuten JP",   period_start: "2026-02-01", period_end: "2026-02-28", gross_sales: 4280000,   platform_fee: 299600,   ad_fee: 428000,    payment_fee: 85600,    refunds: 171200,   net_payout: 3295600,   currency: "JPY", status: "confirmed", fee_rate: 7.0,  orders: 284,  icon: "🇯🇵", color: "#bf0000" },
  { id: 8,  channel: "Yahoo JP",     period_start: "2026-02-01", period_end: "2026-02-28", gross_sales: 2910000,   platform_fee: 174600,   ad_fee: 291000,    payment_fee: 58200,    refunds: 116400,   net_payout: 2269800,   currency: "JPY", status: "pending",   fee_rate: 6.0,  orders: 198,  icon: "🔴", color: "#ff0033" },
  { id: 9,  channel: "Coupang",      period_start: "2026-01-01", period_end: "2026-01-31", gross_sales: 254200000, platform_fee: 27759800, ad_fee: 25420000,  payment_fee: 0,        refunds: 10168000, net_payout: 190852200, currency: "KRW", status: "confirmed", fee_rate: 10.9, orders: 2120, icon: "🇰🇷", color: "#00bae5" },
  { id: 10, channel: "Naver",        period_start: "2026-01-01", period_end: "2026-01-31", gross_sales: 132800000, platform_fee: 7304000,  ad_fee: 13280000,  payment_fee: 3984000,  refunds: 5312000,  net_payout: 102920000, currency: "KRW", status: "confirmed", fee_rate: 5.5,  orders: 1102, icon: "🟢", color: "#03c75a" },
  { id: 11, channel: "TikTok Shop",  period_start: "2026-02-01", period_end: "2026-02-28", gross_sales: 67000,     platform_fee: 8040,     ad_fee: 25000,     payment_fee: 2010,     refunds: 2680,     net_payout: 29270,     currency: "USD", status: "pending",   fee_rate: 12.0, orders: 234,  icon: "🎵", color: "#ff0050" },
];
...
const CHANNELS = [...new Set(MOCK_SETTLEMENTS.map(r => r.channel))];
const CURRENCIES = [...new Set(MOCK_SETTLEMENTS.map(r => r.currency))];
  ...
  const doImport = async () => {
    ...
    try {
      const res = await fetch("/v382/settlements/import", {
        ...
        body: JSON.stringify({ channel: "amazon", period_start: "2026-02-01", period_end: "2026-02-28", gross_sales: 12500000, fees: 1875000, refunds: 625000, net_payout: 10000000, currency: "KRW" }),
      });
    ...
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <div className="hero fade-up">
        <div className="hero-meta">
          ...
          <div>
            <div className="hero-title" style={{ background: "linear-gradient(135deg,#14d9b0,#4f8ef7)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
              {t("settlements.pageTitle") || "Channel Settlements Management"}
            </div>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
          <button className="btn-primary" style={{ fontSize: 11, padding: "6px 14px" }} onClick={doImport} disabled={importBusy}>
            {importBusy ? `⏳ ${t('settlements.processing')}` : "+ Import"}
          </button>
      ...
      <div className="grid4 fade-up fade-up-1" style={{ gridTemplateColumns: "repeat(5,1fr)" }}>
        {[
          ...
          { l: t('settlements.kpiNetPayout'), v: fmtKRWM(totals.netKRW), s: `${t('settlements.kpiPayRate')} ${(totals.netKRW / totals.grossKRW * 100).toFixed(1)}%`, c: "#14d9b0" },
      ...
      {detail && (
        <Drawer onClose={() => setDetail(null)}>
          ...
          <div style={{ marginBottom: 16, padding: "8px 14px", borderRadius: 8, background: "rgba(99,140,255,0.06)", border: "1px solid rgba(99,140,255,0.15)", fontSize: 11 }}>
            ...
            {detail.currency !== "KRW" && (
              <span style={{ color: "var(--text-3)", marginLeft: 8 }}>· {t('settlements.exchangeRate')}: 1{detail.currency} = ₩{FX[detail.currency]}</span>
            )}
          </div>
