...
const _DEMO_PIXEL_CONFIGS = [
  // [281차 P2] pixel_id·capi 배지 포함(편집 UI·설정 배지 데모 표시용)
  { id: "px_demo1", pixel_id: "px_demo1_a1b2c3", name: "메인 쇼핑몰 픽셀", domain: "shop.demo-brand.com", meta_pixel_id: "1029384756", tiktok_pixel_id: "C9A1B2C3D4", created_at: "2026-05-18 09:30",
    capi: { meta: true, tiktok: true, ga4: false, pinterest: false, snap: false, reddit: false, linkedin: false } },
];
...
function PixelConfigTab({ API }) {
  ...
  // [280차] CAPI 목적지 7종 전체를 폼에서 설정 가능(종전 Meta/TikTok 2종만 노출 → 나머지는 켤 방법이 없었다).
  const EMPTY_FORM = {
    name: "", domain: "",
    meta_pixel_id: "", meta_api_token: "", tiktok_pixel_id: "", tiktok_access_token: "",
    ga4_measurement_id: "", ga4_api_secret: "",
    pinterest_ad_account_id: "", pinterest_conversion_token: "",
    snap_pixel_id: "", snap_api_token: "",
    reddit_ad_account_id: "", reddit_conversion_token: "",
    linkedin_conversion_urn: "", linkedin_access_token: "", linkedin_events: "purchase",
  };
  const [form, setForm] = useState(EMPTY_FORM);
  const [showMoreCapi, setShowMoreCapi] = useState(false);
  ...
  const save = async () => {
    ...
    // [280차] 도메인 필수. 수집 신뢰모델이 fail-closed 라 domain 이 비면 Origin 일치 판정이 불가능해
    //   모든 이벤트가 'custom'·value=0 으로 중립화된다(PixelTracking::collect $trusted) → 픽셀이 조용히 무용지물.
    //   종전엔 선택 입력이라 그 함정에 그대로 빠졌다.
    if (!form.domain.trim()) { setMsg(`❌ ${t("pxl.domainRequired")}`); return; }
    setSaving(true);
  ...
  // [281차 P2] 기존 픽셀 편집 — CAPI 자격증명 주입(빈칸=기존값 보존). 279차 이전 생성 픽셀에
  //   GA4/Pinterest/Snap/Reddit/LinkedIn 을 넣을 방법이 없던 것 해소(재생성 시 pixel_id 변경 회피).
  const [editCfg, setEditCfg] = useState(null);   // null=닫힘, {id,name,domain,...}=열림
  const [editForm, setEditForm] = useState({});
  const openEdit = (cfg) => {
    setEditCfg(cfg);
    setEditForm({ name: cfg.name || "", domain: cfg.domain || "",
      meta_pixel_id: "", meta_api_token: "", tiktok_pixel_id: "", tiktok_access_token: "",
      ga4_measurement_id: "", ga4_api_secret: "", pinterest_ad_account_id: "", pinterest_conversion_token: "",
      snap_pixel_id: "", snap_api_token: "", reddit_ad_account_id: "", reddit_conversion_token: "",
      linkedin_conversion_urn: "", linkedin_access_token: "", linkedin_events: "" });
  };
  const saveEdit = async () => {
    if (!editCfg) return;
    setSaving(true);
    // 빈 문자열 필드는 전송하지 않는다(서버가 미변경 처리 → 기존 시크릿 보존).
    const payload = {};
    Object.entries(editForm).forEach(([k, v]) => { if (v !== "" && v != null) payload[k] = v; });
    const r = await API(`/pixel/configs/${editCfg.id}`, { method: "PUT", body: JSON.stringify(payload) });
    setSaving(false);
    if (r.ok) { setMsg(`✅ ${t("pxl.editSaved", "저장되었습니다")}`); setEditCfg(null); load(); }
    else setMsg(`❌ ${r.error || ""}`);
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={CARD}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          ...
          <div><label style={{ fontSize: 12, color: C.muted }}>{t("pxl.domain")} <span style={{ color: "#ef4444" }}>*</span></label>
        ...
        {/* [280차] 서버전환(CAPI) 목적지 확장. ★종전엔 Meta·TikTok 필드만 있어 백엔드에 포워더와 컬럼이
            있는 GA4/Pinterest/Snapchat/Reddit 을 어느 테넌트도 켤 수 없었다(자격증명 입력 UI 부재 = 사실상 미출시).
            LinkedIn(280차 신설) 포함 5종을 여기서 노출한다. 전부 선택 입력 — 비우면 해당 목적지는 no-op. */}
        <div style={{ marginTop: 18 }}>
          <button type="button" onClick={() => setShowMoreCapi(v => !v)}
            style={{ background: "none", border: "none", cursor: "pointer", padding: 0, color: C.accent, fontWeight: 700, fontSize: 13 }}>
            {showMoreCapi ? "▾" : "▸"} {t("pxl.moreCapi")}
          </button>
          {showMoreCapi && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
              <div><label style={{ fontSize: 12, color: C.muted }}>GA4 Measurement ID</label>
                <input style={INPUT} value={form.ga4_measurement_id} onChange={e => setForm({ ...form, ga4_measurement_id: e.target.value })} placeholder="G-XXXXXXX" /></div>
              <div><label style={{ fontSize: 12, color: C.muted }}>GA4 API Secret</label>
                <input style={INPUT} type="password" value={form.ga4_api_secret} onChange={e => setForm({ ...form, ga4_api_secret: e.target.value })} placeholder="xxxxxx..." /></div>

              <div><label style={{ fontSize: 12, color: C.muted }}>Pinterest Ad Account ID</label>
                <input style={INPUT} value={form.pinterest_ad_account_id} onChange={e => setForm({ ...form, pinterest_ad_account_id: e.target.value })} placeholder="549xxxxxxxxxx" /></div>
              <div><label style={{ fontSize: 12, color: C.muted }}>Pinterest Conversion Token</label>
                <input style={INPUT} type="password" value={form.pinterest_conversion_token} onChange={e => setForm({ ...form, pinterest_conversion_token: e.target.value })} placeholder="pina_xxxxxx..." /></div>

              <div><label style={{ fontSize: 12, color: C.muted }}>Snapchat Pixel ID</label>
                <input style={INPUT} value={form.snap_pixel_id} onChange={e => setForm({ ...form, snap_pixel_id: e.target.value })} placeholder="xxxxxxxx-xxxx-..." /></div>
              <div><label style={{ fontSize: 12, color: C.muted }}>Snapchat API Token</label>
                <input style={INPUT} type="password" value={form.snap_api_token} onChange={e => setForm({ ...form, snap_api_token: e.target.value })} placeholder="xxxxxx..." /></div>

              <div><label style={{ fontSize: 12, color: C.muted }}>Reddit Ad Account ID</label>
                <input style={INPUT} value={form.reddit_ad_account_id} onChange={e => setForm({ ...form, reddit_ad_account_id: e.target.value })} placeholder="a2_xxxxxxxx" /></div>
              <div><label style={{ fontSize: 12, color: C.muted }}>Reddit Conversion Token</label>
                <input style={INPUT} type="password" value={form.reddit_conversion_token} onChange={e => setForm({ ...form, reddit_conversion_token: e.target.value })} placeholder="xxxxxx..." /></div>

              <div><label style={{ fontSize: 12, color: C.muted }}>LinkedIn Conversion URN</label>
                <input style={INPUT} value={form.linkedin_conversion_urn} onChange={e => setForm({ ...form, linkedin_conversion_urn: e.target.value })} placeholder="urn:lla:llaPartnerConversion:123456" /></div>
              <div><label style={{ fontSize: 12, color: C.muted }}>LinkedIn Access Token</label>
                <input style={INPUT} type="password" value={form.linkedin_access_token} onChange={e => setForm({ ...form, linkedin_access_token: e.target.value })} placeholder="AQXxxxxxx..." /></div>
              <div style={{ gridColumn: "1 / -1" }}>
                <label style={{ fontSize: 12, color: C.muted }}>{t("pxl.linkedinEvents")}</label>
                <input style={INPUT} value={form.linkedin_events} onChange={e => setForm({ ...form, linkedin_events: e.target.value })} placeholder="purchase" />
                <div style={{ fontSize: 11, color: C.muted, marginTop: 4 }}>{t("pxl.linkedinEventsHint")}</div>
              </div>
            </div>
          )}
        </div>
          ...
          configs.map(cfg => (
            <div key={cfg.id} style={{ padding: "14px 0", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                ...
                {/* [281차 P2] 설정된 CAPI 채널 배지 — 어느 서버전환이 연결됐는지 한눈에 표시 */}
                {cfg.capi && (
                  <div style={{ display: "flex", gap: 5, marginTop: 6, flexWrap: "wrap" }}>
                    {[["meta","Meta"],["tiktok","TikTok"],["ga4","GA4"],["pinterest","Pinterest"],["snap","Snap"],["reddit","Reddit"],["linkedin","LinkedIn"]].map(([k,label]) => (
                      <span key={k} style={{ fontSize: 10, padding: "1px 7px", borderRadius: 10, background: cfg.capi[k] ? "rgba(34,197,94,0.12)" : "var(--surface)", color: cfg.capi[k] ? C.green : C.muted, border: `1px solid ${cfg.capi[k] ? "rgba(34,197,94,0.3)" : C.border}` }}>{cfg.capi[k] ? "✓ " : ""}{label}</span>
                    ))}
                  </div>
                )}
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => openEdit(cfg)} style={{ padding: "6px 14px", borderRadius: 8, border: "none", background: "var(--surface)", color: C.accent, cursor: "pointer", fontSize: 12 }}>✏️ {t("pxl.edit", "편집")}</button>
      ...
      {/* [281차 P2] 픽셀 편집 모달 — CAPI 자격증명 주입(빈칸=기존값 보존) */}
      {editCfg && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={() => setEditCfg(null)}>
          <div style={{ ...CARD, width: "min(560px, 92vw)", maxHeight: "88vh", overflow: "auto" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div><label style={{ fontSize: 12, color: C.muted }}>{t("pxl.pixelName")}</label>
                <input style={INPUT} value={editForm.name} onChange={e => setEditForm({ ...editForm, name: e.target.value })} /></div>
              <div><label style={{ fontSize: 12, color: C.muted }}>{t("pxl.domain")}</label>
                <input style={INPUT} value={editForm.domain} onChange={e => setEditForm({ ...editForm, domain: e.target.value })} placeholder="example.com" /></div>
              {[["meta_pixel_id","Meta Pixel ID"],["meta_api_token","Meta CAPI Token"],["tiktok_pixel_id","TikTok Pixel ID"],["tiktok_access_token","TikTok Access Token"],["ga4_measurement_id","GA4 Measurement ID"],["ga4_api_secret","GA4 API Secret"],["pinterest_ad_account_id","Pinterest Ad Account ID"],["pinterest_conversion_token","Pinterest Token"],["snap_pixel_id","Snapchat Pixel ID"],["snap_api_token","Snapchat Token"],["reddit_ad_account_id","Reddit Ad Account ID"],["reddit_conversion_token","Reddit Token"],["linkedin_conversion_urn","LinkedIn Conversion URN"],["linkedin_access_token","LinkedIn Token"],["linkedin_events","LinkedIn Events"]].map(([k,label]) => (
                <div key={k}><label style={{ fontSize: 12, color: C.muted }}>{label}</label>
                  <input style={INPUT} type={/token|secret/.test(k) ? "password" : "text"} value={editForm[k]} onChange={e => setEditForm({ ...editForm, [k]: e.target.value })} placeholder={/token|secret/.test(k) ? "•••• (유지)" : ""} /></div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 8, marginTop: 16, justifyContent: "flex-end" }}>
              <button onClick={() => setEditCfg(null)} style={{ padding: "8px 20px", borderRadius: 8, border: `1px solid ${C.border}`, cursor: "pointer", background: "var(--surface)", color: C.text, fontSize: 13 }}>{t("pxl.cancel", "취소")}</button>
              <button onClick={saveEdit} disabled={saving} style={{ padding: "8px 24px", borderRadius: 8, border: "none", cursor: "pointer", background: `linear-gradient(135deg, ${C.accent}, #6366f1)`, color: "#fff", fontWeight: 700, fontSize: 13 }}>{saving ? t("pxl.creating") : `💾 ${t("pxl.editSave", "저장")}`}</button>
            </div>
          </div>
        </div>
      )}
