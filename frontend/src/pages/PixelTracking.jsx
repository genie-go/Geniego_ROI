...
export default function PixelTracking() {
  ...
  const tabs = ["Overview","Pixel Status","Event Stream","Diagnostics"];
  const kpis = [{"emoji":"📡","label":"Active Pixels","val":8},{"emoji":"🔥","label":"Events Today","val":2840},{"emoji":"✅","label":"Healthy","val":7},{"emoji":"⚠️","label":"Issues","val":1}];
    ...
    <div style={{ padding: 24, minHeight: "100%", color: "var(--text-1, #1e293b)" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            <div style={{ fontSize: 22, fontWeight: 800, color: "var(--text-1, #1e293b)" }}>🎯 Pixel & Event Tracking</div>
            <div style={{ fontSize: 13, color: "var(--text-3, #64748b)", marginTop: 2 }}>Monitor conversion pixels and tracking events across all channels</div>
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 12 }}>
          {["Real-time event monitoring","Cross-domain tracking","Auto-diagnostics","Tag manager integration"].map((f, i) => (
            <div key={i} style={{
