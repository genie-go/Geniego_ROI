...
const _PL  = { pro:"Pro", enterprise:"Enterprise", starter:"Starter", growth:"Growth", demo:"Demo" };
const _CL  = { monthly:"Monthly", quarterly:"Quarter", biannual:"6개월", yearly:"Annual" };
const _fd  = s => s ? s.slice(0, 10) : "-";
const _fk  = v => (parseInt(v) || 0) > 0 ? "₩" + (parseInt(v)).toLocaleString("ko-KR") : "-";
const INP  = { background:"#0f172a", border:"1px solid #1e3a5f", borderRadius:7, color:"#e2e8f0", padding:"8px 10px", fontSize:12, outline:"none", width:"100%", boxSizing:"border-box" };
  ...
  return (
    <div style={{ display:"grid", gap:12 }}>
      ...
      <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
        ...
        <select value={planFlt} onChange={e => { setPlanFlt(e.target.value); setPage(1); }}
          style={{ background:"#0f172a", border:"1px solid #1e3a5f", borderRadius:7, color:"#94a3b8", padding:"8px 10px", fontSize:12 }}>
          <option value="">All 플랜</option>
          {Object.entries(_PL).filter(([k]) => k !== "demo").map(([k, v]) => <option key={k} value={k}>{v}</option>)}
        </select>
      ...
      <div style={{ fontSize:12, color:"#7c8fa8" }}>Total <strong style={{ color:"#e2e8f0" }}>{total.toLocaleString()}</strong>명의 Paid 회원</div>

      {/* Table Header */}
      <div style={{ display:"grid", gridTemplateColumns:"1.2fr 1.3fr 70px 90px 90px 90px 80px 55px", gap:6, padding:"7px 10px", background:"rgba(255,255,255,0.03)", borderRadius:8, fontSize:10, fontWeight:700, color:"#7c8fa8" }}>
      ...
      {rows.map(u => (
        <div key={u.id} style={{ display:"grid", gridTemplateColumns:"1.2fr 1.3fr 70px 90px 90px 90px 80px 55px", gap:6, padding:"9px 10px", background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.05)", borderRadius:8, alignItems:"center", fontSize:11 }}>
          <div>
            <div style={{ fontWeight:700, color:"#e2e8f0" }}>{u.company || "-"}</div>
          ...
          <button onClick={() => { setDetail(u); setEditing({ ...u }); }}
            style={{ padding:"4px 7px", background:"rgba(79,142,247,0.1)", border:"1px solid rgba(79,142,247,0.3)", borderRadius:6, color:"#4f8ef7", cursor:"pointer", fontSize:10 }}>
            ✏️ 상세
          </button>
        </div>
      ))}
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center" }}
          ...
          <div style={{ background:"#0d1526", border:"1px solid #1c2842", borderRadius:14, padding:24, width:"min(580px,95vw)", maxHeight:"85vh", overflowY:"auto" }}
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              <button onClick={() => { setDetail(null); setEditing(null); }} style={{ padding:"10px 16px", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:8, color:"#94a3b8", cursor:"pointer" }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", zIndex:99999, display:"flex", alignItems:"center", justifyContent:"center" }}
      ...
      <div style={{ background:"#0d1526", border:"1px solid #1c2842", borderRadius:16, padding:28, width:"min(560px,95vw)", maxHeight:"92vh", overflowY:"auto" }}
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20 }}>
          ...
          <div>
            <div style={{ fontWeight:900, fontSize:16, color:"#e2e8f0" }}>비즈니스 Info Register</div>
        ...
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={save} disabled={saving} style={{
            ...
            background: saving ? "rgba(79,142,247,0.2)" : "linear-gradient(135deg,#4f8ef7,#6366f1)",
          ...
          <button onClick={onClose} style={{ padding:"14px 18px", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:10, color:"#94a3b8", cursor:"pointer", fontSize:13 }}>Cancel</button>
        </div>
      </div>
    </div>
  );
...
export function CouponsGrantTab() {
  ...
  const [demoUsers,   setDemoUsers]   = React.useState([]);   // Free회원
  ...
  const loadDemoUsers = React.useCallback(() => {
    const p = new URLSearchParams(demoSearch ? { search: demoSearch } : {});
    fetch(`${_API}/api/v423/admin/demo-users?${p}`, { headers: _ah() })
      .then(r => r.json()).then(d => { if (d.ok) setDemoUsers(d.rows || []); }).catch(() => {});
  }, [demoSearch]);

  React.useEffect(() => { loadCoupons(); loadSubs(); loadDemoUsers(); }, [loadCoupons, loadSubs, loadDemoUsers]);
  ...
  const totPages = Math.ceil(total / LMT);
  const selectedUser = [...subs, ...demoUsers].find(u => String(u.id) === String(form.user_id));
    ...
    <div style={{ display:"grid", gap:16 }}>
      ...
      <div style={{ background:"rgba(79,142,247,0.05)", border:"1px solid rgba(79,142,247,0.15)", borderRadius:12, padding:18 }}>
        ...
        <div style={{ display:"flex", gap:8, marginBottom:14 }}>
          {[
            ...
            <button key={opt.val} type="button" onClick={() => { setTargetType(opt.val); setForm(f => ({ ...f, user_id: "" })); }}
              style={{
        ...
        {targetType === "free" && (
          <>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr auto", gap:8, marginBottom:12 }}>
              ...
              <button onClick={loadDemoUsers} style={{ padding:"8px 12px", background:"rgba(34,197,94,0.12)", border:"1px solid rgba(34,197,94,0.25)", borderRadius:7, color:"#22c55e", cursor:"pointer", fontSize:11 }}>
                🔄
              </button>
            </div>
            <div>
              <div style={{ fontSize:10, color:"#7c8fa8", marginBottom:6 }}>
                Free회원 Select <span style={{ color:"#64748b" }}>({demoUsers.length}명)</span>
              </div>
        ...
        {targetType === "paid" && (
          <div style={{ marginBottom:12 }}>
            <div style={{ fontSize:10, color:"#7c8fa8", marginBottom:4 }}>대상 Paid Member <span style={{ color:"#64748b" }}>(비워두면 All)</span></div>
      ...
      <div>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
          <div style={{ fontWeight:700, fontSize:13 }}>📋 Issue 내역 <span style={{ color:"#7c8fa8", fontWeight:400, fontSize:11 }}>({total}건)</span></div>
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            <select value={fltStatus} onChange={e => { setFltStatus(e.target.value); setPage(1); }}
              style={{ background:"#0f172a", border:"1px solid #1e3a5f", borderRadius:7, color:"#94a3b8", padding:"6px 10px", fontSize:11 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1.4fr 1fr 80px 70px 65px 90px 80px 70px 50px", gap:5, padding:"7px 10px", background:"rgba(255,255,255,0.03)", borderRadius:8, fontSize:10, fontWeight:700, color:"#7c8fa8" }}>
        ...
        {coupons.map(c => (
          <div key={c.id} style={{ display:"grid", gridTemplateColumns:"1.4fr 1fr 80px 70px 65px 90px 80px 70px 50px", gap:5, padding:"8px 10px", background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.04)", borderRadius:7, alignItems:"center", fontSize:11, marginBottom:3 }}>
            <div>
              <div style={{ color:"#e2e8f0", fontWeight:600 }}>{c.issued_to_email || c.redeemer_name || "-"}</div>
