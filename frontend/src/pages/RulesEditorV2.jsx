...
function DraggableField({ name }) {
  return (
    <div
      ...
      style={{ padding: "8px 10px", border: "1px solid #e6e6e6", borderRadius: 10, marginBottom: 8, background: "white", cursor: "grab" }}
      title="drag"
    >
      {name}
    </div>
...
function DropZone({ onDrop, children }) {
  return (
    <div
      ...
      onDrop={(e)=>{ e.preventDefault(); const v = e.dataTransfer.getData("text/plain"); if (v) onDrop(v); }}
      style={{ minHeight: 120, padding: 12, border: "1px dashed #cfcfcf", borderRadius: 14, background: "#fafafa" }}
    >
      {children}
    </div>
  );
