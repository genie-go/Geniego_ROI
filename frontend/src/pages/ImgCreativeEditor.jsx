...
export function ImgCreativeEditor() {
  const { t } = useI18n();
  const [activeTab, setActiveTab] = useState(0);
  const tabs = ["Editor","Templates","My Creatives","AI Generate"];
  const kpis = [{"emoji":"🎨","label":"Templates","val":86},{"emoji":"🖼️","label":"Created","val":234},{"emoji":"⭐","label":"Top Rated","val":18},{"emoji":"📤","label":"Published","val":45}];
    ...
    <div style={{ padding: 24, minHeight: "100%", color: "var(--text-1, #1e293b)" }}>
      <div style={{ borderRadius: 18, padding: "28px 32px", marginBottom: 22, background: "linear-gradient(135deg, rgba(79,142,247,0.08), rgba(99,102,241,0.06))", border: "1px solid rgba(79,142,247,0.12)", backdropFilter: "blur(16px)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            <div style={{ fontSize: 22, fontWeight: 800, color: "var(--text-1, #1e293b)" }}>🎨 Creative Editor</div>
            <div style={{ fontSize: 13, color: "var(--text-3, #64748b)", marginTop: 2 }}>AI-powered image editor for marketing creatives</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ borderRadius: 16, padding: "28px 32px", minHeight: 320, background: "rgba(255,255,255,0.85)", border: "1px solid rgba(0,0,0,0.06)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 12 }}>
          {["AI background removal","Brand kit integration","Batch resize","Smart text placement"].map((f, i) => (
