...
const S = {
    card: { background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: "16px 18px" },
    inp: { padding: "5px 7px", borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface)', color: "#e8eaf6", fontSize: 11, width: "100%", boxSizing: "border-box" },
    btn: (v = "p") => ({
    ...
    th: { fontSize: 9, color: "var(--text-3)", fontWeight: 700, padding: "7px 10px", textAlign: "center", borderBottom: "1px solid rgba(255,255,255,0.07)", letterSpacing: 0.5 },
};
        ...
        return (
            <div key={node.key}>
                <div style={{ display: "flex", alignItems: "center", gap: 9, padding: `${depth === 0 ? 9 : 6}px 12px ${depth === 0 ? 9 : 6}px ${12 + pl}px`, background: depth === 0 && (chk || ind) ? "rgba(79,142,247,0.06)" : "transparent", borderTop: depth === 0 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                    ...
                    {hasC && <span onClick={() => togExp(node.key)} style={{ fontSize: 9, color: 'var(--text-3)', transform: open ? "rotate(90deg)" : "none", transition: "200ms", cursor: "pointer" }}>▶</span>}
                </div>
    ...
    const { fmt } = useCurrency();
        ...
        return (
            <td style={{ padding: "5px 5px", borderBottom: "1px solid rgba(255,255,255,0.05)", verticalAlign: "top", minWidth: 86 }}>
                <input type="number" placeholder="$ Price" value={val} onChange={e => onVal(e.target.value)} style={{ ...S.inp, marginBottom: 3 }} />
                ...
                {fp && fp === Number(val) && <div style={{ fontSize: 9, color: "var(--text-3)", marginTop: 2, textAlign: "center" }}>{fmt(fp)}</div>}
            </td>
        );
    };
        ...
        <div>
            ...
            <div style={{ padding: "12px 18px", borderRadius: 10, background: "rgba(141,164,196,0.06)", border: "1px solid rgba(141,164,196,0.2)", marginBottom: 16, display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: "#4f8ef7" }} >Free 플랜은 per도 패키지 없이 All  접근 Permission이 Auto으로 부여됩니다. 아래에서는 <strong>Growth / Pro / Enterprise</strong> Paid 패키지만 Register하세요.</div>
              </div>
            </div>
            {viewMode === "create" ? (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 540px", gap: 16, alignItems: "start" }}>
                    ...
                    <div style={{ ...S.card, padding: 0, overflow: "hidden" }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 14px", borderBottom: "1px solid rgba(255,255,255,0.07)", background: 'var(--surface)' }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                        {/* 패키지 Name */}
                        <div style={S.card}>
                            <div style={{ fontSize: 10, color: "var(--text-3)", fontWeight: 600, marginBottom: 5, textTransform: "uppercase" }}>Subscription 패키지 Name *</div>
                                                    ...
                                                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                                        ...
                                                        <div style={{ flex: 1 }}>
                                                            ...
                                                            <label style={{ display: "flex", alignItems: "center", gap: 4, cursor: "pointer", marginTop: 3 }}>
                                                                ...
                                                                <span style={{ fontSize: 9, color: "var(--text-3)" }}>Unlimited</span>
                                                            </label>
                                                        </div>
                                                    </div>
                                                </td>
                ...
                <div>
                    {packages.length === 0 ? (
                        <div style={{ ...S.card, textAlign: "center", padding: "48px 20px" }}>
                            ...
                            <div style={{ fontSize: 13, color: "var(--text-3)" }}>Register된 Subscription 패키지가 없습니다</div>
                                        ...
                                        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: open ? 14 : 0 }}>
                                            ...
                                            <div style={{ flex: 1 }}>
                                                ...
                                                <div style={{ fontSize: 10, color: "var(--text-3)" }}>
                                                    ...
                                                    {pkg.price_monthly > 0 && <span style={{ marginLeft: 8, color: "#22c55e" }}>1Tier 월 ${Number(pkg.price_monthly).toLocaleString("en-US")}~</span>}
                                                </div>
                                            </div>
                                            ...
                                            <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 14 }}>
                                                {tierData.length > 0 ? (
                                                    <div style={{ marginBottom: 14, overflowX: "auto" }}>
                                                        <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-3)", marginBottom: 8, textTransform: "uppercase" }}>👥 Accountper Pricing</div>
                                                        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 420 }}>
                                                            ...
                                                            <tbody>
                                                                {tierData.map((t, ti) => {
                                                                    ...
                                                                    const showP = (p, d) => {
                                                                        if (!p) return <span style={{ color: 'var(--text-3)' }}>—</span>;
                                                                        const fp = calcFinal(p, d);
                                                                        return <span><span style={{ color: "#22c55e", fontWeight: 700, marginLeft: 4 }} >${Number(p).toLocaleString("en-US")}</span>{fp && fp !== Number(p) ? <span>→ ${Number(fp).toLocaleString("en-US")}</span> : null}</span>;
                                                                    };
                                                                    return (
                                                                        <tr key={ti}>
                                                ...
                                                ) : <div style={{ fontSize: 11, color: "var(--text-3)", marginBottom: 12 }}>Accountper Pricing 미Settings</div>}
                                                <div>
                                                    <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-3)", marginBottom: 6, textTransform: "uppercase" }}>포함 Menu ({menuKeys.length})</div>
