...
const TYPE_LABEL = { supplier: '매입처', logistics: '물류처', warehouse: '창고처' };
...
export default function PartnerPortal() {
  ...
  const doAction = async (op, extra) => {
    ...
    alert(data.error || '처리에 실패했습니다.'); return false;
  };
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg,#eff6ff,#f0fdf4)', padding: 20 }}>
        <div style={{ ...card, width: 'min(400px,94vw)' }}>
          <div style={{ fontSize: 22, fontWeight: 900, color: '#0f172a', marginBottom: 4 }}>🤝 파트너 포털</div>
          <div style={{ fontSize: 12.5, color: '#64748b', marginBottom: 20 }}>매입처 · 물류처 · 창고처 전용 로그인. 발급받은 ID로 접속하세요.</div>
          <label style={{ fontSize: 12, fontWeight: 700, color: '#475569' }}>로그인 ID</label>
          <input style={{ ...inp, margin: '6px 0 14px' }} value={form.login_id} onChange={e => setForm(f => ({ ...f, login_id: e.target.value }))} placeholder="파트너 ID" autoComplete="username" />
          <label style={{ fontSize: 12, fontWeight: 700, color: '#475569' }}>비밀번호</label>
          ...
          <button style={{ ...btn, width: '100%', opacity: busy ? 0.6 : 1 }} onClick={login} disabled={busy}>{busy ? '로그인 중...' : '로그인'}</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8fafc', padding: '0' }}>
      <div style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <span style={{ fontSize: 17, fontWeight: 900, color: '#0f172a' }}>🤝 파트너 포털</span>
          <span style={{ marginLeft: 12, fontSize: 12.5, color: '#64748b' }}>{typeLabel} · {me?.partner_name || me?.name}</span>
        </div>
        <button onClick={logout} style={{ padding: '7px 14px', borderRadius: 8, border: '1px solid #cbd5e1', background: '#fff', color: '#475569', fontWeight: 700, fontSize: 12.5, cursor: 'pointer' }}>로그아웃</button>
      </div>
...
function SupplierView({ rows, onAction }) {
  ...
  return (
    <>
      <div style={card}>
        <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 12 }}>📥 발주 등록 (입고 예정)</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr)) auto', gap: 8, alignItems: 'end' }}>
          ...
          <input style={inp} placeholder="상품명" value={f.name} onChange={e => setF({ ...f, name: e.target.value })} />
          <input style={inp} type="number" placeholder="수량" value={f.qty} onChange={e => setF({ ...f, qty: e.target.value })} />
          <input style={inp} type="date" value={f.eta} onChange={e => setF({ ...f, eta: e.target.value })} />
          <button style={btn} onClick={async () => { if (await onAction('add_order', { ...f, qty: Number(f.qty) || 0 })) setF({ sku: '', name: '', qty: '', eta: '' }); }}>등록</button>
        </div>
      </div>
      <div style={card}>
        <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 12 }}>📋 발주 내역 ({rows.length})</div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr><th style={th}>SKU</th><th style={th}>상품명</th><th style={th}>수량</th><th style={th}>입고예정</th><th style={th}>상태</th><th style={th}>작업</th></tr></thead>
            <tbody>
              {rows.length === 0 && <tr><td style={{ ...td, textAlign: 'center', color: '#94a3b8' }} colSpan={6}>발주 내역이 없습니다.</td></tr>}
              {rows.map(r => (
                <tr key={r.id}>
                  ...
                  <td style={td}>
                    {r.status !== 'shipped' && <button onClick={() => onAction('update_status', { id: r.id, status: 'shipped' })} style={{ padding: '4px 10px', borderRadius: 6, border: 'none', background: '#4f8ef7', color: '#fff', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>출고확인</button>}
                  </td>
                </tr>
              ))}
            </tbody>
...
function LogisticsView({ rows, onAction }) {
  return (
    <div style={card}>
      <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 12 }}>🚚 배정 출고/피킹 ({rows.length})</div>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr><th style={th}>주문번호</th><th style={th}>SKU</th><th style={th}>상품명</th><th style={th}>수량</th><th style={th}>상태</th><th style={th}>작업</th></tr></thead>
          <tbody>
            {rows.length === 0 && <tr><td style={{ ...td, textAlign: 'center', color: '#94a3b8' }} colSpan={6}>배정된 출고가 없습니다.</td></tr>}
            {rows.map(r => (
              <tr key={r.id}>
                <td style={td}>{r.order_ref}</td><td style={td}>{r.sku}</td><td style={td}>{r.name}</td><td style={td}>{r.qty}</td>
                <td style={td}><span style={{ padding: '2px 8px', borderRadius: 12, fontSize: 11, fontWeight: 700, background: r.status === 'shipped' ? '#dcfce7' : '#fef3c7', color: r.status === 'shipped' ? '#16a34a' : '#b45309' }}>{r.status}</span></td>
                <td style={td}>{r.status !== 'shipped' && <button onClick={() => onAction('update_status', { id: r.id, status: 'shipped' })} style={{ padding: '4px 10px', borderRadius: 6, border: 'none', background: '#4f8ef7', color: '#fff', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>출고완료</button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function WarehouseView({ stock, movements, onAction }) {
  ...
  return (
    <>
      <div style={card}>
        <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 12 }}>📦 입출고 등록</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'auto repeat(auto-fit,minmax(120px,1fr)) auto', gap: 8, alignItems: 'end' }}>
          <select style={inp} value={f.type} onChange={e => setF({ ...f, type: e.target.value })}><option value="Inbound">입고</option><option value="Outbound">출고</option><option value="ReturnsInbound">반품입고</option></select>
          <input style={inp} placeholder="SKU" value={f.sku} onChange={e => setF({ ...f, sku: e.target.value })} />
          <input style={inp} placeholder="상품명" value={f.name} onChange={e => setF({ ...f, name: e.target.value })} />
          <input style={inp} type="number" placeholder="수량" value={f.qty} onChange={e => setF({ ...f, qty: e.target.value })} />
          <button style={btn} onClick={async () => { if (await onAction('add_movement', { ...f, qty: Number(f.qty) || 0 })) setF({ type: 'Inbound', sku: '', name: '', qty: '' }); }}>등록</button>
        </div>
      </div>
      <div style={card}>
        <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 12 }}>📊 재고 현황 ({stock.length})</div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr><th style={th}>SKU</th><th style={th}>상품명</th><th style={th}>재고</th><th style={th}>갱신</th></tr></thead>
            <tbody>
              {stock.length === 0 && <tr><td style={{ ...td, textAlign: 'center', color: '#94a3b8' }} colSpan={4}>재고 데이터가 없습니다.</td></tr>}
      ...
      <div style={card}>
        <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 12 }}>🔄 최근 입출고 ({movements.length})</div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr><th style={th}>구분</th><th style={th}>SKU</th><th style={th}>상품명</th><th style={th}>수량</th><th style={th}>일시</th></tr></thead>
            <tbody>
              {movements.length === 0 && <tr><td style={{ ...td, textAlign: 'center', color: '#94a3b8' }} colSpan={5}>이력이 없습니다.</td></tr>}
              {movements.map(m => (<tr key={m.id}><td style={td}>{({ Inbound: '입고', Outbound: '출고', ReturnsInbound: '반품입고' }[m.type]) || m.type}</td><td style={td}>{m.sku}</td><td style={td}>{m.name}</td><td style={td}>{m.qty}</td><td style={td}>{(m.created_at || '').slice(0, 16)}</td></tr>))}
            </tbody>
          </table>
        </div>
      </div>
