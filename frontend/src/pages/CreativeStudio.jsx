...
const AD_COPY_POOL = {
    ...
    platform: { hl: ["비즈니스를 한 단계 위로", "스마트한 Team을 위한 Select", "복잡한 업무, 이제 Auto화하세요"], body: ["클라우드 기반 Platform으로 Team 생산성을 극대화하세요.", "API Integration 한 번으로 모든 업무 시스템을 Connect하세요."], cta: ["Free 체험 Start", " 신청", "사Capacity Unlimited 플랜"], colors: ["#7c3aed", "#a78bfa"] },
};
...
export default function CreativeStudio({ onUseCampaign }) {
    ...
    const inp = { width: "100%", background: "rgba(15,20,40,0.7)", border: "1px solid rgba(99,140,255,0.2)", borderRadius: 8, color: '#fff', padding: "8px 12px", fontSize: 12, boxSizing: "border-box" };
    const lbl = (text, required) => (<div style={{ fontSize: 11, color: "#ef4444", fontWeight: 600, marginBottom: 5, marginLeft: 3 }} >{text}{required && <span>*</span>}</div>);
                    ...
                    <div style={card}>
                        ...
                        {[
                            ...
                            <div key={label} style={{ marginBottom: 10, padding: "10px 12px", borderRadius: 8, background: "rgba(15,20,40,0.6)", border: "1px solid rgba(99,140,255,0.12)" }}>
                                ...
                                <div style={{ fontSize: big ? 14 : 11, fontWeight: big ? 800 : 400, color: '#fff', lineHeight: 1.5 }}>{val}</div>
                            </div>
                        ))}
