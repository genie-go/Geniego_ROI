...
export default function ApiKeys() {
  ...
  const tabs = ["Overview","Active Keys","Rotation Log","Settings"];
  const kpis = [{"emoji":"🔑","label":"Total Keys","val":24},{"emoji":"✅","label":"Active","val":18},{"emoji":"⚠️","label":"Expiring Soon","val":3},{"emoji":"🔴","label":"Expired","val":2}];
    ...
    <div style={{ padding: 24, minHeight: "100%", color: "var(--text-1, #1e293b)" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            <div style={{ fontSize: 22, fontWeight: 800, color: "var(--text-1, #1e293b)" }}>🔑 API Key Manager</div>
            <div style={{ fontSize: 13, color: "var(--text-3, #64748b)", marginTop: 2 }}>Manage all API keys, tokens and credentials</div>
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 12 }}>
          {["Auto-rotation scheduling","Expiry alerts","Usage analytics per key","Revoke & regenerate"].map((f, i) => (
            <div key={i} style={{
