...
export default function CreativeStudioTab() {
  ...
  const tabs = [
    t('marketing.csTabGallery','Gallery'),
    t('marketing.csTabCreateNew','Create New'),
    t('marketing.csTabPerformance','Performance'),
    t('marketing.csTabBrandAssets','Brand Assets'),
  ];
  const kpis = [
    {"emoji":"🎬","label":t('marketing.csKpiCreatives','Creatives'),"val":156},
    {"emoji":"📱","label":t('marketing.csKpiFormats','Formats'),"val":12},
    {"emoji":"✅","label":t('marketing.csKpiApproved','Approved'),"val":142},
    {"emoji":"📊","label":t('marketing.csKpiTopCtr','Top CTR'),"val":"5.2%"},
  ];
  const features = [
    t('marketing.csFeatMultiFormat','Multi-format export'),
    t('marketing.csFeatAiCopy','AI copy generator'),
    t('marketing.csFeatPerfAnalytics','Performance analytics'),
    t('marketing.csFeatBrandCheck','Brand consistency check'),
  ];
    ...
    <div style={{ padding: 24, minHeight: "100%", color: "#1e293b" }}>
      <div style={{ borderRadius: 18, padding: "28px 32px", marginBottom: 22, background: "linear-gradient(135deg, rgba(79,142,247,0.08), rgba(99,102,241,0.06))", border: "1px solid rgba(79,142,247,0.12)", backdropFilter: "blur(16px)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            <div style={{ fontSize: 22, fontWeight: 800, color: "#1e293b" }}>🎬 {t('marketing.csTitle','Creative Studio')}</div>
            <div style={{ fontSize: 13, color: "#64748b", marginTop: 2 }}>{t('marketing.csSubtitle','Design and manage ad creatives across platforms')}</div>
          </div>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 14, marginBottom: 22 }}>
        {kpis.map((k, i) => (
          <div key={i} style={{ borderRadius: 14, padding: "18px 20px", background: "rgba(255,255,255,0.85)", border: "1px solid rgba(0,0,0,0.06)" }}>
            ...
            <div style={{ fontSize: 22, fontWeight: 800, color: "#1e293b" }}>{k.val}</div>
            <div style={{ fontSize: 11, color: "#64748b", fontWeight: 600, marginTop: 2 }}>{k.label}</div>
          </div>
        ))}
      </div>
      <div style={{ display: "flex", gap: 4, marginBottom: 20, padding: 4, borderRadius: 12, background: "rgba(0,0,0,0.03)", border: "1px solid rgba(0,0,0,0.05)" }}>
        {tabs.map((tab, i) => (
          <button key={i} onClick={() => setActiveTab(i)} style={{ flex: 1, padding: "10px 16px", borderRadius: 10, border: "none", cursor: "pointer", fontWeight: 700, fontSize: 12, transition: "all 0.2s", background: activeTab === i ? "linear-gradient(135deg,#4f8ef7,#6366f1)" : "transparent", color: activeTab === i ? "#fff" : "#475569" }}>{tab}</button>
        ))}
      </div>
      <div style={{ borderRadius: 16, padding: "28px 32px", minHeight: 320, background: "rgba(255,255,255,0.85)", border: "1px solid rgba(0,0,0,0.06)" }}>
        <div style={{ fontSize: 16, fontWeight: 800, color: "#1e293b", marginBottom: 16 }}>{tabs[activeTab]}</div>
        ...
        <div style={{ marginTop: 24, padding: "16px 20px", borderRadius: 12, background: "linear-gradient(135deg, rgba(34,197,94,0.06), rgba(16,185,129,0.04))", border: "1px solid rgba(34,197,94,0.12)", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 700, color: "#16a34a" }}>{t('marketing.csSystemOk','System Operational')}</span>
        </div>
      </div>
    </div>
  );
