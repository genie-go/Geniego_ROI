    ...
    <div style={{ display:'grid', gap:16 }}>
      <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
        {['all','approved','review','active'].map(f => (
          ...
          }}>{f === 'all' ? t('marketing.csFilterAll','전체') : f === 'approved' ? t('marketing.csStatusApproved','승인') : f === 'review' ? t('marketing.csStatusReview','검토중') : t('marketing.csStatusActive','활성')}</button>
        ))}
      </div>
  ...
  const renderCreateNew = () => (
    <div style={{ display:'grid', gap:16 }}>
      <div style={{ ...card, background:'linear-gradient(135deg,rgba(168,85,247,0.08),rgba(79,142,247,0.06))', borderColor:'rgba(168,85,247,0.25)', textAlign:'center', padding:40 }}>
        ...
        <div style={{ fontWeight:900, fontSize:20, color:'#1e293b', marginBottom:8 }}>{t('marketing.csAiGenTitle','AI 크리에이티브 생성')}</div>
        <div style={{ fontSize:12, color:'#64748b', maxWidth:500, margin:'0 auto', lineHeight:1.7, marginBottom:20 }}>
          {t('marketing.csAiGenDesc','카테고리와 채널을 선택하면 AI가 최적의 광고 소재를 자동 생성합니다. 생성된 소재는 바로 캠페인에 활용할 수 있습니다.')}
        </div>
        ...
        }}>⚙ {t('marketing.csStartFromSetup','캠페인 설정에서 시작하기')}</button>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))', gap:12 }}>
        {[
          { icon:'🖼', title:t('marketing.csTypeCarousel','카루셀 광고'), desc:t('marketing.csTypeCarouselDesc','여러 이미지를 슬라이드로 구성'), formats:'Meta · Instagram' },
          { icon:'🎬', title:t('marketing.csTypeVideo','동영상 광고'), desc:t('marketing.csTypeVideoDesc','15~60초 숏폼 영상 자동 제작'), formats:'TikTok · YouTube' },
          { icon:'📐', title:t('marketing.csTypeBanner','배너 광고'), desc:t('marketing.csTypeBannerDesc','반응형 디스플레이 배너 생성'), formats:'Google · Naver' },
          { icon:'📱', title:t('marketing.csTypeStory','스토리 광고'), desc:t('marketing.csTypeStoryDesc','9:16 세로형 풀스크린 소재'), formats:'Instagram · TikTok' },
          { icon:'🛍', title:t('marketing.csTypeShopping','쇼핑 광고'), desc:t('marketing.csTypeShoppingDesc','상품 피드 기반 DPA 소재'), formats:'Meta · Coupang' },
          { icon:'💬', title:t('marketing.csTypeMessage','메시지 광고'), desc:t('marketing.csTypeMessageDesc','카카오톡·LINE 템플릿'), formats:'Kakao · LINE' },
        ].map((f, i) => (
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
            <thead>
              <tr style={{ borderBottom:'2px solid rgba(0,0,0,0.08)' }}>
                {[
                  t('marketing.csColName','소재명'),
                  t('marketing.csColFormat','포맷'),
                  t('marketing.csColPlatform','플랫폼'),
                  'CTR',
                  t('marketing.csConv','전환'),
                  t('marketing.csColStatus','상태')
                ].map(h => (
  ...
  return (
    <div style={{ padding:0, minHeight:"100%", color:"#1e293b" }}>
      ...
      <div style={{ marginTop:24, padding:"14px 20px", borderRadius:12, background: isLight ? 'rgba(34,197,94,0.05)' : "linear-gradient(135deg, rgba(34,197,94,0.06), rgba(16,185,129,0.04))", border:"1px solid rgba(34,197,94,0.12)", display:"flex", alignItems:"center", gap:10 }}>
        ...
        <span style={{ marginLeft:'auto', fontSize:10, color: isLight ? '#374151' : '#64748b' }}>{t('marketing.csLastSync','마지막 동기화: 방금')}</span>
      </div>
    </div>
  );
}
