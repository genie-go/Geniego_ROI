  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      {base && sim && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
          {[
            ...
            { l: "셀러 Deduction Total", bv: fM(base.sellerDed), sv: fM(sim.sellerDed), d: delta.ded, fmt: v => fM(v) },
          ].map(({ l, bv, sv, d, fmt }) => (
      ...
      <div className="card card-glass" style={{ padding: 14, borderLeft: "3px solid #6366f1" }}>
        <div style={{ fontWeight: 700, fontSize: 12, color: "#6366f1", marginBottom: 6 }}>📊 롤업 재버킷팅 가정</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: 8, fontSize: 11 }}>
          {pfR && [
            ...
            ["OrdersCount", pfR.orders + "건"],
            ["반품률", pfR.return_rate + "%"],
            ["Cost Price율", pfR.cogs_pct + "%"],
            ["정산Deduction율", pfR.settled_pct + "%"],
          ].map(([k, v]) => (
        ...
        <div style={{ fontSize: 10, color: "var(--text-3)", marginTop: 8 }}>* Last 30 Days 실적 기준 재버킷팅 Simulation — Deduction 항목 Change 시 위 KPI에 미치는 영향 추정</div>
      </div>
      ...
      <div className="card card-glass" style={{ padding: 16 }}>
        ...
        {pfDeds.length === 0 ? <div style={{ color: "var(--text-3)", fontSize: 12 }}>해당 Platform에 Register된 Deduction 항목 None</div> : (
                ...
                <div key={d.id} style={{ padding: "10px 12px", borderRadius: 8, background: changed ? "rgba(99,102,241,0.07)" : "transparent", border: `1px solid ${changed ? "rgba(99,102,241,0.3)" : "rgba(99,102,241,0.08)"}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      ...
                      {changed && <Badge label="Change됨" color="#6366f1" />}
                    </div>
                    <div style={{ fontSize: 11, color: "var(--text-3)" }}>셀러 실부담: <b style={{ color: PC.SELLER }}>{sellerEff.toFixed(2)}%</b></div>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                    ...
                    {d.payer === "SHARED" && (
                      <div>
                        <div style={{ fontSize: 10, color: "var(--text-3)", marginBottom: 2 }}>셀러 부담 Rate <span style={{ color: "#6366f1" }}>(원본: {d.sp}%)</span></div>
                    ...
                    <div>
                      <div style={{ fontSize: 10, color: "var(--text-3)", marginBottom: 2 }}>부담자</div>
                      <select value={sd.payer} onChange={e => applyChange(d.id, "payer", e.target.value)} style={{ ...iS, width: "100%" }}>
                        <option value="SELLER">셀러 부담</option><option value="PLATFORM">Platform 부담</option><option value="SHARED">분담</option>
                      </select>
                    </div>
                  </div>
                </div>
...
const STATUS_L = { PENDING: "Pending", APPROVED_1: "1차 Approval", APPROVED: "Approval Done", APPLIED: "Apply됨", REJECTED: "Rejected" };
    ...
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10 }}>
        {[
          ...
          { l: "1차 Approval", n: queue.filter(q => q.status === "APPROVED_1").length, c: "#06b6d4" },
          { l: "Apply됨", n: queue.filter(q => q.status === "APPLIED").length, c: "#6366f1" },
      ...
      <div style={{ display: "flex", gap: 1, background: "rgba(9,15,30,0.5)", borderRadius: 8, padding: 2, width: "fit-content" }}>
        {[["pending", "📋 Approval Pending" + (pending.length ? " (" + pending.length + ")" : "")], ["log", "📜 감사 로그"]].map(([v, l]) => (
      ...
      {/* List */}
      <div style={{ display: "grid", gap: 10 }}>
        {shown.length === 0 && <div style={{ color: "var(--text-3)", fontSize: 12, padding: 20, textAlign: "center" }}>항목 None</div>}
        {shown.map(q => (
          <div key={q.id} style={{ padding: "14px 16px", borderRadius: 10, border: `1px solid ${STATUS_C[q.status]}33`, background: `${STATUS_C[q.status]}08` }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
                {["before", "after"].map(key => (
                  <div key={key} style={{ padding: "8px 10px", borderRadius: 6, background: "rgba(9,15,30,0.4)", fontSize: 10 }}>
                    <div style={{ fontWeight: 700, color: key === "before" ? "#ef4444" : "#22c55e", marginBottom: 4 }}>{key === "before" ? "Change 전" : "Change 후"}</div>
            ...
            {tab === "pending" && (
              <div style={{ display: "flex", gap: 8 }}>
                {q.status === "PENDING" && (
                  <button onClick={() => act(q.id, "APPROVE_1")} style={{ padding: "5px 14px", borderRadius: 6, border: "none", background: "#06b6d4", color: "#fff", fontSize: 11, cursor: "pointer", fontWeight: 700 }}>✓ 1차 Approval</button>
                )}
                {q.status === "APPROVED_1" && (
                  <button onClick={() => act(q.id, "APPROVE_2")} style={{ padding: "5px 14px", borderRadius: 6, border: "none", background: "#22c55e", color: "#fff", fontSize: 11, cursor: "pointer", fontWeight: 700 }}>✓✓ 최종 Approval</button>
                )}
                {q.status === "APPROVED" && (
                ...
                {["PENDING", "APPROVED_1"].includes(q.status) && (
                  <button onClick={() => act(q.id, "REJECT")} style={{ padding: "5px 14px", borderRadius: 6, border: "1px solid rgba(239,68,68,0.3)", background: "transparent", color: "#ef4444", fontSize: 11, cursor: "pointer" }}>✗ 거부</button>
                )}
              </div>
            )}
          </div>
...
/* ════ MAIN ════ */
const TABS = [
  { id: "platform", label: "🌐 Platform 매핑", desc: "기준Value 레지스트리" },
  { id: "deduction", label: "💸 Deduction 항목", desc: "부담자 Rate 분해" },
  { id: "kpi", label: "📊 KPI Impact", desc: "롤업 재버킷팅 시뮬" },
  { id: "queue", label: "✅ Approval 큐", desc: "Notification→Approval→Apply→로그" },
];
...
export default function MappingRegistry() {
  ...
  const propose = useCallback(change => {
    const entry = {
      ...
      status: "PENDING",
      ts: tsNow(),
      log: [{ ts: tsNow(), actor: "현재User", action: "Change 제안" }],
    };
    setQueue(q => [entry, ...q]);
    setFlash("✓ Approval 큐에 Register됨"); setTimeout(() => setFlash(""), 2000);
  }, []);
  ...
  const handleQueueAction = useCallback((id, action) => {
    setQueue(q => q.map(item => {
      ...
      const now = tsNow();
      const actorMap = { APPROVE_1: "OwnerA", APPROVE_2: "Team장B", APPLY: "시스템", REJECT: "OwnerA" };
      const statusMap = { APPROVE_1: "APPROVED_1", APPROVE_2: "APPROVED", APPLY: "APPLIED", REJECT: "REJECTED" };
      const msgMap = { APPROVE_1: "1차 Approval", APPROVE_2: "최종 Approval", APPLY: "레지스트리 Apply Done", REJECT: "거부" };
      return { ...item, status: statusMap[action], log: [...item.log, { ts: now, actor: actorMap[action], action: msgMap[action] }] };
    }));
    if (action === "APPLY") setFlash("✓ Change 사항 Apply Done"); setTimeout(() => setFlash(""), 2000);
  }, []);
    ...
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <div className="hero fade-up" style={{ background: "linear-gradient(135deg,rgba(99,102,241,0.07),rgba(168,85,247,0.05))", borderColor: "rgba(99,102,241,0.18)" }}>
        <div className="hero-meta">
          ...
          <div>
            <div className="hero-title" style={{ background: "linear-gradient(135deg,#6366f1,#a855f7)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Mapping Registry 고도화</div>
            <div className="hero-desc">정적 dict → UI Management형 DB · Deduction 부담자 Rate 분해 · KPI Impact 시뮬 · 2-person Approval 큐 + 감사로그</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap", alignItems: "center" }}>
          {[{ l: pf.length + "개 Platform", c: "#6366f1" }, { l: deds.length + "개 Deduction항목", c: "#a855f7" }, { l: "2단계 Approval 큐", c: "#22c55e" }, { l: "감사로그 Auto 기록", c: "#06b6d4" }]
