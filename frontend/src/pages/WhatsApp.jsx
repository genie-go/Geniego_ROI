        ...
        <div className="card card-glass">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                ...
                <div>
                    ...
                    {result && (
                        <div style={{
                            marginTop: 10, padding: '10px', borderRadius: 8, fontSize: 12, lineHeight: 1.6,
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
                    <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                            ...
                            <span className="hero-title" style={{ background: 'linear-gradient(135deg,#25D366,#128C7E)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                                WhatsApp Business
                            </span>
                        </div>
                        <div className="hero-desc">Auth키 Register 즉시 실Integration · 템플릿 Message · 일괄 Send · Send History Management</div>
                        <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
                            <Tag label={status === 'connected' || status === 'ok' ? '✓ Connected' : status === 'demo' ? '🎮 Demo' : '○ 미Connect'} color={statusColor} />
            ...
            <div style={{ display: 'flex', gap: 4, padding: '5px', background: 'rgba(0,0,0,0.25)', borderRadius: 14, flexWrap: 'wrap' }}>
                {TABS.map(t => (
                    <button key={t.id} onClick={() => setTab(t.id)} style={{
                        padding: '7px 14px', borderRadius: 10, border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 11, flex: 1, minWidth: 80,
                        background: tab === t.id ? 'linear-gradient(135deg,#25D366,#128C7E)' : 'transparent', color: tab === t.id ? '#fff' : 'var(--text-2)', transition: 'all 150ms'
                    }}>
                        {t.label}
                    </button>
                ))}
