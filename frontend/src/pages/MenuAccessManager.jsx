...
const SYSTEM_MENUS = [
    ...
    { catKey: "menuAccess.catAI", catDef: "AI 자동화 & 룰 엔진", items: ["AI 전략 생성기", "캠페인 관리", "고객 여정 빌더", "신규 메뉴관리자 접근권한"] },
...
const PLAN_PRESETS = {
    ...
    pro: {
        ...
        color: "#3b82f6",
        perms: { 
            read: ["요약 대시보드", "실시간 모니터링", "알림 피드", "AI 전략 생성기", "캠페인 관리", "광고 성과 분석", "마케팅 인텔리전스", "어트리뷰션 분석", "CRM 세그먼트", "이메일 마케팅", "주문 통합 허브"], 
            create: ["AI 전략 생성기", "캠페인 관리", "이메일 마케팅"], 
            update: ["캠페인 관리"] 
        }
    },
    enterprise: {
...
export default function MenuAccessManager() {
    ...
    const handleSave = () => {
        if(isDemo) {
            alert(t("marketing.demoLocker") || '데모 모드에서는 저장할 수 없습니다.');
            return;
        }
        ...
        <div style={{ padding: 24, paddingBottom: 60, minHeight: '100vh', background: '#0a101d', color: '#fff' }}>
            ...
            <div style={{ marginBottom: 24, padding: "20px 24px", background: "linear-gradient(135deg, rgba(79,142,247,0.1), rgba(168,85,247,0.05))", border: "1px solid rgba(79,142,247,0.2)", borderRadius: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                    ...
                    <h3 style={{ fontSize: 15, fontWeight: 800, color: '#93c5fd' }}>{t("menuAccess.recommendTitle") || "타사 (Salesforce, HubSpot) 사례 기반 플랜 권한 자동 추천"}</h3>
                </div>
            ...
            <div style={{ display: 'flex', gap: 20, position: 'relative' }}>
                <div style={{ flex: 1, background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 20, border: '1px solid rgba(255,255,255,0.1)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, paddingBottom: 16, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            <span style={{ fontSize: 13, fontWeight: 700, color: '#94a3b8' }}>{t("menuAccess.targetObj") || "권한 대상:"}</span>
                            <div style={{ padding: '6px 14px', background: 'rgba(79,142,247,0.15)', color: '#93c5fd', borderRadius: 20, fontSize: 12, fontWeight: 800 }}>
                                {PLAN_PRESETS[selectedTarget]?.name || "Custom Configuration"}
                            </div>
                        </div>
                        <button onClick={handleSave} style={{
                            padding: '8px 18px', background: 'linear-gradient(135deg, #10b981, #059669)', border: 'none', color: '#fff', fontWeight: 800, borderRadius: 8, cursor: 'pointer'
                        }}>💾 {t("menuAccess.saveBtn") || "매트릭스 덮어쓰기 (배포)"}</button>
                    </div>

                    <div style={{ display: 'grid', gap: 12 }}>
                        {SYSTEM_MENUS.map((sec, idx) => (
                            <div key={idx} style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 8, overflow: 'hidden' }}>
                                <div style={{ background: 'rgba(79,142,247,0.08)', padding: '10px 16px', fontWeight: 900, color: '#93c5fd', fontSize: 13 }}>
                                ...
                                <div style={{ padding: '12px 16px', display: 'grid', gap: 8 }}>
                                    {sec.items.map(item => {
                                        ...
                                        return (
                                            <div key={item} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: 'rgba(255,255,255,0.02)', borderRadius: 6, border: '1px solid rgba(255,255,255,0.03)' }}>
                                                <div style={{ fontSize: 13, fontWeight: 700, color: '#e2e8f0' }}>{t(`menuAccess.item_${item}`) || item}</div>
