...
export default function CampaignEnterpriseTabs() {
  const { t } = useI18n();
  const [activeTab, setActiveTab] = useState(0);
  const tabs = ["All Campaigns","Active","Drafts","Analytics"];
  const kpis = [{"emoji":"📋","label":"Campaigns","val":24},{"emoji":"🔄","label":"Active","val":8},{"emoji":"📊","label":"Avg CTR","val":"3.8%"},{"emoji":"💰","label":"Total Spend","val":"$42K"}];
    ...
    <div style={{ padding: 24, minHeight: "100%", color: "var(--text-1, #1e293b)" }}>
      <div style={{ borderRadius: 18, padding: "28px 32px", marginBottom: 22, background: "linear-gradient(135deg, rgba(79,142,247,0.08), rgba(99,102,241,0.06))", border: "1px solid rgba(79,142,247,0.12)", backdropFilter: "blur(16px)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            <div style={{ fontSize: 22, fontWeight: 800, color: "var(--text-1, #1e293b)" }}>📋 Campaign Enterprise</div>
            <div style={{ fontSize: 13, color: "var(--text-3, #64748b)", marginTop: 2 }}>Enterprise campaign management with advanced orchestration</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ borderRadius: 16, padding: "28px 32px", minHeight: 320, background: "rgba(255,255,255,0.85)", border: "1px solid rgba(0,0,0,0.06)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 12 }}>
          {["Multi-channel orchestration","A/B testing","Budget pacing","Creative rotation"].map((f, i) => (
