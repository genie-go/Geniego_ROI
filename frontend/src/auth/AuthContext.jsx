...
const REAL_KEYS_FLAG = "genie_has_real_keys"; // 실제 API 키 등록 여부 플래그
...
export function AuthProvider({ children }) {
                ...
                if (r.ok) {
                    ...
                    try {
                        ...
                        if (d.ok && d.user) {
                            ...
                            // 캐시된 plan이 admin인데 서버가 demo/free를 반환하는 경우 → 로컬 admin 우선 유지
                            const cachedPlan = cached?.plan || "demo";
                            const serverPlan = d.user.plan || "demo";
    ...
    const LOCAL_ACCOUNTS = [
        {
            ...
            password: "admin1234!",
            user: { id: 1, email: "admin@geniego.com", name: "최고 관리자", plan: "admin",
                company: "Geniego", subscription_status: "admin", subscription_expires_at: null, is_local: true },
        },
        {
            email: "admin@genie-roi.com",
            password: "admin1234!",
            user: { id: 1, email: "admin@genie-roi.com", name: "관리자", plan: "admin",
        ...
        },
        {
            ...
            user: { id: 1, email: "master@geniego.com", name: "마스터 관리자", plan: "admin",
                company: "Geniego", subscription_status: "admin", subscription_expires_at: null, is_local: true },
        },
        {
    ...
    const upgradeToPro = useCallback(async (newToken, newUser) => {
        ...
        localStorage.removeItem('genie_demo_sample_override'); // 샘플 데이터 오버라이드 해제
        setHasRealKeys(true);
    ...
    const activateLiveMode = useCallback((updatedUser = null) => {
        localStorage.setItem(REAL_KEYS_FLAG, "1");
        setHasRealKeys(true);
        const newUser = updatedUser || (user ? { ...user, plan: user.plan === "demo" || user.plan === "free" ? "pro" : user.plan } : null);
        if (newUser) {
            setUser(newUser);
     ...
     * - demo/free: DEMO_ALLOWED_MENUS 화이트리스트만 허용 (유료/데모 완전 분리)
    ...
    const hasMenuAccess = useCallback((menuKey) => {
        ...
        // ── 데모(demo/free) 사용자: admin 섹션만 차단, 나머지 전체 허용 ──
    ...
    return (
        <AuthContext.Provider value={{
            user, token, loading,
            login, loginDemo, register, logout, upgrade, upgradeToPro,
