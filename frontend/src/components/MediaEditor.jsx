...
/* ─── Channel별 광고 규격 가이드 ─── */
const buildChannelSpecs = (t) => ({
    meta: [
        { name: t('mediaEditor.spec.feedAd', '피드 광고'), ratio: '1:1', size: '1080×1080', note: t('mediaEditor.spec.metaFeedNote', '정사각형 · JPG/PNG · 최대 30MB') },
        { name: t('mediaEditor.spec.storyAd', '스토리 광고'), ratio: '9:16', size: '1080×1920', note: t('mediaEditor.spec.metaStoryNote', '세로형 · 텍스트 20% 이하 권장') },
        { name: t('mediaEditor.spec.horizontalBanner', '가로 배너'), ratio: '1.91:1', size: '1200×628', note: t('mediaEditor.spec.metaBannerNote', '링크 광고 · 뉴스피드 최적') },
    ],
    instagram: [
        { name: t('mediaEditor.spec.feedAd', '피드 광고'), ratio: '1:1', size: '1080×1080', note: t('mediaEditor.spec.igFeedNote', '기본 정사각형') },
        { name: t('mediaEditor.spec.reelsAd', '릴스 광고'), ratio: '9:16', size: '1080×1920', note: t('mediaEditor.spec.igReelsNote', '세로형 영상 · 최대 60초') },
        { name: t('mediaEditor.spec.storyAd', '스토리 광고'), ratio: '9:16', size: '1080×1920', note: t('mediaEditor.spec.igStoryNote', '15초 이내 권장') },
    ],
    tiktok: [
        { name: t('mediaEditor.spec.inFeedAd', '인피드 광고'), ratio: '9:16', size: '1080×1920', note: t('mediaEditor.spec.ttInFeedNote', '세로형 영상 · 5~60초') },
        { name: t('mediaEditor.spec.topView', '탑뷰'), ratio: '9:16', size: '1080×1920', note: t('mediaEditor.spec.ttTopViewNote', '첫 오픈 시 노출 · 최대 60초') },
    ],
    youtube: [
        { name: t('mediaEditor.spec.bumperAd', '범퍼 광고'), ratio: '16:9', size: '1920×1080', note: t('mediaEditor.spec.ytBumperNote', '6초 스킵 불가') },
        { name: t('mediaEditor.spec.inStream', '인스트림'), ratio: '16:9', size: '1920×1080', note: t('mediaEditor.spec.ytInStreamNote', '5초 후 스킵 가능 · 최대 3분') },
        { name: t('mediaEditor.spec.display', '디스플레이'), ratio: '16:9', size: '300×250', note: t('mediaEditor.spec.ytDisplayNote', '사이드바 디스플레이 광고') },
    ],
    google_search: [
        { name: t('mediaEditor.spec.responsiveAd', '반응형 광고'), ratio: '1:1', size: '1200×1200', note: t('mediaEditor.spec.gsResponsiveNote', '정사각형 기본 이미지') },
        { name: t('mediaEditor.spec.horizontalType', '가로형'), ratio: '1.91:1', size: '1200×628', note: t('mediaEditor.spec.gsHorizontalNote', '가로형 이미지 광고') },
    ],
    naver_search: [
        { name: t('mediaEditor.spec.powerLink', '파워링크'), ratio: '1:1', size: '500×500', note: t('mediaEditor.spec.nsPowerLinkNote', 'Search 광고 이미지') },
        { name: t('mediaEditor.spec.banner', '배너'), ratio: '3:1', size: '1029×258', note: t('mediaEditor.spec.nsBannerNote', '브랜드 Search 배너') },
    ],
    kakao: [
        { name: t('mediaEditor.spec.kakaoFeed', 'Kakao 피드'), ratio: '1:1', size: '800×800', note: t('mediaEditor.spec.kakaoFeedNote', '비즈보드 정사각형') },
        { name: t('mediaEditor.spec.fullView', '풀뷰'), ratio: '2:1', size: '800×400', note: t('mediaEditor.spec.kakaoFullViewNote', '와이드 이미지형') },
    ],
    default: [
        { name: t('mediaEditor.spec.basicSquare', '기본 정사각형'), ratio: '1:1', size: '1080×1080', note: t('mediaEditor.spec.defBasicNote', '범용 SNS 광고') },
        { name: t('mediaEditor.spec.horizontalBannerType', '가로형 배너'), ratio: '16:9', size: '1920×1080', note: t('mediaEditor.spec.defHorizontalNote', '웹 배너·유튜브 썸네일') },
    ],
});

/* ─── 기본 텍스트 레이어 ─── */
const defaultLayer = (t) => ({
    id: Date.now() + Math.random(),
    text: t('mediaEditor.defaultLayerText', '텍스트를 입력하세요'),
    x: 50, y: 40,
    fontSize: 22,
...
export default function MediaEditor({ channelName, channelId, accentColor = '#6366f1', onExport, aiCreative }) {
    ...
    const specsMap = buildChannelSpecs(t);
    const specs = specsMap[channelId] || specsMap.default;
    ...
    useEffect(() => {
        ...
        if (aiCreative.headline || aiCreative.copy) {
            ...
            if (aiCreative.headline) {
                layers.push({ ...defaultLayer(t), id: Date.now() + 1, text: aiCreative.headline, fontSize: 26, y: 40, bold: true, shadow: true });
            }
            if (aiCreative.copy) {
                layers.push({ ...defaultLayer(t), id: Date.now() + 2, text: aiCreative.copy, fontSize: 14, y: 62, bold: false, shadow: true });
            }
            if (aiCreative.cta) {
                layers.push({ ...defaultLayer(t), id: Date.now() + 3, text: aiCreative.cta, fontSize: 16, y: 80, bold: true, bg: true, bgColor: 'rgba(0,0,0,0.55)' });
            }
            if (layers.length > 0) {
                setLayers(layers);
                setAiImported(true);
    ...
    const handleFile = useCallback(file => {
        ...
        if (!file.type.startsWith('image/') && !isVideo) {
            alert(t('mediaEditor.fileTypeAlert', '이미지(JPG·PNG·GIF·WebP) 또는 동영상(MP4·MOV·WebM) 파일을 선택하세요.'));
            return;
        }
        const url = URL.createObjectURL(file);
        setMedia({ url, type: isVideo ? 'video' : 'image', file, name: file.name });
        if (layers.length === 0) {
            setLayers([defaultLayer(t)]);
        }
        setSelId(null);
        setAiImported(false);
    }, [layers.length]);
    ...
    /* ─── 레이어 조작 ─── */
    const addLayer = () => { const l = defaultLayer(t); setLayers(p => [...p, l]); setSelId(l.id); };
    ...
    const importAiText = () => {
        ...
        const newLayers = [];
        if (aiCreative.headline) {
            newLayers.push({ ...defaultLayer(t), id: Date.now() + 1, text: aiCreative.headline, fontSize: 26, y: 38, bold: true });
        }
        if (aiCreative.copy) {
            newLayers.push({ ...defaultLayer(t), id: Date.now() + 2, text: aiCreative.copy, fontSize: 14, y: 60, bold: false });
        }
        if (aiCreative.cta) {
            newLayers.push({ ...defaultLayer(t), id: Date.now() + 3, text: aiCreative.cta, fontSize: 16, y: 80, bold: true, bg: true, bgColor: 'rgba(0,0,0,0.5)' });
        }
    ...
    const exportImage = () => {
        ...
        img.onload = () => {
            ...
            a.click();
            onExport?.(dataUrl);
            setExportMsg(t('mediaEditor.exportDone', '✅ PNG 이미지 Save Done!'));
    ...
    return (
        <div style={{ display: 'grid', gap: 14 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                <div>
                    <div style={{ fontSize: 13, fontWeight: 800, color: accentColor }}>{t('mediaEditor.title', '🖼️ 이미지/동영상 업로드 편집기')}</div>
                    <div style={{ fontSize: 10, color: 'var(--text-3)', marginTop: 2 }}>
                        {t('mediaEditor.subtitle', '직접 만든 이미지·동영상을 업로드하고 텍스트 오버레이를 Add하세요')}
                    </div>
                </div>
                <button
                    ...
                    📐 {showSpecGuide ? t('mediaEditor.closeSpec', '규격 Close') : t('mediaEditor.viewChannelSpec', 'Channel 광고 규격 보기')}
                </button>
            </div>
            ...
            {!media && (
                <div
                    ...
                    <div style={{ fontWeight: 800, fontSize: 14, color: accentColor, marginBottom: 6 }}>
                        {t('mediaEditor.uploadPrompt', '이미지 또는 동영상 업로드')}
                    </div>
                    <div style={{ fontSize: 10, color: 'var(--text-3)', marginBottom: 6, lineHeight: 1.6 }}>
                        {t('mediaEditor.dragDropHint', '드래그&드롭 또는 클릭하여 파일 선택')}<br />
                        <span style={{ opacity: 0.8 }}>{t('mediaEditor.supportedFormats', 'JPG · PNG · GIF · WebP · MP4 · MOV · WebM 지원')}</span>
                    </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px', gap: 14 }}>
                    ...
                    <div style={{ position: 'relative' }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap', alignItems: 'center' }}>
                            <button onClick={addLayer} style={{ ...btn(false), background: accentColor + '18', padding: '6px 14px' }}>{t('mediaEditor.addText', '＋ 텍스트 Add')}</button>
                            {sel && (
                                <>
                                    <button onClick={() => duplicateLayer(selId)} style={{ ...btn(false, '#14b8a6') }}>{t('mediaEditor.duplicate', '📋 복제')}</button>
                                    <button onClick={() => delLayer(selId)} style={{ ...btn(false, '#ef4444') }}>{t('mediaEditor.delete', '🗑️ Delete')}</button>
                                </>
                            )}
                            <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
                                ...
                                <button onClick={() => { URL.revokeObjectURL(media.url); setMedia(null); setLayers([]); setAiImported(false); }}
                                    style={{ ...btn(false, '#f59e0b') }}>{t('mediaEditor.replace', '🔄 교체')}</button>
                            </div>
                        </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 580, overflowY: 'auto', paddingRight: 2 }}>
                        ...
                        {!sel && (
                            <div style={{ fontSize: 10, color: 'var(--text-3)', padding: '16px 10px', textAlign: 'center', border: '1px dashed rgba(150,150,200,0.2)', borderRadius: 10 }}>
                                {t('mediaEditor.emptyPanelLine1', '프리뷰에서 텍스트를 클릭하거나')}<br />{t('mediaEditor.emptyPanelLine2', '＋ 텍스트 Add를 누르세요')}
                            </div>
                        )}
                        ...
                        {sel && (<>
                            ...
                            <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                                <button onClick={() => updateSel({ bold: !sel.bold })} style={btn(sel.bold)}>{t('mediaEditor.bold', 'B 굵게')}</button>
                                <button onClick={() => updateSel({ italic: !sel.italic })} style={btn(sel.italic)}>{t('mediaEditor.italic', 'I 이탤릭')}</button>
                                <button onClick={() => updateSel({ shadow: !sel.shadow })} style={btn(sel.shadow)}>{t('mediaEditor.shadow', '🌔 그림자')}</button>
                                <button onClick={() => updateSel({ bg: !sel.bg })} style={btn(sel.bg)}>{t('mediaEditor.background', '🔲 배경')}</button>
                            </div>
                            ...
                            <div style={{ display: 'flex', gap: 5 }}>
                                {['left', 'center', 'right'].map(a => (
                                    <button key={a} onClick={() => updateSel({ align: a })} style={{ ...btn(sel.align === a), flex: 1 }}>
                                        {a === 'left' ? t('mediaEditor.alignLeft', '⬅ 좌') : a === 'center' ? t('mediaEditor.alignCenter', '↔ 중') : t('mediaEditor.alignRight', '➡ 우')}
                                    </button>
                                ))}
                            </div>
