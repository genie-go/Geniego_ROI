    ...
    return (
        <div style={{ marginBottom: 6 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3, fontSize: 10 }}>
                ...
                <span style={{ fontWeight: 800, color: col }}>{v}<span style={{ color: 'var(--text-3)' }}>/{max}</span></span>
            </div>
...
export default function InfluencerAIPanel({ creators = [], selectedId, style = {} }) {
    ...
    const buildPayload = useCallback(() => ({
        data: creators.map(c => ({
            ...
            graphics: {
                gender: {
                    male: c.graphics?.gender?.male ?? c.audienceMale ?? c.male_pct ?? 50,
                    female: c.graphics?.gender?.female ?? c.audienceFemale ?? c.female_pct ?? 50,
                },
                age: {
                    '10-19': c.graphics?.age?.['10-19'] ?? c.age1019 ?? 12,
                    '20-29': c.graphics?.age?.['20-29'] ?? c.age2029 ?? 32,
                    '30-39': c.graphics?.age?.['30-39'] ?? c.age3039 ?? 34,
                    '40-49': c.graphics?.age?.['40-49'] ?? c.age4049 ?? 15,
                    '50+': c.graphics?.age?.['50+'] ?? c.age50p ?? 7,
                },
                top_regions: c.graphics?.top_regions ?? c.topRegions ?? [
                    { region: '서울', pct: 38 }, { region: '경기', pct: 22 },
                    { region: '부산', pct: 14 }, { region: '인천', pct: 8 }, { region: '대구', pct: 6 },
                ],
                engagement_by_gender: {
                    male_er: c.graphics?.engagement_by_gender?.male_er ?? ((c.audienceMale ?? 50) * 0.032),
                    female_er: c.graphics?.engagement_by_gender?.female_er ?? ((c.audienceFemale ?? 50) * 0.038),
                },
                engagement_by_age: {
                    '10-19': c.graphics?.engagement_by_age?.['10-19'] ?? 5.4,
                    '20-29': c.graphics?.engagement_by_age?.['20-29'] ?? 4.8,
                    '30-39': c.graphics?.engagement_by_age?.['30-39'] ?? 3.2,
                    '40-49': c.graphics?.engagement_by_age?.['40-49'] ?? 2.1,
                    '50+': c.graphics?.engagement_by_age?.['50+'] ?? 1.4,
                },
                purchase_contribution: {
                    by_gender: {
                        male: c.graphics?.purchase_contribution?.by_gender?.male ?? 44,
                        female: c.graphics?.purchase_contribution?.by_gender?.female ?? 56,
                    },
                    by_age: {
                        '20-29': c.graphics?.purchase_contribution?.by_age?.['20-29'] ?? 35,
                        '30-39': c.graphics?.purchase_contribution?.by_age?.['30-39'] ?? 38,
                        '40-49': c.graphics?.purchase_contribution?.by_age?.['40-49'] ?? 18,
                        'other': c.graphics?.purchase_contribution?.by_age?.['other'] ?? 9,
                    },
                    by_region: {
                        '서울/경기': c.graphics?.purchase_contribution?.by_region?.['서울/경기'] ?? 58,
                        '부산/경남': c.graphics?.purchase_contribution?.by_region?.['부산/경남'] ?? 14,
                        '기타': c.graphics?.purchase_contribution?.by_region?.['기타'] ?? 28,
                    },
                },
            },
        })),
    ...
    const CARD = { background: 'linear-gradient(145deg,rgba(255,255,255,0.03),rgba(8,18,38,0.95))', border: '1px solid var(--border)', borderRadius: 12, padding: '12px 14px' };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, ...style }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button onClick={loadHistory}
                        style={{
                            ...
                            background: 'transparent', color: 'var(--text-3)', cursor: 'pointer', fontSize: 11, fontWeight: 700
                        }}>
                        📜 이력
                    </button>
                </div>
            ...
            {status === 'done' && result && (
                <>
                    <div style={CARD}>
                        <div style={{
                            fontSize: 11, fontWeight: 700, color: 'var(--text-3)', marginBottom: 8,
                        ...
                        {(result.portfolio_insights || []).map((ins, i) => (
                            <div key={i} style={{ display: 'flex', gap: 6, marginTop: 6, fontSize: 11, color: 'var(--text-3)' }}>
                    ...
                    {curResult && selCreator && (
                        <div style={{
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
                                <div style={{ background: 'rgba(34,197,94,0.07)', borderRadius: 8, padding: '8px 10px', border: '1px solid rgba(34,197,94,0.15)' }}>
                                    ...
                                    {(curResult.strengths || []).map((s, i) => (
                                        <div key={i} style={{ fontSize: 10, color: 'var(--text-2)', marginBottom: 3 }}>• {s}</div>
                                    ))}
                                </div>
                                <div style={{ background: 'rgba(248,113,113,0.07)', borderRadius: 8, padding: '8px 10px', border: '1px solid rgba(248,113,113,0.15)' }}>
                                    ...
                                    {(curResult.weaknesses || []).map((w, i) => (
                                        <div key={i} style={{ fontSize: 10, color: 'var(--text-2)', marginBottom: 3 }}>• {w}</div>
                                    ))}
                                </div>
                            </div>
                            ...
                            {curResult.ai_insight && (
                                <div style={{ fontSize: 11, color: 'var(--text-2)', lineHeight: 1.7, marginBottom: 6 }}>
                                    🔍 {curResult.ai_insight}
                                </div>
                            )}
            ...
            {history && (
                <div style={CARD}>
                    ...
                    {history.length === 0 && <div style={{ fontSize: 11, color: 'var(--text-3)' }}>이력 None</div>}
                    {history.map(h => (
                        <div key={h.id} style={{ padding: '6px 0', borderBottom: '1px solid var(--border)', fontSize: 11 }}>
                            ...
                            <div style={{ color: 'var(--text-3)', lineHeight: 1.5 }}>{(h.summary || '').slice(0, 85)}…</div>
                        </div>
                    ))}
                </div>
            )}
