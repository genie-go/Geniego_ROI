        ...
        <svg width="100%" viewBox={`0 0 ${width} ${height}`} style={{ overflow: 'visible', display: 'block' }}>
            <g transform={`translate(${pad.l},${pad.t})`}>
                ...
                {gridLines.map(t => {
                    ...
                    return (
                        <g key={t}>
                            ...
                            <text x={-8} y={y + 4} {...FONT} fill="rgba(255,255,255,0.38)" textAnchor="end">{fmt(val)}</text>
                        </g>
                    );
                })}
                {/* X 축 라벨 */}
                {labels && data.map((_, i) => {
                    ...
                    return <text key={i} x={xScale(i)} y={H + 22} {...FONT} fill="rgba(255,255,255,0.38)" textAnchor="middle">{labels[i]}</text>;
                })}
                    ...
                    return (
                        <g key={i}>
                            ...
                            <rect x={x} y={H - bh} width={bw} height={Math.min(12, bh)} rx={4}
                                fill={fc} opacity={0.3} />
                            {/* 값 라벨 */}
                            <text x={x + bw / 2} y={H - bh - 5} {...FONT} fill="rgba(255,255,255,0.6)" textAnchor="middle">{fmt(v)}</text>
                            {/* X 라벨 */}
                            <text x={x + bw / 2} y={H + 24} {...FONT} fill="rgba(255,255,255,0.42)" textAnchor="middle">{d[xKey]}</text>
                        </g>
                    );
                })}
...
export function DonutChart({ data, size = 130, thickness = 24, label, sub }) {
    ...
    return (
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
            ...
            {label && <text x={cx} y={cy - 4} textAnchor="middle" fontSize={size * 0.12} fontWeight="900" fill="white">{label}</text>}
            {sub && <text x={cx} y={cy + 14} textAnchor="middle" fontSize={size * 0.082} fill="rgba(255,255,255,0.42)">{sub}</text>}
        </svg>
    );
}
...
export function seedSeries(base, len = 20, vol = 0.12) {
    return Array.from({ length: len }, (_, i) =>
        base * (1 - vol + (Math.sin(i * 0.9 + base * 0.001) + 1) * vol)
    );
}
