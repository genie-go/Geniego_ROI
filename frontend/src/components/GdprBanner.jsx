        ...
        <div style={{
            ...
            <div style={{ maxWidth: 640, margin: '0 auto' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                    <div>
                        <div style={{ fontWeight: 900, fontSize: 15, color: '#fff' }}>🔒 {t('gdpr.panelTitle')}</div>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <button onClick={handleSave} style={{ flex: 1, padding: '10px 20px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg,#4f8ef7,#6366f1)', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                        {t('gdpr.savePref')}
                    </button>
...
function ConsentBanner({ onAcceptAll, onSettings }) {
    ...
    return (
        <div style={{
            ...
            <div>
                <div style={{ fontWeight: 900, fontSize: 13, color: '#fff', marginBottom: 4 }}>🍪 {t('gdpr.bannerTitle')}</div>
                <div style={{ fontSize: 11, color: 'var(--text-3)', lineHeight: 1.6 }}>
                    {t('gdpr.bannerDesc')}
                </div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={onAcceptAll} style={{ flex: 1, padding: '8px 16px', borderRadius: 8, border: 'none', background: 'linear-gradient(135deg,#4f8ef7,#6366f1)', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                    {t('demo.cookieAcceptAll')}
                </button>
            ...
            <div className="card card-glass">
                ...
                {consent ? (
                    <div style={{ display: 'grid', gap: 8 }}>
                        {CONSENT_TYPES.map(item => (
                            <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', borderRadius: 8, background: 'rgba(0,0,0,0.2)' }}>
