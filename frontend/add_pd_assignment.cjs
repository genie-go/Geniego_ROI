...
// Add en.pricingDetail = {...} BEFORE export default en;
// This is the same pattern as en.pricing.moreItems = "..." which works correctly

const FILE_DATA = [
  {
    fp: 'src/i18n/locales/en.js', v: 'en',
    badge_popular: 'Popular', badge_top: 'Top Tier',
    hl_growth: 'Marketing Growth Plan', hl_pro: 'AI Automation Plan', hl_ent: 'Unlimited Everything Plan',
    tl_free: 'Start Free, Explore Platform', tl_growth: 'Growing SMB Sellers & Brands',
    tl_pro: 'Professional E-commerce Brands & Agencies', tl_ent: 'Large E-commerce·Agency·Group',
  },
  {
    fp: 'src/i18n/locales/ko.js', v: 'ko',
    badge_popular: '인기', badge_top: '최고사양',
    hl_growth: '마케팅 성장 플랜', hl_pro: 'AI 자동화 플랜', hl_ent: '전체 무제한 플랜',
    tl_free: 'Free로 시작, 플랫폼 탐색', tl_growth: '성장기 중소 셀러·브랜드',
    tl_pro: '전문 이커머스 브랜드·에이전시', tl_ent: '대형 이커머스·에이전시·그룹사',
  },
  {
    fp: 'src/i18n/locales/ja.js', v: 'ja',
    badge_popular: '人気', badge_top: '最高仕様',
    hl_growth: 'マーケティング成長プラン', hl_pro: 'AI自動化プラン', hl_ent: '全て無制限プラン',
    tl_free: '無料で始める、プラットフォーム探索', tl_growth: '成長期の中小セラー・ブランド',
    tl_pro: 'プロECブランド・エージェンシー', tl_ent: '大型EC・エージェンシー・グループ',
  },
  {
    fp: 'src/i18n/locales/zh.js', v: 'zh',
    badge_popular: '热门', badge_top: '顶级',
    hl_growth: '营销增长方案', hl_pro: 'AI自动化方案', hl_ent: '无限一切方案',
    tl_free: '免费开始，探索平台', tl_growth: '成长型中小卖家·品牌',
    tl_pro: '专业电商品牌·代理商', tl_ent: '大型电商·代理商·集团',
  },
  {
    fp: 'src/i18n/locales/de.js', v: 'de',
    badge_popular: 'Beliebt', badge_top: 'Top-Tier',
    hl_growth: 'Marketing-Wachstumsplan', hl_pro: 'KI-Automatisierungsplan', hl_ent: 'Alles Unbegrenzt Plan',
    tl_free: 'Kostenlos starten', tl_growth: 'Wachsende KMU-Händler & Marken',
    tl_pro: 'Professionelle E-Commerce-Marken & Agenturen', tl_ent: 'Großes E-Commerce·Agentur·Konzern',
  },
  {
    fp: 'src/i18n/locales/th.js', v: 'th',
    badge_popular: 'ยอดนิยม', badge_top: 'สูงสุด',
    hl_growth: 'Marketing Growth Plan', hl_pro: 'AI Automation Plan', hl_ent: 'Unlimited Everything Plan',
    tl_free: 'Start Free, Explore Platform', tl_growth: 'Growing SMB Sellers & Brands',
    tl_pro: 'Professional E-commerce Brands', tl_ent: 'Large E-commerce·Agency·Group',
  },
  {
    fp: 'src/i18n/locales/vi.js', v: 'vi',
    badge_popular: 'Phổ biến', badge_top: 'Cao cấp',
    hl_growth: 'Marketing Growth Plan', hl_pro: 'AI Automation Plan', hl_ent: 'Unlimited Everything Plan',
    tl_free: 'Start Free, Explore Platform', tl_growth: 'Growing SMB Sellers & Brands',
    tl_pro: 'Professional E-commerce Brands', tl_ent: 'Large E-commerce·Agency·Group',
  },
  {
    fp: 'src/i18n/locales/id.js', v: 'id',
    badge_popular: 'Populer', badge_top: 'Premium',
    hl_growth: 'Marketing Growth Plan', hl_pro: 'AI Automation Plan', hl_ent: 'Unlimited Everything Plan',
    tl_free: 'Start Free, Explore Platform', tl_growth: 'Growing SMB Sellers & Brands',
    tl_pro: 'Professional E-commerce Brands', tl_ent: 'Large E-commerce·Agency·Group',
  },
  {
    fp: 'src/i18n/locales/zh-TW.js', v: 'zhTW',
    badge_popular: '熱門', badge_top: '頂級',
    hl_growth: '行銷成長方案', hl_pro: 'AI自動化方案', hl_ent: '無限一切方案',
    tl_free: '免費開始，探索平台', tl_growth: '成長型中小賣家·品牌',
    tl_pro: '專業電商品牌·代理商', tl_ent: '大型電商·代理商·集團',
  },
];

function buildPricingDetail(d) {
  return `${d.v}.pricingDetail = {
  free_tagline:"${d.tl_free}", free_headline:"Demo Experience Plan",
  ...
  growth_tagline:"${d.tl_growth}", growth_headline:"${d.hl_growth}",
  ...
  pro_tagline:"${d.tl_pro}", pro_headline:"${d.hl_pro}", pro_badge:"${d.badge_popular}",
  ...
  ent_tagline:"${d.tl_ent}", ent_headline:"${d.hl_ent}", ent_badge:"${d.badge_top}",
...
for (const data of FILE_DATA) {
  ...
  // Check if already has correct form
  if (c.includes(`${v}.pricingDetail = {`)) {
    console.log(`⏭ ${v}: already has ${v}.pricingDetail`);
    continue;
  }
  
  // Insert before export default
  const exportStr = `\nexport default ${v};\n`;
  const exportIdx = c.lastIndexOf(exportStr);
  if (exportIdx < 0) {
    // Try without trailing newline
    const alt = `\nexport default ${v};`;
    const altIdx = c.lastIndexOf(alt);
    if (altIdx < 0) { console.log(`⚠ ${v}: export not found`); continue; }
    const patch = '\n' + buildPricingDetail(data) + '\n';
    c = c.slice(0, altIdx) + patch + c.slice(altIdx);
  } else {
    const patch = '\n' + buildPricingDetail(data) + '\n';
    c = c.slice(0, exportIdx) + patch + c.slice(exportIdx);
  }
