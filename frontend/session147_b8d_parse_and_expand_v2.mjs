// session147_b8d_parse_and_expand_v2.mjs
// B8-D v2: Parse 10 batches + Top 500 raw txt -> ja_value lookup map -> expand to 2,505 unmapped rows
//
// V2 ADDITIONS (fixes 6 lookup misses from v1):
//   1. UN-ESCAPE \n, \r, \t in BOTH ja and ko after parsing (fixes reviews.aiReply* triple-miss)
//      Rationale: reviewer writes "\n\n" as literal backslash-n in batch raw .txt for line readability,
//      but ko.js stores REAL line feeds (0x0A). Un-escape restores the canonical form for both
//      the lookup key (ja) and the patched value (ko).
//   2. Pre-lookup ja_value normalization with override map for IME/round-trip corruption:
//      - "適용" (Hangul 용 U+C6A9) → "適用" (Kanji 用 U+7528) — N-147-A IME accident class
//      - "🇯 日本" (truncated flag) → "🇯🇵 日本" (full regional indicator pair)
//      - "🇰 韓国" (truncated flag) → "🇰🇷 韓国" (full regional indicator pair)
//      Override map applied to BOTH the in-memory map keys AND incoming unmapped.csv ja_values
//      so corrupted batch entries still resolve to the canonical unmapped.csv key.
//
// Inputs:
...
// ---------- Escape sequence un-escaper ----------
// Reviewer writes "\n", "\t", "\r" as LITERAL backslash sequences in batch raw .txt
// for line-per-entry readability. ko.js (and unmapped.csv) store the ACTUAL control chars.
// This function converts the literal escapes back to control chars in BOTH ja and ko.
// Side-effect: any genuine "\n" sequence in the source (extremely rare in UI text — would
// only appear as a literal "backslash followed by n" which is itself unusual) would be
// affected. For the GeniegoROI corpus this is safe (no such literal sequences observed).
function unescapeBackslashSeqs(s) {
  if (!s) return s;
  // Process in order to avoid re-processing inserted control chars
  // Use a temporary placeholder for literal backslashes ("\\") to preserve them
  // (also extremely rare in UI text but handled for correctness)
  return s
    .replace(/\\\\/g, '\x00')   // preserve literal backslash-backslash
    .replace(/\\n/g, '\n')
    .replace(/\\r/g, '\r')
    .replace(/\\t/g, '\t')
    .replace(/\x00/g, '\\');     // restore preserved backslashes
}

// ---------- ja_value normalization (round-trip corruption fixes) ----------
// Reviewer's batch raw .txt files have round-trip corruption from chat-message text rendering.
// Pre-lookup normalization restores canonical form so b8b corrupted keys still match
// the canonical ja_value in japanese_b1_b5_unmapped.csv.
//
// Strategy: per-substring replacements applied to ANY ja_value (batch keys + unmapped lookups).
// All substitutions are IDEMPOTENT: applying twice yields the same result.
function normalizeJaForLookup(ja) {
  if (!ja) return ja;
  return ja
    // N-147-A class: Korean Hangul 용 (U+C6A9) accidentally typed where Kanji 用 (U+7528) was meant
    .replace(/適\u{C6A9}/gu, '適用')  // "適용" → "適用"
    // Flag emoji truncation: regional indicator 🇯 (U+1F1EF) without 🇵 (U+1F1F5) → restore 🇯🇵
    // Pattern: 🇯 followed by space and "日本" specifically (avoid over-matching)
    .replace(/\u{1F1EF}(?!\u{1F1F5})( ?日本)/gu, '\u{1F1EF}\u{1F1F5}$1')
    // Flag emoji truncation: 🇰 (U+1F1F0) without 🇷 (U+1F1F7) → restore 🇰🇷
    .replace(/\u{1F1F0}(?!\u{1F1F7})( ?韓国)/gu, '\u{1F1F0}\u{1F1F7}$1');
}

// ko-side flag normalization: same truncation may exist in proposed_ko (reviewer copied
// the corrupted flag from ja to ko). Trigger context is Korean nouns 일본/한국 instead of 日本/韓国.
function normalizeKoForOutput(ko) {
  if (!ko) return ko;
  return ko
    .replace(/\u{1F1EF}(?!\u{1F1F5})( ?일본)/gu, '\u{1F1EF}\u{1F1F5}$1')
    .replace(/\u{1F1F0}(?!\u{1F1F7})( ?한국)/gu, '\u{1F1F0}\u{1F1F7}$1');
}
...
// V2: applies unescapeBackslashSeqs to both ja and ko before returning.
function parsePipeLine(line) {
  ...
  if (line.includes('\t')) {
    ...
    if (!ja) return null;
    ja = unescapeBackslashSeqs(ja);
    ko = unescapeBackslashSeqs(ko);
  ...
  let ja  = line.slice(first + 1, last);
  let ko  = line.slice(last + 1);
  if (!/^\d+$/.test(num)) return null;
  if (!ja) return null;
  ja = unescapeBackslashSeqs(ja);
  ko = unescapeBackslashSeqs(ko);
...
for (const { batch, file } of BATCH_FILES) {
  ...
  for (const raw of lines) {
    ...
    // V2: normalize ja_value to fix known round-trip corruption (IME, flag truncation)
    // so the batch key matches the canonical ja_value in unmapped.csv.
    // Also apply ko-side normalization: if reviewer corrupted ja during chat round-trip,
    // they very likely corrupted ko in the same way (e.g. copying "🇯" to both halves).
    // The canonical user-intended ko form has the proper regional indicator pair.
    const jaNormalized = normalizeJaForLookup(ja);
    const koNormalized = normalizeKoForOutput(ko);
    const normalizationApplied = (jaNormalized !== ja) || (koNormalized !== ko);
    ja = jaNormalized;
    ko = koNormalized;
    ...
    if (normalizationApplied) {
      entryIssues.push({
        level: 'INFO',
        code: 'JA_NORMALIZED',
        msg: `ja_value normalized (round-trip corruption fix applied)`,
      });
    }
...
if (fs.existsSync(IN_TOP500)) {
  ...
  for (const raw of lines) {
    ...
    // V2: normalize ja for canonical lookup, ko for output
    ja = normalizeJaForLookup(ja);
    ko = normalizeKoForOutput(ko);
    if (!map.has(ja)) {
...
for (const row of unmappedData) {
  ...
  // V2 lookup strategy:
  //   1. Try direct match (canonical case — most rows)
  //   2. Fallback: normalize the ja from unmapped.csv (handles case where unmapped.csv
  //      itself has corruption that matches a normalized batch key)
  //   Note: batch keys are ALREADY normalized at insert time (Step 1), so this fallback
  //   is mainly defensive — unmapped.csv is expected canonical.
  let m = map.get(ja);
  if (!m) {
    const jaNorm = normalizeJaForLookup(ja);
    if (jaNorm !== ja) {
      m = map.get(jaNorm);
    }
  }
