// session148_c7b2_apply_tail.mjs
...
//
// CLI:
//   node session148_c7b2_apply_tail.mjs --dry
//   node session148_c7b2_apply_tail.mjs --apply
...
if (!MODE) {
  console.error('Usage: node session148_c7b2_apply_tail.mjs --dry | --apply');
  process.exit(1);
}
...
const IN_MERGED = resolve(ROOT, 'latin_long_c6c_c7b_input.csv');
const BACKUP_DIR = resolve(ROOT, 'backup_session148_C7B_TAIL');
const BACKUP_KO = join(BACKUP_DIR, 'ko.js.bak');
const OUT_LOG = resolve(ROOT, 'c7b2_patch_log.txt');

console.log(`[c7b2] MODE: ${MODE}`);

for (const p of [KO_JS, JA_JS, ZH_JS, EN_JS, IN_MERGED]) {
  if (!existsSync(p)) {
    console.error(`[c7b2] FATAL: missing ${p}`);
    process.exit(1);
  }
}
...
// ---------- 캡처 baseline ----------
console.log('[c7b2] G2/G3: capturing sacred SHA256...');
const sha_ja_before = sha256(JA_JS);
const sha_zh_before = sha256(ZH_JS);
console.log(`[c7b2] ja.js SHA256: ${sha_ja_before.slice(0, 16)}...`);
console.log(`[c7b2] zh.js SHA256: ${sha_zh_before.slice(0, 16)}...`);

// ---------- ko.js / en.js import ----------
console.log('[c7b2] Loading ko.js...');
...
console.log(`[c7b2] G5: ko.js leaf count before: ${koLeavesBefore}`);
...
console.log(`[c7b2] G4: en.js leaf count: ${enLeaves}`);
...
console.log(`[c7b2] merged rows: ${data.length}`);
...
// ---------- dry-run: 패치 시뮬레이션 ----------
console.log('[c7b2] dry-run: validating patches...');
const planned = [];
const skipped = [];
const valueChanged = [];
const noOps = [];
...
console.log(`[c7b2] planned (real changes): ${planned.length}`);
console.log(`[c7b2] no-ops (외래어 유지): ${noOps.length}`);
console.log(`[c7b2] skipped: ${skipped.length}`);
console.log(`[c7b2] value_changed: ${valueChanged.length}`);
console.log(`[c7b2] touched namespaces: ${touchedNs.size}`);
...
`;
  ...
  console.log('[c7b2] DRY-RUN complete. Log:', OUT_LOG);
  process.exit(0);
}

// ---------- APPLY MODE ----------
console.log('[c7b2] APPLY mode: starting...');

// G1 backup
mkdirSync(BACKUP_DIR, { recursive: true });
copyFileSync(KO_JS, BACKUP_KO);
console.log(`[c7b2] G1: backup → ${BACKUP_KO}`);
...
console.log(`[c7b2] object updates: ${applied} applied, ${applyFailed} failed`);
...
console.log(`[c7b2] ns block replacements: ${replaceOk} ok, ${replaceFail} failed`);

// 파일 저장 후 syntax 검증
writeFileSync(KO_JS, currentRaw, 'utf8');

// G7 re-import 검증
console.log('[c7b2] G7: re-importing ko.js...');
let importOk = false;
...
if (!importOk) {
  console.error('[c7b2] G7 FAILED: syntax error');
...
if (koAfterLeaves !== koLeavesBefore) {
  console.error(`[c7b2] G5 FAILED: leaf count ${koLeavesBefore} → ${koAfterLeaves}`);
  copyFileSync(BACKUP_KO, KO_JS);
  writeFileSync(OUT_LOG, `# c7b2 FAILED: leaf count mismatch ${koLeavesBefore} → ${koAfterLeaves}\nbackup restored\n`, 'utf8');
  process.exit(3);
}
...
if (sha_ja_after !== sha_ja_before || sha_zh_after !== sha_zh_before) {
  console.error('[c7b2] G2/G3 FAILED: sacred file modified');
  copyFileSync(BACKUP_KO, KO_JS);
  process.exit(4);
}
...
console.log('[c7b2] DONE. Log:', OUT_LOG);
