...
def load_attribution(cur, default_path="templates/v323/default_attribution.json"):
...
def compute_candidate_scores(rule_cfg, ads_value, idx_exact, idx_prefix, idx_token, prefix_len):
    """Return (normalized_ads_value, candidates_dict)

    candidates_dict: key -> {'score': float, 'mode': str, 'modes': [str]}
    Mode precedence is based on max score among modes found.
    """
    if not ads_value:
        return "", {}
    ...
    def _add(k: str, score: float, mode: str):
        if not k or not _allow(k):
            return
        cur = cand.get(k)
        if cur is None:
            cand[k] = {"score": score, "mode": mode, "modes": [mode]}
        else:
            # keep mode list
            if mode not in cur["modes"]:
                cur["modes"].append(mode)
            # upgrade if higher score
            if score > float(cur["score"]):
                cur["score"] = score
                cur["mode"] = mode

    # exact
    if av in idx_exact:
        _add(av, w_exact, "exact")

    # startswith (prefix index)
    for k in candidate_keys_for_startswith(av, prefix_len, idx_prefix):
        _add(k, w_sw, "startswith")

    # contains (token index)
    for k in candidate_keys_for_contains(av, idx_token):
        _add(k, w_cont, "contains")
    ...
    for rr in regex_rules:
        try:
            ...
            if pat.search(av_raw or ""):
                # if ads regex matched, boost exact match if present
                if av in idx_exact:
                    _add(av, w_regex, "regex_ads")
        except Exception:
            continue

    return av, cand

def best_matches_topn(rule_cfg, ads_value, idx_exact, idx_prefix, idx_token, prefix_len, topn=5):
    """Return (best_result_or_None, top_candidates_list, diagnostics_dict)

    top_candidates_list: [{'key':..., 'score':..., 'mode':..., 'modes':[...]}] sorted by score desc.
    best_result_or_None: {conv, rev, score, matched_key, mode, normalized_ads_value, modes}
    diagnostics_dict: {candidate_count, filtered_by_whitelist(bool), filtered_by_regex(bool)}
    """
    av, cand = compute_candidate_scores(rule_cfg, ads_value, idx_exact, idx_prefix, idx_token, prefix_len)

    top = sorted(
        [{"key":k, "score":float(v["score"]), "mode":v["mode"], "modes":v.get("modes", [v["mode"]])} for k,v in cand.items()],
    ...
    best = top[0]
    val = idx_exact.get(best["key"])
    if not val:
        # should not happen; candidates are keys from indexes
        return None, top, {"candidate_count": len(cand)}
    res = {
        "conv": float(val.get("conv", 0.0)),
        "rev": float(val.get("rev", 0.0)),
        "score": float(best["score"]),
        "matched_key": best["key"],
        "mode": best["mode"],
        "modes": best.get("modes", [best["mode"]]),
        "normalized_ads_value": av,
    }
    return res, top, {"candidate_count": len(cand)}
...
def main():
    ...
    p.add_argument("--fx", default="templates/v323/fx_rates.json")
    p.add_argument("--default-attribution", default="templates/v323/default_attribution.json")
    ...
    p.add_argument("--topn-explain", type=int, default=5)
    p.add_argument("--max-explanations", type=int, default=500)
    args = p.parse_args()

    topn_explain = int(args.topn_explain or 5)
    max_explanations = int(args.max_explanations or 500)
    ...
    unattributed_cost_rows = []
    match_explanations = []  # limited rows for 'why matched' report  # list of dict
    fail_summary = {
        "total_ads_rows": 0,
        "matched_ads_rows": 0,
    ...
    for row in ads_rows:
        ...
        for r in chain:
            ...
            res, topc, diag = best_matches_topn(rule_cfg, ads_val, exact, pre, tok, prefix_len, topn=topn_explain)
            if not res:
            ...
            match = {"matched": True, "rule": r, "mode": res["mode"], "score": res["score"], "key": res["matched_key"], "modes": res.get("modes", [res["mode"]]), "candidates": topc, "candidate_count": (diag.get("candidate_count") if isinstance(diag, dict) else None)}
            break
        ...
        # collect explanation rows (limited)
        if len(match_explanations) < max_explanations:
            match_explanations.append({
                ...
                "ad_id": ad_id,
                "ad_name": ad_name,
                "ads_value_used": (ad_id if last_fail_rule == "ad_id" else (campaign_id if last_fail_rule == "campaign_id" else utm_campaign)) if not match.get("matched") else (ad_id if match.get("rule") == "ad_id" else (campaign_id if match.get("rule") == "campaign_id" else utm_campaign)),
                "matched": bool(match.get("matched")),
                "matched_key": match.get("key"),
                "rule": match.get("rule") if match.get("matched") else last_fail_rule,
                "mode": match.get("mode"),
                "modes": match.get("modes"),
                "score": match.get("score"),
                "candidate_count": match.get("candidate_count"),
                "top_candidates": match.get("candidates", [])[:topn_explain] if match.get("candidates") else [],
                "fail_reason": None if match.get("matched") else last_fail_reason,
                "cost_base": cost_base,
                "impressions": impr,
                "clicks": clk,
            })
    ...
    db.close()
    print(f"[OK] V323 wrote JSON to {out.resolve()} (chain={' > '.join(chain)}, min_score={min_score})")
