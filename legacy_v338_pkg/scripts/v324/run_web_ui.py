...
from scripts.v324.security import (
    hash_password, verify_password, get_secret_key, new_session_token, session_user,
    role_at_least, mask_api_key, hash_api_key,
)

from scripts.v324.permissions import has_perm
from scripts.v324.ops_store import OpsStore
from scripts.v324 import connectors as v324_connectors
from scripts.v324 import marketplace as v324_marketplace
...
class ProjectManager:
    ...
    def create_project(self, project_id: str, name: str):
        ...
        (root/"templates"/"v324").mkdir(parents=True, exist_ok=True)

        init_py = (self.workspace/"scripts"/"v324"/"init_db.py").as_posix()
        ...
        if base_templates.exists():
            for f in base_templates.glob("*.json"):
                shutil_copy(f, root/"templates"/"v324"/f.name)
...
def sync_custom_mappers_file(db_path: str, project_root: pathlib.Path):
    ...
    db.close()
    (project_root/"templates"/"v324").mkdir(parents=True, exist_ok=True)
    (project_root/"templates"/"v324"/"custom_mappers.json").write_text(
...
class App:
    ...
    def require_basic_auth(self, handler: BaseHTTPRequestHandler) -> bool:
        ...
        if not ok:
            ...
            handler.send_header("WWW-Authenticate", 'Basic realm="GENIE_ROI V324"')
    ...
    def refresh_project(self, project_id: str, reason: str = "manual"):
        ...
        gen_py = (self.workspace/"scripts"/"v324"/"generate_dashboard_json.py").as_posix()
        # project-local template defaults
        fx = (proot/"templates"/"v324"/"fx_rates.json").as_posix()
        da = (proot/"templates"/"v324"/"default_attribution.json").as_posix()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if rest_path in ("/login", "/login/"):
            html = (self.server.workspace/"dashboard"/"login_v324.html").read_text(encoding='utf-8')
        ...
        if rest_path in ("/", ""):
            ...
            html = (self.server.workspace/"dashboard"/"projects_v324.html").read_text(encoding='utf-8')
        ...
        if rest_path in ("/ui", "/ui/"):
            ...
            self.serve_file_abs(self.server.workspace/"dashboard"/"ui_v324.html", "text/html; charset=utf-8")
            return
        ...
        if rest_path.startswith("/dashboard/"):
            ...
            rel = rest_path[len("/dashboard/"):] or "index_v324.html"
        ...
        if rest_path.startswith("/api/version"):
            ...
            if not role:
                ...
                if not u: return
                role = self.server.app.role_for(u, pid)
            self._send_json({"version":"V324","project":pid,"project_name":pname,"principal":principal,"role":role,"now":datetime.utcnow().isoformat()+"Z"}); return
        ...
        if rest_path.startswith("/api/projects"):
            ...
            projects, default_id = self.server.pm.list_projects()
            self._send_json({"version":"V324","default":default_id,"projects":projects,"user":u}); return
        ...
        if rest_path.startswith("/api/status"):
            ...
            try:
                ...
                self._send_json({
                    "version":"V324","project":pid,"project_name":pname,
    ...
    def do_POST(self):
        ...
        if rest_path.startswith('/api/connectors/save'):
            ...
            v324_connectors.save_connectors_config(proot, cfg)
        ...
        if rest_path.startswith('/api/products/upsert'):
            ...
            try:
                ...
                self._send_json({'ok':False,'error':'invalid json'}, 400); return
            v324_marketplace.upsert_product(proot, prod)
        ...
        if rest_path.startswith("/api/upload_ads"):
            ...
            return
            ingest_py = (self.server.workspace/"scripts"/"v324"/"ingest_ads.py").as_posix()
            cm = (proot/"templates"/"v324"/"custom_mappers.json").as_posix()
            fx = (proot/"templates"/"v324"/"fx_rates.json").as_posix()
        ...
        if rest_path.startswith("/api/upload_conversions"):
            ...
            self._send_json({'ok':True,'queued':True,'job_id':jid})
            return
            ingest_py = (self.server.workspace/"scripts"/"v324"/"ingest_conversions.py").as_posix()
        ...
        if rest_path.startswith("/api/upload_fx"):
            ...
            try:
                ...
                self._send_json({"ok":False,"error":"invalid json"}, 400); return
            p = proot/"templates"/"v324"/"fx_rates.json"
...
class Worker(threading.Thread):
    """Single worker processing queued jobs (V324)."""
    daemon = True
    def __init__(self, server):
        super().__init__()
        self.server = server
        ...
        while not self.stop_flag:
            ...
            try:
                if jtype == "refresh_dashboard":
                    ...
                    date = payload["date"]
                    out_path = v324_connectors.run_connector(proot, name, date).as_posix()
                    ...
                    out_path = v324_marketplace.export_feed_csv(proot, channel, payload.get("date"))
...
def main():
    ...
    print(f"[GENIE_ROI V324] http://localhost:{args.port}/  (login: admin / admin)")
