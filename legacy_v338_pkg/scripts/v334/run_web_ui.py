...
# -*- coding: utf-8 -*-
"""
GENIE_ROI V334 Web UI (SaaS-feeling + Compliance + Monitoring)
...
from scripts.v334.security import (
    hash_password, verify_password,
    sign_session, verify_session, Session
)
from scripts.v334.ops_store import OpsStore
from scripts.v334.pixel import make_pixel_js
from scripts.v334.connectors_smartstore import SmartStoreOAuth
from scripts.v334 import notifier
from scripts.v334.queue_worker import QueueWorker
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _do_GET(self):
        ...
        # ---- V334 templates (schema / KPIs / connector priority) ----
        if sub == "/api/v334/schema/product":
            schema_path = (_ROOT/"templates"/"v334"/"product_schema_v1.json")
            if not schema_path.exists():
                self._send(404, _json_bytes({"error":"schema not found"}))
                return
            self._send(200, schema_path.read_bytes(), "application/json; charset=utf-8")
            return

        if sub == "/api/v334/templates/connector_priority":
            pth = (_ROOT/"templates"/"v334"/"connector_priority.json")
            if not pth.exists():
                self._send(404, _json_bytes({"error":"connector priority not found"}))
                return
            self._send(200, pth.read_bytes(), "application/json; charset=utf-8")
            return

        if sub == "/api/v334/templates/kpis":
            pth = (_ROOT/"templates"/"v334"/"kpi_templates.json")
            if not pth.exists():
                self._send(404, _json_bytes({"error":"kpi templates not found"}))
                return
            self._send(200, pth.read_bytes(), "application/json; charset=utf-8")
            return

        if sub == "/api/v334/ingest/normalized":
            # Body: {"kind":"orders|reviews|settlements|spend|influencer|products|listings","rows":[...]}
            s = self._session()
            ...
            try:
                if kind == "orders":
                    ok, failed = self.store.ingest_orders_norm(project_id, rows)
                elif kind == "reviews":
                    ok, failed = self.store.ingest_reviews_norm(project_id, rows)
                elif kind == "settlements":
                    ok, failed = self.store.ingest_settlements_norm(project_id, rows)
                elif kind == "spend":
                    ok, failed = self.store.ingest_marketing_spend_norm(project_id, rows)
                elif kind == "influencer":
                    ok, failed = self.store.ingest_influencer_posts_norm(project_id, rows)
                elif kind == "products":
                    for r in rows:
                        self.store.upsert_product_master(project_id, r)
                        ok += 1
                elif kind == "listings":
                    for r in rows:
                        self.store.upsert_channel_listing(project_id, r)
                        ok += 1
                else:
                    self._send(400, _json_bytes({"error":"unknown kind"}))
                    return
            except Exception as e:
                self._send(500, _json_bytes({"error": str(e)}))
                return
            self._send(200, _json_bytes({"ok":True,"kind":kind,"inserted":ok,"failed":failed}))
            return
        ...
        if sub == "/api/smartstore/backfill":
            ...
            job = self.server.queue_worker.enqueue_smartstore_backfill(project_id, b)
            self._send(200, _json_bytes({"ok":True, **job}))
            return


        # V334: id graph split request (analyst+)
        if sub == "/api/id_graph/split_request":
            s = self._session()
            if not require_role(s, "analyst"):
                self._send(403, _json_bytes({"error":"forbidden"}))
                return
            b = self._read_json() or {}
            conflict_key = (b.get("conflict_key") or "").strip()
            conflict_id = b.get("conflict_id")
            if (not conflict_key) and conflict_id is not None:
                cf = self.store.get_conflict_by_index(project_id, int(conflict_id))
                if cf:
                    conflict_key = cf["conflict_key"]
            if not conflict_key:
                self._send(400, _json_bytes({"error":"missing conflict_key or conflict_id"}))
                return
            mode = (b.get("mode") or "virtual").strip()
            note = (b.get("note") or "").strip()
            rid = self.store.create_split_request(project_id, conflict_key, mode=mode, note=note, requested_by=(s.username if s else "unknown"))
            self.store.log_event("INFO","idgraph","SPLIT_REQUEST",f"split request {rid} {conflict_key}", {"project_id": project_id, "mode": mode, "note": note})
            self._send(200, _json_bytes({"ok":True,"request_id": rid, "conflict_key": conflict_key, "mode": mode}))
            return

        # V334: list split requests (manager+)
        if sub == "/api/id_graph/split_requests":
            s = self._session()
            if not require_role(s, "manager"):
                self._send(403, _json_bytes({"error":"forbidden"}))
                return
            ...
            rows = self.store.list_split_requests(project_id, status=status, limit=int(b.get("limit") or 100))
            self._send(200, _json_bytes({"ok":True,"rows": rows}))
            return

        # V334: approve/reject split request (manager+)
        if sub == "/api/id_graph/split_approve":
            s = self._session()
            if not require_role(s, "manager"):
                self._send(403, _json_bytes({"error":"forbidden"}))
                return
            b = self._read_json() or {}
            rid = int(b.get("request_id"))
            approve = bool(b.get("approve", True))
            note = (b.get("note") or "").strip()
            out = self.store.approve_split_request(project_id, rid, approve=approve, approved_by=(s.username if s else "unknown"), note=note)
            self.store.log_event("INFO","idgraph","SPLIT_APPROVAL",f"split approval {rid} -> {out.get('status')}", {"project_id": project_id, "out": out})
            self._send(200, _json_bytes(out))
            return
        ...
        if sub == "/api/id_graph/conflict_action":
            ...
            if action not in ("hold","split","verify","ignore"):
                ...
                return
            result = self.store.apply_conflict_action(project_id, conflict_id, action, note, actor)
...
def main():
    ...
    print(f"[V334] workspace={workspace}  http://{args.host}:{args.port}/dashboard/login_v331.html")
    print("[V334] default users (if first run): admin/admin , analyst/analyst , viewer/viewer")
                    ...
                    for p in proj_root.iterdir():
                        ...
                        if breaches:
                            ...
                            if last_age is None or last_age > cooldown:
                                # V334: attach dashboard/runbook links + recent error summary + optional auto recovery
                                dash_base = os.getenv("GENIE_ROI_DASHBOARD_BASE_URL","").strip()
                                if not dash_base:
                                    dash_base = f"http://localhost:{args.port}"
                                dashboard_url = f"{dash_base}/dashboard/ui_v333.html?project_id={project_id}#monitoring"
                                runbook_url = os.getenv("GENIE_ROI_RUNBOOK_URL","").strip() or f"{dash_base}/docs/v333/V334_RUNBOOK_KO.html"
                                recent_err = store.recent_error_summary(project_id, lookback_minutes=180, limit=5)

                                auto_recover = os.getenv("GENIE_ROI_SLO_AUTO_RECOVER","1").strip() == "1"
                                recover_out = None
                                if auto_recover:
                                    try:
                                        recover_out = queue_worker.run(project_id, max_items=5)
                                    except Exception:
                                        recover_out = None
                                ...
                                notifier.notify("slo_breach", project_id=project_id, title="SLO breach detected",
                                                ...
                                                extra=extra)
                                store.set_last_alert_ts(key, _iso_now())(key, _iso_now())
                                store.log_event("WARN","monitor","SLO_BREACH","Background SLO breach",{"project_id": project_id, "breaches":breaches})
                        # SmartStore lastChangedDate 기반 누락 스캐너 (daily, 09:10 KST)
                        try:
                            import datetime
                            try:
                            ...
                            if now_kst.hour == 9 and now_kst.minute >= 10 and now_kst.minute < 12:
                                day_key = f"smartstore_scan_done::{project_id}::{now_kst.date().isoformat()}"
                                if store.get_config(day_key) != "1":
                                    drop_ratio = float(cfg.get("smartstore_drop_ratio", {}).get("v","0.5"))
                                    min_baseline = int(cfg.get("smartstore_min_baseline", {}).get("v","10"))
                                    queue_worker.smartstore_missing_scan(project_id, drop_ratio=drop_ratio, min_baseline=min_baseline)
                                    store.set_config(day_key, "1")
                        except Exception:
                            pass
            except Exception:
                pass
