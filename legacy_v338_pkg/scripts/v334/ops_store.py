...
class OpsStore:
    def __init__(self, workspace: pathlib.Path):
        ...
        # migration: if v331 db exists and v332 does not, copy forward for seamless upgrade
        v333_path = workspace/"data"/"ops_v333.db"
        if v333_path.exists() and (not self.db_path.exists()):
            try:
                import shutil
                shutil.copy2(v333_path.as_posix(), self.db_path.as_posix())
            except Exception:
                pass
