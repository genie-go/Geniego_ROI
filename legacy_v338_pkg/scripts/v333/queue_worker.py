...
from scripts.v333.connectors_smartstore import SmartStoreClient, iso_utc, parse_any_dt
...
class QueueWorker:
    ...
    def enqueue_smartstore_backfill(self, project_id: str, body: Dict[str,Any]) -> Dict[str,Any]:
        ...
        # V333: track a backfill job for reporting
    ...
    # -------- SmartStore lastChangedDate 기반 누락 스캐너 (V332) --------
    def smartstore_missing_scan(self, project_id: str, *, drop_ratio: float=0.5, min_baseline: int=10) -> Dict[str,Any]:
        ...
        if breach:
            # V333: auto backfill enqueue + quick run (bounded) + report attached to same Slack thread
