...
from scripts.v325.security import (
    hash_password, verify_password, get_secret_key, new_session_token, session_user,
    role_at_least, mask_api_key, hash_api_key,
)

from scripts.v325.permissions import has_perm
from scripts.v325.ops_store import OpsStore
from scripts.v325.org import OrgStore
from scripts.v325.central_templates import CentralTemplates
from scripts.v325.reports import deliver_report
...
from scripts.v325 import connectors as v325_connectors
from scripts.v325 import marketplace as v325_marketplace
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if rest_path in ("/login", "/login/"):
            html = (self.server.workspace/"dashboard"/"login_v325.html").read_text(encoding='utf-8')
        ...
        if rest_path in ("/", ""):
            ...
            html = (self.server.workspace/"dashboard"/"projects_v325.html").read_text(encoding='utf-8')
        ...
        if rest_path in ("/ui", "/ui/"):
            ...
            self.serve_file_abs(self.server.workspace/"dashboard"/"ui_v325.html", "text/html; charset=utf-8")
            return
        ...
        if rest_path.startswith("/dashboard/"):
            ...
            rel = rest_path[len("/dashboard/"):] or "index_v325.html"
        ...
        if rest_path.startswith("/api/version"):
            ...
            if not role:
                ...
                if not u: return
                role = self.server.app.role_for(u, pid)
            self._send_json({"version":"V325","project":pid,"project_name":pname,"principal":principal,"role":role,"now":datetime.utcnow().isoformat()+"Z"}); return
        ...
        if rest_path.startswith("/api/projects"):
            ...
            projects, default_id = self.server.pm.list_projects()
            self._send_json({"version":"V325","default":default_id,"projects":projects,"user":u}); return
        ...
        if rest_path.startswith("/api/status"):
            ...
            try:
                ...
                self._send_json({
                    "version":"V325","project":pid,"project_name":pname,
    ...
    def do_POST(self):
            ...
            if rest_path == "/api/admin/central_templates/deploy":
                ...
                if target.upper()=="ALL":
                    ...
                    for p in plist:
                        ...
                        deployed=self.server.app.central.deploy_to_project(proot, overwrite=True)
                        results[pid]=deployed
                    self.server.ops.audit("central_templates:deploy_all", username=u, project_id=None, resource="ALL", detail={"results":results})
                else:
                    proot=self.server.app.pm.project_root(target)
                    deployed=self.server.app.central.deploy_to_project(proot, overwrite=True)
                    results[target]=deployed
                    self.server.ops.audit("central_templates:deploy", username=u, project_id=target, resource=target, detail={"deployed":deployed})
                self.server.ops._conn().execute("INSERT INTO template_deployments(ts,deployed_by,target,bundle_json,result_json) VALUES (?,?,?,?,?)",
                                                (datetime.utcnow().isoformat()+"Z", u, target, json.dumps(bundle), json.dumps(results)))
                self.server.ops._conn().commit()
        ...
        if rest_path.startswith('/api/connectors/save'):
            ...
            v325_connectors.save_connectors_config(proot, cfg)
        ...
        while not self.stop_flag:
            ...
            try:
                if jtype == "refresh_dashboard":
                    ...
                    date = payload["date"]
                    out_path = v325_connectors.run_connector(proot, name, date).as_posix()
