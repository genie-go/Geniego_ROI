#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""GENIE_ROI V325 Ops Store (audit log, approvals, job queue)

All stored in a single SQLite db under workspace/data/ops_v325.db
Stdlib-only.
...
class OpsStore:
    def __init__(self, workspace: pathlib.Path):
        ...
        self.db_path = (workspace/"data"/"ops_v325.db")
    ...
    def enqueue_job(self, project_id: str, job_type: str, payload: Dict[str,Any]) -> int:
        with self._lock:
            conn=self._conn()
            cur=conn.execute(
                "INSERT INTO job_queue(ts, project_id, job_type, payload_json, status) VALUES (?,?,?,?,?)",
                (datetime.utcnow().isoformat()+"Z", project_id, job_type, json.dumps(payload, ensure_ascii=False), "queued")
            )
    ...
    def fetch_next_job(self) -> Optional[Dict[str,Any]]:
        with self._lock:
            conn=self._conn()
            row=conn.execute(
                "SELECT id, project_id, job_type, payload_json FROM job_queue WHERE status='queued' ORDER BY id ASC LIMIT 1"
            ).fetchone()
            if not row:
                conn.close(); return None
            jid, project_id, job_type, payload_json = row
            conn.execute(
                "UPDATE job_queue SET status='running', started_at=? WHERE id=?",
                (datetime.utcnow().isoformat()+"Z", jid)
            )
            conn.commit()
            conn.close()
        try: payload=json.loads(payload_json or "{}")
