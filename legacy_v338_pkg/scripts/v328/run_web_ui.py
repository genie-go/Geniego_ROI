...
from scripts.v328.security import (
    hash_password, verify_password, get_secret_key, new_session_token, session_user,
    role_at_least, mask_api_key, hash_api_key,
)

from scripts.v328.permissions import has_perm
from scripts.v328.ops_store import OpsStore
from scripts.v328.org import OrgStore
from scripts.v328.central_templates import CentralTemplates
from scripts.v328.reports import deliver_report
from scripts.v328 import connectors as v327_connectors
report
...
from scripts.v328 import connectors as v327_connectors
from scripts.v328 import marketplace as v325_marketplace
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if rest_path in ("/pixel/e", "/pixel/e/"):
            ...
            ops.add_pixel_event(project_id=proj, anonymous_id=anon, event_name=str(ename), event=payload)
            if ename == 'identify':
                ids = pixel_mod.normalize_identity_payload(payload.get('props') or {})
                for k,v in ids.items():
                    ops.add_id_edge(project_id=proj, src_type='anon', src_value=anon, dst_type=k, dst_value=str(v), confidence=1.0)
        ...
        if rest_path.startswith("/api/version"):
            ...
            if not role:
                ...
                if not u: return
                role = self.server.app.role_for(u, pid)
            self._send_json({"version":"V327","project":pid,"project_name":pname,"principal":principal,"role":role,"now":datetime.utcnow().isoformat()+"Z"}); return
...
if rest_path.startswith("/api/projects"):
            ...
            projects, default_id = self.server.pm.list_projects()
            self._send_json({"version":"V327","default":default_id,"projects":projects,"user":u}); return
        ...
        if rest_path.startswith("/api/status"):
            ...
            try:
                ...
                self._send_json({
                    "version":"V327","project":pid,"project_name":pname,
