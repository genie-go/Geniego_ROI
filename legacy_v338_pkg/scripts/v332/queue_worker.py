...
from scripts.v332.connectors_smartstore import SmartStoreClient, iso_utc, parse_any_dt
...
class QueueWorker:
    ...
    def smartstore_missing_scan(self, project_id: str, *, drop_ratio: float=0.5, min_baseline: int=10) -> Dict[str,Any]:
        ...
        if breach:
            self.store.log_event("WARN","connector","SMARTSTORE_DROP","SmartStore order volume drop detected", out)
            notifier.notify("smartstore_missing_suspected", project_id=project_id,
                           title="SmartStore order missing suspected",
                           text=json.dumps(out, ensure_ascii=False, indent=2),
                           extra=out)
        else:
            self.store.log_event("INFO","connector","SMARTSTORE_DROP_CHECK","SmartStore drop check OK", out)
        return out
