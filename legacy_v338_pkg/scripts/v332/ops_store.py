...
class OpsStore:
    def __init__(self, workspace: pathlib.Path):
        ...
        self.db_path = workspace/"data"/"ops_v332.db"
        # migration: if v331 db exists and v332 does not, copy forward for seamless upgrade
        v331_path = workspace/"data"/"ops_v331.db"
        if v331_path.exists() and (not self.db_path.exists()):
            try:
                import shutil
                shutil.copy2(v331_path.as_posix(), self.db_path.as_posix())
            except Exception:
                pass
    ...
    def rebuild_groups(self, project_id: str) -> Dict[str,Any]:
        ...
        # Apply split rules: detach ambiguous email_hash from grouping
        split_email_hashes=set()
        for sr in split_rules:
            ck = sr["conflict_key"]
            if ck.startswith("email_hash:"):
                split_email_hashes.add(ck.split(":",1)[1])
        ...
        for e in edges:
            ...
            if (a[0]=="email_hash" and a[1] in split_email_hashes) or (b[0]=="email_hash" and b[1] in split_email_hashes):
                continue
            ...
            for root, members in groups.items():
                ...
                for cf in conflicts:
                    ...
                    if d.get("email_hash") and ("email_hash", d["email_hash"]) in members:
                        ...
                        if d["email_hash"] in split_email_hashes:
