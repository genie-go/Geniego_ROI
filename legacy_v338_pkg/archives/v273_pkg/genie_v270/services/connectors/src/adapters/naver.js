...
// V163: Expand to AdGroup-level budget/bid updates (scaffold defaults; verify fields per account/product).
const NAVER_TARGET_ADGROUP_IDS = (process.env.NAVER_TARGET_ADGROUP_IDS || "").split(",").map(s=>s.trim()).filter(Boolean);
// Field names are configurable because SearchAd products can vary by type/account.
const NAVER_ADGROUP_BUDGET_FIELD = process.env.NAVER_ADGROUP_BUDGET_FIELD || "dailyBudget";
const NAVER_ADGROUP_BID_FIELD = process.env.NAVER_ADGROUP_BID_FIELD || "defaultBid";
...
function parseBidDeltaPct(actions){
  const a = (actions||[]).find(x => (x||{}).type === "BID_DELTA_PCT");
  const v = a ? Number(a.value) : 0;
  if (!isFinite(v)) return 0;
  return v;
}
...
async function snapshotBeforeChange(execution_id, campaignIds, adgroupIds){
  ...
  // Snapshot adgroups (optional)
  for (const id of (adgroupIds||[])){
    try{
      const g = await naverFetch("GET", `/ncc/adgroups/${encodeURIComponent(id)}`, null);
      adgroups.push({ id, data: g });
    }catch(e){
      adgroups.push({ id, error: String(e.message || e) });
    }
  }
...
async function applyAdGroupUpdate({ adgroupId, budgetDeltaPct, bidDeltaPct, dry_run }){
  const before = await naverFetch("GET", `/ncc/adgroups/${encodeURIComponent(adgroupId)}`, null);
  ...
  // budget
  if (budgetDeltaPct !== 0){
    const cur = Number(before?.[NAVER_ADGROUP_BUDGET_FIELD]);
    if (isFinite(cur)){
      const next = clampBudgetKrw(cur * (1 + budgetDeltaPct/100));
      if (next !== clampBudgetKrw(cur)){
        patch[NAVER_ADGROUP_BUDGET_FIELD] = next;
        fields.push(NAVER_ADGROUP_BUDGET_FIELD);
      }
    }
  }
  ...
  // bid
  if (bidDeltaPct !== 0){
    const curBid = Number(before?.[NAVER_ADGROUP_BID_FIELD]);
    if (isFinite(curBid)){
      const nextBid = clampBudgetKrw(curBid * (1 + bidDeltaPct/100)); // keep integer KRW
      if (nextBid !== clampBudgetKrw(curBid)){
        patch[NAVER_ADGROUP_BID_FIELD] = nextBid;
        fields.push(NAVER_ADGROUP_BID_FIELD);
      }
    }
  }

  if (fields.length === 0){
    return { adgroupId, ok:true, skipped:true, reason:"no_change_or_missing_fields", before };
  }

  if (dry_run){
    return { adgroupId, ok:true, dry_run:true, before, patch, fields };
  }

  const uri = `/ncc/adgroups/${encodeURIComponent(adgroupId)}?fields=${encodeURIComponent(fields.join(","))}`;
  const updated = await naverFetch("PUT", uri, patch);
  return { adgroupId, ok:true, updated, fields, patch };
}

async function rollbackAdGroup({ adgroupId, beforeData, dry_run }){
  if (!beforeData) return { adgroupId, ok:false, error:"missing_snapshot" };
  const patch = {};
  const fields = [];
  if (beforeData[NAVER_ADGROUP_BUDGET_FIELD] !== undefined){
    patch[NAVER_ADGROUP_BUDGET_FIELD] = beforeData[NAVER_ADGROUP_BUDGET_FIELD];
    fields.push(NAVER_ADGROUP_BUDGET_FIELD);
  }
  if (beforeData[NAVER_ADGROUP_BID_FIELD] !== undefined){
    patch[NAVER_ADGROUP_BID_FIELD] = beforeData[NAVER_ADGROUP_BID_FIELD];
    fields.push(NAVER_ADGROUP_BID_FIELD);
  }
  if (fields.length === 0){
    return { adgroupId, ok:true, skipped:true, reason:"nothing_to_restore" };
  }
  if (dry_run){
    return { adgroupId, ok:true, dry_run:true, restore: patch };
  }
  const uri = `/ncc/adgroups/${encodeURIComponent(adgroupId)}?fields=${encodeURIComponent(fields.join(","))}`;
  const updated = await naverFetch("PUT", uri, patch);
  return { adgroupId, ok:true, updated, restore: patch };
}
...
async function execute({execution_id, channel, actions, dry_run, cap, targets}) {
  ...
  const targetCampaignIds = (targets && targets.campaign_ids && targets.campaign_ids.length) ? targets.campaign_ids : NAVER_TARGET_CAMPAIGN_IDS;
  const targetAdGroupIds  = (targets && targets.adgroup_ids  && targets.adgroup_ids.length)  ? targets.adgroup_ids  : NAVER_TARGET_ADGROUP_IDS;
  ...
  const bidDeltaPct = parseBidDeltaPct(actions);
  const snap = await snapshotBeforeChange(execution_id, targetCampaignIds, targetAdGroupIds);
...
async function rollback({execution_id, channel, dry_run, targets}) {
  ...
  const adgroups = Array.isArray(snap?.adgroups) ? snap.adgroups : [];
  for (const g of adgroups){
    const gid = g?.id;
    const beforeData = g?.data;
    if (!gid || !beforeData){
      adgroupResults.push({ adgroupId: gid || "unknown", ok:false, error:"missing_original_adgroup_in_snapshot" });
      continue;
    }
    try{
      const r = await rollbackAdGroup({ adgroupId: gid, beforeData, dry_run: useDry });
      adgroupResults.push(r);
    }catch(e){
      adgroupResults.push({ adgroupId: gid, ok:false, error:String(e.message || e) });
    }
  }

  return {
    execution_id, channel, dry_run: useDry,
    ok: results.every(r=>r.ok) && adgroupResults.every(r=>r.ok),
    snapshot_path: sp,
    snapshot: snap,
    summary: { restored_campaigns: results.filter(r=>r.ok).length, failed_campaigns: results.filter(r=>!r.ok).length, restored_adgroups: adgroupResults.filter(r=>r.ok).length, failed_adgroups: adgroupResults.filter(r=>!r.ok).length },
    campaign_results: results,
    adgroup_results: adgroupResults
  };
}
