# GENIE_ROI AdOps OS — V270 (Ops-grade platform w/ no-code policy DSL + key rotation automation)

Generated: 2026-02-25

## What's new in V270 (incremental over V269 and all prior versions)

### 1) "정말 마지막 마감재": API Key 회전/만료 정책의 **스케줄 기반 자동화**
- 테넌트별 `rotate_days/grace_days/mode(ALERT|ENFORCE)`를 DB에 저장하고, 게이트웨이 백그라운드 워커가 주기적으로 점검합니다.
- **ALERT**: 회전 필요/만료 임박/회전 overdue를 `incidents`에 기록(운영 알림·감사 근거)
- **ENFORCE**: `rotate_days + grace_days`를 넘긴 키는 자동 `revoked_at` 처리(강제 만료)
- Admin UI에서 정책 설정 및 "due keys" 확인 가능: `/v270/admin`

> 보수적 설계: 자동으로 새 키를 생성/배포하지 않습니다(보안 사고 방지). 운영자가 UI/API로 새 키를 발급한 뒤 교체합니다.

### 2) Google/Amazon 룰을 **정책 DSL로 UI에서 정의(코드 수정 없이)**
- 테넌트 정책(`tenant_policies.policy`)에 `connector_dsl`을 저장합니다.
- V270부터 `connector_dsl`이 포함된 정책은 게이트웨이에서 최소 스키마 검증을 수행합니다(버전/필드 타입/주요 제약).

예시:
```json
{
  "allowed_channels": ["google_ads","amazon_ads"],
  "connector_dsl": {
    "version": 1,
    "providers": {
      "google_ads": {
        "allow": ["campaign.update_budget"],
        "deny": ["account.delete"],
        "constraints": {"budget_delta_pct_max": 15}
      },
      "amazon_ads": {
        "allow": ["campaign.update_budget"],
        "deny": [],
        "constraints": {"budget_delta_pct_max": 10}
      }
    }
  }
}
