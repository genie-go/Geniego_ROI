...
r.POST("/v1/marketplace/publisher_members", auth.RequireRole("admin"), func(c *gin.Context) {
  ...
  // V301: channel credentials (stored per-tenant)
  r.POST("/v1/commerce/channel/credentials", auth.RequireRole("admin"), func(c *gin.Context) {
    tenant := tenantOf(c)
    var body struct{ Channel string `json:"channel"`; Creds map[string]any `json:"creds"` }
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
    if body.Channel=="" { c.JSON(400, gin.H{"error":"channel required"}); return }
    if body.Creds==nil { body.Creds = map[string]any{} }
    if err := st.CommerceSetChannelCreds(c, tenant, body.Channel, body.Creds); err != nil {
  ...
  r.POST("/v1/commerce/products/upload", auth.RequirePermission("commerce:write"), func(c *gin.Context) {
    ...
    if err != nil { c.JSON(500, gin.H{"error":err.Error()}); return }
    jobID, err := st.CommerceEnqueueJob(c, tenant, body.Channel, "products", map[string]any{"run_id":runID, "products": body.Products})
    if err != nil { c.JSON(500, gin.H{"error":err.Error()}); return }
    c.JSON(200, gin.H{"run_id":runID, "job_id":jobID, "status":"queued"})
  })
  ...
  r.POST("/v1/commerce/prices/update", auth.RequirePermission("commerce:write"), func(c *gin.Context) {
    ...
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
    runID, err := st.CommerceStartSync(c, tenant, body.Channel, "prices")
    if err != nil { c.JSON(500, gin.H{"error":err.Error()}); return }
    jobID, err := st.CommerceEnqueueJob(c, tenant, body.Channel, "prices", map[string]any{"run_id":runID, "prices": body.Prices})
    if err != nil { c.JSON(500, gin.H{"error":err.Error()}); return }
    c.JSON(200, gin.H{"run_id":runID, "job_id":jobID, "status":"queued"})
  })
  ...
  r.POST("/v1/commerce/orders/sync", auth.RequirePermission("commerce:read"), func(c *gin.Context) {
    ...
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
    runID, err := st.CommerceStartSync(c, tenant, body.Channel, "orders")
    if err != nil { c.JSON(500, gin.H{"error":err.Error()}); return }
    jobID, err := st.CommerceEnqueueJob(c, tenant, body.Channel, "orders", map[string]any{"run_id":runID})
    if err != nil { c.JSON(500, gin.H{"error":err.Error()}); return }
    c.JSON(200, gin.H{"run_id":runID, "job_id":jobID, "status":"queued"})
  })
  ...
  r.POST("/v1/commerce/inventory/sync", auth.RequirePermission("commerce:write"), func(c *gin.Context) {
    ...
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
    runID, err := st.CommerceStartSync(c, tenant, body.Channel, "inventory")
    if err != nil { c.JSON(500, gin.H{"error":err.Error()}); return }
    jobID, err := st.CommerceEnqueueJob(c, tenant, body.Channel, "inventory", map[string]any{"run_id":runID, "inventory": body.Inventory})
    if err != nil { c.JSON(500, gin.H{"error":err.Error()}); return }
    c.JSON(200, gin.H{"run_id":runID, "job_id":jobID, "status":"queued"})
  })

r.GET("/v1/commerce/jobs/:job_id", auth.RequirePermission("commerce:read"), func(c *gin.Context) {
  tenant := tenantOf(c)
  jobID := c.Param("job_id")
  out, err := st.CommerceGetJobSummary(c, tenant, jobID)
  if err != nil { c.JSON(404, gin.H{"error": "not found"}); return }
  c.JSON(200, out)
})

r.GET("/v1/commerce/jobs/:job_id/items", auth.RequirePermission("commerce:read"), func(c *gin.Context) {
  tenant := tenantOf(c)
  jobID := c.Param("job_id")
  items, err := st.CommerceListJobItems(c, tenant, jobID, 500)
  if err != nil { c.JSON(500, gin.H{"error": err.Error()}); return }
  c.JSON(200, gin.H{"job_id": jobID, "items": items})
})
