...
// V299+ Enterprise admin: audit logs, roles/permissions (RBAC), SSO scaffold, billing/plans.
r.GET("/v1/audit/logs", auth.RequirePermission("audit:read"), func(c *gin.Context) {
  limitStr := c.Query("limit")
  limit := 100
  if limitStr != "" {
    if v, err := strconv.Atoi(limitStr); err == nil { limit = v }
  }
  logs, err := st.ListAuditLogs(c, tenantOf(c), limit)
  if err != nil { c.JSON(500, gin.H{"error":"audit"}); return }
  c.JSON(200, gin.H{"items": logs})
})

r.GET("/v1/billing/plans", auth.RequirePermission("billing:read"), func(c *gin.Context) {
  plans, err := st.ListPlans(c)
  if err != nil { c.JSON(500, gin.H{"error":"plans"}); return }
  c.JSON(200, gin.H{"plans": plans})
})

r.POST("/v1/billing/plans", auth.RequirePermission("billing:write"), func(c *gin.Context) {
  var p store.BillingPlan
  if err := c.BindJSON(&p); err != nil { c.JSON(400, gin.H{"error": err.Error()}); return }
  if p.PlanID == "" { p.PlanID = uuid.NewString() }
  if err := st.UpsertPlan(c, p); err != nil { c.JSON(500, gin.H{"error":"upsert_plan"}); return }
  c.JSON(200, gin.H{"ok": true, "plan_id": p.PlanID})
})

r.POST("/v1/billing/subscription", auth.RequirePermission("billing:write"), func(c *gin.Context) {
  var body struct { PlanID string `json:"plan_id"` }
  if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error": err.Error()}); return }
  if body.PlanID == "" { c.JSON(400, gin.H{"error":"plan_id required"}); return }
  if err := st.SetSubscription(c, tenantOf(c), body.PlanID); err != nil { c.JSON(500, gin.H{"error":"subscription"}); return }
  c.JSON(200, gin.H{"ok": true})
})

// SSO endpoints are scaffolding: store config and provide placeholders for OIDC flow.
r.GET("/v1/admin/sso/oidc", auth.RequirePermission("admin:*"), func(c *gin.Context) {
  cfgRow, err := st.GetOIDCConfig(c, tenantOf(c))
  if err != nil { c.JSON(404, gin.H{"error":"not_configured"}); return }
  c.JSON(200, cfgRow)
})

r.POST("/v1/admin/sso/oidc", auth.RequirePermission("admin:*"), func(c *gin.Context) {
  var body store.OIDCConfig
  if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error": err.Error()}); return }
  body.TenantID = tenantOf(c)
  if err := st.UpsertOIDCConfig(c, body); err != nil { c.JSON(500, gin.H{"error":"upsert_oidc"}); return }
  c.JSON(200, gin.H{"ok": true})
})
...
r.GET("/v1/roi/drilldown", auth.RequireRole("analyst","marketer","admin"), func(c *gin.Context) {
  from := c.Query("from")
  to := c.Query("to")
  if from=="" || to=="" { c.JSON(400, gin.H{"error":"from & to required (YYYY-MM-DD)"}); return }
  res, err := st.GetROIDrilldown(c, tenantOf(c), from, to)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, res)
})


// --- V294 Analytics drilldowns ---
r.GET("/v1/roi/analytics/cohort", auth.RequireRole("analyst","marketer","admin"), func(c *gin.Context) {
  from := c.Query("from")
  to := c.Query("to")
  cohortEvent := c.DefaultQuery("cohort_event","signup")
  convEvent := c.DefaultQuery("conversion_event","purchase")
  windowDays, _ := strconv.Atoi(c.DefaultQuery("window_days","30"))
  if from=="" || to=="" { c.JSON(400, gin.H{"error":"from & to required (YYYY-MM-DD)"}); return }
  rows, err := st.GetCohortReport(c, tenantOf(c), from, to, cohortEvent, convEvent, windowDays)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

r.GET("/v1/roi/analytics/funnel", auth.RequireRole("analyst","marketer","admin"), func(c *gin.Context) {
  from := c.Query("from")
  to := c.Query("to")
  if from=="" || to=="" { c.JSON(400, gin.H{"error":"from & to required (YYYY-MM-DD)"}); return }
  steps := c.QueryArray("step")
  ordered := c.DefaultQuery("ordered","0")
  var rows any
  var err error
  if ordered=="1" || strings.ToLower(ordered)=="true" {
    rows, err = st.GetOrderedFunnelReport(c, tenantOf(c), from, to, steps)
  } else {
    rows, err = st.GetFunnelReport(c, tenantOf(c), from, to, steps)
  }
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

r.GET("/v1/roi/analytics/segments", auth.RequireRole("analyst","marketer","admin"), func(c *gin.Context) {
  from := c.Query("from")
  to := c.Query("to")
  purchaseEvent := c.DefaultQuery("purchase_event","purchase")
  if from=="" || to=="" { c.JSON(400, gin.H{"error":"from & to required (YYYY-MM-DD)"}); return }
  rows, err := st.GetSegmentDrilldown(c, tenantOf(c), from, to, purchaseEvent)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

// --- V294 CI-based deal recommendation ---
r.GET("/v1/deals/recommend_ci", auth.RequireRole("analyst","marketer","admin"), func(c *gin.Context) {
  productId := c.Query("product_id")
  from := c.Query("from")
  to := c.Query("to")
  if productId=="" || from=="" || to=="" { c.JSON(400, gin.H{"error":"product_id, from, to required"}); return }
  rows, err := st.GetDealRecommendationsCI(c, tenantOf(c), productId, from, to)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})


r.GET("/v1/deals/recommend_bayes", auth.RequireRole("analyst","marketer","admin"), func(c *gin.Context) {
  productId := c.Query("product_id")
  from := c.Query("from")
  to := c.Query("to")
  if productId=="" || from=="" || to=="" { c.JSON(400, gin.H{"error":"product_id, from, to required"}); return }
...
mdeF := 0.0
roiMinF := 0.0
if q := c.Query("alpha"); q != "" {
  if v, e := strconv.ParseFloat(q, 64); e == nil { alphaF = v }
}
if q := c.Query("power"); q != "" {
  if v, e := strconv.ParseFloat(q, 64); e == nil { powerF = v }
}
if q := c.Query("mde"); q != "" {
  if v, e := strconv.ParseFloat(q, 64); e == nil { mdeF = v }
}
if q := c.Query("roi_min"); q != "" {
  if v, e := strconv.ParseFloat(q, 64); e == nil { roiMinF = v }
}
rows, err := st.GetDealRecommendationsBayes(c, tenantOf(c), productId, from, to, alphaF, powerF, mdeF, roiMinF)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

// --- V294 Marketplace ---
type UpsertCatalogReq struct {
  ID string `json:"id"`
  SKU string `json:"sku"`
  Name string `json:"name"`
...
r.GET("/v1/marketplace/catalog", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
  rows, err := st.ListMarketplaceCatalog(c, tenantOf(c))
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

r.POST("/v1/marketplace/catalog", auth.RequireRole("admin","marketer"), func(c *gin.Context) {
  var req UpsertCatalogReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.ID=="" { req.ID = uuid.New().String() }
  if req.SKU=="" || req.Name=="" { c.JSON(400, gin.H{"error":"sku & name required"}); return }
  if req.Currency=="" { req.Currency="USD" }
  if req.ListPrice=="" { req.ListPrice="0" }
  if err := st.UpsertMarketplaceCatalogItem(c, tenantOf(c), req.ID, req.SKU, req.Name, req.Description, req.Category, req.Currency, req.ListPrice, req.IsActive); err != nil {
    c.JSON(500, gin.H{"error":"db"}); return
  }
  c.JSON(200, gin.H{"ok":true, "id":req.ID})
})

type UpsertOfferReq struct {
  ID string `json:"id"`
...
r.GET("/v1/marketplace/offers", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
  rows, err := st.ListMarketplaceOffers(c, tenantOf(c))
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

r.POST("/v1/marketplace/offers", auth.RequireRole("admin","marketer"), func(c *gin.Context) {
  var req UpsertOfferReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.ID=="" { req.ID = uuid.New().String() }
  if req.OfferCode=="" || req.Name=="" { c.JSON(400, gin.H{"error":"offer_code & name required"}); return }
  if req.Type=="" { req.Type="discount" }
  if req.Status=="" { req.Status="draft" }
  if err := st.UpsertMarketplaceOffer(c, tenantOf(c), req.ID, req.OfferCode, req.Name, req.Type, req.RulesJSON, req.StartAt, req.EndAt, req.Status); err != nil {
    c.JSON(500, gin.H{"error":"db"}); return
  }
  c.JSON(200, gin.H{"ok":true, "id":req.ID})
})

type UpsertSettlementReq struct {
  ID string `json:"id"`
...
r.GET("/v1/marketplace/settlements", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
  rows, err := st.ListMarketplaceSettlements(c, tenantOf(c))
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

r.POST("/v1/marketplace/settlements", auth.RequireRole("admin"), func(c *gin.Context) {
  var req UpsertSettlementReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.ID=="" { req.ID = uuid.New().String() }
  if req.SettlementDate=="" || req.PartnerID=="" { c.JSON(400, gin.H{"error":"settlement_date & partner_id required"}); return }
  if req.PartnerType=="" { req.PartnerType="influencer" }
  if req.Currency=="" { req.Currency="USD" }
  if req.GrossRevenue=="" { req.GrossRevenue="0" }
  if req.Fees=="" { req.Fees="0" }
  if req.Payout=="" { req.Payout="0" }
  if req.Status=="" { req.Status="open" }
  if err := st.UpsertMarketplaceSettlement(c, tenantOf(c), req.ID, req.SettlementDate, req.PartnerType, req.PartnerID, req.Currency, req.GrossRevenue, req.Fees, req.Payout, req.Status, req.Metadata); err != nil {
    c.JSON(500, gin.H{"error":"db"}); return
  }
  c.JSON(200, gin.H{"ok":true, "id":req.ID})
})
...
rep, err := st.GetExperimentReport(c, tenantOf(c), expID, holdoutPct, alphaF, powerF, mdeF)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rep)
})
  ...
  r.POST("/v1/connectors/accounts", func(c *gin.Context) {
    ...
    if len(body.Auth) == 0 { body.Auth = []byte(`{}`) }
    if len(body.Token) == 0 { body.Token = []byte(`{}`) }
    var expPtr *time.Time = nil
    if body.TokenExpiresAt != "" {
      if t0, err := time.Parse(time.RFC3339, body.TokenExpiresAt); err == nil { expPtr = &t0 }
    }
    if body.Status == "" { body.Status = "ACTIVE" }
    if err := st.UpsertConnectorAccount(c, tenantOf(c), strings.ToLower(body.Provider), body.AccountID, []byte(body.Config), []byte(body.Auth), []byte(body.Token), expPtr, body.Status); err != nil {
      c.JSON(500, gin.H{"error":"db"}); return
    }
    c.JSON(200, gin.H{"ok": true})
  })
  ...
  r.GET("/v1/connectors/accounts", func(c *gin.Context) {
    ...
    for _, r0 := range rows {
      out = append(out, gin.H{
        ...
        "config_json": json.RawMessage(r0.ConfigJSON),
        "auth_json": json.RawMessage(r0.AuthJSON),
        "token_json": json.RawMessage(r0.TokenJSON),
        "token_expires_at": r0.TokenExpiresAt,
        "last_sync_at": r0.LastSyncAt, "updated_at": r0.UpdatedAt,
      })

  // --- V290: connector token updates (used by LIVE collectors) ---
  r.POST("/v1/connectors/accounts/token", func(c *gin.Context) {
    var body struct{
      Provider string `json:"provider"`
      AccountID string `json:"account_id"`
      Token json.RawMessage `json:"token_json"`
      TokenExpiresAt string `json:"token_expires_at"`
    }
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error": err.Error()}); return }
    if body.Provider=="" || body.AccountID=="" { c.JSON(400, gin.H{"error":"provider/account_id required"}); return }
    ...
    if err := st.UpdateConnectorAccountToken(c, tenantOf(c), strings.ToLower(body.Provider), body.AccountID, []byte(body.Token), expPtr); err != nil {
      c.JSON(500, gin.H{"error":"db"}); return
    }
    c.JSON(200, gin.H{"ok": true})
  })

  r.POST("/v1/connectors/accounts/sync_mark", func(c *gin.Context) {
    var body struct{ Provider string `json:"provider"`; AccountID string `json:"account_id"` }
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error": err.Error()}); return }
    if body.Provider=="" || body.AccountID=="" { c.JSON(400, gin.H{"error":"provider/account_id required"}); return }
    if err := st.MarkConnectorAccountSync(c, tenantOf(c), strings.ToLower(body.Provider), body.AccountID); err != nil {
      c.JSON(500, gin.H{"error":"db"}); return
    }
    c.JSON(200, gin.H{"ok": true})
  })
...
r.POST("/v1/influencers", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
  ...
  if body.ContentTypes == nil { body.ContentTypes = []any{} }
  if body.Channels == nil { body.Channels = []any{} }
  if body.Audience == nil { body.Audience = map[string]any{} }
  id, err := st.CreateInfluencer(c, tenantOf(c), body.Name, body.Handle, body.Categories, body.ContentTypes, body.Channels, body.Audience, body.Notes)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, gin.H{"id": id})
})
...
r.GET("/v1/influencers", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
  ...
  out := []gin.H{}
  for _, r0 := range rows {
    var cats any; var cts any; var chs any; var aud any
    _ = json.Unmarshal(r0.Categories, &cats)
    _ = json.Unmarshal(r0.ContentTypes, &cts)
    _ = json.Unmarshal(r0.Channels, &chs)
    _ = json.Unmarshal(r0.AudienceJSON, &aud)
    out = append(out, gin.H{"id": r0.ID, "name": r0.Name, "handle": r0.Handle, "categories": cats, "content_types": cts, "channels": chs, "audience": aud, "notes": r0.Notes, "created_at": r0.CreatedAt})
  }
...
r.GET("/v1/products", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
  ...
  out := []gin.H{}
  for _, r0 := range rows {
    var cats any; var attrs any; var cts any; var pref any
    _ = json.Unmarshal(r0.Categories, &cats)
    _ = json.Unmarshal(r0.AttributesJSON, &attrs)
    _ = json.Unmarshal(r0.ContentTypes, &cts)
    _ = json.Unmarshal(r0.PreferredChannels, &pref)
    out = append(out, gin.H{"id": r0.ID, "sku": r0.SKU, "name": r0.Name, "categories": cats, "attributes": attrs, "content_types": cts, "preferred_channels": pref, "price": r0.Price, "margin": r0.Margin, "created_at": r0.CreatedAt})
  }
  c.JSON(200, gin.H{"items": out})
})
...
r.POST("/v1/channels/traits", auth.RequireRole("admin","marketer"), func(c *gin.Context) {
  var body struct{
    Channel string `json:"channel"`
    Traits any `json:"traits"`
  }
  if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"bad_request"}); return }
  if body.Channel == "" { c.JSON(400, gin.H{"error":"channel_required"}); return }
  if body.Traits == nil { body.Traits = map[string]any{} }
  if err := st.UpsertChannelTraits(c, tenantOf(c), body.Channel, body.Traits); err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, gin.H{"ok": true})
})
r.GET("/v1/channels/traits/:channel", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
  ch := c.Param("channel")
  row, err := st.GetChannelTraits(c, tenantOf(c), ch)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  if row == nil { c.JSON(404, gin.H{"error":"not_found"}); return }
  var traits any
  _ = json.Unmarshal(row.TraitsJSON, &traits)
  c.JSON(200, gin.H{"channel": row.Channel, "traits": traits, "updated_at": row.UpdatedAt})
})

r.POST("/v1/providers/rate_limits", auth.RequireRole("admin"), func(c *gin.Context) {
  var body struct{
    Provider string `json:"provider"`
    Limits any `json:"limits"`
  }
  if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"bad_request"}); return }
  if body.Provider == "" { c.JSON(400, gin.H{"error":"provider_required"}); return }
  if body.Limits == nil { body.Limits = map[string]any{} }
  if err := st.UpsertProviderRateLimits(c, tenantOf(c), body.Provider, body.Limits); err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, gin.H{"ok": true})
})
r.GET("/v1/providers/rate_limits/:provider", auth.RequireRole("admin","analyst"), func(c *gin.Context) {
  p := c.Param("provider")
  row, err := st.GetProviderRateLimits(c, tenantOf(c), p)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  if row == nil { c.JSON(404, gin.H{"error":"not_found"}); return }
  var limits any
  _ = json.Unmarshal(row.LimitsJSON, &limits)
  c.JSON(200, gin.H{"provider": row.Provider, "limits": limits, "updated_at": row.UpdatedAt})
})
...
r.POST("/v1/influencers/products/stats/ingest", auth.RequireRole("admin","analyst"), func(c *gin.Context) {
  ...
  // --- V290: Influencer pricing + deal estimation ---
  r.POST("/v1/influencers/rate_cards", auth.RequireRole("admin","marketer"), func(c *gin.Context) {
    var body struct{
      ...
      RevShare float64 `json:"rev_share"`
      Meta json.RawMessage `json:"meta_json"`
    }
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error": err.Error()}); return }
    if body.InfluencerID=="" || body.Channel=="" || body.ContentType=="" { c.JSON(400, gin.H{"error":"influencer_id/channel/content_type required"}); return }
    if len(body.Meta)==0 { body.Meta = []byte(`{}`) }
    if err := st.UpsertInfluencerRateCard(c, tenantOf(c), body.InfluencerID, strings.ToLower(body.Channel), strings.ToLower(body.ContentType), body.BaseFee, body.CPM, body.CPC, body.RevShare, []byte(body.Meta)); err != nil {
      c.JSON(500, gin.H{"error":"db"}); return
    }
    c.JSON(200, gin.H{"ok": true})
  })

  r.POST("/v1/influencers/deals/estimate", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
    var body struct{
      InfluencerID string `json:"influencer_id"`
      ...
      ExperimentID *string `json:"experiment_id"`
      UseIncremental *bool `json:"use_incremental"` // if true, fee bounds are based on incremental (lift) profit
    }
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error": err.Error()}); return }
    if body.InfluencerID=="" || body.ProductID=="" || body.Channel=="" || body.ContentType=="" { c.JSON(400, gin.H{"error":"influencer_id/product_id/channel/content_type required"}); return }
    if body.HorizonDays <= 0 { body.HorizonDays = 14 }

    // historical window: last 30 days
    to := time.Now()
    from := to.AddDate(0,0,-30)
    agg, err := st.AggInfluencerProductStats(c, tenantOf(c), body.InfluencerID, body.ProductID, strings.ToLower(body.Channel), from, to)
    if err != nil { c.JSON(500, gin.H{"error":"db"}); return }

    prod, err := st.GetProductByID(c, tenantOf(c), body.ProductID)
    if err != nil { c.JSON(500, gin.H{"error":"db"}); return }

    // parse price/margin (stored as text)
    price := 0.0
    margin := 0.3
    if prod.Price != nil {
      if v, err := strconv.ParseFloat(*prod.Price, 64); err == nil { price = v }
    }
    if prod.Margin != nil {
      if v, err := strconv.ParseFloat(*prod.Margin, 64); err == nil {
        // margin may be percent (0-1) or (0-100)
        if v > 1.0 { v = v / 100.0 }
        margin = v
      }
    }

    // derive baseline KPIs from history; fallback to conservative priors if no data
    impr := float64(agg.Impressions)
    clicks := float64(agg.Clicks)
    conv := float64(agg.Conversions)
    rev := float64(agg.Revenue)

    ctr := 0.012
    cvr := 0.02
    aov := price
    if impr > 0 { ctr = clicks / impr }
    if clicks > 0 { cvr = conv / clicks }
    if conv > 0 && rev > 0 { aov = rev / conv }
    if aov <= 0 { aov = price; if aov <= 0 { aov = 25.0 } }

    // expected volume scaling: use horizon_days and historical daily averages
    hist_days := 30.0
    exp_impr := impr / hist_days * float64(body.HorizonDays)
    exp_clicks := exp_impr * ctr
    exp_conv := exp_clicks * cvr
    exp_rev := exp_conv * aov

    gross_profit := exp_rev * margin


// Optional: incorporate incrementality (Lift) from experiments into deal economics
lift_pct := 0.0
lift_revenue := 0.0
base_scaled := 0.0
if body.ExperimentID != nil && *body.ExperimentID != "" {
  holdoutPct, _, err := st.GetExperiment(c, tenantOf(c), *body.ExperimentID)
  if err == nil {
    rep, err2 := st.GetExperimentReport(c, tenantOf(c), *body.ExperimentID, holdoutPct, 0, 0, 0)
    if err2 == nil {
      // scaled holdout revenue to treatment size (best-effort)
      treat_units := float64(rep.Treatment.Units)
      hold_units := float64(rep.Holdout.Units)
      hold_rev := float64(rep.Holdout.Revenue)
      if hold_units > 0 {
        base_scaled = hold_rev * (treat_units / hold_units)
      }
      lift_revenue = rep.LiftRevenue
      if base_scaled > 0 {
        lift_pct = lift_revenue / base_scaled
      } else if rep.Treatment.Revenue > 0 {
        lift_pct = lift_revenue / float64(rep.Treatment.Revenue)
      }
    }
  }
}

// incremental view (use lift_pct)
incremental_rev := exp_rev * lift_pct
incremental_profit := incremental_rev * margin


    // pricing using rate card if available
    fee_by_card := 0.0
    if rc, err := st.GetInfluencerRateCard(c, tenantOf(c), body.InfluencerID, strings.ToLower(body.Channel), strings.ToLower(body.ContentType)); err == nil {
      fee_by_card = rc.BaseFee + rc.CPM*(exp_impr/1000.0) + rc.CPC*exp_clicks
      if rc.RevShare > 0 {
        rs := exp_rev * rc.RevShare
        if rs > fee_by_card { fee_by_card = rs }
      }
    }

    // recommend fee range: ensure Profit ROI (profit / fee) >= target
    target_profit_roi := 1.5

    // choose profit basis: total profit by default; optional incremental profit if requested and lift is known
    profit_basis := gross_profit
    if body.UseIncremental != nil && *body.UseIncremental && lift_pct > 0 {
      profit_basis = incremental_profit
    }

    max_fee := 0.0
    if profit_basis > 0 { max_fee = profit_basis / target_profit_roi }
    min_fee := max_fee * 0.6

    // If rate card suggests higher, keep it as floor
    if fee_by_card > 0 && fee_by_card > min_fee { min_fee = fee_by_card }
    if fee_by_card > 0 && fee_by_card > max_fee { max_fee = fee_by_card * 1.15 }

    proposed := -1.0
    if body.ProposedFee != nil { proposed = *body.ProposedFee }
    expected_roas := 0.0
    expected_profit_roi := 0.0
    expected_incremental_profit_roi := 0.0
    if proposed > 0 {
      expected_roas = exp_rev / proposed
      expected_profit_roi = gross_profit / proposed
      // incremental profit ROI (if lift known)
      if lift_pct > 0 { expected_incremental_profit_roi = incremental_profit / proposed }
    }

    c.JSON(200, gin.H{
      "influencer_id": body.InfluencerID,
      "product_id": body.ProductID,
      "channel": strings.ToLower(body.Channel),
      "content_type": strings.ToLower(body.ContentType),
      "horizon_days": body.HorizonDays,
      "expected": gin.H{
        "impressions": int64(exp_impr),
        "clicks": int64(exp_clicks),
        "conversions": int64(exp_conv),
        "revenue": exp_rev,
        "gross_profit": gross_profit,
        "lift_pct": lift_pct,
        "lift_revenue": lift_revenue,
        "incremental_revenue": incremental_rev,
        "incremental_profit": incremental_profit,
        "ctr": ctr, "cvr": cvr, "aov": aov, "margin": margin,
      },
      "pricing": gin.H{
        "rate_card_fee_hint": fee_by_card,
        "recommended_fee_min": min_fee,
        "recommended_fee_max": max_fee,
        "profit_basis": func() string { if body.UseIncremental!=nil && *body.UseIncremental && lift_pct>0 { return "incremental" }; return "total" }(),
        "proposed_fee": proposed,
        "expected_roas": expected_roas,
        "expected_profit_roi": expected_profit_roi,
        "expected_incremental_profit_roi": expected_incremental_profit_roi,
      },
      "notes": "Estimates use last-30d influencer_product_stats priors; refine with channel traits + content performance once available.",
    })
  })
// --- V295 Marketplace Apps (Appstore workflows) ---
...
r.GET("/v1/marketplace/apps", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
  rows, err := st.ListMarketplaceApps(c, tenantOf(c))
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

r.POST("/v1/marketplace/apps", auth.RequireRole("admin"), func(c *gin.Context) {
  var req UpsertAppReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.AppKey=="" || req.Name=="" { c.JSON(400, gin.H{"error":"app_key & name required"}); return }
  if req.Status=="" { req.Status = "listed" }
  err := st.UpsertMarketplaceApp(c, tenantOf(c), store.MarketplaceApp{
    ID: req.ID, AppKey: req.AppKey, Name: req.Name, Publisher: req.Publisher,
    Description: req.Description, Scopes: req.Scopes, InstallURL: req.InstallURL, Status: req.Status,
  })
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, gin.H{"ok":true})
})

type UpsertInstallReq struct {
...
r.GET("/v1/marketplace/installs", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
  rows, err := st.ListMarketplaceInstalls(c, tenantOf(c))
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

r.POST("/v1/marketplace/installs", auth.RequireRole("admin"), func(c *gin.Context) {
  var req UpsertInstallReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.AppID=="" { c.JSON(400, gin.H{"error":"app_id required"}); return }
  if req.Status=="" { req.Status = "installed" }
  if req.InstalledBy=="" { req.InstalledBy = "admin" }
  if err := st.UpsertMarketplaceInstall(c, tenantOf(c), req.AppID, req.InstalledBy, req.Status, req.Config); err != nil {
    c.JSON(500, gin.H{"error":"db"}); return
  }
  c.JSON(200, gin.H{"ok":true})
})
...
r.GET("/v1/marketplace/reviews", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
  appID := c.Query("app_id")
  if appID=="" { c.JSON(400, gin.H{"error":"app_id required"}); return }
  rows, err := st.ListMarketplaceReviews(c, tenantOf(c), appID)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

r.POST("/v1/marketplace/reviews", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
  var req CreateReviewReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.AppID=="" || req.Rating<1 || req.Rating>5 { c.JSON(400, gin.H{"error":"app_id & rating(1..5) required"}); return }
  if req.Reviewer=="" { req.Reviewer = "user" }
  if err := st.CreateMarketplaceReview(c, tenantOf(c), store.MarketplaceReview{
    AppID: req.AppID, Reviewer: req.Reviewer, Rating: req.Rating, Title: req.Title, Body: req.Body,
  }); err != nil {
    c.JSON(500, gin.H{"error":"db"}); return
  }
  c.JSON(200, gin.H{"ok":true})
...
// --- V296 Marketplace: scopes approval + version deploy + publisher console ---
type UpsertScopeReq struct {
  ScopeKey string `json:"scope_key"`
  Title string `json:"title"`
  Description string `json:"description"`
}

r.GET("/v1/marketplace/scopes", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
  rows, err := st.ListMarketplaceScopes(c)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

r.POST("/v1/marketplace/scopes", auth.RequireRole("admin"), func(c *gin.Context) {
  var req UpsertScopeReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.ScopeKey=="" || req.Title=="" { c.JSON(400, gin.H{"error":"scope_key & title required"}); return }
  if err := st.UpsertMarketplaceScope(c, req.ScopeKey, req.Title, req.Description); err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, gin.H{"ok":true})
})
...
r.GET("/v1/marketplace/app_versions", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
  appID := c.Query("app_id")
  if appID=="" { c.JSON(400, gin.H{"error":"app_id required"}); return }
  rows, err := st.ListMarketplaceAppVersions(c, tenantOf(c), appID)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

r.POST("/v1/marketplace/app_versions", auth.RequireRole("admin"), func(c *gin.Context) {
  var req UpsertVersionReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.AppID=="" || req.Version=="" { c.JSON(400, gin.H{"error":"app_id & version required"}); return }
  id, err := st.UpsertMarketplaceAppVersion(c, tenantOf(c), req.AppID, req.Version, req.ReleaseNotes, req.PackageURL)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, gin.H{"id":id})
})

type VersionActionReq struct {
  VersionID string `json:"version_id"`
  Reviewer string `json:"reviewer"`
}

r.POST("/v1/marketplace/app_versions/submit", auth.RequireRole("admin"), func(c *gin.Context) {
  var req VersionActionReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.VersionID=="" { c.JSON(400, gin.H{"error":"version_id required"}); return }
  if err := st.SubmitMarketplaceAppVersion(c, tenantOf(c), req.VersionID); err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, gin.H{"ok":true})
})

r.POST("/v1/marketplace/app_versions/approve", auth.RequireRole("admin"), func(c *gin.Context) {
  var req VersionActionReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.VersionID=="" { c.JSON(400, gin.H{"error":"version_id required"}); return }
  if req.Reviewer=="" { req.Reviewer = "admin" }
  if err := st.ApproveMarketplaceAppVersion(c, tenantOf(c), req.VersionID, req.Reviewer); err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, gin.H{"ok":true})
})

r.POST("/v1/marketplace/app_versions/publish", auth.RequireRole("admin"), func(c *gin.Context) {
  var req VersionActionReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.VersionID=="" { c.JSON(400, gin.H{"error":"version_id required"}); return }
  if req.Reviewer=="" { req.Reviewer = "admin" }
  if err := st.PublishMarketplaceAppVersion(c, tenantOf(c), req.VersionID, req.Reviewer); err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, gin.H{"ok":true})
})

r.POST("/v1/marketplace/app_versions/rollback", auth.RequireRole("admin"), func(c *gin.Context) {
  var req VersionActionReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.VersionID=="" { c.JSON(400, gin.H{"error":"version_id required"}); return }
  if req.Reviewer=="" { req.Reviewer = "admin" }
  if err := st.RollbackMarketplaceAppVersion(c, tenantOf(c), req.VersionID, req.Reviewer); err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, gin.H{"ok":true})
})

type ScopeRequestReq struct {
  AppVersionID string `json:"app_version_id"`
  ScopeKey string `json:"scope_key"`
}

r.POST("/v1/marketplace/scope_requests", auth.RequireRole("admin"), func(c *gin.Context) {
  var req ScopeRequestReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.AppVersionID=="" || req.ScopeKey=="" { c.JSON(400, gin.H{"error":"app_version_id & scope_key required"}); return }
  if err := st.UpsertMarketplaceScopeRequest(c, tenantOf(c), req.AppVersionID, req.ScopeKey); err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, gin.H{"ok":true})
})

r.GET("/v1/marketplace/scope_requests", auth.RequireRole("admin","marketer","analyst"), func(c *gin.Context) {
  vid := c.Query("app_version_id")
  if vid=="" { c.JSON(400, gin.H{"error":"app_version_id required"}); return }
  rows, err := st.ListMarketplaceScopeRequests(c, tenantOf(c), vid)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

type ReviewScopeReq struct {
  RequestID string `json:"request_id"`
  Status string `json:"status"` // approved|denied
  Reviewer string `json:"reviewer"`
}

r.POST("/v1/marketplace/scope_requests/review", auth.RequireRole("admin"), func(c *gin.Context) {
  var req ReviewScopeReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.RequestID=="" || req.Status=="" { c.JSON(400, gin.H{"error":"request_id & status required"}); return }
  if req.Reviewer=="" { req.Reviewer = "admin" }
  if err := st.ReviewMarketplaceScopeRequest(c, tenantOf(c), req.RequestID, req.Status, req.Reviewer); err != nil { c.JSON(400, gin.H{"error": err.Error()}); return }
  c.JSON(200, gin.H{"ok":true})
})

type UpsertPublisherReq struct {
  Name string `json:"name"`
  ContactEmail string `json:"contact_email"`
}

r.GET("/v1/marketplace/publishers", auth.RequireRole("admin"), func(c *gin.Context) {
  rows, err := st.ListMarketplacePublishers(c, tenantOf(c))
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

r.POST("/v1/marketplace/publishers", auth.RequireRole("admin"), func(c *gin.Context) {
  var req UpsertPublisherReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.Name=="" { c.JSON(400, gin.H{"error":"name required"}); return }
  id, err := st.UpsertMarketplacePublisher(c, tenantOf(c), req.Name, req.ContactEmail)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, gin.H{"id":id})
})

type UpsertPublisherMemberReq struct {
  PublisherID string `json:"publisher_id"`
  MemberEmail string `json:"member_email"`
  Role string `json:"role"`
}

r.GET("/v1/marketplace/publisher_members", auth.RequireRole("admin"), func(c *gin.Context) {
  pid := c.Query("publisher_id")
  if pid=="" { c.JSON(400, gin.H{"error":"publisher_id required"}); return }
  rows, err := st.ListMarketplacePublisherMembers(c, tenantOf(c), pid)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})

r.POST("/v1/marketplace/publisher_members", auth.RequireRole("admin"), func(c *gin.Context) {
  var req UpsertPublisherMemberReq
  if err := c.BindJSON(&req); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
  if req.PublisherID=="" || req.MemberEmail=="" { c.JSON(400, gin.H{"error":"publisher_id & member_email required"}); return }
  if err := st.AddMarketplacePublisherMember(c, tenantOf(c), req.PublisherID, req.MemberEmail, req.Role); err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, gin.H{"ok":true})
})
  ...
  // ---- V300 Launch-final: OIDC SSO sessions ----
  r.GET("/v1/me", func(c *gin.Context) {
    p := auth.GetPrincipal(c)
    if p.TenantID == "" {
      c.JSON(http.StatusUnauthorized, gin.H{"error":"unauthorized"}); return
    }
    c.JSON(200, gin.H{"tenant_id":p.TenantID,"user_id":p.UserID,"role":p.Role,"permissions":p.Permissions})
  })

  r.POST("/v1/auth/logout", func(c *gin.Context) {
    authz := c.GetHeader("Authorization")
    if strings.HasPrefix(strings.ToLower(authz), "bearer ") {
      tok := strings.TrimSpace(authz[7:])
      _ = st.DeleteSession(c, tok)
    }
    c.JSON(200, gin.H{"ok":true})
  })

  r.GET("/v1/auth/oidc/start", func(c *gin.Context) {
    tenant := c.Query("tenant_id")
    redirect := c.Query("redirect")
    if tenant == "" { c.JSON(400, gin.H{"error":"tenant_id required"}); return }
    cfg2, err := st.GetOIDCConfigFull(c, tenant)
    if err != nil || !cfg2.Enabled { c.JSON(400, gin.H{"error":"oidc not configured"}); return }

    state := uuid.New().String()
    nonce := uuid.New().String()
    _ = st.SaveOIDCLoginState(c, tenant, state, nonce, redirect, 10*time.Minute)

    authURL := cfg2.IssuerURL + "/authorize" +
      "?response_type=code" +
      "&client_id=" + url.QueryEscape(cfg2.ClientID) +
      "&redirect_uri=" + url.QueryEscape(cfg2.RedirectURL) +
      "&scope=" + url.QueryEscape(cfg2.Scopes) +
      "&state=" + url.QueryEscape(state) +
      "&nonce=" + url.QueryEscape(nonce)
    c.JSON(200, gin.H{"auth_url":authURL,"state":state})
  })

  // Simplified callback: expects id_token posted (for local/dev). In production you would exchange code for tokens.
  r.POST("/v1/auth/oidc/callback", func(c *gin.Context) {
    var body struct{
      TenantID string `json:"tenant_id"`
      State string `json:"state"`
      IDToken string `json:"id_token"`
    }
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
    stRec, err := st.ConsumeOIDCLoginState(c, body.TenantID, body.State)
    if err != nil || time.Now().After(stRec.ExpiresAt) { c.JSON(400, gin.H{"error":"invalid state"}); return }

    cfg2, err := st.GetOIDCConfigFull(c, body.TenantID)
    if err != nil { c.JSON(400, gin.H{"error":"oidc config missing"}); return }
    claims, err := auth.VerifyRS256JWT(body.IDToken, cfg2.JWKSJSON, cfg2.IssuerURL, cfg2.ClientID, stRec.Nonce)
    if err != nil { c.JSON(401, gin.H{"error":"token verify failed", "detail":err.Error()}); return }

    // Map subject/email to user
    userID := claims.Sub
    if claims.Email != "" { userID = claims.Email }
    role := "admin"
    token := uuid.New().String()
    _ = st.CreateSession(c, body.TenantID, userID, role, token, 24*time.Hour)
    // Frontend can store session token and use Authorization: Bearer <token>
    c.JSON(200, gin.H{"session_token":token,"user_id":userID,"tenant_id":body.TenantID})
  })

  // ---- V300: Stripe billing integration (reference) ----
  r.POST("/v1/billing/stripe/config", auth.RequireRole("admin"), func(c *gin.Context) {
    tenant := tenantOf(c)
    var body map[string]string
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
    err := st.UpsertStripeConfig(c, tenant, body["stripe_secret_key"], body["stripe_webhook_secret"])
    if err != nil { c.JSON(500, gin.H{"error":err.Error()}); return }
    c.JSON(200, gin.H{"ok":true})
  })

  r.POST("/v1/billing/stripe/checkout_session", func(c *gin.Context) {
    tenant := tenantOf(c)
    var body struct{
    ...
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
    url2, err := billingCreateStripeCheckoutSession(c, st, tenant, body.PlanID, body.SuccessURL, body.CancelURL)
    if err != nil { c.JSON(500, gin.H{"error":err.Error()}); return }
    c.JSON(200, gin.H{"checkout_url":url2})
  })

  r.POST("/v1/billing/stripe/webhook", func(c *gin.Context) {
    // public endpoint; verifies signature inside helper
    handled, err := billingHandleStripeWebhook(c, st)
    if err != nil { c.JSON(400, gin.H{"error":err.Error()}); return }
    c.JSON(200, gin.H{"ok":handled})
  })

  // ---- V300: Plan bundles (policy templates packaged with plans) ----
  r.POST("/v1/billing/plan_bundle", auth.RequireRole("admin"), func(c *gin.Context) {
    tenant := tenantOf(c)
    var body struct{
      PlanID string `json:"plan_id"`
    ...
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
    if body.BundleName=="" { body.BundleName="default" }
    err := st.AddPlanPolicyBundle(c, tenant, body.PlanID, body.PolicyTemplateID, body.BundleName)
    if err != nil { c.JSON(500, gin.H{"error":err.Error()}); return }
    c.JSON(200, gin.H{"ok":true})
  })

  // ---- V300: Commerce Hub (multi-marketplace) ----
  r.POST("/v1/commerce/products/upload", auth.RequirePermission("commerce:write"), func(c *gin.Context) {
    tenant := tenantOf(c)
    var body struct{ Channel string `json:"channel"`; Products []map[string]any `json:"products"` }
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
    runID, err := st.CommerceStartSync(c, tenant, body.Channel, "products")
    if err != nil { c.JSON(500, gin.H{"error":err.Error()}); return }
    // reference: just store products locally; real connector calls happen in connectors service
    _ = st.CommerceUpsertProducts(c, tenant, body.Products)
    _ = st.CommerceFinishSync(c, tenant, runID, "success", "stored locally; connector stub")
    c.JSON(200, gin.H{"run_id":runID})
  })

  r.POST("/v1/commerce/prices/update", auth.RequirePermission("commerce:write"), func(c *gin.Context) {
    tenant := tenantOf(c)
    var body struct{ Channel string `json:"channel"`; Prices []map[string]any `json:"prices"` }
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
    runID, _ := st.CommerceStartSync(c, tenant, body.Channel, "prices")
    _ = st.CommerceFinishSync(c, tenant, runID, "success", "connector stub")
    c.JSON(200, gin.H{"run_id":runID})
  })

  r.POST("/v1/commerce/orders/sync", auth.RequirePermission("commerce:read"), func(c *gin.Context) {
    tenant := tenantOf(c)
    var body struct{ Channel string `json:"channel"` }
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
    runID, _ := st.CommerceStartSync(c, tenant, body.Channel, "orders")
    // stub: no external fetch; placeholder
    _ = st.CommerceFinishSync(c, tenant, runID, "success", "connector stub")
    c.JSON(200, gin.H{"run_id":runID})
  })

  r.POST("/v1/commerce/inventory/sync", auth.RequirePermission("commerce:write"), func(c *gin.Context) {
    tenant := tenantOf(c)
    var body struct{ Channel string `json:"channel"`; Inventory []map[string]any `json:"inventory"` }
    if err := c.BindJSON(&body); err != nil { c.JSON(400, gin.H{"error":"bad json"}); return }
    runID, _ := st.CommerceStartSync(c, tenant, body.Channel, "inventory")
    _ = st.CommerceUpsertInventory(c, tenant, body.Inventory)
    _ = st.CommerceFinishSync(c, tenant, runID, "success", "stored locally; connector stub")
    c.JSON(200, gin.H{"run_id":runID})
  })
...
r.GET("/v1/experiments/sequential_report", auth.RequireRole("analyst","admin","marketer"), func(c *gin.Context) {
  experimentId := c.Query("experiment_id")
  if experimentId=="" { c.JSON(400, gin.H{"error":"experiment_id required"}); return }
  alpha,_ := strconv.ParseFloat(c.DefaultQuery("alpha","0.05"), 64)
  power,_ := strconv.ParseFloat(c.DefaultQuery("power","0.8"), 64)
  mde,_ := strconv.ParseFloat(c.DefaultQuery("mde","0.02"), 64)
  looks,_ := strconv.Atoi(c.DefaultQuery("looks","6"))
  rep, err := st.GetSequentialExperimentReport(c, tenantOf(c), experimentId, alpha, power, mde, looks)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rep)
})

r.GET("/v1/experiments/holdout_decay", auth.RequireRole("analyst","admin","marketer"), func(c *gin.Context) {
  experimentId := c.Query("experiment_id")
  metric := c.DefaultQuery("metric","revenue_per_user")
  days,_ := strconv.Atoi(c.DefaultQuery("days","28"))
  if experimentId=="" { c.JSON(400, gin.H{"error":"experiment_id required"}); return }
  rep, err := st.GetHoldoutDecayReport(c, tenantOf(c), experimentId, metric, days)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rep)
})

r.GET("/v1/marketplace/finance_summary", auth.RequireRole("admin","analyst"), func(c *gin.Context) {
  from := c.Query("from"); to := c.Query("to")
  if from=="" || to=="" { c.JSON(400, gin.H{"error":"from,to required"}); return }
  sum, err := st.GetMarketplaceFinanceSummary(c, tenantOf(c), from, to)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, sum)
})

r.POST("/v1/marketplace/security_auto_review", auth.RequireRole("admin"), func(c *gin.Context) {
  var body struct{ AppID string `json:"app_id"` }
  _ = c.BindJSON(&body)
  if body.AppID=="" { c.JSON(400, gin.H{"error":"app_id required"}); return }
  res, err := st.RunMarketplaceSecurityAutoReview(c, tenantOf(c), body.AppID)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, res)
})

r.GET("/v1/insights/reviews", auth.RequireRole("analyst","admin","marketer"), func(c *gin.Context) {
  appId := c.Query("app_id")
  from := c.Query("from"); to := c.Query("to")
  if appId=="" || from=="" || to=="" { c.JSON(400, gin.H{"error":"app_id, from, to required"}); return }
  res, err := st.GetReviewInsights(c, tenantOf(c), appId, from, to)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, res)
})

r.GET("/v1/insights/influencers", auth.RequireRole("analyst","admin","marketer"), func(c *gin.Context) {
  appId := c.Query("app_id")
  from := c.Query("from"); to := c.Query("to")
  if appId=="" || from=="" || to=="" { c.JSON(400, gin.H{"error":"app_id, from, to required"}); return }
  rows, err := st.GetInfluencerInsights(c, tenantOf(c), appId, from, to)
  if err != nil { c.JSON(500, gin.H{"error":"db"}); return }
  c.JSON(200, rows)
})
