...
app.post("/v1/execute", async (req, res) => {
  ...
  const provider = (p.provider || (p.payload?.provider as string) || "stub").toLowerCase();
  ...
  try {
    ...
    // In production, connectors would POST snapshot/evidence back to gateway or a shared store.
    // Here we just return it to gateway which stores the result_json.
    res.json(result);
  } catch (e: any) {
