...
func (s *Store) CreateExecution(ctx context.Context, id, tenant, approvalID, idem, channel, provider, actionType, status string, reqJSON []byte) error {
  _, err := s.db.Pool.Exec(ctx, `
    INSERT INTO executions(id,tenant_id,approval_id,idempotency_key,channel,provider,action_type,status,request_json)
    VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)
    ON CONFLICT (tenant_id,idempotency_key) DO NOTHING
  `, id, tenant, approvalID, idem, channel, provider, actionType, status, reqJSON)
  return err
}
...
func (s *Store) SaveSnapshot(ctx context.Context, tenant, execID, provider string, snapshot []byte) error {
  _, err := s.db.Pool.Exec(ctx, `
    INSERT INTO provider_snapshots(tenant_id,execution_id,provider,snapshot_json)
    VALUES($1,$2,$3,$4)
    ON CONFLICT (tenant_id,execution_id) DO UPDATE SET snapshot_json=excluded.snapshot_json
  `, tenant, execID, provider, snapshot)
  return err
}

func (s *Store) GetSnapshot(ctx context.Context, tenant, execID string) ([]byte, error) {
  var b []byte
  err := s.db.Pool.QueryRow(ctx, `
    SELECT snapshot_json FROM provider_snapshots WHERE tenant_id=$1 AND execution_id=$2
  `, tenant, execID).Scan(&b)
  return b, err
}
