...
func callAI(aiURL string, req map[string]any) (AIResponse, []byte, error) {
	...
	httpReq, _ := http.NewRequest("POST", aiURL+"/v257/ai/recommend", strings.NewReader(string(b)))
...
func callConnectorExecute(connURL, execID, channel string, actions []map[string]any, dryRun bool) error {
	...
	resp, err := http.Post(connURL+"/v257/connectors/execute", "application/json", strings.NewReader(string(reqBody)))
	if err != nil {
		return err
	}
	defer resp.Body.Close()
...
func callConnectorRollback(connURL, execID, channel string, dryRun bool) error {
	...
	resp, err := http.Post(connURL+"/v257/connectors/rollback", "application/json", strings.NewReader(string(reqBody)))
	if err != nil {
		return err
	}
	defer resp.Body.Close()
...
func main() {
	...
	r.POST("/v257/ops/policy/reload", opsAuth, func(c *gin.Context) {
	...
	r.GET("/v257/ops/outbox", opsAuth, func(c *gin.Context) {
	...
	r.POST("/v257/executions/approve", func(c *gin.Context) {
	...
	r.GET("/v257/executions/:id", func(c *gin.Context) {
		id := c.Param("id")
	...
	r.GET("/v257/audit/:id", func(c *gin.Context) {
		id := c.Param("id")
	...
	r.GET("/v257/reports/lift/:id", func(c *gin.Context) {
		id := c.Param("id")
		row := pool.QueryRow(ctx, "SELECT shadow_mode, confidence FROM executions WHERE execution_id=$1", id)
		var shadow bool
		var conf *float64
		if err := row.Scan(&shadow, &conf); err != nil {
			c.JSON(404, gin.H{"error": "not_found"})
			return
		}
		c.JSON(200, gin.H{
			...
			"shadow_mode": shadow,
			"confidence": conf,
			"lift_report": gin.H{"status": "placeholder", "note": "Compute uplift using real spend/conversion data + experiment design."},
		})
	})
