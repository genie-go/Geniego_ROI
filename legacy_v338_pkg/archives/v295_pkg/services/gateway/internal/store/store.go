...
func (s *Store) GetDealRecommendationsBayes(ctx context.Context, tenant, productID, from, to string) ([]DealCIReco, error) {
  rows, err := s.db.Pool.Query(ctx, `
    SELECT ips.influencer_id::text, i.name,
           ips.product_id::text, p.name,
           ips.channel,
  ...
  out := []DealCIReco{}
  z := 1.96
  // Priors: weakly informative Beta(1,1)
  alpha0 := 1.0
  beta0 := 1.0
  ...
  for rows.Next() {
    ...
    minClicks := 200.0
    minCost := 100.0
    maxCPA := 100.0 // demo guardrail; in reality per-product/segment
    cpa := 0.0
    if conv > 0 { cpa = costNum / conv }

    // Posterior for conversion rate
    a := alpha0 + conv
    b := beta0 + math.Max(0.0, clicks-conv)
    mean := a / (a + b)
    varp := (a*b)/((a+b)*(a+b)*(a+b+1.0))
    sd := math.Sqrt(varp)

    pLow := math.Max(0.0, mean - z*sd)
    pHigh := math.Min(1.0, mean + z*sd)
    ...
    // Decision logic (Bayesian + guardrails)
    // SCALE if profit lower bound > 0 and CPA guardrail passes and sample size ok.
    // STOP if profit upper bound < 0 or CPA way above guardrail with enough data.
    // otherwise HOLD.
    sufficient := clicks >= minClicks && costNum >= minCost
    cpaOk := (conv == 0) || (cpa <= maxCPA)

    if sufficient && cpaOk && r.ProfitLow > 0 && r.ROILow > 0.12 {
      r.Decision = "SCALE"
      width := math.Abs(r.ProfitHigh - r.ProfitLow)
      r.Confidence = math.Max(0.55, math.Min(0.95, 1200.0/(width+1200.0)))
    } else if sufficient && (r.ProfitHigh < 0 || (conv > 5 && cpa > maxCPA*1.5)) {
      r.Decision = "STOP"
      width := math.Abs(r.ProfitHigh - r.ProfitLow)
      r.Confidence = math.Max(0.60, math.Min(0.95, 1200.0/(width+1200.0)))
    } else {
      r.Decision = "HOLD"
      width := math.Abs(r.ProfitHigh - r.ProfitLow)
      base := 0.35
      if sufficient { base = 0.45 }
      r.Confidence = math.Max(base, math.Min(0.85, 1200.0/(width+1200.0)))
      if !sufficient { r.Confidence = math.Min(r.Confidence, 0.55) }
    }
...
func (s *Store) ListMarketplaceApps(ctx context.Context, tenant string) ([]MarketplaceApp, error) {
  rows, err := s.db.Pool.Query(ctx, `
    SELECT id::text, tenant_id, app_key, name, publisher, description, scopes_json, install_url, status, created_at, updated_at
  ...
  for rows.Next() {
    ...
    var scopes []byte
    if err := rows.Scan(&r.ID, &r.TenantID, &r.AppKey, &r.Name, &r.Publisher, &r.Description, &scopes, &r.InstallURL, &r.Status, &r.CreatedAt, &r.UpdatedAt); err != nil { return nil, err }
