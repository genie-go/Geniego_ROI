...
const V295_EVAL = {
  platform_definition: "마케팅/광고/딜/파트너까지 ‘실행 → 검증 → 의사결정’을 전 채널로 연결하는 톱티어급 Growth Operating System (Orchestration + Measurement + Decisioning + Marketplace)",
  absolute_scores: [
    { area:"Journey 오케스트레이션(노드형 빌더 + 상시 워커 + 재시도/DLQ + 승인/감사)", score:9.9, note:"엔진 안정성 + 빌더 UX를 함께 갖춤. 운영 실패를 ‘데이터로 관리’하는 수준." },
    { area:"코호트/퍼널/세그먼트 드릴다운(Ordered funnel 포함)", score:9.8, note:"실행/실험/딜 데이터와 같은 화면에서 분석까지 이어지는 운영 루프." },
    { area:"실험/인크리멘털(가드레일·샘플사이즈 프록시)", score:9.6, note:"실험 결과를 딜/예산 의사결정에 바로 연결. 고급 통계 자동화는 향후 여지." },
    { area:"딜 추천(Bayesian CI + Guardrails)", score:9.7, note:"하한 기반 SCALE/HOLD/STOP + 표본/CPA 가드레일로 의사결정 품질 상승." },
    { area:"어트리뷰션(운영 결합형)", score:9.5, note:"캠페인/실험/딜 리포트와 결합. MMM/MTA 고도화는 확장 포인트." },
    { area:"Marketplace(오퍼/정산/카탈로그/앱스토어 + 설치/리뷰)", score:9.4, note:"거래·정산·리스팅·설치·리뷰까지 ‘상거래 운영’ 기본 골격 완성." },
    { area:"커넥터/채널 실행(템플릿화)", score:9.6, note:"운영 표준화에 강점. 생태계 확장 속도가 경쟁 포인트." },
    { area:"엔터프라이즈(권한/감사/정책)", score:9.6, note:"대부분 운영 요구 충족. 더 촘촘한 RBAC/SCIM/데이터 레지던시 등은 확장 가능." }
  ],
  competitor_scores: [
    { area:"Journey & Orchestration", score:9.6, note:"Braze/Iterable 급 UX에 근접 + 운영 안정성(재시도/DLQ) 강점" },
    { area:"Analytics Drilldown (Cohort/Funnel/Segments)", score:9.4, note:"MA 단독 분석보다 ‘운영-측정-결정’ 결합이 강점" },
    { area:"Experiment & Incrementality", score:9.1, note:"톱티어 Measurement 제품 수준의 자동 통계는 더 필요" },
    { area:"Deal Decisioning (Bayesian + Guardrails)", score:9.3, note:"프로덕트화된 의사결정 루프가 강점. 고급 모델링은 확장 여지" },
    { area:"Marketplace / Appstore", score:9.0, note:"앱스토어 기본 기능(리스팅/설치/리뷰/정산) 확보. 파트너 규모는 시간 변수" }
  ],
  market_entry: [
    { title:"도입 효과(속도)", detail:"‘실행→검증→결정’ 폐루프를 한 제품에서 돌려 캠페인/딜 의사결정 리드타임을 크게 단축." },
    { title:"도입 효과(비용)", detail:"MA+BI+실험+딜 운영이 분산된 스택에서 발생하는 데이터 불일치/운영 장애/협업 오버헤드를 감소." },
    { title:"차별화 포인트", detail:"마케팅 자동화만이 아니라 ‘딜/오퍼/정산/앱스토어’까지 결합해 파트너 매출 확장에 직접 연결." },
    { title:"리스크/관건", detail:"생태계(커넥터/앱/파트너) 확장 속도와 엔터프라이즈 레퍼런스 확보가 시장 침투의 핵심." }
  ]
};
...
function EvaluationView() {
  ...
  return (
    <div>
      <Card title="V295 플랫폼 정의">
        <div style={{fontSize:16, fontWeight:900, lineHeight:1.4}}>{V295_EVAL.platform_definition}</div>
        <div style={{marginTop:10, color:"#6b7280", fontWeight:700}}>
      ...
      <Card title="기능별 절대평가 (10점 만점)">
        <table style={tableStyle}>
          ...
          <tbody>
            {V295_EVAL.absolute_scores.map((r,idx)=>(
              <tr key={idx}>
      ...
      <Card title="경쟁사 대비 상대평가 (10점 만점)">
        <table style={tableStyle}>
          ...
          <tbody>
            {V295_EVAL.competitive_scores.map((r,idx)=>(
              <tr key={idx}>
                <td style={thtd}><b>{r.area}</b></td>
                <td style={thtd}><b>{r.score.toFixed(1)}</b></td>
                <td style={thtd}>{r.note}</td>
      ...
      <Card title="시장 진입 시 예상 효과 (예측)">
        <ul style={{margin:0, paddingLeft:18}}>
          {V295_EVAL.market_entry_effects.map((x,idx)=>(
