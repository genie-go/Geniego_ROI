...
func main() {
	...
	r.GET("/v163/admin/api/incidents", func(c *gin.Context) {
		rows, err := pool.Query(ctx, "SELECT id, tenant_id, kind, message, created_at FROM incidents ORDER BY created_at DESC LIMIT 200")
		if err != nil { c.JSON(500, gin.H{\"ok\":false,\"error\":err.Error()}); return }
		defer rows.Close()
		items := []gin.H{}
		for rows.Next() {
			var id int64; var tenant, kind, msg string; var created time.Time
			_ = rows.Scan(&id, &tenant, &kind, &msg, &created)
