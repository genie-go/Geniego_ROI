...
def throttle(cur, tenant: str, channel: str, scope: str = "default"):
    """Distributed (DB-backed) token bucket throttling.

    Uses:
      - commerce_rate_limits (rps, burst) configured per (tenant,channel,scope)
      - commerce_rate_limit_state (tokens, last_refill_at) as shared state

    Behavior:
      - Refill tokens by elapsed_seconds * rps, capped at burst.
      - Consume 1 token per call; if insufficient, sleep a short amount and retry.
    """
    cur.execute(
        "SELECT rps, burst FROM commerce_rate_limits WHERE tenant_id=%s AND channel=%s AND scope=%s",
        (tenant, channel, scope),
    )
    row = cur.fetchone()
    rps = float(row[0]) if row else 2.0
    burst = float(row[1]) if row else max(2.0, rps * 5.0)

    # Ensure state row exists
    cur.execute(
        """
        INSERT INTO commerce_rate_limit_state(tenant_id, channel, scope, tokens, last_refill_at, updated_at)
        VALUES(%s,%s,%s,%s,now(),now())
        ON CONFLICT (tenant_id, channel, scope) DO NOTHING
        """,
        (tenant, channel, scope, burst),
    )
    ...
    for _ in range(50):
        cur.execute(
            """
            SELECT tokens, EXTRACT(EPOCH FROM last_refill_at)
            FROM commerce_rate_limit_state
            WHERE tenant_id=%s AND channel=%s AND scope=%s
            FOR UPDATE
            """,
            (tenant, channel, scope),
        )
        tok, last_refill = cur.fetchone()
        nowt = time.time()
        elapsed = max(0.0, nowt - float(last_refill))
        tokens = min(burst, float(tok) + elapsed * max(0.01, rps))

        if tokens >= 1.0:
            tokens -= 1.0
            cur.execute(
                """
                UPDATE commerce_rate_limit_state
                   SET tokens=%s,
                       last_refill_at=to_timestamp(%s),
                       updated_at=now()
                 WHERE tenant_id=%s AND channel=%s AND scope=%s
                """,
                (tokens, nowt, tenant, channel, scope),
            )
            return

        # Not enough tokens: compute minimal sleep to next token
        need = 1.0 - tokens
        wait = min(2.0, need / max(0.01, rps))
        # Update state with refilled tokens and bump last_refill_at
        cur.execute(
            """
            UPDATE commerce_rate_limit_state
               SET tokens=%s,
