...
/**
 * rakuten connector — commercial-grade scaffold.
 * Auth: key-based (Rakuten APIs vary by region).
 *
 * Required creds: service_secret, license_key
 * Provide a correct `base_url` for your region/sandbox if needed.
 */
function cfg(creds: any) {
  const c = requireFields(creds, ["service_secret", "license_key"], "rakuten");
  ...
  async execute(p: ExecutePayload) {
    try {
      ...
      if (p.action_type === "fetch_orders") {
        return okResp(p, "rakuten", { orders: [] }, { kind, note: "implement real API calls + pagination" });
      }
      if (p.action_type === "sync_inventory") {
        return okResp(p, "rakuten", { successes: [], failures: [] }, { kind, note: "implement inventory sync" });
      }
      if (p.action_type === "upsert_products") {
        return okResp(p, "rakuten", { successes: [], failures: [] }, { kind, note: "implement product upsert" });
      }
      if (p.action_type === "update_prices") {
        return okResp(p, "rakuten", { successes: [], failures: [] }, { kind, note: "implement price update" });
      }
      return errResp(p, "rakuten", `unsupported_action:${p.action_type}`);
    } catch (e: any) {
      return errResp(p, "rakuten", e?.message || "error");
    }
  },
};
