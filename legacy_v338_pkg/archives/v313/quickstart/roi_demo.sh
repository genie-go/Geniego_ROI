...
curl -sS -X POST http://localhost:8080/v1/roi/metrics/ads   -H "Content-Type: application/json" -H "X-Tenant-ID: ${TENANT_ID:-demo-tenant}" -H "X-API-Key: ${API_KEY:-}"   --data-binary @- <<JSON | jq .
{
  "day": "${DAY}",
  "rows": [
...
curl -sS -X POST http://localhost:8080/v1/roi/conversions   -H "Content-Type: application/json" -H "X-Tenant-ID: ${TENANT_ID:-demo-tenant}" -H "X-API-Key: ${API_KEY:-}"   --data-binary @- <<JSON | jq .
{
  "events": [
