# GENIE_ROI AdOps OS — V262 (Turning weaknesses into strengths: Ops/Governance/Integration-ready)

Generated: 2026-02-25

V262 is a **safe-by-default, ops-grade** reference implementation for an "Execution-Grade AdOps OS":
...
> V262 is safe: no real ad-platform changes unless you implement adapters and set DRY_RUN=false.
...
- **V262 API 호환 경로**: 외부 API는 `/v262/*`로 접근 가능(내부는 `/v261` 핸들러로 라우팅)
...
Open Admin UI:
- http://localhost:8080/v261/admin?token=change-me
...
## Key APIs
- POST `/v261/workflows/run` (idempotent with `Idempotency-Key`)
- POST `/v261/executions/approve`
- GET  `/v261/executions/:id`
- GET  `/v261/audit/:id`
- POST `/v261/shadow/events` (record shadow outcomes)
- GET  `/v261/reports/lift/:id` (placeholder lift view based on shadow_events)

Ops (token protected via `X-Ops-Token` header or `?token=` for demo):
- GET  `/v261/ops/outbox`
- GET  `/v261/ops/dlq`
- GET  `/v261/ops/metrics` (JSON)
- GET  `/v261/metrics` (Prometheus text)
- POST `/v261/ops/policy/reload`
...
## What V262 does to turn V259 weaknesses into strengths
...
- `/v261/metrics` (Prometheus) + `/v261/ops/metrics` (JSON snapshot)
...
## Run demo
```bash
curl -s -X POST http://localhost:8080/v261/workflows/run \
...
Approve:
```bash
curl -s -X POST http://localhost:8080/v261/executions/approve \
  -H "Content-Type: application/json" \
  -H "X-Role: operator" \
...
See ops:
```bash
curl -s http://localhost:8080/v261/ops/metrics -H "X-Ops-Token: change-me" -H "X-Role: admin" | jq
```
