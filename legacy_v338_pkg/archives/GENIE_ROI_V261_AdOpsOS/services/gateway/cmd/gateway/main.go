...
func callAI(aiURL string, req map[string]any) (AIResponse, []byte, error) {
	...
	httpReq, _ := http.NewRequest("POST", aiURL+"/v261/ai/recommend", strings.NewReader(string(b)))
...
func callConnectorExecute(connURL, execID, channel string, actions []map[string]any, dryRun bool) error {
	...
	resp, err := http.Post(connURL+"/v261/connectors/execute", "application/json", strings.NewReader(string(reqBody)))
	if err != nil {
		return err
	}
	defer resp.Body.Close()
...
func callConnectorRollback(connURL, execID, channel string, dryRun bool) error {
	...
	resp, err := http.Post(connURL+"/v261/connectors/rollback", "application/json", strings.NewReader(string(reqBody)))
	if err != nil {
		return err
	}
	defer resp.Body.Close()
