...
/**
 * Cafe24 provider (V303).
 * - Auth: OAuth2 bearer with refresh token.
 * - Default base_url points to included MOCK server.
 *
 * Creds:
...
async function ensureToken(c: any) {
  if (!c.access_token || isExpired(c.token_expires_at)) {
    const { token, evidence } = await oauthRefresh(c.base_url, "/oauth/cafe24/token", {
      client_id: c.client_id, client_secret: c.client_secret, refresh_token: c.refresh_token || "rt-demo", mall_id: c.mall_id
    });
...
async function upsertProducts(p: ExecutePayload, c: any, at: string) {
  ...
  const url = `${c.base_url}/api/cafe24/products`;
  const r = await jsonFetch(url, { method:"POST", headers: bearer(at), body: JSON.stringify({ products }) });
  if (r.status >= 400) throw new Error(`cafe24_products_failed:${r.status}`);
  return okResp(p, "cafe24", { products: products.map(normalizeProduct) }, { url, status: r.status });
}
...
async function updatePrices(p: ExecutePayload, c: any, at: string) {
  ...
  if (r.status >= 400) throw new Error(`cafe24_prices_failed:${r.status}`);
  return okResp(p, "cafe24", { prices }, { url, status: r.status });
}

async function fetchOrders(p: ExecutePayload, c: any, at: string) {
  const url = `${c.base_url}/api/cafe24/orders`;
  ...
  return okResp(p, "cafe24", { orders: events.map((e:any)=>e.order), order_events: events }, { url, status: r.status });
}

async function syncInventory(p: ExecutePayload, c: any, at: string) {
  const inventory = (p.payload?.data?.inventory || []);
  const url = `${c.base_url}/api/cafe24/inventory`;
  const r = await jsonFetch(url, { method:"POST", headers: bearer(at), body: JSON.stringify({ inventory }) });
  if (r.status >= 400) throw new Error(`cafe24_inventory_failed:${r.status}`);
  return okResp(p, "cafe24", { inventory: inventory.map(normalizeInventoryRow) }, { url, status: r.status });
}
...
export const cafe24Adapter: ProviderAdapter = {
  ...
  async execute(p: ExecutePayload) {
    try {
      ...
      else return errResp(p, "cafe24", `unsupported_action:${p.action_type}`);
      ...
      return errResp(p, "cafe24", e?.message || String(e));
    }
  },
};
