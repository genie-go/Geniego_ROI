...
from .google_ads_real import list_campaigns_via_google_ads_sdk, GoogleAdsRealClientError
...
class GoogleConnector:
    ...
    async def list_campaigns(self, tenant_id: str, auth: Dict[str, Any], *, account_id: Optional[str]=None) -> List[ExternalCampaign]:
        ...
        """
        mode = (auth.get("mode") or "stub").lower()
if mode == "real":
    try:
        return list_campaigns_via_google_ads_sdk(auth, account_id=account_id)
    except GoogleAdsRealClientError as e:
        # fall back to stub to keep the platform running safely
