...
def sync_custom_mappers_file(db_path: str, root: pathlib.Path):
    ...
    db.close()
    (root/"templates"/"v320").mkdir(parents=True, exist_ok=True)
    (root/"templates"/"v320"/"custom_mappers.json").write_text(
    ...
    def do_POST(self):
        ...
        if self.path in ("/api/upload_fx", "/api/upload_attribution"):
            ...
            if self.path == "/api/upload_fx":
                ...
                dst = self.server.app.root / "templates" / "v320" / "fx_rates.json"
                dst.write_bytes(data)
            else:
                # attribution rules
                if "chain" not in obj:
                    self.send_error(400, "attribution json must include chain"); return
                dst = self.server.app.root / "templates" / "v320" / "default_attribution.json"
                dst.write_bytes(data)
                # also store into DB for persistence
                db = sqlite3.connect(self.server.app.db_path)
                cur = db.cursor()
        ...
        if self.path in ("/api/upload_ads", "/api/upload_conversions"):
            ...
            if self.path == "/api/upload_ads":
                ...
                cmd = [sys.executable, "scripts/v320/ingest_ads.py", "--db", self.server.app.db_path, "--csv", dst.as_posix(), "--channel", channel]
            else:
                cmd = [sys.executable, "scripts/v320/ingest_conversions.py", "--db", self.server.app.db_path, "--csv", dst.as_posix()]

            code, out = run(cmd)
            if code == 0:
                code2, out2 = run([sys.executable, "scripts/v320/generate_dashboard_json.py", "--db", self.server.app.db_path, "--out", "out"])
                out = out + "\n" + out2
            self.send_response(200)
            self.send_header("Content-Type","text/plain; charset=utf-8")
            self.send_header("Cache-Control","no-store")
