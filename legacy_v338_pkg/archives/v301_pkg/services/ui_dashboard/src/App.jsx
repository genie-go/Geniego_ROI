...
async function api(path, { method = "GET", body } = {}) {
  ...
  const sess = (typeof window!=="undefined" && window.localStorage) ? window.localStorage.getItem("sessionToken") : "";
  if (sess) headers["Authorization"] = "Bearer " + sess;
  if (!sess && DEFAULT_KEY) headers["X-API-Key"] = DEFAULT_KEY;
...
function TemplateEditor() {
  ...
  const [commerceChannel, setCommerceChannel] = useState("shopify");
  const [commerceProductsJSON, setCommerceProductsJSON] = useState('[{"sku":"SKU-001","title":"Sample","description":"Demo","price":19.99,"currency":"USD"}]');
  const [commerceInventoryJSON, setCommerceInventoryJSON] = useState('[{"sku":"SKU-001","on_hand":50,"reserved":0}]');
  const [commerceLastRun, setCommerceLastRun] = useState(null);

  async function runCommerce(kind){
    try{
      setErr("");
      let body = { channel: commerceChannel };
      if (kind==="products") body = { channel: commerceChannel, products: JSON.parse(commerceProductsJSON) };
      if (kind==="inventory") body = { channel: commerceChannel, inventory: JSON.parse(commerceInventoryJSON) };
      if (kind==="prices") body = { channel: commerceChannel, prices: [] };
      const path = kind==="orders" ? "/v1/commerce/orders/sync"
        : kind==="products" ? "/v1/commerce/products/upload"
        : kind==="inventory" ? "/v1/commerce/inventory/sync"
        : "/v1/commerce/prices/update";
      const r = await api(path, { method:"POST", body });
      setCommerceLastRun(r);
    }catch(e){
      setErr(String(e));
    }
  }
...
function App() {
  ...
  useEffect(() => {
    // Handle SSO callback in SPA (if backend returns session_token via URL in your deployment)
    try{
      const qp = new URLSearchParams(window.location.search);
      const st = qp.get("session_token");
      if (st) {
        window.localStorage.setItem("sessionToken", st);
        qp.delete("session_token");
  ...
  "Marketplace Pro": "marketplace:write",
  "Marketplace": "marketplace:read",
  "Experiment Pro": "experiments:read",
  "Deal CI": "deals:read",
  "Commerce Hub": "commerce:read"
};
const hasPerm = (want) => {
  if (!want) return true;
  for (const p of (perms||[])) {
    if (p === "*" || p === want) return true;
    if (p.endsWith(":*") && want.startsWith(p.slice(0,-1))) return true;
  }
  return false;
};
const allTabs = ["Overview","Segments","Templates","Journey Builder","Connectors","ROI","Drilldown","Analytics","Experiment Pro","Deal CI","Marketplace","Marketplace Pro","Insights","About","Approvals","Webhooks","Attribution","Influencers","Products","Recommendations","Commerce Hub","Billing","Admin Console","Help"];
const tabs = allTabs.filter(t => !tabPerm[t] || hasPerm(tabPerm[t]));;
...
async function saveOIDC(next) {
  ...
  if (tabPerm[tab] && !hasPerm(tabPerm[tab])) {
    return (
      <div style={{padding:20,fontFamily:"system-ui"}}>
        <h2>403 Forbidden</h2>
        <div>이 메뉴는 권한이 필요합니다: <b>{tabPerm[tab]}</b></div>
        <div style={{marginTop:8}}>관리자에게 권한을 요청하거나, 다른 메뉴를 선택해 주세요.</div>
      </div>
    );
  }
...
{tab==="Commerce Hub" && (
  <div>
    <Card title="Commerce Hub (Amazon/Shopify/Qoo10/Rakuten/Coupang/Naver/Cafe24)">
      <div style={{color:"#6b7280", marginBottom:8}}>
        제품 등록/가격 업데이트/주문 수집/재고 동기화는 커넥터 스텁(레퍼런스)으로 구성되어 있으며,
        실제 연동은 각 채널 API 자격증명 설정 후 connectors 서비스 구현을 확장하는 방식입니다.
      </div>
      <div style={{display:"flex", gap:8, flexWrap:"wrap", alignItems:"center", marginBottom:10}}>
        <div>Channel:</div>
        <select value={commerceChannel} onChange={(e)=>setCommerceChannel(e.target.value)}>
          <option value="amazon">Amazon</option>
          <option value="shopify">Shopify</option>
          <option value="qoo10">Qoo10</option>
          <option value="rakuten">Rakuten</option>
          <option value="coupang">Coupang</option>
          <option value="naver">Naver SmartStore</option>
          <option value="cafe24">Cafe24</option>
        </select>
        <Button onClick={()=>runCommerce("products")}>Upload products</Button>
        <Button onClick={()=>runCommerce("orders")}>Sync orders</Button>
        <Button onClick={()=>runCommerce("inventory")}>Sync inventory</Button>
      </div>

      <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:12}}>
        <div>
          <div style={{fontWeight:700, marginBottom:6}}>Products JSON</div>
          <textarea style={{width:"100%", minHeight:140}} value={commerceProductsJSON} onChange={(e)=>setCommerceProductsJSON(e.target.value)} />
        </div>
        <div>
          <div style={{fontWeight:700, marginBottom:6}}>Inventory JSON</div>
          <textarea style={{width:"100%", minHeight:140}} value={commerceInventoryJSON} onChange={(e)=>setCommerceInventoryJSON(e.target.value)} />
        </div>
      </div>

      <div style={{marginTop:10}}>
        <div style={{fontWeight:700}}>Last run</div>
        <pre style={{whiteSpace:"pre-wrap", background:"#f9fafb", padding:10, borderRadius:8}}>
          {commerceLastRun ? JSON.stringify(commerceLastRun, null, 2) : "No runs yet."}
        </pre>
      </div>
    </Card>
  </div>
