...
if (aStatus) {
  ...
  for (const rn of GOOGLE_ADS_CAMPAIGN_RESOURCE_NAMES) {
    ...
    // (4) Keyword status updates (pause/resume)
    if (aKeyword) {
      if (!GOOGLE_ADS_KEYWORD_RESOURCE_NAMES.length) throw new Error("Google Ads missing GOOGLE_ADS_KEYWORD_RESOURCE_NAMES");
      const desired = (aKeyword.type==="resume_keyword") ? "ENABLED" : (aKeyword.type==="pause_keyword") ? "PAUSED" : (String(aKeyword.status||aKeyword.value||"PAUSED").toUpperCase());
      for (const rn of GOOGLE_ADS_KEYWORD_RESOURCE_NAMES) {
        const snap = await searchKeyword(rn);
        fs.writeFileSync(snapPath(execution_id, rn + "_kw"), JSON.stringify(snap, null, 2));
        const oldStatus = snap.status;
        if (String(oldStatus).toUpperCase() === String(desired).toUpperCase()) {
          results.push({ keyword_resource: rn, skipped:true, reason:"idempotent", oldStatus, newStatus: desired });
          continue;
        }
        if (dry_run) { results.push({ keyword_resource: rn, dry_run:true, oldStatus, newStatus: desired }); continue; }
        try {
          await mutateKeywordStatus(rn, desired);
          results.push({ keyword_resource: rn, ok:true, oldStatus, newStatus: desired });
        } catch (e) {
          // rollback
          try {
            await mutateKeywordStatus(rn, oldStatus);
            results.push({ keyword_resource: rn, failed:true, error:String(e&&e.message||e), rollback_attempted:true, rollback_to: oldStatus });
          } catch (re) {
            results.push({ keyword_resource: rn, failed:true, error:String(e&&e.message||e), rollback_failed:true, rollback_to: oldStatus, rollback_error:String(re&&re.message||re) });
          }
        }
      }
    }
...
async function searchKeyword(keywordResourceName) {
  const q = `SELECT ad_group_criterion.resource_name, ad_group_criterion.status FROM ad_group_criterion WHERE ad_group_criterion.resource_name = "${keywordResourceName}" LIMIT 1`;
  const body = { query: q };
  const resp = await fetchJson(`${GOOGLE_ADS_API_BASE}/customers/${GOOGLE_ADS_CUSTOMER_ID}/googleAds:search`, {
    method: "POST",
    headers: headers(),
    body: JSON.stringify(body)
  });
  const row = (resp.results && resp.results[0] && resp.results[0].adGroupCriterion) ? resp.results[0].adGroupCriterion : null;
  if (!row) throw new Error("keyword_not_found");
  return { resource_name: row.resourceName || keywordResourceName, status: row.status };
}

async function mutateKeywordStatus(keywordResourceName, status) {
  const op = { update: { resourceName: keywordResourceName, status }, updateMask: "status" };
  const body = { operations: [op] };
  return fetchJson(`${GOOGLE_ADS_API_BASE}/customers/${GOOGLE_ADS_CUSTOMER_ID}/adGroupCriteria:mutate`, {
    method: "POST",
    headers: headers(),
    body: JSON.stringify(body)
  });
