...
func (s *Store) CreateInfluencer(ctx context.Context, tenant, name string, handle *string, categories any, notes *string) (string, error) {
  b, _ := json.Marshal(categories)
  var id string
  err := s.db.Pool.QueryRow(ctx, `INSERT INTO influencers(id, tenant_id, name, handle, categories, notes)
    VALUES(gen_random_uuid(), $1, $2, $3, $4::jsonb, $5) RETURNING id`,
    tenant, name, handle, string(b), notes).Scan(&id)
  return id, err
}

func (s *Store) ListInfluencers(ctx context.Context, tenant string) ([]InfluencerRow, error) {
  rows, err := s.db.Pool.Query(ctx, `SELECT id, tenant_id, name, handle, categories, notes, created_at FROM influencers WHERE tenant_id=$1 ORDER BY created_at DESC`, tenant)
  if err != nil { return nil, err }
  defer rows.Close()
  out := []InfluencerRow{}
  for rows.Next() {
    var r InfluencerRow
    if err := rows.Scan(&r.ID, &r.TenantID, &r.Name, &r.Handle, &r.Categories, &r.Notes, &r.CreatedAt); err != nil { return nil, err }
    out = append(out, r)
  }
  return out, nil
}

func (s *Store) CreateProduct(ctx context.Context, tenant, name string, sku *string, categories any, price *string, margin *string) (string, error) {
  b, _ := json.Marshal(categories)
  var id string
  err := s.db.Pool.QueryRow(ctx, `INSERT INTO products(id, tenant_id, sku, name, categories, price, margin)
    VALUES(gen_random_uuid(), $1, $2, $3, $4::jsonb, $5::numeric, $6::numeric) RETURNING id`,
    tenant, sku, name, string(b), nullableNumeric(price), nullableNumeric(margin)).Scan(&id)
  return id, err
}
...
func (s *Store) ListProducts(ctx context.Context, tenant string) ([]ProductRow, error) {
  rows, err := s.db.Pool.Query(ctx, `SELECT id, tenant_id, sku, name, categories, price::text, margin::text, created_at FROM products WHERE tenant_id=$1 ORDER BY created_at DESC`, tenant)
  if err != nil { return nil, err }
  defer rows.Close()
  out := []ProductRow{}
  for rows.Next() {
    var r ProductRow
    if err := rows.Scan(&r.ID, &r.TenantID, &r.SKU, &r.Name, &r.Categories, &r.Price, &r.Margin, &r.CreatedAt); err != nil { return nil, err }
    out = append(out, r)
  }
  return out, nil
}
...
func (s *Store) RecommendInfluencersForProduct(ctx context.Context, tenant, productID string, fromDate string, toDate string, limit int) ([]RecommendationRow, error) {
  // Score = 0.45*ROAS + 0.35*CVR + 0.20*ENG (all normalized safely)
  // ROAS = revenue/cost, CVR = conversions/clicks, ENG from influencer_channel_stats.engagement_rate
  q := `
  WITH ips AS (
  ...
  SELECT influencer_id, influencer_name, channel,
    (0.45*COALESCE(roas_n,0) + 0.35*COALESCE(cvr_n,0) + 0.20*COALESCE(eng_n,0)) AS score,
    jsonb_build_object(
      'roas', roas, 'cvr', cvr, 'engagement_rate', eng,
      'revenue', revenue, 'cost', cost, 'conversions', conv, 'clicks', clicks
    )::jsonb::text::bytea AS evidence_json
  FROM norm
  ORDER BY score DESC
  LIMIT $5;
  `
  rows, err := s.db.Pool.Query(ctx, q, tenant, productID, fromDate, toDate, limit)
  if err != nil { return nil, err }
  defer rows.Close()
  out := []RecommendationRow{}
  for rows.Next() {
    var r RecommendationRow
    if err := rows.Scan(&r.InfluencerID, &r.InfluencerName, &r.Channel, &r.Score, &r.EvidenceJSON); err != nil { return nil, err }
    out = append(out, r)
  }
  return out, nil
}
