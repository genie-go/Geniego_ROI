...
function ExperimentProView() {
  const [experimentId, setExperimentId] = useState("exp__1");
  ...
  async function load() {
    try {
      ...
      const a = await fetch("/v1/experiments/sequential_report?"+q1, {headers:{"x-tenant":""}}).then(r=>r.json());
      const b = await fetch("/v1/experiments/holdout_decay?"+q2, {headers:{"x-tenant":""}}).then(r=>r.json());
      setSeq(a); setDecay(b);
...
function MarketplaceProView() {
  ...
  const [appId, setAppId] = useState("app__1");
  const [reviewRes, setReviewRes] = useState(null);

  async function loadFinance() {
    const q = new URLSearchParams({from, to}).toString();
    const a = await fetch("/v1/marketplace/finance_summary?"+q, {headers:{"x-tenant":""}}).then(r=>r.json());
    setFinance(a);
  }
  async function runAutoReview() {
    const a = await fetch("/v1/marketplace/security_auto_review", {
      method:"POST",
      headers:{"Content-Type":"application/json","x-tenant":""},
...
function InsightsView() {
  ...
  async function load() {
    ...
    const a = await fetch("/v1/insights/reviews?"+q, {headers:{"x-tenant":""}}).then(r=>r.json());
    const b = await fetch("/v1/insights/influencers?"+q, {headers:{"x-tenant":""}}).then(r=>r.json());
...
function App() {
  ...
  const tabs = ["Overview","Segments","Templates","Journey Builder","Connectors","ROI","Drilldown","Analytics","Experiment Pro","Deal CI","Marketplace","Marketplace Pro","Insights","About","Approvals","Webhooks","Attribution","Influencers","Products","Recommendations"];
