...
## V301 highlights (Commerce Hub upgrade)

- Commerce Hub upgraded from "local stub" to a **job queue + worker** model (retries, backoff).
- Shopify connector upgraded to perform **real Admin API calls** (products, prices, orders, inventory) when credentials are provided.
- New admin endpoint to store per-tenant channel credentials: `POST /v1/commerce/channel/credentials`.
...
## V299
- Commercial launch pack: Enterprise SSO/RBAC/Audit, Billing & plans, Connector SDK + marketplace publish automation, Tenant policy templates.
- Beginner docs: docs/V299_BEGINNER_GUIDE.md
