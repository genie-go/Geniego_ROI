...
func (s *Store) InsertPolicy(ctx context.Context, tenant string, version int, pol []byte) error {
  _, err := s.db.Pool.Exec(ctx, `
    INSERT INTO policies(tenant_id,version,policy_json) VALUES($1,$2,$3)
    ON CONFLICT (tenant_id,version) DO UPDATE SET policy_json=excluded.policy_json
  `, tenant, version, pol)
  return err
}
...
func (s *Store) GetLatestPolicy(ctx context.Context, tenant string) (PolicyRow, error) {
  ...
  err := s.db.Pool.QueryRow(ctx, `
    SELECT tenant_id, version, policy_json
    FROM policies WHERE tenant_id=$1
    ORDER BY version DESC LIMIT 1
  `, tenant).Scan(&r.TenantID, &r.Version, &r.PolicyJSON)
  return r, err
}

func (s *Store) CreateApproval(ctx context.Context, id, tenant, actionType, status, requestedBy, reason string) error {
  _, err := s.db.Pool.Exec(ctx, `
    INSERT INTO approvals(id,tenant_id,action_type,status,requested_by,reason)
    VALUES($1,$2,$3,$4,$5,$6)
  `, id, tenant, actionType, status, requestedBy, reason)
  return err
}

func (s *Store) DecideApproval(ctx context.Context, id, status, by, reason string) error {
  _, err := s.db.Pool.Exec(ctx, `
    UPDATE approvals SET status=$2, approved_by=$3, reason=$4, decided_at=now()
    WHERE id=$1
  `, id, status, by, reason)
  return err
}

func (s *Store) CreateExecution(ctx context.Context, id, tenant, approvalID, idem, channel, actionType, status string, reqJSON []byte) error {
  _, err := s.db.Pool.Exec(ctx, `
    INSERT INTO executions(id,tenant_id,approval_id,idempotency_key,channel,action_type,status,request_json)
    VALUES($1,$2,$3,$4,$5,$6,$7,$8)
    ON CONFLICT (tenant_id,idempotency_key) DO NOTHING
  `, id, tenant, approvalID, idem, channel, actionType, status, reqJSON)
  return err
}

func (s *Store) UpdateExecutionResult(ctx context.Context, tenant, id, status string, result []byte) error {
  _, err := s.db.Pool.Exec(ctx, `
    UPDATE executions SET status=$3, result_json=$4, updated_at=now()
    WHERE tenant_id=$1 AND id=$2
  `, tenant, id, status, result)
  return err
}
...
func (s *Store) EnqueueOutbox(ctx context.Context, tenant, execID string, payload []byte) error {
  _, err := s.db.Pool.Exec(ctx, `
    INSERT INTO outbox(tenant_id,execution_id,payload_json,status) VALUES($1,$2,$3,'PENDING')
  `, tenant, execID, payload)
  return err
}

func (s *Store) NextOutbox(ctx context.Context) (int64, string, string, []byte, error) {
  // Simple claim without SKIP LOCKED for scaffold
  row := s.db.Pool.QueryRow(ctx, `
    SELECT id, tenant_id, execution_id, payload_json
    FROM outbox
    WHERE status='PENDING' AND next_attempt_at <= now()
    ORDER BY id ASC LIMIT 1
  `)
  var id int64
  var tenant, exec string
  var payload []byte
...
func (s *Store) MarkOutbox(ctx context.Context, id int64, status string, nextAttempt time.Time) error {
  _, err := s.db.Pool.Exec(ctx, `
    UPDATE outbox SET status=$2, attempts=attempts+1, next_attempt_at=$3
    WHERE id=$1
  `, id, status, nextAttempt)
  return err
}
...
func (s *Store) Audit(ctx context.Context, tenant, execID, eventType string, event map[string]interface{}) error {
  ...
  _, err := s.db.Pool.Exec(ctx, `
    INSERT INTO audit_events(tenant_id,execution_id,event_type,event_json)
    VALUES($1,$2,$3,$4)
  `, tenant, execID, eventType, b)
  return err
}
...
func (s *Store) UpsertContact(ctx context.Context, tenant string, body map[string]interface{}) error {
  ...
  _, err := s.db.Pool.Exec(ctx, `
    ...
    ON CONFLICT (tenant_id,contact_id) DO UPDATE
      SET email=excluded.email, phone=excluded.phone, attrs=excluded.attrs, updated_at=now()
...
func (s *Store) UpsertJourney(ctx context.Context, tenant string, body map[string]interface{}) error {
  ...
  _, err := s.db.Pool.Exec(ctx, `
    ...
    ON CONFLICT (tenant_id,journey_id) DO UPDATE
      SET name=excluded.name, status=excluded.status, definition_json=excluded.definition_json, updated_at=now()
...
func (s *Store) TickJourneys(ctx context.Context, tenant string) error {
  // This is a scaffold-only scheduler:
  // - find due enrollments
  // - create a controlled action execution (EMAIL_SEND) for step payload
  // Real production would be a separate worker with policy enforcement, frequency caps, consent checks, etc.
  _, _ = tenant, ctx
  return nil
}
